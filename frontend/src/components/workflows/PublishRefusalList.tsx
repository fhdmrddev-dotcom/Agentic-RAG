 ...
 * to be detected PER ENTRY. `isArgumentRefusal` below is that detection for these five
 * codes; everything it declines falls through to the shipped generic renderer, unchanged.
 * ⚠ An entry this component cannot phrase HONESTLY is declined rather than approximated —
 * a refusal with no argument name to print would render empty curly quotes, and a publish
 * blocked with an unrenderable refusal is a workflow nobody can fix.
 */
import {
...
/**
 * One `named_failures` entry this surface can phrase, narrowed to exactly the fields it
 * reads. ⚠ The wire entry carries two MORE keys (the slug and the server diagnostic) and
 * they are deliberately absent from this type: a field that is not in scope cannot be
 * rendered by accident.
 */
export interface ArgumentRefusal {
  /** One of the five `ArgumentGapKind` strings — the union's declaring home is the
   *  vocabulary module, and this is an IMPORT of it, never a second copy. */
   ...
   *  `upstream_unreachable` and meaningless elsewhere. */
 ...
 *
 * ⚠ This is the difference between a detection set that follows the vocabulary and one
 * that drifts from it. A sixth kind added to the union makes `REFUSAL_FOR_KIND` a type
 * error until it gains an entry, and the moment it does, this set contains it — with no
 * edit here and nobody having to remember.
 */
const REFUSAL_CODES: ReadonlySet<string> = new Set(Object.keys(REFUSAL_FOR_KIND))
 ...
 * `named_failures` consumer owes. The second is the fence: `upstream_unreachable` without an
 * upstream, or any kind but `shape_unknown` without an argument, is an entry whose sentence
