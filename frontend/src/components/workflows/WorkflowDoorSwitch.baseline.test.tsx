 ...
 * rendered DOM of the tree as it stands at `CAPTURE_SHA` — `DoorHeaderStrip.tsx` and
 * `doorVocabulary.ts` both still nonexistent — by running `doorCapture` above and writing what
 * it produced into this file by SUBSTITUTION. Not one attribute here was typed from the
 * source, computed by hand, or reasoned about.
 *
 * OBSERVED TWICE on the unchanged tree before it was committed, and the two runs agreed byte
 * for byte — so it is a baseline rather than one sample of something that might vary. These
 * doors read no clock, no randomness and no measurement API, so a capture that differed between
 * two runs would be a REAL FINDING to report and not flake to re-roll.
 */
const DOOR_HTML_BASELINE: Record<string, string> = {
  CHOOSER_STANDALONE: "<div data-testid=\"workflow-doors\" class=\"flex h-full flex-col bg-background\"><div class=\"flex flex-col gap-1 border-b border-border px-6 py-4\"><h1 class=\"text-[18px] font-semibold text-foreground\">How do you want to build this?</h1><p class=\"text-[13px] text-muted-foreground\">Pick the fast path or full control — nothing is locked, you can switch anytime.</p></div><div class=\"grid min-h-0 flex-1 content-start gap-4 overflow-y-auto px-6 py-6 md:grid-cols-2\"><button type=\"button\" data-testid=\"door-card-describe\" class=\"flex flex-col gap-2 rounded-xl border border-border bg-card p-5 text-left transition-colors hover:border-primary/60\"><span aria-hidden=\"true\" class=\"text-3xl\">⚡</span><span class=\"font-mono text-[10px] uppercase tracking-wider text-muted-foreground\">loose · fastest path</span><span class=\"text-[16px] font-semibold text-foreground\">Describe &amp; run</span><span class=\"text-[13px] text-muted-foreground\">Say what recurring work this should do — the AI drafts the phases and sets the strictness.</span><span class=\"mt-1 text-[13px] font-medium text-primary\">Open <span aria-hidden=\"true\">›</span></span><span class=\"mt-1 text-[11px] italic text-muted-foreground\">nothing locked — switch to Author &amp; govern anytime</span></button><button type=\"button\" data-testid=\"door-card-govern\" class=\"flex flex-col gap-2 rounded-xl border border-border bg-card p-5 text-left transition-colors hover:border-accent-violet/60\"><span aria-hidden=\"true\" class=\"text-3xl\">🔧</span><span class=\"font-mono text-[10px] uppercase tracking-wider text-muted-foreground\">power · full control</span><span class=\"text-[16px] font-semibold text-foreground\">Author &amp; govern</span><span class=\"text-[13px] text-muted-foreground\">Open the full Builder — every advanced control, the live strictness tier, the locked judge.</span><span class=\"mt-1 text-[13px] font-medium text-accent-violet\">Open <span aria-hidden=\"true\">›</span></span><span class=\"mt-1 text-[11px] italic text-muted-foreground\">citation policy · gate set · per-phase scope &amp; model</span></button></div></div>",
...
describe("WorkflowDoorSwitch 193-01 — the pre-move rendered DOM, byte for byte", () => {
  ...
  it("both CHOOSER captures carry both door cards and both shipped door names", () => {
    for (const key of ["CHOOSER_STANDALONE", "CHOOSER_INLINE"]) {
      ...
      // The two shipped door names, as they appear in the rendered HTML (`&` is escaped).
      expect(html).toContain("Describe &amp; run")
      expect(html).toContain("Author &amp; govern")
  ...
  it("both GOVERN captures carry the return control, the locked judge and the shipped govern label", () => {
    for (const key of ["GOVERN_STANDALONE", "GOVERN_INLINE"]) {
      ...
      expect(html).toContain("Author &amp; govern")
      // The band's own words, which AUTH-01 moves into `doorVocabulary.ts`.
      expect(html).toContain("‹ both doors")
