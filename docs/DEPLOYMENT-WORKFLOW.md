...
| **A new app setting / seed row** | Settings live in `app_settings`/`user_settings`. A fresh cloud DB may be **missing seed rows** (`pg_dump --schema-only` skips data). Insert/verify the row in cloud (e.g. `app_settings 'global'`). **Migration 087 (`skill_creator_reborn`) is data-carrying** — the built-in `skill-creator` seed is NOT in `full-schema.sql`. After deploy, verify `SELECT count(*) FROM public.skills WHERE id='00000000-0000-0000-0000-000000000010' AND is_system=true` returns 1 in cloud; if 0 (env bootstrapped from `full-schema.sql`), apply migration 087 via the SQL editor (idempotent — safe to re-run). |
