...
A RAG-based AI agent platform where users organize documents into nested folders and interact with a customizable AI agent. The agent remembers preferences across threads, extracts tables and images from documents, shows confidence and citations, runs code in a sandbox, and can be taught new skills that persist. As of v2.7 the agent also keeps a per-thread workspace of versioned files, manages a todo list, can spawn sub-agents, and can pause to ask the user a question — all surfaced live in a collapsible right-side workspace panel. The Deep Midnight visual design delivers a glassmorphic, mobile-responsive experience with knowledge health dashboards and user feedback loops.
...
## Last Shipped: v2.7 Agent Workspace & Panel

**Shipped:** 2026-05-30 (3 days, 6 phases, 28 plans, 226 commits, ~52K LOC delta)

**Goal:** Gave the agent a persistent per-thread workspace and a visible right-side panel — the "Claude.ai artifacts" moment for a self-hosted, multi-provider agent platform. The agent can now keep a scratchpad of versioned files, manage a todo list, spawn sub-agents, and pause to ask the user a question, with all of it surfaced live in a collapsible panel.

**Scope brief:** `.planning/PRDs/v2.7.md` (drafted 2026-05-12, scoped to Themes A+C+D+H). 22 REQ-IDs Validated across 6 phases (083-088).

**Key deliverables:**
- Tool-dispatch chain extracted from `threads.py` into a registry-pattern `tool_dispatcher.py` (G-5 satisfied); 4 carried v2.6 bugs closed
- Per-thread workspace filesystem — write/read/list/delete/version/diff, hybrid inline-Postgres (≤256 KB) / Supabase Storage, FK-chain RLS, SSE write/delete events
- 3 new agent tools — `write_todos`, `task` (sub-agent: 1-level nesting + per-run/global concurrency caps), `ask_user` (human-in-the-loop pause/resume via the codebase's first Redis pub/sub)
- StreamsProvider SSE demux → dedicated Zustand stores; panel updates trigger zero chat message-list re-renders (PANEL-06)
- Right-side workspace panel — controlled open/rail/hidden state machine, todos, file browser with per-type preview, client-side unified-diff viewer, ask_user answer surface + chat↔panel seam
- WCAG 2.1 AA on all panel surfaces (vitest-axe gate, contrast fixed both themes, keyboard nav, focus rings)
- SEED-034 folded (text-only universal tool-use directive, eval-gated across 6 providers); D-17 gemini-3 thought_signature closed-verified

**Seeds consumed / planted:** SEED-034 folded; SEED-037/038/039 planted (087 panel deferrals — in-panel office/PDF viewing + download, artifacts unification, panel reliability). Carry-forwards to v2.8: title-gen live-verify (BUG-260527-01), Google secondary-model 404, per-provider tool-use gaps (eval script is the harness seed), chat-tool-card unification (BUG-260529-02).

## Current Milestone: v2.8 Harness Engine & Workflow Mode (in progress)

**Goal:** Give the agent a deterministic, auditable workflow runtime — locked ordered phases, dispatcher-enforced per-phase tool whitelists, and validation gates between steps — plus a Deep/Harness dual-mode toggle, built on v2.7's proven primitives (`tool_dispatcher`, `task`/`ask_user`, run-backed Redis streaming).

**Target features:**
- **Harness Engine** — state-machine runtime with 5 phase types (`programmatic`, `llm_single`, `llm_agent`, `llm_batch_agents`, `llm_human_input`), validation gates between phases, versioned/immutable-on-publish workflow definitions, and Postgres-persisted resumable phase state (survives uvicorn restarts)
- **Dual-mode UX (MODE-01)** — Deep Mode (today's free chat, default) ↔ Harness Mode (locked phases), mid-thread switch with workflow-lock; live phase timeline in the existing v2.7 panel
- **Iteration-cap "Continue" button (SEED-029)** — resume past the step cap instead of silently dropping tool calls; Harness Mode's per-phase `max_steps` is its natural home
- **Reliability/accuracy folded in** — cross-provider eval-harness regression gate (SEED-034, `scripts/eval_cross_provider.py`), `task()` global-concurrency fair-share for `llm_batch_agents` (SEED-036a), tool-count budget guard at `get_tools()` (SEED-035)
- **Polish riders** — sketch-first chat tool-card unification (BUG-260529-02 + timer/step-count + download) and evidence-first Anthropic cross-provider parity (summary-tail BUG-260514-02 + iteration-bloat BUG-260523-04); kickoff carry-forward UAT sweep (BUG-260527-01 title-gen, Google-404, download payload) + SEED-037 download `/gsd:quick`

**Scope discipline:** The 6-type **Plugin Contract** (PLUGIN-01..03), the PPTX reference plugin, and the `super_admin`/operator role tier are **deferred to a new milestone v2.9 "Plugin Contract & Extension System"** (slotted before v3.0 Skill Studio) — the plugin contract is cross-milestone load-bearing and a wrong contract is expensive to change, so it locks on real harness telemetry (per the v2.7 PRD's own Q-08 risk note; mirrors the v2.7 split). **G-5:** the `threads.py`/`agent_runner` extraction lands FIRST so the harness sits in a clean module, not bolted onto the 3,186-LOC file. Migrations renumber from the real head (**056+**) — the v2.7 PRD's 125-139 reservation is stale fiction from before the split.

## Current State

**Shipped:** v2.7 (Agent Workspace & Panel) — 2026-05-30 (6 phases, 28 plans, 226 commits, ~52K LOC delta)
**Current:** v2.8 Harness Engine & Workflow Mode — started 2026-05-30 (defining requirements). Plugin Contract deferred to v2.9.
...
### Active (v2.8 — Harness Engine & Workflow Mode)

*Milestone started 2026-05-30. Locked scope: HARNESS-01..03 (state-machine workflow runtime, 5 phase types, validation gates, resumable runs), MODE-01 (dual Deep/Harness mode), plus folded-in SEED-029 (Continue button), SEED-034 (eval-harness regression gate), SEED-035 (tool-count budget), SEED-036a (`task()` concurrency fair-share), and two polish riders (chat tool-card unification + Anthropic cross-provider parity). Requirements formalized in `.planning/REQUIREMENTS.md`; phases in `.planning/ROADMAP.md`. PLUGIN-01..03 + `super_admin`/operator role tier deferred to v2.9 (before v3.0). See D-v2.8-01.*
