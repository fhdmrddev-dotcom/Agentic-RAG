...
## Last Shipped: v2.6 Foundation — RAG Quality + Multi-Worker + Polish

**Shipped:** 2026-05-27 (16 days, 35 phases, 91 plans, 846 commits)

**Goal:** Built the production-ready substrate the next four milestones depend on — closed the user-observed PDF-DOCX extraction inconsistency, lifted the single-worker concurrency ceiling, hoisted the streams subscription surface out of `useMessages`, and absorbed six carry-forward seeds before they compound.

**Scope brief:** `.planning/PRDs/v2.6.md` (locked 2026-05-10, signoff 2026-05-12). All 6 PRD gates passed. 24 REQ-IDs Validated. 35 phases (068-082 including inserts).

**Key deliverables:**
- Per-aspect extraction dispatcher with swappable engines (camelot tables, pymupdf_full images, legacy text)
- Multi-worker uvicorn enabled (WORKER_COUNT=2) with D-PRD-12 ADR
- StreamsProvider context lift (useMessages < 100 LOC)
- 9 LLM providers integrated (OpenAI, Anthropic, Google, DeepSeek, Kimi, MiniMax, GLM, OpenRouter, Ollama)
- Live-execution UX refactor (RunCard + ToolCallPanel + Focus Mode)
- Settings architecture unification (DB-backed, settings_override.json eliminated)
- asyncpg pool in hot paths with token telemetry (runs.input_tokens/output_tokens)
- Confidence recalibration on post-071.3 extraction defaults
- Cross-cutting verification gate: 5/5 SCs GREEN, 24/24 REQ-IDs Validated
