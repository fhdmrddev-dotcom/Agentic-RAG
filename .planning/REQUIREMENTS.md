...
- [ ] **DEBT-06**: Phases **238, 240, 241** and **242-246** each receive an independent §6.3 review,
      ...
      **Still unmet (RE-DERIVED 2026-09-18, after the operator's ruling):** 239 `owed` · 241 key absent · 242 `false` · 244 key absent · 245 has no verification file · 251 `owed` · 252 `owed` · 253 `owed` — **eight rows**. ⚠ The clause is re-derived from each `*-VERIFICATION.md` on disk, never edited in place — 249 and 250 left this clause because their VALUE changed, not because anyone struck them out.
      **Already accounted for (RE-DERIVED 2026-09-18):** 238 `complete` · 240 `complete` · 243 `refused` · 246 `done` · **249 `refused`** · **250 `refused`** — **six rows**, and ⛔ not one of them may be named as owed again.
      ⭐ **249 AND 250 MOVED HERE ON 2026-09-18 BY AN OPERATOR RULING, NOT BY A REVIEW.** The operator adopted the drafted refusals (`249-REVIEW-REFUSAL.md`, `250-REVIEW-REFUSAL.md`), recorded the ruling as the answer to **`BUS-251`** and **`BUS-250`** — both now `[CLOSED]` — and claude ruled on nothing (D-05). ⛔ **ACCOUNTED FOR IS NOT REVIEWED**: both keep `verification_mode: self-verified`, no §6.3 review ran on either, and the accepted risk is that a builder read its own work. RE-OPEN: gemini reviewing either phase at ANY time voids its refusal. ⚠ **THE OPERATOR DELIBERATELY LEFT 251 / 252 / 253 OPEN** — `BUS-249` / `BUS-256` / `BUS-257` are still `[OPEN]` — so the available review capacity goes to the two security-bearing rows and the one carrying two live criticals. That is a graded decision, not an omission.
