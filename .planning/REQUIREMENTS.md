...
| REQ-ID | Phase | Status |
|---|---|---|
| SHIP-01 | 242 — Ship It | **Code-complete, UAT-owed.** Three arms, each measured: (a) migration **178** adds CHECK constraints on `multimodal_max_vision_calls` AND `vision_max_pages` (`d3eae092b`) — clamp proven at 1001→1000 and 0→1, NULL arm proven, re-run proven a no-op, all inside rolled-back transactions; **applied to LOCAL only, cloud is operator-owed**. (b) the Search tab sends **changed fields only** (`46292bb81`) — two planted defects driven RED, `SettingsPage.tsx` restored md5-identical. (c) the refusal **names the stored value** at all four bound sites, and can never turn a 400 into a 500. ⭐ **UAT DRIVEN 2026-09-11 — 4 of 5 rows PASS in a real browser** (`242-UAT-RESULTS.md`). Row 2 IS this requirement: with `1001` stored, changing only *Search breadth* sent `{"hnsw_ef_search":50}` → **200** — the save that used to return a 400 about images. ⚠ **Still unticked**, and only on its production half: **Row 5 (cloud) is owed to the operator**, and SC#1's words name the database production serves from. ⭐ **The class fix is executable, not prose:** `test_242_settings_bounds_have_schema_constraints.py` reds when a Python bound arrives with no CHECK, over an **EMPTY** allow-list |
