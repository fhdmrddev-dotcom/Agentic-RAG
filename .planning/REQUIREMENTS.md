...
- [x] **COLL-02**: The `template_input` resolver is scoped to the current run — a template uploaded in one run is not visible or accessible in another. (v3.1 STRETCH carry-forward; depends on Phase 120 COLL-01 run-scope seam.)
- [x] **SRH-01**: When a skill's script is non-Python (JS, shell, etc.), the agent surfaces an honest "cannot execute this skill type" signal rather than silently failing or narrating the code as if it ran. (v3.1 STRETCH carry-forward; DISC-01 Layer 1; SEED-044.)
...
- [x] **FND-01** *(foundation, added 2026-07-09 — operator pivot: foundation pass before more features)*: Run-lifecycle honesty — the chat run-state signal is authoritative and reflects backend reality in **both** directions: a genuinely-streaming run is visible + stoppable (Stop actually cancels), and a finished run finalizes with no phantom "running" / dead Stop. Postgres `runs.status` is authoritative; Redis `runs:active` is a derived mirror co-written with the status on every transition (register on start, ZREM on terminal), reconciled via fetch on reconnect per D-v2.5-03; the frontend derives running/Stop from `runs.status` (via get_snapshot), and `runs:active` + the reconciler + admin backpressure stay consistent with it (corrected root cause, c12ff264 / D-145-01/02). The run-lifecycle / `runs:active` logic is extracted OUT of `threads.py` into a dedicated, tested module (satisfies the overdue G-5 `threads.py` extraction). (BUG-260709-01 cross-provider repro — finished run → phantom running + dead Stop; live streaming run → `runs:active` empty → no Stop — plus BUG-260702-02 orphaned runs on restart. Pairs the SEED-094 run-end honesty already shipped in Phase 138; Phase 145.)
