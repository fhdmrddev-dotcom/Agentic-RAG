...
| AUTH-03 | 193 → **RE-OPENED → Phase 193.1 (inserted 2026-08-14)** | ⚠ **ANSWERED WRONGLY, not delivered.** 193 labelled and conditionally hid the **run-time** upload box; the requirement (rewritten above) is about attaching a template **when authoring**. The work 193 did on this is not wasted — the three-state predicate, the card mark and the hidden-box logic all remain correct — but the requirement is **not met** and must not be ticked. **The re-opened half now has a home: Phase 193.1 Template-First Authoring.** Two pieces have since landed OUTSIDE a phase and are inputs to it, not satisfactions of it: 193's own AUTH-03 frontend work (`000539a3` — the Builder can now WRITE a template descriptor, the draft-then-attach half) and quick task `260814-q5r` (`22244732` — `GET /workflows/{id}/template/placeholders`, plus the UAT proving a bound template's fields render). **What is still missing, and is the whole of 193.1:** the AI drafts BLIND to the template (`SEED-157` — `/generate` has accepted `template_placeholders` since Phase 103 and the frontend has never sent it). Do not tick this row until a draft is grounded in a real template's real placeholder keys. |
| RUN-01 | 194 |
| RUN-02, RUN-03 | 195 |
| AUTH-04 | 196 |
| AUTH-02 | 197 |
