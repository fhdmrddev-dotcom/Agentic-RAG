...
- [ ] **FLAG-01**: Operator can disable a misbehaving capability (per-feature kill-switches: web search, sandbox, self-improve, workflows) and enable a maintenance/read-only mode — built on the existing `app_settings` TTL-cached substrate, fail-closed, no new flag infrastructure
