...
- [x] **VIS-01**: Advanced/technical features (eval studio, model management, trigger tuner) are hidden from end users and visible only to operator-tier users — enforced at the API layer (not UI-only gating), with the visibility map defined per feature
