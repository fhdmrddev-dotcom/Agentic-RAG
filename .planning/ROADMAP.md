...
- [ ] **Phase 075.2: ToolCallPanel Dedup + Final Outputs Download Link** (INSERTED 2026-05-21) — Close 2 defects surfaced by Phase 075.1 Chrome MCP UAT: (1) BUG-260521-01 ToolCallPanel renders same tool_call_id as duplicate card on first tool of a run (cross-provider, frontend dedup-by-id audit + fix at `ToolCallPanel.tsx:538` + StreamsProvider tool_call_id stability check); (2) BUG-260521-02 pinned Final Outputs panel in `MessageItem.tsx` renders filenames as plain text — wire `<a href={file.url}>` with the per-cell link component for download affordance + visual consistency. Bonus: optional pickup of 075.1-REVIEW WR-01 (`onToolEnd` name+status → id matching) + WR-02 (two-probe getSnapshot race) as the same wave. (2 plans)
