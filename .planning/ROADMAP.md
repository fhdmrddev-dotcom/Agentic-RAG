...
- [ ] **Phase 076.2: Provider Streaming Parity + DeepSeek Full Integration** (INSERTED 2026-05-26) — Dedicated provider polish phase closing gaps from 076.1 live testing. **DeepSeek gaps:** (1) thinking mode full support — reasoning_content round-trip in agent loop in-memory message chain (SEED-032 Gap 1, groundwork from migration 050 + streaming accumulator already in place), (2) real-time UI reflection — tool panels don't render progressively during multi-tool runs (content appears all at once on completion, unlike Anthropic where each batch renders live), (3) sticky timer bar not reflecting accurate step/action state during DeepSeek runs, (4) polluted/leaked data appearing in UI during streaming (repeated narration or raw content). **Cross-provider:** (5) sub-agent/title/suggestion model resolution follows Settings active_provider instead of the chat's actual model — causes wrong provider's sub-agent to handle titles when user switches models without switching Settings, (6) Moonshot/MiniMax/Zhipu untested with real API keys — basic live smoke test needed. Absorbs SEED-032.
