...
| 187. Business Vocabulary + AI-Seeded Canvas | 29/29 | **In progress — gap-closure round 5 (waves 13-16).** Plans 01-25 shipped across rounds 1-4; `187-26` (GAP A, wave 13) landed 2026-08-04 — the loose door can now bind a knowledge base BEFORE the AI drafts, and the choice is proved to reach `generateWorkflow`'s `project_folder_id` on the request the client sends. `187-27` (GAP B, wave 14) landed 2026-08-04 — opening a draft now ISSUES a check, and until it answers the publish trigger is disabled with an honest sentence instead of claiming "the static checks pass · checked by the server" over a check nobody made. `187-28` (GAP C, wave 15) landed 2026-08-04 — `workflows.py`'s `_ROUTE_ASSIGNED_CODES` comment now states what the code does (a route-assigned verdict stays out of the SERVER publish gate but DOES gate the author's Publish CONTROL, deliberately per `BUG-260731-03`), and both halves carry a behavioural fence: the server half quantified over the whole set in a new `test_187_route_assigned_reach.py`, the client half pinned in the canvas suite; the source grep is ranked BENEATH the property as a regression pin. Zero behaviour change — comment-only diff, zero mount-cap spend. `187-29` (wave 16) landed 2026-08-04 — the three suites carrying round 5’s honesty estate (`DescribeKbPicker.test.tsx` 30, `ProblemsTray.test.tsx` 30, `verdictModel.test.ts` 29) are now pinned in `vitest-count-gate.cjs`, every number read from the gate’s own `actual` column over two agreeing pre-pin samples and each pin OBSERVED producing a `[count-decrease]` on a genuinely deleted `it(` block (P-28/P-29/P-30); pinned total 715 → 804 and the derived success line moved 18/18 → 21/21 on its own. `187-VALIDATION.md` gained the round-5 section as a PURE INSERTION with every figure carrying its command and every “before” derived at that commit, and the manual board grew 14 → 17 (M15 born-bound / M16 the first second / M17 the fast path still fast) with the operator’s seeded-rows caveat recorded as BINDING on each. **All 29 plans have now shipped, but the PHASE IS NOT COMPLETE — verification has not run, and all 17 manual rows are UNPERFORMED.** VOCAB-01/02/03 deliberately NOT marked — the orchestrator marks REQ-IDs at phase end, once behaviour is observable | - |
