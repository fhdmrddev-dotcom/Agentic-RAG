...
- [ ] `241-01-PLAN.md` — the measurement that can FAIL: replace the three Phase 230 artifacts in place, one home for `EVAL_PROBES`, both layers, honest miss, non-zero exit on unmeasurable, honesty guard on the canonical gate (wave 1)
- [ ] `241-02-PLAN.md` — the bench corpus: `recall_bench` on local Docker Postgres from `supabase/full-schema.sql`, real chunks verbatim, synthetic bulk by perturbation, tenant skew, teardown (wave 1)
- [ ] `241-03-PLAN.md` — the remedy as a PRODUCT CAPABILITY: migration 176, `hnsw.ef_search` + `hnsw.iterative_scan` as settings, the `SET LOCAL` seam, two controls on the shipped Retrieval card (wave 1)
- [ ] `241-04-PLAN.md` — the verdict: operator applies 176, schema regenerated, bench at scale, before/after local, cloud parity + cloud AS-IS, `241-VALIDATION.md`, `SEED-265`, ledger last (wave 2, operator-gated)
