...
## Prior Milestone Archive: v3.7 Workflow Product Completion
...
| 198 | Node Vocabulary (research-first) | 1/1 | Complete | 2026-08-24 |
...
| 200.2 | **(INSERT)** The Run Column Stops Repeating Itself | 6/6 | Complete | 2026-08-23 |
| 200.3 | **(INSERT)** Publish & Run Human-in-the-Loop Workflows | 2/2 | Complete | 2026-08-24 |
...
- [x] **Phase 192.2: Does This One Work?** — the library card answers *which of these is worth running*, not just *which is which* (LIB-06). ⚠ **INSERTED 2026-08-19 from sketch 179**, which is the first sketch in this project to RENDER the shipped component rather than redraw it — and rendering it **overturned the premise**: the card is not information-poor, it is nine rows deep, and it ALREADY answers LIB-05 on screen (`3 share this name · changed 2 days ago`). So this phase is **subtraction plus one field**, not a redesign. **G-2 SATISFIED — sketch 179, winner C.** ⚠ **G-5 on `WorkflowCard.tsx` (8/3/818) is UNDISCHARGED and this phase discharges it FIRST**, in the order D-01 requires — the presentation seam ships before the feature it makes room for. ✅ **CLOSED 2026-08-20 by the v3.7 close audit** — 5/5 SCs verified against the tree at `e0c57ef4`, not against a SUMMARY: `runFacts.ts`' four arms (`success`/`failure`/`never`/`not-by-you`, plus the honest `unknown`), the `_LAST_RUN_LATERAL_SQL` + outer-projection `has_any_run` on all four feeds (no per-card fetch), `cardFace.ts` as the extracted lead/defer decision (G-5 DISCHARGED), the six CUT atoms asserted absent on every face by `WorkflowCard.baseline.test.tsx` §D-03, and the emoji/system-vocabulary sweep. **Teardown paid**: `frontend/src/dev/` no longer exists and `main.tsx` is byte-clean. ⚠ **10 human-UAT rows stay OWED** (`192.2-HUMAN-UAT.md`, 0 of 10 driven) — closed as a DECISION under the G-7 rule that owed manual rows are legitimate, not as a claim they ran. ⚠ `BUG-260819-01` stays `open` and folds into the next library phase.

  **Plan progress: 11 / 11 executed · GAP-CLOSURE ROUND 1 COMPLETE — ALL 3 WAVES (`192.2-07`…`192.2-11`)** — Wave 1 (`192.2-01`, measurement-only) COMPLETE at base SHA `82dd2efd13459e419d8e6036cdae19fae9ee574b`. Gates at base, all recorded verbatim in `192.2-01-SUMMARY.md`: `tsc` **33 / 19 files** (pre-existing, **zero under `library/`**) · count gate **`total 4455 · failed 0 · pinned total 4328` — `OK — 92/92`** · library suites + `WorkflowsPage` **9 files / 501 tests / 0 failed** · backend **62 failed / 2289 passed** (SEED-056 rot; **all six workflow suites green**). ⚠ **The card's 17 resting atoms are inventoried as literal strings BEFORE any source byte changes**, and the finding that matters is structural: **D-03's six CUT atoms live in exactly TWO JSX nodes** — `<WorkflowSoul scale="card" />` and `<p data-testid="fork-consequence">` — so Wave 4's subtraction is a two-line deletion, not a rewrite of five renderers, and it must NOT touch `WorkflowSoul.tsx` (its `run`/`pub` consumers are out of scope). ⚠ **D-04 confirmed in source: `43 share this name · changed 2 days ago` ALREADY SHIPS — LIB-05 stays COMPLETE.** ⚠ **Two plan-text defects corrected under Rule 3**: the count gate lives at the **repo root**, not under `frontend/` (and the plan's `| tail` form made the module-not-found **exit 0**), and the mark map is **`FACE`**, not `ROW_FACE`. ✅ **All three hot-file triples re-derived and all three MATCH** (`WorkflowCard.tsx` 8/3/818 · `api.ts` 171/98/6174 · `api/workflows.py` 36/18/1984); **G-5 confirmed FIRING and UNDISCHARGED**, Wave 2 discharges it. ⚠ **CLAUDE.md's count-gate constants have rotted a FOURTH time** (carries `4170/4096/83`, measured `4455/4328/92`) — recorded here, **Wave 5 owns the same-commit sync**. **ZERO source files modified by Wave 1, verified four ways.** ⚙ **WAVE 2 COMPLETE — `192.2-02` + `192.2-03`, run in PARALLEL worktrees, merged at `00b81f63` / `7bd88426`.** ⚠ **G-5 on `WorkflowCard.tsx` is now DISCHARGED**: the lead/defer decision extracted to `cardFace.ts`, and the proof is the ORDERING — the characterization pin `WorkflowCard.baseline.test.tsx` was committed at `b1017d20`, ONE COMMIT BEFORE `cardFace.ts` existed (24/24 green), and `git diff --numstat` on it across the routing commit is **empty**. ⚠ **The business words were IMPORTED, not re-spelled** — `STATE_RUNNABLE`/`STATE_DRAFT`/`STATE_STARTER` already ship in `libraryVocabulary.ts:297-307`, so spelling them in `cardFace.ts` as the plan said would have created the exact second copy T-06 forbids; the consequence is stated rather than smoothed — **the starter word is `Shared starter`, not the plan's `Starter`**, and Wave 4 now picks in ONE place. `./cardFace.ts` was added to `LIBRARY_SUBTREE_PATHS` although the plan's `files_modified` did not name it: an unlisted module is swept by six fences **not at all**. `192.2-03` joined `last_run_at` + `last_run_status` onto all three feeds through one shared `_LAST_RUN_LATERAL_SQL` — **no migration, no column, no write** — and drove all three routes live over real HTTP with `response_model` in force (`/published` 120 rows, 32 populated / 88 explicit-null · `/starters` 3 · `/drafts` 80, 16 populated). ⚠ **`/starters` needed an owner-scoping signature change the plan did not name** — `list_starter_workflows` took no user at all, while five `is_system_global` published rows carry 20/15/11/7/1 real runs belonging to ONE user on a pool that bypasses RLS; it gained a keyword-only, `None`-defaulted, **fail-closed** `user_id`, and a cross-tenant probe on a real world-readable row proves the runner sees the facts and a stranger sees the row and two `null`s. ⚠ **`models/harness.py` was NOT modified — the plan's `files_modified` was WRONG**: both wire models live in `api/workflows.py`, and `WorkflowDraftRow` does not exist in the backend at all (it is the FRONTEND type name). ⚠ **Three pre-existing test needles were over-specified and were corrected beside their originals** — one pinned a TRAILING SPACE in `"AS token, updated_at "`, i.e. that `updated_at` was the LAST column, which that test never claimed. Gates post-merge, verdict lines read verbatim: `tsc -p tsconfig.app.json` **33, unmoved, ZERO under `library/`** · count gate **`OK — total 4506 · failed 0 · pinned total 4328 · 92/92`** · backend **62 failed / 2328 passed** (failure set identical to the Wave 1 baseline; `+39` passed is 192.2-03's own suite) · backend workflow + run-facts suites **162 passed / 0 failed**. **For Wave 4: `last_run_status` arrives RAW (a `switch` needs a total default arm), and an ABSENT key is a THIRD state distinct from `null` — D-08 forbids rendering either one blank or green.** ⚙ **WAVE 3 COMPLETE — `192.2-04`, run SEQUENTIALLY on the main working tree (4 task commits `8c5ec85b` / `1c90a1d9` / `4618c148` / `0d846456`).** The run truth now travels wire → `LibraryRow` → `runFacts.ts` → `CardFace`, and **STILL NO PIXEL CHANGED**: `WorkflowCard.baseline.test.tsx` passes with a `git diff --numstat` of exactly nothing against both `8c32d986` and the base SHA. ⚠ **`LibraryRow`'s two run fields are typed `string | null | undefined`, NOT the plan's `string | undefined`** — the neighbouring `updatedAt` normalizer deliberately collapses `null` into `undefined`, and doing the same here erases the difference between *the backend says there is no run* and *the backend never mentioned runs*, which IS T-13; both normalizers therefore pass the wire value **verbatim**, with no `??`. `runFacts(row, now)` returns a **three-armed** discriminated union — `ran` / `never` / `unknown` — where neither absence arm carries an `outcome` or a `when` member at all, so neither is *structurally* readable as a run. ⚠ **An unrecognised status resolves to `unknown`, never success**, via `hasOwnProperty.call` rather than `TABLE[key] ?? fallback` (which this repo has measured returning `[Function Object]`), and the suite drives `succeeded`, `COMPLETED`, `""`, the three IN-FLIGHT statuses and five inherited property names. ⚠ **`relativeChanged.ts` was SPLIT, not copied** — `relativeBand` is the same nine bands without the `changed ` prefix, so the fourth relative-time formatter in this repository was never written (T-16); its own suite passes **unedited**, which is what proves no band boundary moved. The five run words went to `libraryVocabulary.ts` (T-06), **not** into `runFacts.ts` and **not** borrowed from `runVocabulary.ts`, whose words are the CANVAS's and describe one step of a run being watched. ⚠ **THE APPROVED SKETCH ITSELF HAS THE T-13 BUG** — `dev/SketchLibraryCard.tsx`'s `runWords` claims a three-armed unknown in its comment and ships two arms plus a `Never run` catch-all; it was deliberately not copied. ⚠ **Two of this plan's own fences were wrong and were fixed by RUNNING them**: a source needle red on the docblock documenting its own rule (the 187-24 trap, fourth time in this subtree — now swept over comment-stripped source with a scoping control), and the inherited import-set regex **could not cross a line**, so it read TWO specifiers where the module has THREE. ⚠ **`libraryRow.ts` was the WRONG path for the normalizers** (they live in `libraryFilter.ts`; `libraryRow.ts` emits zero runtime code) — the THIRD wrong `files_modified` entry in this phase. Gates, verdict lines read verbatim from the **repo root**: `tsc -p tsconfig.app.json` **33, byte-identical to the base set, ZERO under `library/`** (measured after every task) · count gate **`OK — total 4574 · failed 0 · pinned total 4328 · 92/92`**, and the `+68` is exactly `52 + 11 + 5` with no residual · library subtree **12 files / 620 tests / 0 failed** · **no file under `backend/` or `supabase/` touched**. **For Wave 4: hoist ONE `now` per render into `cardFace(row, now)` — 107 rows each reading their own clock is P-1; and `libraryVocabulary.ts` measures `8 / 4 / 584` with NO ledger row, which Wave 5 owes.** ⚙ **WAVE 4 COMPLETE — `192.2-05`, run SEQUENTIALLY on the main working tree (4 task commits `e2ebbbfc` / `70b7863b` / `98b47c19` / `68f2ac71`).** ✅ **LIB-06 IS SATISFIED ON SCREEN**: the card renders sketch 179 variant C — a 3px run gutter, the NAME still leading line 1 with the version deferred to its right (D-02; variant B was not chosen), and **line 2 saying the run truth then the state in BUSINESS words**. Three rows sharing ONE name and differing only by their last run now say **three different things**, asserted as a mutual-distinctness property and RE-ASSERTED after every `class` attribute is stripped off the rendered DOM — so *colour is never the only carrier* is proved, not promised. ⚠ **THE SUBTRACTION LANDED AS EXACTLY SIX ATOMS AND THE DELTA IS PROVED RATHER THAN CLAIMED.** Wave 1's finding held exactly: the cut is **TWO JSX nodes**, `<WorkflowSoul scale="card" />` and the fork-consequence `<p>`. The characterization pin was **RE-BASELINED BY INVERSION** — each of the six flips from *asserted present* to **asserted ABSENT**, **not one assertion was deleted** — plus a new sweep asserting all six absent on all three provenance faces; the pin went `24 → 29` cases, i.e. it grew STRICTER while growing. ⚠ **THREE SURVIVING ATOMS CHANGED THEIR LITERAL AND NONE DEPARTED** (D-06 — the mark, the folder chip, the state word): an auditor counting *"exactly six left"* must count DEPARTURES, not edits. ⚠ **`WorkflowSoul.tsx` and `lib/phaseGlyph.tsx` are BYTE-UNTOUCHED** (`git diff --stat` → 0 lines): the soul is CONSUMED not owned and still renders at `scale="run"` / `scale="pub"`, and the phase-glyph map is TOTAL OVER PHASE TYPES — `icon-convention.md` §4 forbids a phase-type glyph as a category icon BY NAME, so **the plan's `files_modified` naming `phaseGlyph.tsx` was DECLINED rather than obeyed. That is the FOURTH wrong path in this phase's plans** (after `models/harness.py` and `libraryRow.ts`). The three marks became **`lucide-react`** icons — the house chrome set this very file already drew three marks from, so no fourth icon path was introduced. ⚠ **THE IDENTITY LINE MOVED FROM DOM POSITION 2 TO 3**, deliberately: D-01 numbers the run truth as line 2 and 179-C is SILENT on the order (it rendered no identity line at all). Five child-order assertions moved with it in the same wave — which is exactly what asserting placement by child order was FOR. **LIB-05 was NOT re-opened**: no field, word or resolver of `rowIdentity.ts` changed. ⚠ **TWO CONSEQUENCES ARE RECORDED RATHER THAN SMOOTHED.** (1) On a name-colliding row `resolveIdentity` picks the STATE AXIS as its discriminator, so `Ready to run` now renders **TWICE** — line 2 and the identity line — found by a card-wide `getByText` throwing *"Found multiple elements"*, not by reasoning; **not fixed here** (D-04 forbids re-opening LIB-05, `rowIdentity.ts` is outside `files_modified` with 72 pinned cases, and the card *invents no part and drops none*), re-open trigger: an operator reads the stutter on the real shelf. (2) The filter still matches on `purpose` but the purpose hero is gone, so **a search hit is no longer self-explaining** — pinned in that direction with the trade written beside it. ⚠ **THE FORK CONSEQUENCE MOVED INTO THE `⋯` MENU, IT DID NOT DIE** (T-21) — `aria-describedby` round trip intact, and it is a plain `<p>` so Radix's roving focus and typeahead skip it. **P-1 discharged**: ONE hoisted `now` reaches `cardFace(row, now)`, the same instant the page already hoists for `resolveIdentity`. Gates, verdict lines read verbatim from the **repo root**: `tsc -p tsconfig.app.json` **33, byte-identical to the base set, ZERO under `library/`** (measured after every task) · count gate **`OK — total 4594 · failed 0 · pinned total 4328 · 92/92`**, and the `+20` is exactly `15 + 5` with **no residual** · library subtree + `WorkflowsPage` **12 files / 640 tests / 0 failed** (`WorkflowsPage.test.tsx` held at **55/55** — no pinned file decreased) · **no file under `backend/` or `supabase/` touched**. **For Wave 5: `WorkflowCard.tsx` re-derives to `11 / 4 / 1104` against a ledger row reading `8 / 3 / 818` — a FIFTH stale/absent ledger obligation alongside `libraryVocabulary.ts` (`8 / 4 / 584`, no row at all) and the count-gate constants.** ⚙ **WAVE 5 COMPLETE — `192.2-06`, the phase's closing plan, run SEQUENTIALLY on the main working tree(4 task commits `b38764ca` / `23334f0d` / `079cca07` / `eb279c08`). ✅ ALL 6 PLANS EXECUTED.** ✅ **THE SKETCH SURFACE IS GONE, AND IT IS PROVED RATHER THAN ASSERTED**: `grep -rn"sketch-card|SketchLibraryCard" frontend/src` → **no match, exit 1**; `frontend/src/dev/` no longerexists; and `main.tsx` is **byte-identical to its pre-sketch shape** (`git diff 17a508d7 --frontend/src/main.tsx` → empty). The component and its route branch deleted **in one commit**, as thesketch's own teardown note required. ⚠ **TWO DOCBLOCKS STILL POINTED AT THE DELETED PATH AND WERE AMENDED IN THE SAME COMMIT** —`library/runFacts.ts` and `library/WorkflowCard.tsx`. **Nothing in this repository typechecks prose**, soa dangling pointer is exactly the class of wrong reference that survives every gate; neither was deleted,both now say where the surface went. **The sketch itself was NOT torn down** — it is the phase's G-2evidence — and its README now records the teardown as DISCHARGED with the original wording preservedbeside it, plus the fact that **the sketch's own `runWords` two-armed T-13 bug did NOT ship: the correctthree-armed resolution lives in `runFacts.ts`.** ⚠ **THE LEDGER DEBT WAS LARGER THAN ANY PLAN THOUGHT, AND THE GAP IS THE FINDING.** The plan namedSEVEN files; the phase's real diff names **TWELVE** non-test source files. **FIVE existing rows werestale** and are corrected BESIDE their originals — `WorkflowCard.tsx` `8/3/818 → 12/4/1105` ·`lib/api.ts` `171/98/6174 → 172/99/6227` · `db/workflows.py` `36/18/1656 → 37/19/1811` ·`api/workflows.py` `36/18/1984 → 37/19/2073` · `WorkflowsPage.tsx` `34/12/1176 → 36/14/1186` (itsFIFTH consecutive staling). **THREE rows were added because they were ABSENT, not because they werenew**: `libraryVocabulary.ts` **8/4/584** (flagged by Wave 3), and — **found by nobody until this pass— `libraryFilter.ts` 4/4/341** and **`libraryRow.ts` 3/3/162, sitting EXACTLY at the threshold.** Allthree were over/at G-5 with no row, so the guardrail could never have fired on them at any count. ✅ **`WorkflowCard.tsx`'s cell read *⚠ obligation UNDISCHARGED* for four phases and is now DISCHARGED(Wave 2), with the NEXT SEAM NAMED** — the `⋯` overflow menu and its armed-delete state machine —rather than marked `satisfied` with no successor, which is the exact state three of the five rows foundstale on 2026-08-17 were in. ✅ **`api.ts`'s 197 seam-DECLINE HOLDS and its re-open trigger did NOTfire** (`+53 / −0`, zero new runtime exports, measured by grep over the diff) — stated explicitlyrather than left as a silent cell. Four young rows added below the threshold on the `fileIcon.tsx`precedent (`cardFace.ts`, `runFacts.ts`, `relativeChanged.ts`, `main.tsx`), and the **five wrong`files_modified` paths in this phase's plan artifacts** are collected in one table because the RATE isthe finding — the fifth being this plan's own prompt calling `relativeBand` a MODULE when it is an**export at `relativeChanged.ts:103`**. ⚠ **CLAUDE.md's count-gate constants have rotted a FOURTH time and the correction is recorded BESIDEthe originals, never over them**: `4170 / 4096 / 83` (2026-08-17) → **`4594 / 4328 / 92`** — `+424`cases in **TWO DAYS**. 192.2's own arc is published with every increment attributed and no residual(`4455 → 4506 → 4574 → 4594`). ⚠ **A deliberate deletion is the ONE thing that can legitimatelydecrease the total, and it did NOT here — because no gated suite ever covered `src/dev/`. The gate couldnever have told anyone the dev surface had outlived its sketch.** The cap held at `2` on every run ofevery plan and was never adjusted. ✅ **THE G-4 ROWS EXIST BEFORE THE CLAIM**: `192.2-VALIDATION.md` carries **NINE** rows, every `result:`empty, including **U4 the deploy-skew unknown arm** (two concrete setups; ⚠ delete the KEYS, not nullthem) and **U5 *the resting card reads quieter*** — both chosen because the implementation cannottrivially satisfy them. **The 4-axis bandwidth is stated for EVERY axis including the three that do notapply, each with its reason** — cross-provider does not apply (no streaming, no agent loop, no providerrouting; the card neither reads nor branches on `provider`), and the caveat that WOULD make it apply iswritten down. **Both `192.2-05` findings are carried with concrete re-open triggers**, and the state-wordduplication is additionally FILED as **`BUG-260819-01`** (`status: open`, `surface: Agentic-RAG`) — arecord living only in a phase summary is invisible to every touchpoint that scans for open bugs. Gates at close, verdict lines read verbatim from the **repo root**: `tsc -p tsconfig.app.json` **33,byte-identical to the base set, ZERO under `library/`** · count gate **`OK — total 4594 · failed 0 ·pinned total 4328 · 92/92`** — unchanged by the deletion · library subtree + `WorkflowsPage` **12 files/ 640 tests / 0 failed** · `node scripts/check-claude-md-size.cjs` **exit 0, 77661 chars, 51.8 % oflimit, headroom 72339** · **no file under `backend/` or `supabase/` touched by this plan.** ⛔ **OWED AT CLOSE, stated as a DECISION and never as a claim that everything ran: all NINE G-4lived-experience rows.** They must be driven on the **real** library, by reading the screen — **never by`getElementById`** (D-27). **Run U4 FIRST** (the deploy-skew unknown arm): it is the row most likely tobe skipped and it guards the phase's central honesty claim, that a frontend deployed ahead of itsbackend never says *"Never run"* about a workflow that has run. **Then U1** — the phase's thesis. ⚙ **WAVE 1 COMPLETE — `07` + `08` + `09` in THREE parallel worktrees**, merged `59d68ba2` / `5e0f398e` / `4c4010f5`; zero cross-branch file overlap, zero deletions. ⚠ **The plan-checker caught a cross-plan fence collision BEFORE execution** (07 pins an EXACT literal count over `GUTTER_TONE`/`RUN_TONE`; 11 adds a sixth arm and named that fence NOWHERE, while running the whole subtree in its own verification) — fixed by giving 11 ownership plus a mandatory RE-DERIVE. **A SECOND, INTRA-WAVE instance of the same shape was briefed to both agents and HELD: 07's fence passes 11/11 against 09's modified `WorkflowCard.tsx` post-merge**, because 07 bounded its extraction brace-wise and 09 put its classes in a separate module constant — both proved with controls, not asserted. ⚠ **`?raw` CANNOT READ CSS UNDER VITEST — IT RETURNS THE EMPTY STRING, SILENTLY** (four forms measured, all length 0; `test.css` defaults to `false`, so the query is swallowed with no throw and no warning). **Any fence reading CSS that way defends nothing while staying green.** 07's caught it only because non-vacuity is asserted BEFORE contents. Swept repo-wide: no other `?raw` CSS reader exists, so it is a LATENT trap, not an active silent-green fence. ⚠ **08 proved its `EXISTS` with a REAL PLANT**: a deliberately miscorrelated `r2.definition_id = r2.definition_id` flips all four sites to 100% true (`28/28`, `120/120`, `3/3`, `80/80`) — the OPPOSITE lie to CR-01, and green everywhere — and reds three fences. Per-site correlation proved with a no-run row returning `false` at each projection, checked against `workflow_runs` directly rather than inferred from the projection under test. ⚠ **The plan's "2 of 3 starters" figure was corrected DOWN to 1 of 3 by measurement** (only `weekly-status-report` has ever run) — matching `192.2-03`'s existing `rows: 3 populated: 1`, so the correction moves TOWARD the prior evidence. ⚠ **A wrong `files_modified` is now SEVEN of this phase's EIGHT plans.** ⚠ **AN ORCHESTRATOR-SUPPLIED BASELINE WAS WRONG AND 07 CAUGHT IT**: *"library subtree 12 files / 640 tests"* was INHERITED from plan 05's report rather than re-derived at this base; measured it is **11 files / 585 tests**, and a plan trusting the stale figure would have read a 44-test DECREASE that never happened. Gates post-merge, verdict lines verbatim: count gate **`OK — total 4646 · failed 0 · pinned total 4328 · 92/92`** · `tsc` **33, unmoved, ZERO under `library/` or `WorkflowsPage`** · backend **62 failed / 2350 passed** (rot set unchanged) · backend workflow + run-facts **184 passed / 0 failed**. ⏸ **Seven hot-file ledger cells are OWED and deliberately DEFERRED to the round's close** — 09 re-derived them and found all four rows accurate at base with only line counts moving and NO phase count moving (so no new G-5 obligation); waves 2 and 3 touch the same files, so applying them now would go stale within the hour. ⚙ **WAVE 2 COMPLETE — `192.2-10`, the wire→row→word seam + WR-02, run SEQUENTIALLY on the main working tree (3 task commits `ff351da1` / `75952617` / `d568f434`).** `has_any_run` now reaches `LibraryRow.hasAnyRun` **three-valued**, the library owns `RUN_NOT_BY_YOU = "Run by someone else"` in ONE home, and the three docblocks CR-01 named stop asserting a caller-scoped `null` as a row-level fact — each **corrected BESIDE its original, never over it**. ⚠ **`api.ts`'s 197 seam-DECLINE HOLDS and the proof is THREE-WAY rather than the plan's single grep**, which the brief correctly called necessary but not sufficient: the complete NON-COMMENT added content of a `+131 / −1` diff is **two lines, both `has_any_run?: boolean | null`**; the ONLY removed line is a docblock terminator; and a token scan over every added line — comments included — for `export`/`function`/`const`/`let`/`var`/`class`/`=>` returns **nothing at all**, so no runtime value can be hiding inside an existing export statement. The re-open trigger did NOT fire. ⚠ **`RUN_NEVER`'s literal is BYTE-UNCHANGED and that is the point** — the word was always right; what was missing was its PRECONDITION, so the arm now fires only when the wire affirms `has_any_run === false`, and the review's suggested reword to *"You haven't run this"* stays declined (it makes every card caller-scoped to repair the few rows where the scope shows). ⚠ **THE PLAN'S OWN ACCEPTANCE GREP IS UNSATISFIABLE AND THAT IS RECORDED RATHER THAN SIDE-STEPPED**: `grep "?? false"` over `libraryFilter.ts` exits 0 on **the comment forbidding `?? false`** — the 195-07 trap, and the comment is load-bearing so it was NOT deleted to satisfy a criterion; the discriminating check strips comments first (**CODE-ONLY hits: 0**, exactly two bare `hasAnyRun: row.has_any_run,` assignments) and is now a test with a synthetic control. ⚠ **The self-satisfying-count trap MEASURED, not warned about**: the three new cross-reference docblocks discuss `last_run_status` by name, so a naive whole-file count reads **18** where the truth is **3** — the WR-02 fence is line-anchored, and the contrast is pinned as its own case. **Four RED drives against REAL plants, each with an md5-verified restore**: `?? false` at both normalizers (**6 failed**) · a FOURTH `last_run_status` in `api.ts` (**3 failed**) · one shipped marker deleted (**4 failed**) · a re-spelled word in `runFacts.ts` (**1 failed**). ⚠ Note which cases did NOT fire on the first plant: **`true` and `false` both survive a `?? false`** — only `null` and `undefined` are destroyed, which is exactly why `toBe` over truthiness was mandated. ⚠ **[Rule 2] THE FENCE WOULD NEVER HAVE RUN**: `src/lib/` has NO bare-directory `TARGETS` entry (two file-level lines only), so `apiRunFields.fences.test.ts` got a `TARGETS` entry **and** a `BASELINE` pin at **12** in its creating commit — `+42 / −0`, because ADDING a pin is not RAISING one. ⚠ **`files_modified` was CORRECT but INCOMPLETE BY FIVE** — three `tsc`-forced `LibraryRow` construction sites (the required field enumerated them, which is what requiredness is FOR), two test files Task 3's own acceptance needs (there is no `libraryVocabulary.test.ts` in this repo), and the count gate: **eight of nine plans** now. ⚠ **The plan's "D-08 arm 3 as amended" phrasing was DECLINED** — `RUN_UNKNOWN` already ships as arm 3, so the word is documented as the FOURTH arm rather than renumbering a shipped vocabulary. Gates, verdict lines read verbatim from the **repo root**: count gate **`OK — total 4675 · failed 0 · pinned total 4340 · 93/93`**, and the **`+29` is exactly `12 + 12 + 5` with no residual** · `tsc -p tsconfig.app.json` **33, unmoved, ZERO under `library/`, `WorkflowsPage` or `apiRunFields`** (measured after every task) · library subtree **12 files / 650 tests / 0 failed** · `WorkflowCard.baseline.test.tsx` **byte-unchanged, 29/29 unedited** · **zero files under `backend/` or `supabase/`, zero deletions across all three commits.** ⏸ **FIVE more ledger cells re-derived and OWED at the round's close** (`api.ts` **174/99/6357** · `libraryVocabulary.ts` **10/4/727** · `libraryFilter.ts` **6/4/435** · `libraryRow.ts` **4/3/201** · `vitest-count-gate.cjs` **102/18/3301**) — deliberately NOT applied mid-round, per 192.2-09 / 197-10. ⚠ **`192.2` was ABSENT from the count gate's phase-bucket list at base and this commit adds it (17 → 18 phases).** **For Wave 3: `RUN_NOT_BY_YOU` is EXPORTED AND UNCONSUMED BY DESIGN** — adding the fourth `RunFact` member makes `runGutterOf` non-total, a typecheck error on purpose, so `192.2-11` lands the union member and its two colour maps atomically; a test pins that `runFacts.ts` contains neither `RUN_NOT_BY_YOU` nor `not-by-you` today. **CR-01 is NOT closed on screen until `192.2-11` ships.** ⚙ **WAVE 3 COMPLETE — `192.2-11`, THE PLAN THAT CLOSES CR-01 ON SCREEN, run SEQUENTIALLY on the main working tree (3 task commits `9206f62c` / `b9fceff9` / `aa2b0029`). ✅ ALL 11 PLANS EXECUTED; GAP ROUND 1 COMPLETE.** ✅ **CR-01 IS CLOSED IN CODE**: `never` now fires ONLY on an affirmed `has_any_run === false`, `RunFact` gains a fourth arm `not-by-you`, and a row somebody else ran reads **`Run by someone else`** with **no occurrence of `Never run` anywhere on the card** — the regression asserted over the WHOLE card, not line 2 alone. ⚠ **The old behaviour was MEASURED, not inferred**: four pre-existing card cases went RED the instant the precondition landed, each reading *expected 'Not recorded|…' to contain 'Never run'* on a `status: null` row. ⚠ **DEC-11-B held exactly as the operator was shown it** — an ABSENT bit resolves to `unknown`, never `never`; compared with `=== true` / `=== false` and proved by a four-value matrix reaching THREE arms (a truthiness test would produce two and re-ship CR-01 for the stale-backend case), plus a `status: undefined` × four-value matrix proving rule 1 still short-circuits before the new field is read. ✅ **07's FENCE RE-DERIVED, NOT BUMPED, AND THE ARITHMETIC IS PUBLISHED**: `TONE_SHAPE` `{entries:10, unique:9, perMap:5}` → **`{entries:12, unique:10, perMap:6}`** — `entries` **+2** (raw occurrences, one per map), `unique` **+1** (only `bg-muted-foreground` is a NEW literal; `text-muted-foreground` is the THIRD occurrence of one already named), `perMap` **+1**. **A uniform `+2` guess would have landed on `unique: 11` — wrong while looking plausible.** Derived BOTH ways: by reading the two edited object bodies back, and confirmed against the fence's own failing diff (*expected [ 'bg-success', …(11) ] to have a length of 10 but got 12*). **The assertion is still EXACT** — no `>=`, no range, no removed check — and **the fence carries FIVE synthetic controls, not the two the plan named**; all five re-run and all five fire. ⚠ **WR-03's FIX IS `DropdownMenuGroup`, NOT `DropdownMenuLabel`, AND BOTH HALVES OF THE REVIEW'S OWN FIX BLOCK ARE REFUTED BY MEASUREMENT**: Radix's `Menu.Label` is a bare `Primitive.div` with **no `role` prop** (`react-menu/dist/index.mjs:356-362`), so swapping to it left the five-role sweep **still red** — `div[role=none]` where it had said `p[role=none]`, a renamed defect rather than a fixed one — **and** it merges `text-sm font-semibold` under our className, so the note would have rendered SEMIBOLD, a pixel change D-03 did not sanction. `Menu.Group` renders `role="group"` and nothing else: one node, permitted role, identical paint, `aria-describedby` round trip passing UNEDITED. ⚠ **AXE DID NOT CATCH WR-03 AND THE SUITE SAYS SO**: driven through the real trigger against the unedited `<p>`, `vitest-axe` / axe-core 4.11.4 passed **GREEN** under jsdom — so the fence that holds WR-03 is the explicit five-permitted-roles sweep over the menu's direct element children (driven RED **twice**, naming the offending node each time), and the axe case ships **labelled as a broad backstop** with its silence written into its own comment. ⚠ **A DEFAULT PARAMETER FIRES ON AN EXPLICIT `undefined`** — read out of a real red, and the SAME absence-destroying shape `192.2-10` recorded for `?? false`; every absent-bit case now builds its row directly and both helpers say they CANNOT express it. ✅ **`WorkflowCard.baseline.test.tsx` byte-unchanged and 29/29 UNEDITED** (DEC-11-D), `git diff --numstat` vs base empty. Gates, verdict lines read verbatim from the **repo root**: count gate **`OK — total 4690 · failed 0 · pinned total 4340 · 93/93`** · `tsc -p tsconfig.app.json` **33, unmoved, ZERO under `library/`** · library subtree **12 files / 665 tests / 0 failed** · with `WorkflowsPage` **13 / 724 / 0** · `check-gap-closure-rounds.cjs 192.2` **`G-7 clear`** (1 round, cap 2, no new capability in a closure round) · `check-claude-md-size.cjs` **exit 0** · **zero files under `backend/` or `supabase/`, zero deletions across all three commits.** ⚠ **THE UAT SCRIPT WAS AMENDED AND NOTHING WAS DRIVEN**: `192.2-VALIDATION.md` goes `rows: 9 → 10` / `owed: 9 → 10` with **`driven: false` and all ten `result:` fields EMPTY, grep-verified**. **U4's override key list was WRONG and would have PASSED WHILE BLIND** — it named two keys, the payload now carries three, and rule 1's short-circuit means a payload still carrying `has_any_run` STILL renders *Not recorded*, so the row would have gone green having tested two keys of three; corrected with the original preserved as superseded. **U10 added** for the fourth arm, with its honest limitation stated in the row (the operator is the one user who has run everything on this box, so the override is the practical instrument). ⏸ **The ledger cell for `WorkflowCard.tsx` re-derives to `15 / 4 / 1262` against a row reading `8 / 3 / 818` and was FLAGGED, NOT EDITED** — `CLAUDE.md` / `docs/HOT-FILE-LEDGER.md` are outside this plan's `files_modified` and the same-commit sync rule binds them together (the `197-10` precedent); G-5 still FIRES and its obligation stays DISCHARGED with the `⋯` overflow-menu seam still NAMED, because WR-03 is a one-node role swap and no second concern arrived. ⛔ **TEN G-4 lived-experience rows remain OWED to the operator** — run **U10 and U4** first; they are the two that guard the honesty claim from opposite sides.

  ⚙ **GAP-CLOSURE ROUND 1 — PLANNED 2026-08-19, 5 plans / 3 waves, NOT YET EXECUTED.** Verification scored **4/6** (`192.2-VERIFICATION.md`) and code review **1 critical / 4 warning** (`192.2-REVIEW.md`). **G-7 is CLEAR** — `check-gap-closure-rounds.cjs` reports `6 total · 0 gap-closure`, and **ROADMAP SC#1 is genuinely UNMET**, which is the one thing that justifies a round. ⚠ **Only ONE of the two recorded gaps is plannable.** Gap 2 (the nine G-4 rows, `driven: false`, 0/9) has **NO plan by decision**: no plan can close it, only an operator looking at the real 117-row library can, and substituting an automated proxy is the exact failure G-4 was ratified to stop. It routes to `/gsd:verify-work 192.2`.

  | Wave | Plan | Closes | Files |
  |---|---|---|---|
  | 1 | `192.2-07` | **WR-01** — `bg-warning`/`text-warning` compile to nothing, so the `stopped` gutter renders unpainted, visually identical to the arm its own docblock says is DELIBERATELY unpainted | `tailwind.config.js`, `index.css`, + a fence landed RED first |
  | 1 | `192.2-08` | **CR-01, backend half** — the existence-only `has_any_run` projection on all three feeds | `db/workflows.py`, `api/workflows.py`, `test_library_run_facts.py` — ⚠ `autonomous: false` as a **DB-serialization flag** (CLAUDE.md rule 4), not a human checkpoint |
  | 1 | `192.2-09` | **WR-04** — three chips + the search select on atoms D-03 cut from the card | `libraryFilter.ts`, `libraryVocabulary.ts`, `WorkflowCard.tsx`, `WorkflowsPage.tsx` |
  | 2 | `192.2-10` | the wire→row→word seam + **WR-02** (three lookalike `last_run_status` fields; one column, two wire names) | `api.ts`, `libraryRow.ts`, `libraryFilter.ts`, `libraryVocabulary.ts` |
...
- [x] **Phase 193: Authoring Doors + Template Placement** — the two doors are tellable apart before choosing; template supply has a findable home (AUTH-01, AUTH-03) ⚠ **CLOSED 2026-08-20 WITH A NAMED RESIDUAL** — SC#3 (template supply has a findable home) is VERIFIED and driven (`193-VERIFICATION.md` U4, byte-exact provenance line). **SC#1 and SC#2 are NOT verified and are not merely owed UAT: `SEED-156` is still measurably TRUE at `e0c57ef4`.** Re-measured this audit from the phase's own committed captures: `GOVERN_STANDALONE` still renders the describe screen — same `What recurring work should this automate?` heading, the same `describe-template-row`, the same `describe-hint` (*"the AI writes the steps, sets how strict it is"*) and the same `Write the first draft` button. **The govern door's promise appears nowhere on the govern door's first screen.** Phases 197, 199 and 200 all touched this surface and none took it. **Routed to `200.1` as a named line, not re-opened.**
...
- [x] **Phase 194: Stop a Running Workflow** — a run can be stopped mid-execution and reports `cancelled` honestly (RUN-01) ✅ **CLOSED 2026-08-20 by the v3.7 close audit — and SC#2's recorded FAILURE is REPAIRED, verified in source rather than inherited.** `194-VERIFICATION.md` scored 2/3 and failed SC#2 (*"the user pressed Stop, got 204, and the run reports `completed`"* — at `WORKER_COUNT=2`, ~half of all stops). At HEAD `finish_run` carries the terminal-status guard (`WHERE … status NOT IN ('completed','failed','cancelled') OR status = $2`, `db/workflows.py:2018`), and `complete_phase` carries the `IS DISTINCT FROM 'cancelled'` fence — so a later `completed` write can no longer replace a `cancelled` at either grain. SC#1 is the `StopControl variant="page"` mount at `WorkflowRunPage.tsx:1292` (194.1 plan 07, the fourth mount). ⚠ **TWO RESIDUALS, both routed forward, neither hidden by the tick:** **L-01** — the far-worker producer KEEPS RUNNING; the run now REPORTS honestly, the work does not STOP (needs a Redis cancel channel or an in-loop status re-read; `finish_run`'s own comment says so). **WR-04** — three non-owner callers (`admin.py` operator Kill, `admin.py` disable-user sweep, `workflows.py` delete cascade) drive `_cancel_run_internals`, so `runVocabulary.ts` says **"Stopped by you"** about a stop the reader did not make.
...
- [x] **Phase 195: Show the Deliverable** — a produced file is shown from the run surface, reusing the shipped file presentation (RUN-02, RUN-03) ✅ **CLOSED 2026-08-20 by the v3.7 close audit** — 3/3 SCs re-verified at `e0c57ef4` (`195-VERIFICATION.md` scored 12/12 and its method already refused the SUMMARYs). Confirmed surviving Phase 200's run-surface re-port: `WorkflowRunPage.tsx` still imports `FileRow` + `fileRowUtils`, still sorts `byNewestFirst` (`:562`), and still renders `COPY_DELIVERABLE_HEADING = "Files in this run's workspace"` (`:216`). ⚠ **SC#3 remains proved by test and UNEXERCISED live** — no run in the local DB has ever produced 2+ files, recorded at verification and still true. ⚠ **The file-only deliverables region is NOT a 195 gap** — 195's goal is explicitly *a workflow that produces a FILE*; the TEXT-answer arm is new scope and is `200.1` line 1.
- [x] **Phase 196: Registry-Backed Model Picker (canvas)** — a step's model comes from the live registry, never typed (AUTH-04)
- [x] **Phase 197: Guided Authoring** — drafting from a description guides the decisions that change the result (AUTH-02) ✅ **CLOSED 2026-08-20 by the v3.7 close audit** — 3/3 SCs verified in code (`197-VERIFICATION.md`, 11/11 plan must-haves). ⚠ **9 G-4 UAT rows authored, 4 PASS · 2 PARTIAL · 5 NOT DRIVEN** (`197-HUMAN-UAT.md`) — closed as a DECISION, not as a claim they ran. **Drive U4 first** (the requirement row on ≥2 providers; anthropic + openai are the owed pair). ⚠ SC#2's mechanical half (the fast door stays fast) passed by numstat; SEED-156 above is the standing reason the *chooser* half is still unproven.
...
**Goal**: A person who opens a finished run is told what it produced — whichever KIND of thing that
was — and while a run is going the live line looks live.
**Depends on**: Phase 200 (this repairs one of its criteria and finishes its run surface).
**Requirements**: RUN-04 (new — see `REQUIREMENTS.md`)
**Inserted**: 2026-08-20, by the v3.7 close audit. Two operator observations plus one defect the
audit measured while checking Phase 200's SC#3.
**Flags**: **G-5 fires** on `frontend/src/pages/WorkflowRunPage.tsx` (23 commits / 6 phases) and on
`backend/app/db/workflows.py` (42 / 20) — read their sections in `docs/HOT-FILE-LEDGER.md` before
planning. **G-2 does NOT fire**: sketch 200 is the acceptance bar and already exists
(`.planning/sketches/200-journey-interactive/screens/run-surface.html`, `acceptance_bar: true`) —
plan 3 ports its markup DIRECTLY, which is the loop Phase 200 proved and the derivation layer it
deleted. **G-7 does not apply — this is a phase, not a closure round**, and it says so because the
temptation to file it as one is exactly what G-7 exists to catch.

⚠ **THIS IS EXPLICITLY NOT A PHASE 195 GAP.** Phase 195's goal is verbatim *"A workflow that
produces a file shows it"* and all three of its success criteria are about files. It shipped what
it promised and its criteria are ticked. A run whose deliverable is a TEXT ANSWER is a capability
195 never scoped, so building it inside a 195 closure round would be the *"closure rounds smuggle in
features"* mechanism named in `CLAUDE.md` § G-7, which turned Phase 187 from 15 plans into 29.

---

### Why plan 1 exists: a defect the audit measured, not a refactor

**Phase 200 SC#3 is built, gated, green and structurally unreachable.** Measured 2026-08-20 against
the live DB, and the split is total rather than partial:

| Measurement | Figure |
|---|---|
| `workflow_phases` rows with a non-null `output` | **588** |
| …`jsonb_typeof(output) = 'string'` (double-encoded) | **527** |
| …of the **`completed`** rows specifically | **484 of 484 — every one** |
...
| `output.output_file` / `path` present | **60** |

**The mechanism, and it is migration 122's root cause one column over.**
`complete_phase` (`backend/app/db/workflows.py:1606`) binds `json.dumps(output)` into a `$2::jsonb`
parameter, on a pool where `dependencies._init_pg_connection` already registers a jsonb codec with
`encoder=json.dumps` on every pooled connection (D-073-06). Encoded twice ⇒ the column holds a jsonb
**STRING SCALAR**. `declared_phase_measure` (`backend/app/models/thread.py:48`) then opens with
`if not isinstance(raw, dict): return None, None` and degrades **silently**. `fail_phase` and
`record_phase_not_sent` bind the same way; the `object` rows are `pending` / `cancelled` /
`skipped`, written by a path that passes the dict directly — which is what makes the status split
diagnostic rather than coincidental.

⚠ **Nothing about the render is dishonest, and that is precisely why nobody caught it.** D-07's
absent arm does its job: no count, never a `0`, never a dash. The count gate is green, `tsc` is
green, both wire models are correct, and `FlowEdge`'s single-source rule holds. **This is the
Phase-118 built-but-unreachable lesson with a live mechanism** — and the reason it is plan 1 is that
the TEXT arm below reads `output` through the same door.

⚠ **The fix is a DECISION, not an obvious patch, and the planner must take it explicitly.** Two
shapes, and they are not equivalent:
  ...
  * **(b) repair the WRITER** — drop `json.dumps` at the three `$N::jsonb` bind sites and let the
  ...
  ⚠ **STATE.md's standing decision on the sibling columns is (a)-shaped and must be honoured or
  overturned in writing**: `inputs` (230 rows) and `workflow_definitions.definition` (261 rows) were
  DELIBERATELY left in the old shape because *"flipping a writer alone gives a table with two shapes
  in it."* `output` already has two shapes, so that argument does not transfer unexamined.

---

### The two things the operator named

**1 · The run surface shows the deliverable BY TYPE.** Today the deliverables region is file-only —
`WorkflowRunPage.tsx:216` heads it `Files in this run's workspace` and `:218` renders
`This run produced no files.` on the terminal empty arm. That sentence is TRUE and USELESS about a
run whose deliverable was a text answer, which is **the common case: 479 phase rows carry
`output.text` against 60 carrying a file — 8×.** The answer is already sitting in
`workflow_phases.output.text` on the final step; `GET /workflow-runs/{id}` already `SELECT`s
`output` (`api/workflow_runs.py:636`) and the serializer extracts only `_measure`, because
`WorkflowRunPhaseRead` declares no `output` field and `response_model` drops what is not declared.
**No migration. One narrow declared field.**

⚠ **ONLY `text` MAY SHIP, AND THE ROUTE'S OWN COMMENT IS THE STANDING ARGUMENT** (`:627-633`):
*"this jsonb carries field_map, citations and prompts … the client never sees a prompt."* Measured
key census on the same 588 rows: `citations` **314** · `source_refs` **314** ·
`similarity_scores` **254** · `sub_run_id` **239** · `field_map` **66** · `tool_call_id` **34**.
A `.select("*")`-shaped widening, or shipping `output` whole, is refused — the exposure stays
bounded by the declared field, exactly as `step_count` / `step_noun` are.

**Four arms, and each must be a distinct render** — the `runFacts` / `DecisionsList` / `phaseDuration`
finding for the fifth time: folding *"no files"* and *"nothing at all"* prints *"nothing happened"*
about a run that produced an answer.

| arm | what is true | what it says |
|---|---|---|
| file(s) | ≥1 workspace file | today's row list, unchanged |
| text only | `output.text` on the terminal step, no file | the answer, named as the deliverable |
| both | both | both, the file leading (it is the artefact) |
| genuinely nothing | terminal, neither | today's `This run produced no files.` — **re-worded**, since it is now one of four and no longer the only absence |

**2 · The run log's left column.** Ported from `screens/run-surface.html` markup DIRECTLY, not
derived from a change-log — the instrument failure that cost Phase 200 its first pass. Measured
sheet-vs-shipped:

| | sheet (`run-surface.html:202,230-234`) | shipped (`RunTranscript.tsx:429`) |
|---|---|---|
| width | `w-16` (64px) | `w-14` (56px) |
| size / weight | `text-data-sm` = **12px / 500** | `text-[11px]` mono, 400 |
| settled colour | `text-[#464651]` (`outline-variant`) | `text-muted-foreground/60` |
| **live row clock** | `text-indigo animate-pulse` | **no live treatment at all** |
| **spinner** | `animate-spin` glyph, 16px, indigo, beside the live line | **absent** |

⚠ **The live-row treatment is the substance of this item; the width and size are the smaller half.**
⚠ **`RunTranscript`'s two shipped REFUSALS are not reopened by this and must survive**: the live
line stays FULL-STRENGTH (the sheet's dim lines are in-flight NARRATION between bright RESULT lines,
and with one line per step that mapping inverts), and the sheet's per-line authored narration
(*"Connecting to Northwind CRM instance…"*) is copy this product does not have. **A spinner is not
narration** — it says *this is happening now*, which is a fact the product HAS and currently does
not show.
...
**Success criteria** (what must be TRUE):
  ...
  5. Gates hold: count gate `failed 0` with no per-file decrease (baseline at `e0c57ef4`:
     **total 5365 · pinned 5004 · 110/110**), `tsc -p tsconfig.app.json --noEmit` at its 33-error /
     ...
     migration unless plan 1 chooses one — in which case it is pasted into the SQL editor and
     `full-schema.sql` is regenerated, per `CLAUDE.md`.
...
- [ ] `200.1-01-PLAN.md` — **the read defect** (wave 1). Adds `phase_output_object` beside
      `declared_phase_measure` and routes the read through it (repair (a)); stops the three terminal
      `workflow_phases` writers double-encoding (repair (b)); takes **both**, with **no migration**,
      recorded in the source as `D-200.1-01` — the `inputs` / `workflow_definitions.definition`
      deferral is HONOURED with the reason it does not transfer written down. The string-scalar
      counterfactual is DRIVEN (returns nothing **without raising**). Closes with a blocking operator
...
- [ ] `200.1-02-PLAN.md` — **the deliverable by type** (wave 2). One narrow declared field,
      ...
      **ALLOW-LIST, not a deny-list** — the key census measured **eight live `output` keys absent from
      this ROADMAP's list of six**, so a deny-list is already incomplete on today's data. Proved by a
...
- [ ] `200.1-03-PLAN.md` — **the log's live line** (wave 1). Ported from
      ...
      `#A3A5FF` is EXACTLY this tree's dark `--primary`, so `text-primary` is a token match rather
