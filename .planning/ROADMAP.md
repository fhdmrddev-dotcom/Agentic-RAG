...
| 187. Business Vocabulary + AI-Seeded Canvas | 26/29 | **In progress — gap-closure round 5 (waves 13-16).** Plans 01-25 shipped across rounds 1-4; `187-26` (GAP A, wave 13) landed 2026-08-04 — the loose door can now bind a knowledge base BEFORE the AI drafts, and the choice is proved to reach `generateWorkflow`'s `project_folder_id` on the request the client sends. Remaining: `187-27` (GAP B, the never-ran fail-open), `187-28` (GAP C, the stale doc claim), `187-29` (the round-5 pins + record). VOCAB-01/02/03 deliberately NOT marked — the orchestrator marks REQ-IDs at phase end, once behaviour is observable | - |
