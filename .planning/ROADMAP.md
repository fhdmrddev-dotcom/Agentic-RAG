...
Plans:
- [ ] 188.1-01-PLAN.md — Wave 0: re-scope the twelve narrowed `?raw` negative fences to a subtree source that self-includes the two D-01 modules, and make SC#4's no-focusable-control walk non-vacuous (measured: both shipped renders are non-editable, so the layer never mounts) (wave 1)
- [ ] 188.1-02-PLAN.md — Wave 0: capture the `AFFORDANCE_SHAPE` geometry baseline from the SHIPPED tree, twice-agreed and order-independent, so SC#2 has something to be identical to (wave 2)
- [ ] 188.1-03-PLAN.md — The extraction (D-01): `editAffordance.ts` + `PlaneEditingLayer.tsx`, verbatim move of the two contiguous regions (311 L), exactly three import removals, the ESM-cycle fence with a positive control, both fences observed RED (wave 3)
- [ ] 188.1-04-PLAN.md — SC#6: the five WR-04 own-guards in the tree's own `Object.prototype.hasOwnProperty.call` spelling + WR-07 `encodeURIComponent(runId)`, six falsifications each observed RED first (wave 4)
- [ ] 188.1-05-PLAN.md — Close: flip the G-5 ledger row (correcting its stale 1574 L → measured 1593), register UF-1, pin the D-02 lazy-import debt, drive operator rows M1–M3 (wave 5)
...
| 188.1 (INSERTED). WorkflowCanvas Extraction Refactor | 0/5 | **Planned 2026-08-05** — 5 plans, 5 waves. Measured at planning: `WorkflowCanvas.tsx` **1593 L** (the ROADMAP's 1574 was stale), `eslint src/components/workflows/` **6 errors** (D-01's gate is 6→5, not 2→1), `tsc -p tsconfig.app.json` **33**, count gate **2462/2462 zero slack**. Two findings the inputs did not carry: SC#4's no-focusable-control assertion is **vacuous today** (both shipped renders are non-editable, so `PlaneEditingLayer` never mounts — Plan 01 closes it), and `PlaneEditingLayer` also needs `CANVAS_LAYOUT`, which research's import-closure table omitted (harmless — `canvasModel` was already in the verified closure). — **G-5 debt owed from 188.** The ledger records this extraction as due in Phase 188; 188 honoured its budget by measurement (13 ins / 2 del vs a ≤15/≤4 cap), which capped the diff without reducing the 1574 L file. Inserted 2026-08-05 so 189 adds its node type to a paid-down file. Behaviour-preserving: **no new capability**, lean UAT (prove *unchanged*). Live constraint from the 185-10 seam: `WorkflowCanvas` imports `FlowEdge`'s VALUE at module scope for `edgeTypes`, so the extracted module must NOT import back (ESM cycle). Also carries the three bounded security fixes owed from 188's audit — WR-04 `own()` ×5, WR-07 `encodeURIComponent(runId)`, UF-1 register `last_workflow_run_id` | - |
