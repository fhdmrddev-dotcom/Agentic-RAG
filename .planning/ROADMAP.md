...
**Plans**: TBD via `/gsd:plan-phase 066` — likely 4–5 plans:
  - 066-01: Backend lifecycle split — extend `runs.status` CHECK constraint to include `timed_out`; new SQL migration; update Pydantic `MessageResponse.run_status` Literal; backend writes `timed_out` on budget fire vs `cancelled` on user-DELETE; capture `runs.error` on every non-completed terminal state.
  - 066-02: Backend timeout machinery — replace `asyncio.timeout(run_hard_timeout_seconds)` total-deadline with per-LLM-call budget that resets on each tool-call boundary; OR raise the total to 600s default (decision in plan-phase). Add `LLM_CALL_TIMEOUT_SECONDS` setting if going with per-call. Producer task survives consumer disconnect (re-validate Phase 061's shielding still holds).
  - 066-03: Frontend lifecycle UI — render distinct banner for `timed_out` runs ("Agent reached time limit. [Resume]") vs `cancelled` runs ("Response stopped"). Resume button on `timed_out` re-fires the original prompt with full conversation context.
  - 066-04: Backend integration test — synthetic slow-agent fixture that deliberately exceeds the budget; assert `runs.status='timed_out'`, `runs.error` non-NULL, no `GeneratorExit` exception leak in logs, SSE consumer receives `timed_out` terminal event.
  - 066-05: Live UAT — re-run the user's Gap-006 prompt ("search for research authored by Fahed Mrad → professional short report → charts/diagrams → docx") end-to-end; verify completion + observable LangSmith trace clean.
**Risks / pitfalls**:
