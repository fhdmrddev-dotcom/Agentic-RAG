...
| 189. Governed External-Action Node Model | 0/? | Not started — **UNBLOCKED 2026-08-07.** The G-5 gate that held it is discharged: 188.2 shipped the `PhaseNodeCard.tsx` refactor (797 → 274 L) and the `CLAUDE.md` hot-file ledger row now reads *satisfied (188.2 — 2026-08-07)*, so a discuss-phase scan of 189's `files_modified` no longer matches a G-5-firing row on this file. 188.2 is complete on all four gates. **Two items ride into 189's discuss-phase:** `BUG-260807-01` (the unguarded WR-04 sink in `editAffordance.ts`, filed by 188.2's secure-phase) and 188.2's two owed manual UAT rows (A2 ⛔ needs a live run; A1's visual half) — neither blocks planning. `189-CONTEXT.md` is already locked. | - |
