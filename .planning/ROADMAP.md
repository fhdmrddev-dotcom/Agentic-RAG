     ...
     THREAT MODEL MANDATORY             ONE family, proving the seam
        +------------+---------------+
              ...
              233  THE PREVIEW                       no migration - G-2 SKETCH
                     v
              234  THE WATCH LOOP + FIRST REAL SYNC  migs 155-158
                     |  RETIRES THE CLAUDE.md MANUAL-UPLOAD-ONLY RULE, SAME COMMIT
                     |  THREAT MODEL MANDATORY
                     v
              235  THE SOURCE SAYS WHAT IT DID       mig 159 - G-2 SKETCH
                     v
              236  THE CORPUS UNDER ATTACK           no migration - GA gate
                     v
              237  ONE RULE ENGINE                   mig 160
        +------------+---------------+
        v                            v
238  MICROSOFT GRAPH             239  ANY MCP SERVER WITH FILES
     if not SMALL, 232 was wrong      mig 161 (reserve) - rows-not-code proof
        +------------+---------------+
                     v
              240  MAIL IS A SHAPE                   mig 162 - own discuss-phase, LAST
                     v
              241  RECALL AT CORPUS SCALE            mig 163 (reserve) - HARD
```
...
**UI hint**: yes
**Migrations**: **154** — `154_connection_scoped_visibility.sql` (`ingest_visibility` + `source_connection_id` + the four-site widening, **in one transaction**).
...
**UI hint**: yes
**Migrations**: **159** — `159_connector_sync_runs.sql` (per-source run history: counts, errors, completeness).
...
**UI hint**: yes
**Migrations**: **160** — `160_classification_rules_scope.sql` (`rule_scope` discriminator + the `SOURCE_FIELDS` whitelist).
...
**UI hint**: yes
**Migrations**: **161** reserved — the per-connection tool binding stored as **data on the watch row** (skip the number if it fits on an existing column).
...
**Migrations**: **162** — `162_documents_thread_key.sql` (the retrieval-grouping column and its index).
...
**UI hint**: no
**Migrations**: **163** reserved (index parameters / a filtered-search index; skip the number if none is needed).
...
| 231 | **154** | `154_connection_scoped_visibility.sql` — the four-site widening + `ingest_visibility` + `source_connection_id`, **in one transaction** |
| 232 | — | none expected |
| 233 | — | none expected |
| 234 | **155 · 156 · 157 · 158** | `connector_watches` · `connector_watch_items` · `documents_source_state` · reserve (audit action types / freeze state) |
| 235 | **159** | `connector_sync_runs` |
| 236 | — | none expected |
| 237 | **160** | `classification_rules_scope` |
| 238 | — | none expected |
| 239 | **161** (reserve) | MCP tool binding on the watch row |
| 240 | **162** | `documents_thread_key` |
| 241 | **163** (reserve) | filtered-search index parameters |
...
|-------|----------------|--------|-----------|
| 228. v3.9 Closeout — The Debt Gets a Number | 0/? | Not started | — |
| 229. The One Ingest Splice | 0/? | Not started | — |
| 230. The Durable Ingestion Queue | 0/? | Not started | — |
| 231. Connection-Scoped Visibility | 0/? | Not started | — |
