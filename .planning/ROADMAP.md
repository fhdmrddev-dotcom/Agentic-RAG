...
**Plans**: 5 plans expected (Plan 01 stream-close-on-timeout fix; Plan 02 context-aware in-flight UI affordance; Plan 03 agent agentic-behavior overhaul system prompt; Plan 04 skill-load UX clarity; Plan 05 closing UAT — re-runs synthetic-timeout protocol + Fahed Mrad multi-tool pipeline). Plan 05 is `autonomous: false` (Chrome MCP + Supabase MCP + LangSmith MCP human checkpoints).
**Risks / pitfalls**:
