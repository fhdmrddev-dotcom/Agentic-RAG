...
- [ ] 095.1-06-PLAN.md — GAP-1 (WORKSPACE-PARITY): WorkspacePanel.hasActivity omits the derived selector → the activity-derived panel is structurally unreachable on the no-write_todos parity case. Include useDerivedPanel in hasActivity so TodosSection mounts; guard test renders the REAL panel+TodosSection through the gate (the path the unit tests bypassed). [gap-closure]
- [ ] 095.1-07-PLAN.md — GAP-2 (RUN-HONESTY): live attribution lag + turn divergence (live=iterationCount+1 vs reload=1, no provider·model live). Surface the already-resolved model/provider onto the dispatch response → stamp the live message; reconcile the run-sub turn to a reload-stable value so live==reload. Additive only — shared SSE/chunk path byte-identical (grep-proven). [gap-closure]
...
| 095.1 Cross-Provider Run Honesty & Workspace Parity | 5/5 | All plans complete ✅ 2026-06-06 — ready for /gsd:verify-work (095.1-01..04 shipped; 095.1-05: MessageItem honesty — D-06 flat Generated-files list (hero crown/split removed, is_hero written-but-unread, no backend change) + D-07 deliverable-aware Resume (!producedDeliverables, BUG-260518-01 CLOSED); finalOutputs 11/11, tsc baseline 37, vite build 0, dangerouslySetInnerHTML==0). WORKSPACE-PARITY + RUN-HONESTY + PROVIDER-ERR all delivered | - |
