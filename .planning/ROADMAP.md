...
| 206.3 | **(INSERT)** A workflow that reaches outside can be published | The publish gauntlet's golden run stops swallowing its own exception, names what actually failed, and a workflow carrying an external_action step reaches `published: true` — **starting with a re-diagnosis, because the planted cause was refuted** | CONN-02 (follow-up), CONN-03 (follow-up) | Planned — **registered 2026-08-25 from `SEED-201`, operator decision.** ⚠ The seed's causal story is REFUTED; scope the re-diagnosis before the fix |
...
- [ ] **Phase 206.3 (INSERT): A workflow that reaches outside can be published** — ⚠ **REGISTERED 2026-08-25 FROM `SEED-201`, BY OPERATOR DECISION — AND THE SEED'S OWN DIAGNOSIS IS REFUTED, SO THIS PHASE BEGINS BY RE-DIAGNOSING, NOT BY FIXING.** Driven in real Chromium 2026-08-25: an MCP-shaped `external_action` draft pressed through `◆ Publish` returned `published: false`, `blocked_stage: golden_run_error`, `named_failures: ["golden run could not complete: None"]` — the cause is literally the string `None`, so **even the named failure names nothing**. ⚠ **DO NOT CONFUSE THIS WITH THE STAGE-2.6 BLOCKER `206.2-01` FIXED** — that one WAS about MCP, and it is CLOSED and proved end to end (`✓Grounding` appears in this very verdict, with the MCP step present). This blocker is one stage later. ⚠ **THE PLANTED CAUSE — *"every `external_action` step always stops for approval, so every such workflow is unpublishable"* — WAS REFUTED AT PHASE 206.2's VERIFICATION** by a passing shipped test: `backend/tests/unit/test_publish_service.py::test_v20_an_external_action_workflow_publishes` drives a **real armed checkpoint** through a **real golden-run ctx** for a **capability-shaped** step and asserts a successful publish, and a sibling test is a deliberate fence against exactly the hang the seed describes; **both pass on the current tree.** Only the MCP shape was ever actually driven. The real cause is very likely MCP-specific and **swallowed by a broad `except Exception`** in `backend/app/services/harness/publish_service.py`'s golden-run path — an exception-swallowing bug that **predates this entire feature** (Phase 102-09, commits `78aad9ee` / `18eff014`). ⚠ **DO NOT WEAKEN THE D-04 / Phase-185 APPROVAL CHECKPOINT** (*"this step reaches outside your workspace, so it always stops and asks you first — that cannot be switched off"*): the evidence says it is not the blocker, and trading a publish bug for a safety one is the wrong direction. ⚠ **A SECOND, SEPARABLE DEFECT the same drive found:** the verdict reports `golden_run_id: null` beside rendered copy claiming *"this blocked before stage 3, so there is no run to open"* — **both halves are false.** It blocked INSIDE stage 3 and the run exists (`5d67d004…`, still `active` minutes later). The person is told there is nothing to look at, about the one artifact that would explain the failure. **This half should be fixed regardless of which route the main fix takes.**
