...
- [ ] 092-07-PLAN.md — Gap-closure (F4, 6 tasks, planned 2026-05-31): one id-routing fix family across 3 facets — A) sub-agent `parent_run_id` FK (add `producer_run_id` to the engine ctx for `runs.parent_run_id`, fail-closed guard), B) SSE routing (thread `stream_run_id` through `run_workflow` + `_run_phase_with_gates` → every `_emit`, + the `ask_user_prompt` emit transport — events reach the watched producer stream), C) resume + Continue (BOTH resume ctx sites mint + terminalize a NON-NULL producer `runs` row; repair the post-reload Continue-404 owner-scoped at `/runs/{id}/continue`). + live-DB FK regression test + the binding 4-axis native-7 UAT gate (re-runs 092-06 rows 4-10: SC#2 natural/Cancel + SC#3 + SC#5 + CONT-01 + native-7 + ask_user render + resume×Continue + Deep byte-identical). Additive + harness-scoped; Deep stays byte-identical (wave 1; UAT task autonomous:false)
