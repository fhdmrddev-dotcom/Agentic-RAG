...
- [x] **Phase 239: Any MCP Server With Files** — a file-serving MCP server becomes a watchable source by rows, not code (SRC-04) *(⚠ **BUILT 2026-09-08, 3 plans, NOT CLOSED.** Adapter resolves by PROTOCOL with no vendor branching above `adapters/`; tool-name variance lives in `config["source_tools"]` as data; **no migration — reserved 174 unused**. Backend gate diffed as a SET: 71→71, zero new. ⭐ Wave 3 caught a **FALSE GREEN in its own plan** — `Boolean(mcp_server_url)` would have printed "✓ Ready as source" on rows the server resolves no adapter for. ✅ **SC#2 MET 2026-09-09** — driven against GitHub MCP on a public repo, zero-code proven by commit hash (HEAD identical, 0 code commits, clean tree); browse/recursion/preview all 200. ⛔ **G-4 UAT row OWED**; `BUG-260907-01` stays `folded`, not `closed`, until it runs. ⛔ **No agent reviewer — `OV-239-01`**; `/code-review ultra` is the only independent gate.)*
