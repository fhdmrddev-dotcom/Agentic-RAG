...
- [x] 149-12-PLAN.md — Suggestion think-leak: strip `<think>` reasoning blocks before the line-parse so compat-model chips are clean questions (Test 7 minor)
...
- [x] 150-01-PLAN.md — Cipher foundation: secret_cipher (MultiFernet + enc:v1: envelope + sweep/status helpers), env var, dependency, unit tests
- [x] 150-02-PLAN.md — Migration 100: add the 10 missing app_settings secret text columns (D-150-08); apply live + regenerate full-schema [BLOCKING]
- [x] 150-03-PLAN.md — Encrypt/decrypt seams in user_settings.py (write + read, fail-soft to env) + SC#1 ciphertext-at-rest proof
- [x] 150-04-PLAN.md — Lifespan key-validation (hard-fail) + eager idempotent sweep + D-150-07 failed-save HTTP 500
- [x] 150-05-PLAN.md — Control Plane secrets_encryption tile (D-150-02): additive /admin/backpressure block + HealthSignals tile
...
- [x] 151-02-PLAN.md — D-10 migration: `skill_files(skill_id, filename)` unique index (author + BLOCKING operator apply, enables D-07 upsert)
- [x] 151-03-PLAN.md — SC#3 upload allowlist widen: `validate_upload` + lockstep `TemplateUpload.tsx` accept= (D-09)
...
- [x] 152-02-PLAN.md — WFIN-03 delete cascade backend: FK-safe cascade + delete-preview helpers + distinct cascade route + cancel-first (D-08, api/workflows.py)
...
- [x] 153-02-PLAN.md — Citation interaction foundation: nav context (useCitationNav.openDocument, owner-scoped) + App/IngestionPage wiring + marker↔row flash contract + additive citation CSS [CITE-01]
...
- [x] 153-04-PLAN.md — CitationPeek hover-peek → click-to-pin popover (chunk + full-doc variants, calm degrade, a11y pin/Esc) [CITE-01]
...
- [x] 155-07-PLAN.md — Opacity-offender sweep, remaining clusters (chat / studio / settings / ingestion / layout long tail)
...
- [x] 155-06-PLAN.md — Run modal + Citation UI + 154-surface a11y suites
...
- [x] 159-03-PLAN.md — Filter-toggle persistence: migration 103 + settings readback chain, operator-gated apply (D-159-04)
