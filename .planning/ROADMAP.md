...
| 187. Business Vocabulary + AI-Seeded Canvas | 27/29 | **In progress — gap-closure round 5 (waves 13-16).** Plans 01-25 shipped across rounds 1-4; `187-26` (GAP A, wave 13) landed 2026-08-04 — the loose door can now bind a knowledge base BEFORE the AI drafts, and the choice is proved to reach `generateWorkflow`'s `project_folder_id` on the request the client sends. `187-27` (GAP B, wave 14) landed 2026-08-04 — opening a draft now ISSUES a check, and until it answers the publish trigger is disabled with an honest sentence instead of claiming "the static checks pass · checked by the server" over a check nobody made. Remaining: `187-28` (GAP C, the stale doc claim), `187-29` (the round-5 pins + record). VOCAB-01/02/03 deliberately NOT marked — the orchestrator marks REQ-IDs at phase end, once behaviour is observable | - |
