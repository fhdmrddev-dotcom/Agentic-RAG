...
- 🚧 **v3.5 UX Consolidation & Chat Polish** — **CORE Phases 174-177 + STRETCH Phases 178-180** (started 2026-07-22; roadmap created 2026-07-22). Clear the parked `surface: Agentic-RAG` chat-surface bug backlog + consolidate the accumulated UI/UX rough edges (incl. the new v3.4 org surfaces) into one coherent, honest experience. A Medium cleanup milestone — bug-fix + polish, no large net-new build. **Phases 169-173 RESERVED** for the deferred v3.4 STRETCH carry-forwards (`.planning/v3.4-STRETCH-CARRYFORWARD.md`) — not reused here.
...
- [ ] **Phase 174: Run-State & Lifecycle Honesty** — no empty/orphaned cancel-kill bubbles, stop indicator survives nav+reload, "setting up agent" shows live activity, workflow-run timers/avatars stay accurate on nav (STATE-01..04)
...
- [ ] **Phase 176: Chat Render Correctness + Exec Reliability** — one user bubble, un-folded final answer, no silent send-drop, live version-pointer, reliable `execute_code` library install (RENDER-01..04, EXEC-01)
...
- [ ] 174-01-PLAN.md — STATE-03 pre-answer reasoning honesty (`outerBannerLabel` reasoningActive → "Reasoning…") [Wave 1]
- [ ] 174-02-PLAN.md — STATE-01a + STATE-02 verify-and-close (cancelled-no-output + stop-indicator reload-derive) [Wave 1]
- [ ] 174-03-PLAN.md — STATE-01b killed-workflow amber block (403 catch branch + composer unlock) [Wave 2]
- [ ] 174-04-PLAN.md — STATE-04 workflow-run timer anchor + single avatar (startedAt stamp + pre-runId dedup) [Wave 3]
**UI hint**: yes
...
| 175. Cross-Provider Streaming Fidelity | 0/? | Not started | - |
