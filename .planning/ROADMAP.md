...
- [x] **Phase 236: The Corpus Under Attack** — an adversarial corpus that fails when a defence is removed, driven on the full native roster, with the payload planted in a real synced document (TRUST-02) *(⚠ **CLOSED ON AN OPERATOR DECISION, NOT ON SC#1.** MET: SC#2 — 8/8 mutations caught at the point of USE, re-driven by the reviewer — and SC#3. ⛔ OWED: **SC#1, this phase's GA-GATE criterion**, plus SC#10's behavioural half — today's evidence is offline tests and a fully mocked rehearsal, so nothing is synced and no model is called. First drive: Google Drive, live connection + live model creds, `236-VALIDATION.md` §3 Step 4B. **Re-open trigger: the moment the live drive runs; if it has not by `/gsd:complete-milestone`, TRUST-02 is NOT delivered.** Decision record: `STATE.md` § ⚖)*
