...
| **245** | ⚠ **CORRECTED 2026-09-13 (Phase 245) — NOT BLOCKED, and was not blocked when this table was written.** The row above is preserved, not deleted, because the rot being visible IS the finding. All nine of 238's M rows were **DRIVEN LIVE on 2026-09-07** (`238-VERIFICATION.md:213-231` — 7 full pass, 2 half at the time; **four defects found by driving and NONE by the 15-case unit suite**). The operator completed the Azure registration *hours after* `238-SUMMARY.md` was written. ⭐⭐ **This is the highest-traffic home of the false claim in the whole set — it is what an orchestrator reads to decide whether 245 is blocked.** | **Everything.** SC#1 closed by `245-03`, SC#2 by `245-02`'s live drive, SC#3/SC#4 by `245-01`. ⚠ One residue, named not hidden: **M-9's Microsoft/OneDrive arm** needs a `/Finance/` folder in the OneDrive account — an operator action, trigger recorded in `245-VERDICT.md` |
...
|-------|------|------|--------------|-----|-----------|-------|
| 242 | Ship It — and Prove What Already Shipped | An operator can save the settings that control search, on the database production actually serves from, and every SHIP claim in the register has been replaced by a measurement | SHIP-01, SHIP-02, SHIP-03, SHIP-04 | 2 plans + 2 fast | ~~**177**~~ **178** `178_app_settings_vision_calls_bound.sql` | ⚠⚠ **THE INTAKE PREMISE IS STALE — read the corrections block first.** The push already landed (`1f313670b`); row 5 is already spent and retired in writing (`dbd63864b`). **G-5**: `settings.py` (**36/19/854**), `SettingsPage.tsx` (**45/23/1751**), `user_settings.py` (**50/32/1561**) — all three FIRE, all honoured by construction. ⚠ **A new bound on an existing settings column is a MIGRATION, not just an API change.** ⚠ **Blocked on a read-capable cloud DSN for 2 of 5 SC.** `check-deploy-drift.sh` gates. UI hint (the Settings tab + the refusal) |
...
| **245** | ⚠ **CORRECTED 2026-09-13 (Phase 245)** — the row above is preserved, not deleted | — | — | — | — | ⚠ **"Blocked on ONE Azure app registration — M-1 first, it unblocks the other 8" is FALSE.** All nine of 238's M rows were **DRIVEN LIVE on 2026-09-07** (`238-VERIFICATION.md:213-231` — 7 full pass, 2 half at the time; **four defects found by driving and NONE by the 15-case unit suite**). The operator completed the Azure registration *hours after* `238-SUMMARY.md` was written. **The blocker was discharged six days before six live registers stopped saying so.** Everything else in the row stands |
...
- [x] **Phase 242: Ship It — and Prove What Already Shipped** — ✅ **CLOSED 2026-09-11** (self-verified, `OV-SOLO-01`). **4 of 5 UAT rows driven and PASS in a real browser**; Row 5 (production) **DEFERRED under D-242-08** — measured unrunnable, 242 is not deployed. Migration **178 applied to cloud** by the operator; cloud migration parity **25/25**; security advisor clean. SHIP-02/03/04 **closed with evidence**; SHIP-01 code-complete and deliberately unticked (G-4). ⭐ The cloud-DSN blocker DISSOLVED — the Supabase MCP answers arbitrary SQL against production, read-only, no credential handling. ⚠ The CHECK sweep the CONTEXT deferred was **two columns, not five** (174/176 had already done three), so migration **178** closed both and the fence's allow-list is EMPTY. ⚠⚠ **Migration 177 IS applied to cloud** — `BUG-260911-01` remediated in production, contrary to D-242-07. ⛔ Owed: 178 → cloud, the non-code parity walk, and `242-VALIDATION.md`
- [x] **Phase 243: The Thinking Block and the Follow-Scroll Seam** ✅ **COMPLETE 2026-09-11** — CHAT-01..05 all delivered and DRIVEN; L-2/L-3/L-4/L-5/L-6-settled pass in a real browser; `BUG-260823-01` + `BUG-260707-03`#2 closed. ⚠ Owed: L-1, L-6's live frame, the cross-provider board (attempted, abandoned — provider attribution unverifiable by the driver). — — one calm line while a model thinks, expandable into a timeline; deltas that repaint on a cadence instead of per token; a scroll position that survives a tool call; and reasoning visible on a plain answer (CHAT-01..05) ⚠ G-2 SKETCH MANDATORY · CHAT-02 + CHAT-03 ARE ONE MECHANISM
- [x] **Phase 244: The Chat Shell and the Composer** ✅ **COMPLETE 2026-09-13** — SHELL-01..05 all delivered and DRIVEN; SC#3 closed on a real-browser drive in both directions. ⚠ Not covered: the fail-closed arms (never exercised) and the Deep-mode ask_user path (undriven). — chat scrolls inside chat, a cap-paused run leaves you able to act, an approval is answerable in the thread, a local file attaches to a message, and a source that stopped reading reaches you in the shell (SHELL-01..05) ⭐ SURF-03's home RULED: the app shell
...
- [x] **fast** — the register corrected (`681b82680`, D-242-06): SHIP-02/03/04 close with evidence inline, the ROADMAP's own migration number fixed 177 → 178, `SEED-242` **re-armed** not closed, and `242-VALIDATION.md` authored because **G-4 fires and neither plan carried a row** (SHIP-02, SHIP-04)

**UI hint**: yes (the Settings → Search tab, its payload and its refusal copy).
**Migrations**: ~~**177**~~ **178** `178_app_settings_vision_calls_bound.sql` — the CHECK constraints plus the in-range clamps. ⚠⚠ **RENUMBERED, AND THE ORIGINAL IS STRUCK RATHER THAN OVERWRITTEN, because the register being wrong about which migration a phase shipped is exactly what SC#5 exists to stop.** `177` was taken during this phase's own discussion by `177_rls_app_settings_user_settings.sql` — `BUG-260911-01`, a live production exposure (`app_settings` and `user_settings` with RLS disabled and `anon` holding all privileges). That one shipped first. ⭐ **And it is now APPLIED TO CLOUD** — measured 2026-09-11 over the Supabase MCP: 7/7 PASS on its own VERIFY block, and the security advisor's `rls_disabled_in_public` ERROR is GONE. `242-CONTEXT.md` D-242-07 says it is *"written and NOT applied"*; that sentence is now false. ⚠ **178 covers TWO columns, not one** — `multimodal_max_vision_calls` AND `vision_max_pages` — because migrations 174 and 176 were measured to have ALREADY constrained the other three columns the CONTEXT deferred, so the remaining sweep was one column and six lines. See `242-01-SUMMARY.md`. ⚠ Applied to **local** via `scripts/apply_migration_178.py` (hard-coded 127.0.0.1 DSN, one transaction, row counts asserted, re-run proven a no-op, clamp and NULL arms driven inside rolled-back transactions). ⛔ **NOT applied to cloud** — a production write needs per-action operator approval. Low urgency: cloud holds 100 and 50, both in range. `scripts/regenerate-full-schema.sh` run afterwards, no `--reset`.
...
- [x] 243-04-PLAN.md — ✅ **DONE 2026-09-11** (`b9819b7ab`, `96ca3909c`, `2a988c3de`) — V1's thin rule shipped: the body drops the four classes and steps to `text-sm` over REAL paragraphs; the tail clamps at 300px with a control that **removes itself** below the MEASURED threshold (the sketch-050 mechanism copied, not mounted); the fold reads `Thought for N seconds` only when a span was measured. ⛔ **D-243-13 honoured mechanically** — `grep -c "/ 180"` is **0**, a DB-loaded message shows no digit, and `git diff --stat -- supabase/ backend/` is **EMPTY**. ⚠ **ONE DECLARED DIFFERENCE from the acceptance bar**, with its reason, for `243-VERIFICATION.md` to carry: the mockup labels EVERY message, the shipped surface labels only the ones it watched stream. ⭐ Two plan errors found by driving: §10's uniqueness needle matched **ZERO** files after the render shape changed, and `MessageItem.tsx` had to take the prop while absent from `files_modified` (edited, ledger row updated same commit). `BUG-260718-02` → **`folded`, not closed** (the perceptual half is owed to G-4 UAT); `SEED-269` planted. Gate `254/254, failed 0`; typecheck set diff empty; four ledger triples re-derived (CHAT-01 appearance)
- [x] 243-05-PLAN.md — ✅ **DONE 2026-09-11** (`5e03d12b0`, `63e56e79f`) — **OUTCOME 1: branch only.** `MessageItem` loses the `StreamingNarration` arm (`-9/+37`); a live tool-bearing turn now takes the SAME renderers as the settled answer, below the thinking line, caret at the live edge. Document order asserted two ways (`compareDocumentPosition`), not claimed. ⭐ **The navigation path was driven through the provider's OWN callbacks and reconcile and was GREEN on first run** — so `BUG-260707-03` residual #2's *reconcile* half **already shipped at Phase 176 (RENDER-02 / D-07)** and the routing note calling it owed was never re-measured; only the render branch remained. ⚠ A `hasRunningTools`-narrowed fold was **rejected on a measured ground** (it oscillates once per iteration; holding it open needs state, which re-hollows the 227 discharge). The three ROADMAP-named collateral surfaces checked **by name**: streaming cursor / narration banner / citation branch — 27 suites, 276 tests, 0 failed; `StreamingNarration.tsx` **byte-identical** and now **zero-caller** (retirement OWED, not taken). ⚠ 15 reds triaged by **SET DIFF** on a one-file revert — 14 provably inherited, 1 genuinely this plan's (`ThinkingBlock` §11 anchored the answer on the FOLD). `tsc` caught two fixtures **green and wrong**. `count gate OK — 255/255, failed 0, 8030`; typecheck **67, set diff empty**. `SEED-049` **NOT re-opened** — criterion 5 *was* reachable from vitest. `BUG-260707-03` stays **`folded`, not closed**: every fence is synthetic and residual #1 has been open since 2026-07-07. **`MessageItem.tsx` not re-hollowed** (`useState` 3→3, `useEffect` 0→0, props 5→5, all 9 deletions enumerated); `StreamsProvider.tsx` was in `files_modified` and **needed no edit**
...
**Success Criteria** (what must be TRUE):
  ...
  1. Each of Phase 238's **nine** UAT rows reads **pass**, **⛔ blocked with its reason and its blocking id**, or **retired with a named trigger**. No row is silently absent. ⚠ **M-1 is driven first — it unblocks the other eight**; S-1 / S-2 may be retired on `SEED-256` (no work/school tenant) alone, provided the retirement says so (DEBT-01).
     ⚠ **CORRECTED 2026-09-13 (Phase 245) — the criterion above is preserved and is STILL EXACTLY
     RIGHT; only its blocking claim was false.** *"M-1 is driven first — it unblocks the other eight"*
     described work **already done**: All nine of 238's M rows were **DRIVEN LIVE on 2026-09-07** (`238-VERIFICATION.md:213-231` — 7 full pass, 2 half at the time; **four defects found by driving and NONE by the 15-case unit suite**). The operator completed the Azure registration *hours after* `238-SUMMARY.md` was written. ✅ **SC#1 is now
     CLOSED** — 238's eleven rows (`M-1`…`M-9` plus `S-1`/`S-2`) read **8 PASS · 1 ⛔ BLOCKED with a
     reason, `BUG-260913-01` and a named trigger (M-9) · 2 ⛔ RETIRED (`SEED-256`)** · **0 HALF**.
...
- [x] 245-02-PLAN.md — DEBT-02 + DEBT-01's M-9 arm: 233's five G-4 rows driven live (row 2 first, measured against the DB before anything is imported) and M-9 driven through BOTH doors with the rule created through `RuleBuilderPanel`.
- [x] 245-03-PLAN.md — DEBT-01's paper arm: M-8 flipped off a self-contradicting HALF PASS, S-1/S-2 retired across three registers in one commit, EVERY live-register home of the stale Azure claim corrected BESIDE its original (derived over two phrases, 32 hits swept, exclusions named), `SEED-177` refuted, 240's five mail rows deferred with triggers.
...
⚠ **CORRECTED 2026-09-13 (Phase 245) — the Flags block above is preserved, not deleted. TWO of its
claims were measured FALSE at HEAD:**
1. *"Blocked on ONE Azure app registration"* — **false.** All nine of 238's M rows were **DRIVEN LIVE on 2026-09-07** (`238-VERIFICATION.md:213-231` — 7 full pass, 2 half at the time; **four defects found by driving and NONE by the 15-case unit suite**). The operator completed the Azure registration *hours after* `238-SUMMARY.md` was written.
   `238-VERIFICATION.md:139` has read *"The operator completed the Azure app registration, so the rows
   below stopped being owed"* **ever since**. ⭐ **The bottom register was right the whole time and six
   live registers above it were wrong for six days** — `feedback_a_review_is_a_claim_about_code_not_the_code`.
2. *"`SEED-177` still reads `status: planted`"* — **REFUTED (D-15).** The seed reads
   **`status: partially-answered`**, and its own frontmatter already records the retire-the-egress-fence
   trigger as *"ANSWERED by Phase 206 (2026-08-25)"*. **Half the claim is false and half was already
   handled. What rotted was the PROSE in two registers — this block and `CLAUDE.md` — not the seed**,
   which is left byte-unchanged. Its remaining arms are a capability decision, not bookkeeping;
   trigger: `SEED-013` / Open Platform getting a phase number.
⛔ Everything else in the block **stands**: DEBT-03 is an honesty requirement and must not become a
re-review; G-3 applies hard; 2-3 plans; a driven row's defect does not silently expand the phase.
⭐ **And 237's rule-builder was CLICKED for the first time on 2026-09-13** (`245-02`) — it covered
MORE than expected; see `245-VERDICT.md`.
...
| Number | File | Phase |
|---|---|---|
| ~~**177**~~ **178** | `178_app_settings_vision_calls_bound.sql` — two CHECK constraints + generalised in-range clamps. ⚠ `177` is `177_rls_app_settings_user_settings.sql` (`BUG-260911-01`), which shipped first and **is applied to cloud** | 242 |
