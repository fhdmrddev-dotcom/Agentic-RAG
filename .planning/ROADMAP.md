...
- [ ] 214-02-PLAN.md - the run-phase read seam (D-214-23): `output._failure_reason` projected through `phase_output_object` on the jsonb STRING-SCALAR arm, both wire models widened in one commit, and every `lib/api/workflows.ts` widening the phase needs
...
- [ ] 214-14-PLAN.md - `visual_workflow_canvas` flips at its COLD default (D-214-19, `live_connectors` does not), and the five cross-plan seams proven by tests that mock neither side
- [ ] 214-15-PLAN.md - the ledger re-derived and synced row-with-section, the gate pinned, the registers written, a mitigation→test map for all fifteen threat models, and the G-4 operator drive (blocking)
