...
- [ ] 214-16-PLAN.md - ⭐ **the run-supply wire SC#2 has no other channel for**: `postMessage.inputs` → `MessageCreate.inputs` → the merge at `threads.py:917` **and its twin at `workflow_kickoff.py:500`**, proven by a real `POST /threads/{id}/messages` read back out of `workflow_runs.inputs`; D-103-CONF-1 amended in writing
