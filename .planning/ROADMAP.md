...
- [ ] **Phase 257: Cost in Dollars, and What It Cannot See** — an effective-dated rate registry, ONE token→USD conversion, and an operator view of spend per run and per org that states what it cannot see (METER-01, METER-02, METER-07). ⚠ **4/4 plans EXECUTED, but the independent review returned REVISE** (`257-REVIEW.md`, claude reviewed / gemini built, `469dd2826`). **SC#2 and SC#3 hold. SC#1 and SC#4 do NOT:** measured on 1,595 live runs, **1 of 78 models in real use has a rate** (98.3% unrated, headline $0.2666), and the run-level badge **never mounts** because no route outside `/admin/spend` emits `cost_usd`/`is_rated` — the fields were added to two type files with **no producer**. Closed in **257.1**. ⛔ **Phase 258 is BLOCKED on SC#1 by its own dependency line** (*"nothing is priceable until cost is attributable"*).
