...
- [ ] **Phase 222: One Click Connects Any MCP Server** — RFC 9728 discovery + OAuth 2.1/PKCE on the MCP door, reusing the PKCE machinery `oauth_service.py` already has and the MCP client has never called, so a service connects by logging in rather than by pasting a token minted by hand.
