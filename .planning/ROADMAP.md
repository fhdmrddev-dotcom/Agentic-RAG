...
**Plans**: **10 of 10 EXECUTED, 2026-08-15** (planned the same day; 10 plans in 6 waves). ⏸ **The
phase is deliberately NOT marked complete and NO success criterion is marked satisfied that a person
has driven**, because all five UAT rows are OWED — see `193.2-UAT.md`.
...
|---|---|---|---|
| **SC#1** | the author is not asked to restate the same intent; the requirement arrives proposed, editable, never blank and never silently derived | prompt asks for it; **20/20** non-empty over 20 real paid generations against a pre-fix **0/N**; server-side provenance stamp; the visible `AI-proposed` mark demoting on any edit | ⏸ **U1 moment 1 — OWED** |
| **SC#2** | the draft publishes without hand-editing the canvas; a genuine refusal names the step by its **visible label** and offers an action | `llm_human_input` **0/20** against a pre-fix **2/2-with-a-template** / 4/6 overall; the refusal rewritten in two arms that share no sentence, naming the canvas label, with the model-authored label clamped and single-lined | ⏸ **U1 moment 2 — OWED.** ⚠ **A MEASURED REDUCTION, NEVER AN ABSENCE (D-08) — the publish gate stays** |
| **SC#3** | a just-published workflow is findable without knowing its name | the two author feeds now `ORDER BY updated_at DESC` (starters stay alphabetical, D-16); rendered position **17 of 109 → 4 of 109**, pinned as arithmetic; the post-publish Run CTA's **first automated coverage ever** | ⏸ **U1 moment 3 + U4 — OWED.** ⚠ **Position 4 is not position 1, and no `ORDER BY` change can make it 1** — whether 4 is "findable" is the operator's judgement (OQ-5) |
| **SC#4** | nothing regresses `AUTH-03` — the template-first run still fills the bound template end to end | the `render_template` **control did not fall**: 5/5 per arm on a **like-for-like** re-drive against 193.1's 3/3 | ⏸ **U1 moments 4-5 — OWED.** ⚠ **That control counts a `render_template` PHASE IN A DRAFT; it does NOT prove a template gets FILLED** — which is exactly why D-21 exists |
