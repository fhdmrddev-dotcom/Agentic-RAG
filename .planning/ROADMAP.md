...
- [ ] **Phase 072: Multimodal Lift + DOCX Completeness** — Replace `_MAX_VISION_CALLS` / `_MAX_B64_BYTES` module constants with `app_settings` keys; persist empty-vision-description rows (`description=''`); DOCX `related_parts` walk catches floating shapes + headers/footers. **Widened 2026-05-16:** adds opt-in `vision_sweep` engine (per-page rasterize + vision-LLM "list figures + bboxes" prompt) to `IMAGE_ENGINES_PDF` per-aspect dispatcher slot; migration 048 widens `app_settings.extraction_image_engine_pdf` CHECK to include `'vision_sweep'`. Dual-target SC#1: default (pymupdf_full) ≥35%; opt-in (vision_sweep) ≥80% of ~59 visible figures on operator's thesis. SEED-021 stays planted for v3.x layout-aware ML. (4 plans)
