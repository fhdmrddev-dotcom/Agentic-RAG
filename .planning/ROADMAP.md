...
- [x] **Phase 188.1 (INSERTED): WorkflowCanvas Extraction Refactor** — G-5 debt owed from 188: lift `PlaneEditingLayer` + `EDIT_AFFORDANCE` out of `WorkflowCanvas.tsx` (1574 L) with zero user-visible change, so 189 adds its node type to a paid-down file; carries three bounded security fixes owed from 188's audit (no requirement — refactor, no new capability)
...
> ✅ **188-13 is COMPLETE — the board was driven, 2026-08-05.** It opened at its checkpoint with **0 of 17 rows driven** and that refusal to auto-approve was correct: driving the board then found **seven defects (F1–F7)**, three of which no plan, no review and no unit test had caught. Final: **16 PASS · 0 FAIL · 1 ⛔ · 0 pending**. Row 17 was run first as directed, and **`BUG-260609-04` is now `closed`** (`verified_closed_by: 188`) — its closure condition was met on the fixture the defect was photographed on. The single ⛔ is row 6 (`minimax`, `SC10-188-RUN` — failed after 600 s), recorded with its reason rather than dropped. **Rows 12/14/16 were driven separately at `/gsd:verify-work 188`**, after an audit found their PASS rested on evidence gathered while driving *other* rows — the phase's own failure mode, caught in the phase's own record.
...
### Phase 188.1: WorkflowCanvas extraction refactor — lift PlaneEditingLayer and EDIT_AFFORDANCE out of WorkflowCanvas.tsx (INSERTED)

**Goal:** `WorkflowCanvas.tsx` shrinks below the G-5 threshold by extracting `PlaneEditingLayer` + the exported `EDIT_AFFORDANCE` table into their own module, with **zero user-visible change** — so Phase 189 can add a node type to a file that has been paid down rather than to the largest workflow file in the tree. This is **debt owed, not new work**: the CLAUDE.md hot-file ledger records this extraction as due in Phase 188, and 188 honoured its budget by *measurement* (13 ins / 2 del against a ≤15/≤4 cap), which capped the diff without reducing the file.

**Requirements**: none — this is a refactor phase. It delivers **no new user-facing capability**; a plan that adds one is out of scope by construction.
**Depends on:** Phase 188 (SECURED 2026-08-05 — the canvas run-state surface must be settled before the file moves under it).
**Plans:** 5 plans across 5 waves

**Success Criteria** (what must be TRUE):

  1. `PlaneEditingLayer` and the exported `EDIT_AFFORDANCE` table live in their own module; `WorkflowCanvas.tsx` is measurably smaller (record the before/after line count — 1574 L is the pinned starting point) and the ledger row flips from *G-5 fires* to *satisfied*.
  2. **No user-visible change.** The canvas renders byte-identically before and after for every shipped state — editing affordances, the 185 governance corner seal, the 187 node-face ladder, and the 188 run readings all behave exactly as they do at `7e1bff34`.
  3. **The ESM-cycle constraint holds.** `WorkflowCanvas` imports `FlowEdge`'s **value** at module scope for the `edgeTypes` map, so the extracted module **must not import back into `WorkflowCanvas`**. This is the live constraint 185-10 discovered when it named the seam — a cycle here fails at runtime, not at typecheck.
  4. Both shipped canvas invariants survive and stay mechanically enforced: **a third badge is a typecheck error** (the max-2 `BadgeSlots` tuple), and **no focusable control lives inside the card** (the ✕ and ＋ live on the lane).
  5. The frontend count gate stays green with **no loss of coverage** — every moved test moves with its subject and remains inside **both** `TARGETS` (it runs) and `BASELINE` (it is pinned). A suite that silently stops running is the failure mode this phase must not introduce; `tsc -p tsconfig.app.json` stays at its measured 33.
  6. Three **bounded** hardening fixes owed from Phase 188's security audit land here, because they sit in exactly this subtree: `own()` applied at the five WR-04 prototype-key sites (`ICON_TINT[data.phaseType] ?? DEFAULT_TINT` returns a *function* for a prototype key — `??` never fires — and it is interpolated into CSS at `PhaseNodeCard.tsx:738`), `encodeURIComponent(runId)` in the exported `getWorkflowRun` (`api.ts:3751`, WR-07), and a threat-model entry registering `last_workflow_run_id` on the un-gated `GET /threads/{id}/workflow` (UF-1). Each is a few lines; none is a capability.
...
- [x] 188.1-04-PLAN.md — SC#6: the five WR-04 own-guards in the tree's own `Object.prototype.hasOwnProperty.call` spelling + WR-07 `encodeURIComponent(runId)`, six falsifications each observed RED first (wave 4)
- [x] 188.1-05-PLAN.md — Close: flip the G-5 ledger row (correcting its stale 1574 L → measured 1593), register UF-1, pin the D-02 lazy-import debt, drive operator rows M1–M3 (wave 5)

### Phase 188.2: PhaseNodeCard extraction refactor — pay down the G-5 debt on PhaseNodeCard.tsx along the seams Phase 189 needs, and close BUG-260806-01 (INSERTED)

**Goal:** `PhaseNodeCard.tsx` drops below the G-5 threshold by extracting along the seams Phase 189 actually needs (a 7th `phase_type`'s face, a conditional badge in slot 1, a capability-derived subtitle), with **zero user-visible change** — so 189 adds a node type to a file that has been paid down rather than to a G-5-firing one. **Debt owed, not new work**: a plan that adds a user-facing capability is out of scope by construction (the 188.1 shape).

**Requirements**: none — refactor phase. Traceability is to the success criteria below and to the CONTEXT decision ids `D-01`–`D-11`; no plan carries a REQ-ID and none may call `requirements.mark-complete`.
**Depends on:** Phase 188.1 (the sibling canvas extraction; `PhaseNodeCard` sits under `WorkflowCanvas`, and 188.1's `?raw` cycle fence covers this subtree).
**Plans:** 7 plans across 6 waves (planned 2026-08-06)

**Success Criteria** (what must be TRUE):

  1. `PhaseNodeCard.tsx` hits D-03's three NAMED targets, each measured with the SAME named classifier before and after — total **797 → ≤ 380**, CODE **249 → ≤ 120**, and the BODY (the exact range from the `// ── The card ──` separator to EOF) **427 → ≤ 175**. A missed target is reported as a measured shortfall in lines, never rounded.
  2. The card renders exactly what it rendered: three whole-`innerHTML` captures and two geometry matrices — all captured on the **UNMOVED** tree, before any destination file existed — still hold after the cut. The rendered DOM is byte-identical; the moved SOURCE is not, and the phase does not claim it is.
  3. All **17** negative source fences (not the 2 CONTEXT named by hand) plus 4 haystacks read a six-file subtree source whose path list named every destination before it existed, and falsification controls C-1…C-9 were each observed RED.
  4. The five new modules cannot import back into `PhaseNodeCard` in any form (value, type-only or dynamic); the cut has one direction and left **no re-export shim** (D-06).
  5. The gates are unmoved: `tsc --noEmit -p tsconfig.app.json` = **33**, `eslint src/components/workflows/` = **5**, count gate green with `failed 0`, no `[count-decrease]`, `TARGETS` unedited and one upward pin move.
  6. **`BUG-260806-01` is closed**: a *selected* card no longer occludes its own `✕`, proved by a driven `elementFromPoint` row **with the card selected and the node measured at z-index 1000**, AND by a z-index relation assertion that re-runs every build with both numbers named (D-10 / D-11). The analysis alone does not close it.
  7. The record is honest: the CLAUDE.md ledger row flips from *G-5 fires* to *satisfied (188.2)* with re-derivable figures and their commands; three measured stale doc pointers are corrected; and both UAT boards carry their OWN evidence in disjoint columns with no row reading `[pending]` under a completed header.

**Why now (G-5, measured not inherited):** `wc -l` = **797 L**; `git log -- <file>` = **4 distinct phases** (184, 185, 188, 188.1) — over the 3-phase threshold. The operator hardened the ledger row from *Watch* to **G-5 fires** on 2026-08-06 at the 188.1-05 checkpoint, and chose the prescribed route at `/gsd:discuss-phase 189`: the refactor ships FIRST. **Sequencing is deliberately inverted** — 189's discuss ran BEFORE this refactor so the cut is not blind; the recorded rationale and the seam list live in `189-CONTEXT.md`'s `<gate>` block.

**Folded bug:** `BUG-260806-01` — a *selected* phase card occludes its own `✕` remove control (xyflow `z-index: 1000` on the selected node vs the affordance portal's `auto`; `elementFromPoint` at the `✕`'s own centre returns the card). Pre-existing, found while driving 188.1's board; `affected_areas: [frontend/workflow-canvas, frontend/editing-affordances]`.
...
- [x] 188.2-04-PLAN.md — Create additively: `phaseNodeCardContract.ts` (the 6 slot types) and `ownProperty.ts` (a zero-import leaf), both verbatim moves, nothing deleted (wave 3)
- [x] 188.2-05-PLAN.md — Create additively: `NodeCornerMarks.tsx`, `NodeIconWell.tsx`, `NodeRunOverlay.tsx`; C-2/C-5/C-8 observed RED against violations planted in real destination files (wave 4)
- [x] 188.2-06-PLAN.md — THE CUT: delete the seven ranges, wire three terse call sites, re-point both importers with no shim, rewrite the docblock (door b), and measure D-03's three numbers with the named classifier (wave 5)
- [x] 188.2-07-PLAN.md — Close: flip the G-5 ledger row, correct three stale doc pointers, one upward pin move, and drive BOTH UAT boards with disjoint evidence (wave 6, has checkpoints) — **COMPLETE 2026-08-07: Task 1 (`615b2a06`); the checkpoint was RETURNED with nothing substituted, the operator authorised the orchestrator to drive the boards via Chrome MCP, and Tasks 2–3 landed as `0492181a` — 4 of 5 rows DRIVEN AND PASSED, `BUG-260806-01` CLOSED. ⚠ Row A2 is ⛔ BLOCKED and A1's visual half was not performed — both OWED, A2 first.**
...
- [x] 189-01-PLAN.md — Wave 0: the three new backend falsification suites (V06/V09/V10/V11/V13), every one observed RED (wave 1)
- [x] 189-02-PLAN.md — Wave 0: capture BOTH publish-gauntlet conflicts RED at HEAD using SHIPPED types, plus the D-20 leak guard (wave 1)
- [x] 189-03-PLAN.md — SC#3: `docs/CONNECTOR-ARCHITECTURE.md` + the `D-v3.6-01` pointer entry + the CLAUDE.md line (D-10, D-11) (wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 189-04-PLAN.md — D-20: `EXTERNAL_ACTION_CAPABILITIES` + the fidelity-only union + the falsified identity prose (wave 2)
- [x] 189-05-PLAN.md — D-19: thread `is_golden_run`; the armed checkpoint stops killing publishes, live runs still pause (wave 2)
- [x] 189-06-PLAN.md — Migration 115: authored, **applied (operator-authorised)**, `full-schema.sql` regenerated (D-08, D-17) (wave 2, checkpoint) — **constraint 5 → 6 literals; one-line dump diff; V13 3 skips → 3 PASSES.** ⚠ **Cloud parity OWED** (standing queue, migs 099 onward). ⚠ Apply method deviated from the plan's stated truth (via `psycopg2`, not the SQL editor) — ban on `db push`/`db reset` held, proved by surviving row counts; see `189-06-SUMMARY.md`

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 189-07-PLAN.md — The 7th `phase_type` + the D-04 structural arming pin + the D-26 docblock corrections (wave 3)
- [x] 189-08-PLAN.md — The client derivation layer + the developer panel's own honest word (D-07) (wave 3) — **the ONE derivation carries the sixth slug** (`DB_PHASE_STATUS` key + the 8th `CanvasReading` member + a one-line switch arm; `default:` floor and WR-04 guard UNTOUCHED), and the panel names it `↛ Not sent` in its own harness vocabulary. Fail-closed baseline MEASURED before it was superseded (`phaseStatusFromDb("recorded_not_sent")` → `unknown`, driven probe PASSED). Six wrong fixes planted, each observed RED — sharpest: *"the not-sent word must not be Complete"*. ⚠ **`tsc` is 40, NOT 33, and that is BY DESIGN: the union widening ARMS 7 exhaustive tables as 189-10's worklist** (RESEARCH §B9 undercounts them as 4 — it omits the two canvas TEST files). The intermediate canvas state was measured HONEST, not argued: word `State unknown`, ring DOTTED, never `Complete`. Count gate 2508 → 2521, both pins moved in-commit. See `189-08-SUMMARY.md`

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 189-09-PLAN.md — `_exec_external_action`: the closed-set step that records and sends nothing (D-02, D-05, D-22) (wave 4)
- [x] 189-10-PLAN.md — The canvas run vocabulary + the 8th ring reading, four arcs and still (D-16, D-07) (wave 4) — **the compiler-generated worklist CONSUMED TO EXHAUSTION: `tsc` 40 → 38 → 33, all seven `TS2741` closed, every one of 189-08's cited line numbers re-derived by symbol and HELD.** The word is `Not sent — recorded` (D-16, byte-exact, em dash asserted by CODEPOINT), the clause is `null` with its reason at the table, and `RUN_READING_BORDER`'s FIFTH deliberate absence is recorded AND pinned. The ring is UI-SPEC §4b implemented, not re-designed — dash .15 / gap .10 / **repeats 4** / gapCentre .125 / **spinning FALSE** — and every number it predicted was recomputed before it was trusted (`4 × (.15 + .10) = 1` exactly; gaps at .125/.375/.625/.875, so **none at 12 o'clock**; dashoffset 16.022, positive). **Three whole-table invariants that existed NOWHERE now do** — exact tiling, four-arc uniqueness, pairwise-distinct geometry — each stated over the table so the NINTH reading inherits it, and **five literal `7`s were DERIVED from `ALL_READINGS.length` rather than re-pinned at 8**. Colour spent NOTHING: the stroke-set size is unmoved at 5 while the readings grew to 8. **Six wrong fixes planted, every one RED** (P the word reads `Complete` · Q an EN dash · R the ring SPINS · S a gap at 12 o'clock · T `repeats: 2` colliding with `failed`, which fired the new tiling invariant · U a stroke claiming success); all reverted, md5-verified. `CARD_READING_SHAPES`' 8th row is **CAPTURED** (observed twice, md5-identical) and pins three ABSENCES nothing else reaches — the default border branch, the missing spin class, no pause chip. **`CARD_SUBTREE_PATHS` UNMOVED at 6**; only `NodeRunOverlay.tsx` touched inside the fence, at 6 insertions. ⚠ **jsdom cannot prove greyscale — U3 (189-16) is still OWED.** New suite `runVocabulary.test.ts` (23); count gate 2521 → 2546, all pins moved in-commit. 2 commits (`d240836f` · `5b2cc2e1`). See `189-10-SUMMARY.md`

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 189-11-PLAN.md — The D-05 seam: `record_phase_not_sent`, the run that continues, and **V20, the headline gate** (wave 5) — **V20 IS GREEN: an `external_action` workflow PUBLISHES.** The fifth phase-keyed write is ONE `pool.execute` / ONE `UPDATE` in `complete_phase`'s shape (not `fail_phase`'s merge — `_failure_reason` count 0, and the identifier is unspelled even in the denial); the engine's **third branch** consumes the **imported** `RECORDED_INTENT_KEY` at **+78 / −0**, so no shipped line changed. **"The run CONTINUES" is bought by PLACEMENT and PROVEN BY THE NEXT PHASE RUNNING** — a status assertion alone cannot tell "continued" from "halted here". The receipt question is answered explicitly: **no `phase_completed` row** for a phase that recorded; the ledger rides the EXISTING `phase_transition` kind (`via: recorded_not_sent`), so **D-09 holds and `test_audit_event_registration.py` passes UNCHANGED**. ⚠ **No SSE either, with the cost stated as `D-189-DEF-03`** — `phase_completed` maps to a "✓ Complete" card, so emitting it would paint the lie; emitting nothing lets `finalizeAllPhasesForThread` sweep the card to `done`, so a live viewer sees "Complete" until reconcile (Phase 190 closes it). **⭐ BOTH ORIGINAL REDS WERE RE-OBSERVED THROUGH V20 ITSELF**: reverting 189-05's golden-run branch → `blocked_stage='golden_run_error'` naming *"the ask channel was subscribed with timeout_seconds=None"*; reverting 189-04's union → `blocked_stage='grounding_fidelity'` with the `unregistered_tool` finding. Both restored md5-identical. **So V20 mocks NEITHER stage 2.6 NOR the golden run** — only the leaf registry reads, the DB edges and the judge — and still runs in 0.88 s because the ask channel raises. Plants **J** (record skipped) / **K** (status `completed`) / **L** (receipt written) / **M** (sentinel renamed) each RED; ⚠ **under L both V20 and the D-05 core test stayed GREEN** — the fourth time this phase the headline proof was blind to a real defect. ⚠ **And one found by accident: V20's first run passed every publish assertion over an EMPTY phase spine.** ⚠ RESEARCH §A3's seam pointer `:1605-1623` is **68 lines stale** (it is `:1673-1680`), and **`test_publish_gate.py` is the Phase-136 SKILL gate for the THIRD plan running** (V20 lives in `tests/unit/test_publish_service.py`; the gate file was run and left unedited, 11 passed). Nine-file scope 2 failed / 181 passed (+3); `tests/unit` 62 / **1745** (+1 = V20, zero regressions); SC#4 fence 0; `frontend/` + `supabase/` diffs EMPTY. 2 commits (`7ce83c0d` · `61084eec`). See `189-11-SUMMARY.md`
- [x] 189-12-PLAN.md — The client 7th type: `definitionOps` + the silent count-6 pins (wave 5) — **THE NODE IS PLACEABLE (SC#1).** The picker offers seven choices with the shipped six in their shipped positions (asserted as a PREFIX — a bare count passes an insert), and `slugForType` + `minimalPhaseFor` build a phase the backend union accepts. **`tsc` 33 → 37 → 33**, measured in three steps: the two SILENT edits raised **four** errors (`SLUG_BASE`'s `satisfies`, a **knock-on `TS7053` at `slugForType` no document lists**, `requiredConfigFor`'s `never` guard, and `REQUIRED_CONFIG_KEYS` in a TEST file) while **`PHASE_TYPE_ORDER` raised NOTHING** — the plan's named risk confirmed rather than assumed, since `satisfies` admits a subset and the one silent edit is the one that makes the type reachable at all. `SLUG_BASE` is **`act`**, not `send`: the token is keyed by TYPE (one for three capabilities) and nothing is sent in 189. ⚠ **`requiredConfigFor` emits a REAL `capability`**, the one departure from the switch's empty-string register — `prompt`/`fn` are plain `str` so an empty one PARSES, but `capability` is a closed `Literal` (D-15) so `""` 422s the WHOLE definition on the first save: the step would be placeable and then unstorable. `available_tools` is excluded twice over (backend default + D-03's server-side derivation). **⚠ D-23 IS NOW MECHANICAL, NOT EDITORIAL** — the suite reads `backend/app/models/harness.py` through the `?raw` loader (`PublishGauntlet.test.tsx:46`'s shipped cross-language idiom) and asserts the client mirror equals the backend union's `phase_type` literals **IN ORDER**, plus that the emitted capability is a member of the server's closed `Literal`, with positive controls on all three extractors so an empty read cannot make it vacuous. The docblock pointer **`harness.py:157-167` → `:288-299`** was stale TWICE over and is corrected — and the fence is what survives the next time that number rots. **Three plants, each observed RED**: **V** an 8th client-only member (3 failed — and **only the D-23 fence can see it**), **W** `capability: "wire_transfer"` (1 failed), **X** `capability: ""`, the plausible wrong fix (2 failed); restored md5-identical, `grep -c PLANT` → 0. ⚠ **THE PIN LIST WAS RE-DERIVED AND `189-RESEARCH.md` IS WRONG IN BOTH DIRECTIONS — 5 sites it does not list, 6 it lists that do not move here.** Four of the five are invisible to a `toHaveLength(6)` grep because they are SHAPE assertions (`'external_actionundefined'` from an `undefined` subtitle concatenated into an expectation, a missing `<svg>`, a sentence read from a map with no entry); they were found by **RUNNING the suite**, and the plan's predicted signature — a green `tsc` and a red vitest — is exactly what appeared. **19 sites moved across 5 suites, 12 of them counts, every one DERIVED from `PHASE_TYPE_ORDER.length`** rather than re-pinned at 7 (189-10's lesson), so the EIGHTH type inherits every guard. The subtree-path, contract-export and icon-tint six-counts are **UNMOVED with a zero-line diff**. ⚠ **Two FALSIFIABLE placeholders ship**, one tripwire each for 189-13 (`TYPES_AWAITING_A_3D_MARK`) and 189-14 (`TYPES_AWAITING_A_VOCABULARY_ENTRY`): each excluding case ASSERTS the entry is genuinely missing, so the owing plan's arrival is a RED test rather than a silent skip. Until then a placed node reads `external_action` with the `•` fallback — the shipped TOTALITY floor, never a fabrication. ⚠ **UI-SPEC §7b's *"nothing chosen yet"* state is no longer reachable from the picker** (a placed node carries `send_email`); **189-13 owns finishing that choice**. Count gate **2546 → 2560**, both pins in-commit (`definitionOps.test.ts` 232 → 243, `StepTypePicker.test.tsx` 43 → 46), no per-file decrease, `TARGETS` untouched. **ONE commit** (`dcfdfb38`) covering both tasks, because Task 2's own action mandates it. `toolOptions` NOT widened; V22 green; backend and `supabase/` diffs EMPTY. **CONN-01 stays `Pending` — 4 plans owed.**
...
| 188. Non-Technical Run Observability | 13/13 | **CLOSED — Code COMPLETE · UAT COMPLETE (16 PASS · 0 FAIL · 1 ⛔ · 0 pending) · SECURED 46/46 (`97b2bd29`).** Security verdict: `register_authored_at_plan_time: true`, so mitigations were VERIFIED, not re-scanned; every CLOSED verdict cites shipped source at `file:line` and five recorded measurements were re-derived rather than inherited (`WorkflowCanvas.tsx` 13/2 vs the ≤15/≤4 cap · zero dependency churn · count gate 2462 pinned == 2462 actual, 45/45 · backend gate suites 20 passed · `check-deploy-drift.sh` exit 0); all ten fence suites confirmed inside BOTH `TARGETS` and `BASELINE`. Nothing blocking under `block_on: high`, but three warnings recorded: **RD-1** — `T-188-08-05`/`T-188-09-04` were accepted on the claim that *no client-side flag check exists*, and CR-05 (`bce384ef`) then shipped one in four places in `ChatLayout.tsx`; read at source, **D-181-02 is about refusal polarity and D-14 about run-state derivation — neither forbids a client feature gate**, so the acceptance rested on a misreading at authoring time (the gate is fail-closed defence-in-depth over an intact server boundary; the risk stands, the wording is corrected). **F7 restated as a lesson:** `T-188-06-01`'s invariance fence on the ⛨ seal held throughout and the seal still never rendered on the run surface — an invariance fence on a shared component says nothing about whether its output is REACHABLE on a new surface. **Two of the 10 deferred WARNINGs are security findings under a non-security label** — WR-04 (`ICON_TINT[…] ?? DEFAULT_TINT` returns a *function* for a prototype key, interpolated into CSS at `PhaseNodeCard.tsx:738`, deferred on exactly the reachability argument `phaseState.ts:73-74` rejects) and WR-07 (unencoded `runId` in the exported `getWorkflowRun`, `api.ts:3751`); plus UF-1, `last_workflow_run_id` on the un-gated `GET /threads/{id}/workflow`, in no threat model. Owed to the next phase touching those files: `own()` at the five WR-04 sites, `encodeURIComponent(runId)`, and a threat-model entry for `last_workflow_run_id`. Prior close-out record follows. 13 of 13 plans across 11 waves (2026-08-05). `188-13` opened at its `human-verify` checkpoint with **0 of 17 rows driven** and deliberately not auto-approved — **that refusal paid for itself: driving the board found SEVEN defects (F1–F7), all fixed and re-verified live**, three of which no plan, no review and no test had caught (**F6 was INTRODUCED by F3's own UAT fix** — the anchor moved, the tick gate did not, so the live clock never ticked on any run; **F7** was found by checking the phase against its own success criteria, not by any row; **the `unknown` reading** had never been driven at all). Rows 12/14/16 were driven at `/gsd:verify-work 188` after an audit found their PASS rested on evidence gathered while driving other rows. `BUG-260609-04` → **CLOSED** (`verified_closed_by: 188`; row 17 met its stated condition on the fixture it was photographed on, and not on 188-04's reducer tests, which its own trigger forbade). The one ⛔ is row 6 (`minimax`, `SC10-188-RUN`). **A code review then found 6 BLOCKERS and all 6 are FIXED** (round 0 of gap-closure — G-7 clear; fixed directly because SC#1/#5/#6 were genuinely unmet, not because a must_have scored failed): the run surface never re-read the run so a live run showed *● Running* forever · the launch path opened no SSE stream so nodes never advanced · `finish_run` NULLs the anchor the "Open the run" receipt reads, making a finished run unreachable · the route leaked its existence at 405/307 while the canvas was off · the `workflow-run` view was ungated · and the live reconcile still painted skipped steps as executing. Two review prescriptions were **refuted by measurement** and corrected rather than applied (CR-02's `selectThread` does not arm the stream; CR-06's symptom is *running*, not *done*). **10 WARNINGs deferred** with a trigger. Measured at close: count gate **2442 / failed 0 / 45-45 pinned** (188-12 pinned every suite the gate runs; `BASELINE_TOTAL` equals the measured total, so zero slack) · `tsc -p tsconfig.app.json` **33**, unmoved · `vite build` exit 0 · backend canvas/gate suites **20 passed**; full-suite non-attribution proved by diffing the failure SET (the 17 `tests/unit` rows sum to exactly 62, so the old "62-red" figure is the same failures over a narrower scope — measured, not refuted) · `WorkflowCanvas.tsx` **13 ins / 2 del** against the ≤15/≤4 cap, so **G-5 honoured by measurement** (plus the `lib/phaseState.ts` extraction the ledger's own 188 row names; the `PlaneEditingLayer` lift stays **OWED**) · **zero migrations**. Both fail-opens fixed with RED observed first — the SPEC named the unreachable `?? "done"`, RESEARCH found the **reachable** `finalizeAllPhasesForThread` sweep. `BUG-260609-04` stays **folded**: code closure landed in 188-04 (and a second, previously unrecorded half — `phaseType` flattened to `unknown`), flip to `closed` only on the live single-phase run, row 17. **New, phase-attributable:** `WorkflowRunPage.tsx` imports `WorkflowCanvas` statically, defeating the lazy split — recorded with a re-open trigger, not fixed | - |
| 188.1 (INSERTED). WorkflowCanvas Extraction Refactor | 5/5 | **EXECUTED 2026-08-06 — 5/5 plans, UAT 3/3 DRIVEN (0 blocked, 0 pending).** Measured at close: `WorkflowCanvas.tsx` **1593 → 1292 L** (22 ins / 323 del; the moved bodies diff byte-identical against the pre-move file, because the extraction was split into *create additively* then *cut*) · count gate **2477/2477 zero slack, 45/45, failed 0** · `tsc -p tsconfig.app.json` **33** unmoved · `eslint src/components/workflows/` **6 → 5** (D-01's gate met) · `vite build` exit 0, zero circular warnings · `git diff --stat -- backend/ supabase/migrations/ frontend/package.json` **0 files** · `check-deploy-drift.sh` PASS. **The G-5 ledger row is FLIPPED to `satisfied (188.1 — 2026-08-06)`** and two stale figures corrected (1574→measured 1593; 478→measured 797). **`PhaseNodeCard.tsx` hardened to `G-5 fires — extraction due` by operator direction** — measured 7 plans / 4 phases, +67 % growth — so **Phase 189's discuss-phase owes a refactor recommendation on that file as its FIRST option**. UF-1 REGISTERED, never re-gated (T-188.1-03, accept, zero backend diff). **NINE plan-inherited claims measured FALSE across the five plans** — the re-derive rule earned its keep every wave; notable: `[INEFFECTIVE_DYNAMIC_IMPORT]` is **2** not the "exactly one" four consecutive plans carried, `PhaseNodeCard` was 797 L not 755 (755 was true at phase START), and the research's "no `WorkflowCanvas-*.js` chunk exists at all" is false (a 151-byte re-export shim does exist). **Operator board, each row with its OWN evidence:** M1 — hover reveal driven as a controlled A/B (off-canvas `0,0,0,0,0` → on-plane `1,1,1,1,1`), 5/5 reachable by `elementFromPoint`, picker holds **300 px / `transform: none`** at zoom 1.69 (the counter-scale, matching 188.1-02's `PICKER_WIDTH`) · M2 — vertical drag moves card **+76 y** and its ✕ **+76 y** (no stranded control) while the neighbouring ＋ moves **+38 y**, exactly half: `verticalOffsetFor` computing the slanted-connector midpoint from its new module; ⌥ swaps the **subtitle**, not the title; 185 seal confirmed **top-right** · M3 — operator-driven end-to-end run, elapsed confirmed to **climb through intermediate values** rather than jump (the F6 defect class). Freshness proven **mechanically** against the dev server rather than by asserting a hard reload — the served canvas no longer defines `EDIT_AFFORDANCE` or `PlaneEditingLayer`, both new modules do, neither imports back, re-confirming the ESM-cycle fence against the RUNNING app. **Four findings, dispositioned differently on purpose:** F-3 the run surface rendered its status **twice** (`band.sentence` at `:888` and `:921`, both `shrink-0` so neither scrolled away) — **FIXED** under G-3, guard observed RED first · F-1 a **selected** card (xyflow `z-index: 1000`) occludes its own ✕ (portal at `z-index: auto`), invisible to `.click()` and to jsdom — measured **PRE-EXISTING**, filed **BUG-260806-01** · F-2 affordances scale with zoom (43.9 px → 12.3 px) rather than holding constant size — shipped behaviour · F-4 dead space above the spine (`fitView` vs a `flex-1` container) — **NOT fixed**, a layout decision wanting a sketch under G-2 and out of a refactor charter. **SEED-139 planted**: 193 of 711 threads (27 %) are workflow runs and only 3 are distinguishable, because `active_workflow_run_id` is a liveness pointer cleared on completion. **CLOSED 2026-08-06** — verification PASSED 6/6 (`91c37b5c`), **SECURED 18/18** (`c2a5571f`), validated 16/16 `nyquist_compliant` (`29c344b2`). This cell previously ended `NEXT: /gsd:secure-phase 188.1`, which stayed stale for a day after that command had run and misrouted a session — corrected against the commit trail. **NEXT: `/gsd:discuss-phase 189`.** Prior planning record follows. **Planned 2026-08-05** — 5 plans, 5 waves. Measured at planning: `WorkflowCanvas.tsx` **1593 L** (the ROADMAP's 1574 was stale), `eslint src/components/workflows/` **6 errors** (D-01's gate is 6→5, not 2→1), `tsc -p tsconfig.app.json` **33**, count gate **2462/2462 zero slack**. Two findings the inputs did not carry: SC#4's no-focusable-control assertion is **vacuous today** (both shipped renders are non-editable, so `PlaneEditingLayer` never mounts — Plan 01 closes it), and `PlaneEditingLayer` also needs `CANVAS_LAYOUT`, which research's import-closure table omitted (harmless — `canvasModel` was already in the verified closure). — **G-5 debt owed from 188.** The ledger records this extraction as due in Phase 188; 188 honoured its budget by measurement (13 ins / 2 del vs a ≤15/≤4 cap), which capped the diff without reducing the 1574 L file. Inserted 2026-08-05 so 189 adds its node type to a paid-down file. Behaviour-preserving: **no new capability**, lean UAT (prove *unchanged*). Live constraint from the 185-10 seam: `WorkflowCanvas` imports `FlowEdge`'s VALUE at module scope for `edgeTypes`, so the extracted module must NOT import back (ESM cycle). Also carries the three bounded security fixes owed from 188's audit — WR-04 `own()` ×5, WR-07 `encodeURIComponent(runId)`, UF-1 register `last_workflow_run_id` | - |
| 188.2 (INSERTED). PhaseNodeCard Extraction Refactor | 7/7 | **COMPLETE — 7/7 plans (188.2-01 … 188.2-07; 2026-08-07) and ALL FOUR GATES CLOSED: executed 7/7 · VERIFIED 7/7 must-haves (`d6cfe63b`, `status: human_needed`) · SECURED 34/34 threats (`6ff2ef83`) · VALIDATED 12 green / 7 PARTIAL over 19 rows (`b07d1a07` tests + `d9963d03` doc). ⚠ THIS CELL READ "NOT YET VERIFIED — verify-work and secure-phase are still owed" UNTIL 2026-08-07, while `188.2-VERIFICATION.md` had existed since `d6cfe63b`** — corrected only because the operator questioned a routing line generated from this stale text rather than from the artifact; the standing rule is to read the phase's own VERIFICATION/SECURITY/VALIDATION frontmatter before emitting any `▶ /gsd:` routing. **`secure-phase` was not a rubber stamp:** it found the ESM-cycle fences in `PhaseNodeCard.test.tsx` **and** the 188.1 sibling in `WorkflowCanvas.test.tsx` both claimed to forbid "ANY import form" while missing the `.tsx`-suffixed specifier — legal here under `moduleResolution: bundler` + `allowImportingTsExtensions: true`, so a real TDZ cycle could land under a green fence, green `tsc` and green `eslint`; widened with an optional `(\.[jt]sx?)?` group, proven as a PROPERTY (old needle EVADES / new CAUGHT on all four forms; `?raw` still misses under both branches) and **driven RED end-to-end** against a real back-import planted in `NodeRunOverlay.tsx`. The two ASVS V5 rows (the WR-04 `own<T>()` verbatim move — diff against `git show 82dd29a7^` is EMPTY, `Object.hasOwn(` 0 tree-wide — and fence N10's `dangerouslySetInnerHTML` ban, whose green-vacuous-glob boundary is mechanically closed by per-destination `toContain` + `length > 0`) were verified individually, never on a SUMMARY's word. **`validate-phase` closed 4 per-file claims nothing asserted** — the row map claimed per-FILE properties backed only by the joined `cardSubtreeSource` haystack, which cannot distinguish "true in this file" from "true somewhere in the subtree"; +4 `it()` (124 → 128, pin EXTENDED, `TARGETS` untouched), with 05-T2 observed RED against a planted `export const RING_LEAK`. **Two of that auditor's own claims were refuted on re-check**: its "15/19 FILLED" headline disagreed with the Status column it wrote in the same pass (real figure **12/7/19**), and its "`runVocabulary` is unguarded" rested on the absence of a same-named test file when the source is fenced `?raw` from two suites (`PhaseNodeCard.test.tsx:2019-2021` loops `[cardSubtreeSource, runVocabularySource]` — that IS the guard). **⚠ NEW BUG FILED — `BUG-260807-01`:** `verticalOffsetFor` (`editAffordance.ts:214-224`) is a sixth, **unguarded** WR-04 sink returning `NaN` for a prototype-key slug, which invalidates the `translate(…)` on the three elements Plan 02 just gave `zIndex: AFFORDANCE_Z` (1002) — so a mispositioned affordance now lands **above** the cards rather than behind them; slug unconstrained end-to-end (`harness.py:202`), **not** covered by `D-188.2-DEF-02` (whose four sites are each already correct), routed to Phase 189. Gates at close: `tsc` **33** unmoved · `eslint src/components/workflows/` **5** unmoved · count gate **2506/2506, failed 0, 45/45 files**. ⚠ **MANUAL UAT DEBT CARRIED OUT OF THIS PHASE, stated as a decision rather than as a claim that everything ran: stated as a decision rather than as a claim that everything ran: row A2 is ⛔ BLOCKED and A1's subjective visual half was not performed. RUN A2 FIRST** — it needs a live workflow run (both DB runs open as ordinary chat threads because `ChatLayout.tsx:344-352` routes to the run surface only on an **active** run id, and `GET /workflows/runs/{id}` answers **404**, the collection **405** POST-only), so it is a *look-while-you're-there* row on the next run anyone launches: read the arc presentation attributes for two readings and confirm the seven readings stay distinguishable **by shape alone** in greyscale, the running arc spins, and a card with no reading is still. A1's half is zero-cost — glance at one Builder card. Both are registered as `D-188.2-DEF-07` / `D-188.2-DEF-08`. `615b2a06`·`360399b2`·`c8ac1d0e`·`0492181a`: **Task 1 DONE** — the `CLAUDE.md` hot-file ledger row for `PhaseNodeCard.tsx` flipped from *G-5 fires — extraction due* to **satisfied (188.2 — 2026-08-07)**, carrying all three D-03 numbers as before → after (**total 797→274 / CODE 249→100 / BODY 427→169**) plus the honest **subtree 797 → 1332 L (+67.1 %)**, the five destination modules, and the commands to re-derive. **The classifier was REBUILT and re-validated in this plan rather than inherited** — its first form reproduced the totals but not `797/518/249/30`, and the 143-line gap identified the missing rule exactly (JSX `{/* … */}` blocks count as comment); only then were the after-numbers trusted, and all three reproduce 188.2-06 exactly. **FOUR stale doc pointers corrected, and three of the four were ALREADY WRONG before this phase moved anything**: `icon-convention.md:66`'s ⛨ said `PhaseNodeCard.tsx:440` when the true pre-cut home was `:653` (stale by **213 lines**) → now `NodeCornerMarks.tsx:266`; its `GOVERNANCE_SEAL_LABEL` cited a root-level `definitionOps.ts:388` that **does not exist** → `components/workflows/definitionOps.ts:466` (Rule 2 deviation — a fourth pointer the plan did not name); `:69`'s ＋/✕ said `WorkflowCanvas.tsx:637, :680` when the pre-188.1 sites were **`:641`/`:684`** and 188.1 had since moved both → `PlaneEditingLayer.tsx:186`/`:234`; and `SKILL.md`'s *"always read `PhaseNodeCard.tsx`"* is now per-fact across the six-file subtree (⚠ the 248px/radius-22/42-20-20 facts **stayed** in the card and the 260×104 box was never in it — a blanket "read the new modules" would have been wrong too). **ONE upward pin move, EXTENSIONS ONLY**: `PhaseNodeCard.test.tsx` **107→124**, `WorkflowCanvas.editing.test.tsx` **58→61**, `BASELINE_TOTAL` note **2482→2502**, all read from the gate's own `actual` column across **two agreeing runs**, `TARGETS` untouched, the only removed pin lines being the two raised, and a verification run showing **pinned == actual, `count gate OK`, failed 0**, no `[count-decrease]`. Gates: `tsc` **33**, eslint **5/0**, `git diff --stat -- frontend/src backend/ supabase/migrations/ package*.json` **0 files changed**. `188.2-DEFERRED.md` created — six deferrals, each with an observable trigger. **⚠ THE PHASE-FINDING, recorded in TWO places (the deferred register D-188.2-DEF-01 and the gate script's own comment): the count gate's `failed` line is NOT a usable regression backstop on this machine today** — two back-to-back runs with **identical per-file columns and an identical total of 2502** read `failed 1` then `failed 0`; the one failure was `src/pages/WorkflowBuilderPage.canvas.test.tsx > 184-11 … POSITIVE CONTROL`, which passes **128/128 in isolation** and which **no 188.2 plan touches** (the orchestrator's own tally: `failed` varied **1 → 21 → 50 → 10 → 0 → 0** across eight runs, all `Axe is already running` or 5 s timeouts, pre-existing SEED-056 rot at a higher count than that seed records). The gate's COUNT columns were stable throughout and remain sound. **TASKS 2 AND 3 — the two UAT boards — RESOLVED `0492181a`, and the gate was ANSWERED rather than bypassed.** The executor that reached the blocking checkpoint had no browser-driving tool and **substituted nothing** — not a unit test, not a green gate, not the served-bundle read it took as a freshness precondition and explicitly disclaimed as evidence — so it returned the checkpoint. The **operator then authorised the orchestrator to drive the rows via Chrome MCP**, and it did. `188.2-UAT.md` now reads `status: PARTIAL — 4 driven / 1 blocked` (`rows_passed: 4`, `rows_blocked: 1`, `rows_not_yet_driven: 0`) — **four driven rows and one open one, never "the board passed"**, the frontmatter being arithmetic over its own rows so a header cannot read as completion (Phase 188 shipped a board whose header read COMPLETE while six rows read `[pending]`). **THE HEADLINE RESULT — B1 passed WITH A FALSIFICATION CONTROL, which is the only instrument in this estate that can see the defect class** (jsdom applies no CSS, computes no stacking contexts, and `.click()` bypasses hit-testing, which is how this bug survived from 184-12 with every unit test green): with step 2 **selected** (computed `z-index: 1000` — the failing state from the report's own table) and the affordance revealed by a **physical** pointer (`opacity: 1`, `:hover` true), `document.elementFromPoint` at the centre of `canvas-remove-emit` returned **the control**; dropping that control to `z-index: 999` — the pre-fix world — made the *identical* hit test return **`canvas-node-emit`**, the card painting over its own remove control, **reproducing the defect exactly**; restoring `1002` returned the control again. **The instrument was observed swinging BOTH ways in one session.** **B2 then passed deselected** (node `z-index: 0`, `✕` still `1002`, same hit test still reachable), which is what makes B1 a **comparison** rather than a single observation — the node's elevation is the only variable that moved. **A1 PASS** on measured geometry (`width 248px` / `radius 22px` / `padding 42px 20px 20px` / `backdrop-filter blur(4px)`, identical on both cards, matching `PhaseNodeCard.tsx:176` post-cut; the `getBoundingClientRect` 419 px is the React-Flow ~1.69× zoom, cross-checked against the well measuring 104.8 for a 62 px source in A3) — **but its subjective half was NOT performed and the row says so.** **A3 PASS** with a **fresh** selection of `retrieve` (not B1's `emit`): all three marks present at once, verdict ∩ icon well **FALSE** (143.6 px gap), verdict ∩ seal **FALSE**, seal surviving the selected border treatment, every class signature read off the **post-cut** modules. **A2 ⛔ BLOCKED — recorded with its reason and a one-action re-open instruction, never omitted and never inferred from a neighbour** (188.2-03 guards the arc's *geometry*, but that coverage is exactly as blind to **animation and greyscale distinguishability** as jsdom is, and those are A2's whole subject). Disjoint-evidence compliance was **checked, not asserted**. Two environment findings recorded because they cost real time: **the app has no URL router** (`ActiveView` is React state at `App.tsx:102`, so visiting `/workflows` renders chat — the URL is inert), and synthetic `computer` clicks were **unreliable on the sidebar nav but reliable on the canvas**, which is why every claim on the board is measured with `elementFromPoint` rather than inferred from a click outcome. The board also carries both recorded DECISIONS: the disjoint-evidence rule, and **SC#10's non-application** (no streaming, no agent loop, no provider routing, no UI state machine; `git diff --stat -- backend/` is 0 files across the phase; the canvas makes no model call, so there is no per-provider variable to vary — eight identical PASSes would be the false confidence the rule is written against). **`BUG-260806-01` → `status: closed` / `verified_closed_by: 188.2`**, closed on **the driven B1 row plus its falsification control plus the unit guard**, never on the stacking analysis — which discharges SC#6's explicit condition and lifts the fix's SUFFICIENCY above the MEDIUM that RESEARCH rated it. Prior plan detail below is unchanged. **PLANNED 2026-08-06 — 7 plans, 6 waves.** **PLANNED 2026-08-06 — 7 plans, 6 waves.** G-5 debt on `PhaseNodeCard.tsx`, measured not inherited: `wc -l` **797 L**, `git log -- <file>` **4 distinct phases** (184, 185, 188, 188.1) = 7 plans, +67 % since the ledger's last figure. Operator hardened the ledger row from *Watch* to **G-5 fires** at the 188.1-05 checkpoint and chose the prescribed route at `/gsd:discuss-phase 189`: the refactor ships FIRST, in the 188.1 shape (verbatim extraction, measured, zero user-visible change, lean prove-unchanged UAT). **Sequencing deliberately inverted** — 189's discuss ran BEFORE this refactor so the cut is not blind; the seams 189 needs (a 7th `phase_type`'s face, a conditional badge in slot 1, a capability-derived subtitle) are recorded in `189-CONTEXT.md`'s `<gate>` block. Folds **`BUG-260806-01`** (a *selected* card occludes its own ✕ — xyflow `z-index: 1000` vs the affordance portal's `auto`; pre-existing, found while driving 188.1's board). Must-survive invariants, both type/test-enforced: a **third badge is a typecheck error** and **no focusable control lives inside the card**; **badge slot 1 stays empty — reserved for 189 (D-12)**. No migration, no schema, no new endpoint, no new glyph; red line D-14 · **188.2-01 (Wave 0) DONE** `471104d4`·`28f7ee64`·`8cb82a6b`: all **17** negative `?raw` fences + **4** haystacks re-pointed at a six-path `cardSubtreeSource` that names all five destination modules BEFORE any exists — **green against a completely unchanged production tree** (0 production files touched), which is the proof the re-scope is behaviour-neutral. **D-09's third-badge typecheck invariant asserted for the FIRST time** and observed RED at exactly **34** (`TS2578 Unused '@ts-expect-error' directive`) against a widened `readonly BadgeSlot[]`, back to **33** on revert; measured first — `BadgeSlot2Tuple` had ZERO callers, `@ts-expect-error` occurred **0** times in the whole workflows directory. Gates: vitest **112/112** (107→112), `tsc -p tsconfig.app.json` **33** / 0 in `src/components/workflows/`, eslint **5/0**, `count gate OK` (+5; pin move is Plan 07's). Two inherited claims measured FALSE (12 of 14 held): `const RING_RADIUS` is at `:315` not `:313`; `grep -n phaseNodeCardSource` returns **3** code lines after Task 2, not 4 (P6's two operands share one line) · **188.2-02 (the phase's SECOND contract) DONE** `7f77af44`·`fe3de624`·`30c2e66f`: **`BUG-260806-01` fixed at the PORTAL tier with `PhaseNodeCard.tsx` untouched** (`git diff --stat` = 0 files) — all three affordance groups (`＋`, `✕` AND picker) carry `zIndex: AFFORDANCE_Z` = **1002**, above `@xyflow`'s inline `SELECTED_NODE_Z` = **1000**; production change is **83 insertions / 0 deletions**. Mechanism re-derived from `node_modules` (all six readings held): neither `.react-flow__nodes` nor `.react-flow__viewport-portal` opens a stacking context, so a positioned portal child escapes into the viewport's context and can be ranked against the nodes; 1002 avoids `.react-flow__connectionline`'s 1001. Library control ships in the **RENDERED** form, chosen from a printed measurement (selected node `"1000"`, unselected `"0"` — the report's table reproduced); the `node_modules` `?raw` fallback was NOT taken. Guard observed RED twice (lowered constant → relation + wiring fail; removed property → wiring ALONE fails at `expected '' to be '1002'`). The one authorised `AFFORDANCE_SHAPE_BASELINE` re-capture is proven, not asserted: two byte-identical dumps + a parse of the OLD literal showing **12 of 72 fields differ, ALL `style`, each by exactly the appended ` z-index: 1002;`**. Gates: vitest **61/61** (58→61), `tsc` **33**, eslint **5/0**, `count gate OK` (2482→**2490**, no pin edited). **⚠ Two acceptance criteria were UNSATISFIABLE as written** — the "exactly two files" `git diff` (tree carries ~230 unrelated `.claude/` GSD-update mods; measured scoped instead) and the style-line grep (**arithmetically impossible** — removed lines can never contain the new term; met by the added-lines-only form plus the stronger field parse). **⚠ Board B's driven `elementFromPoint` row is still OWED → Plan 07** (no unit test in this estate can see occlusion); Plan 07's pin move covered, **as of 188.2-02**, two files (`PhaseNodeCard.test.tsx` 107→112 AND `WorkflowCanvas.editing.test.tsx` 58→61) · **188.2-03 (Wave 2) DONE** `99a383c7`·`1fc193b4`·`b1942579`: the pre-move capture, taken on the **UNMOVED** tree — `ls` on all five destination modules answers *No such file or directory* at every commit in this plan, and `git diff --stat -- PhaseNodeCard.tsx` is **0 files changed**. `CARD_HTML_BASELINE` pins whole-`innerHTML` byte-identity for three rows (maximal running, maximal waiting — the only pause-chip reading — and the minimal Builder row), each proven by its own marker `it(` to have rendered the blocks it claims. `CARD_SHAPE` + `CARD_READING_SHAPES`/`CARD_VERDICT_SHAPES`/`CARD_BORDER_SHAPES` (aggregated as `CARD_SHAPE_BASELINE`) pin 7 readings × 3 verdicts × 4 border branches. **Every literal captured by SCRIPT SUBSTITUTION from two byte-identical dumps** (`cmp` clean both times) — no string hand-typed or hand-edited. **C-6 RED on TWO instruments under two different one-value edits**: `RING_RADIUS` 34→35 fails the reading matrix with a diff naming `canvas-node-ring-arc` (`r` 34→35, dasharray 55.543→57.177); the seal's `right-[17px]`→`right-[16px]` fails `MAXIMAL_RUNNING`'s `innerHTML`. Both reverted. Totality proven both ways: one deleted reading key moved `tsc` **33 → 34 (`TS2741`) → 33**. **⚠ THREE plan claims measured FALSE**: (1) `RING_RADIUS` is at `:315` not `:313` (the same stale figure Plan 01 already corrected); (2) MAXIMAL_RUNNING does NOT exercise the grounded-and-selected border branch — `runReadingBorder('running')` returns `border-primary` so the RUN arm shadows both, which is why a third literal `CARD_BORDER_SHAPES` was added (Rule 2) covering all four arms AND their precedence; (3) the seven readings are **NOT** distinct on the arc's `class`/`style` — measured **3**, because the arc's whole geometry rides SVG PRESENTATION attributes, so `CARD_SHAPE` captures eleven attributes not five (that finding is itself now a pinned assertion). Gates: vitest **124/124** (112→124), `tsc` **33** / 0 in workflows, eslint **5/0**, `count gate OK` **failed 0** (2482→**2502**, pin untouched). **Plan 07's pin move now covers THREE files** (`PhaseNodeCard.test.tsx` 107→**124**, `WorkflowCanvas.editing.test.tsx` 58→61, plus whatever 04–06 add) · **188.2-04 (Wave 3) DONE** `82dd29a7`·`155b3832`: the two camelCase leaves created **additively**, both moves EMPTY-DIFF verbatim. `phaseNodeCardContract.ts` (**214 L**) = `PhaseNodeCard.tsx:139-285` — **6** exported type names, **0** runtime exports, 5 `import type` / 0 value imports; the directory's **FIRST types-only module** (measured: all 11 camelCase leaves export ≥ 1 runtime value). `ownProperty.ts` (**86 L**) = `:289-310` with **ZERO imports** (un-cyclable by construction, so it needs no "allowed import" non-vacuity line), its four body lines diffing EMPTY against the card's and the `export` keyword the only declaration change. Both files were **assembled by script** (`git show HEAD:<file> \| sed -n 'A,Bp'` piped into the destination), so verbatim-ness is a property of the construction. **ADDITIVE: `PhaseNodeCard.tsx` (797 L), `PhaseNode.tsx` and `runVocabulary.ts` byte-unchanged at both commits; Plan 03's five baseline literals untouched — zero re-capture.** The subtree fence's REACH into each new file was **proven, not assumed** (Rule 2 — the plan asked for no such proof; `import.meta.glob` contributes the empty string for an unmatched path *in silence*): N5 `@xyflow` planted in `phaseNodeCardContract.ts` and N10 `dangerouslySetInnerHTML=` planted in `ownProperty.ts` each drove the named guard RED, then reverted — **the first 2 of the 5 destination paths to resolve to a real file**. **D-05 recorded IN PLACE**: the superseded *"two small private copies"* paragraph is quoted, and what changed is named as arithmetic (two sinks now split across two destinations ⇒ three copies); `runVocabulary.ts:84-88` keeps its differently-signed private copy, its four call sites and its fences. **⚠ Rule 1 deviation — the plan's literal STATE-OF-THE-EXTRACTION wording ("were CUT … a HARD CUT") would have shipped a FALSE docblock** at an additive commit; both headers state the additive position in bold and attribute the cut to `188.2-06`. Gates: vitest **124/124 UNCHANGED** (adds no test), `tsc` **33** / 0 in workflows, eslint **5/0**, `count gate OK` **failed 0** (total **2502**, pin untouched). **⚠ THREE inherited claims measured FALSE (16 of 19 held)**: `grep -c "import type"` reads **6** not 5 (5 statements + the header sentence explaining them; `^import type` = 5); the WR-04 falsifications are at `PhaseNodeCard.test.tsx:2214-2288` **not `:2000-2063`** (that figure was measured against the pre-188.2-01 file, 2063 lines TOTAL); the stale-sentence site in `nodePresentation.ts` is **`:144-145`** not `:145-146`. **OWED TO PLAN 06:** `nodePresentation.ts:144-145` claims `PhaseNodeCard.NodeVerdictMark` is an alias of `VerdictMarkKind` — still true at this commit, false the moment 06 cuts; deliberately NOT edited here (outside `files_modified`, and D-01 forbids folding a correction into a move commit) · **188.2-05 (Wave 4) DONE** `7b417f71`·`53247c69`·`d6dfe716`: the three presentational modules created **additively**, with **NINE moved spans and NINE EMPTY diffs**. `NodeCornerMarks.tsx` (**247 L**) = `PhaseNodeCard.tsx:410-434` + `:567-656` — the WR-04 sink-4 verdict derivation, the verdict mark and the ⛨ seal, kept in their SHIPPED ORDER (verdict above seal) with the seal's `{grounded ? ( … ) : null}` TERNARY intact, because `sealJsxBlock` walks backwards from the seal's test id and THROWS on a miss. `NodeRunOverlay.tsx` (**301 L**) = `:287` + `:312-355` + `:440-443` + `:658-739` + `:741-762`; all five `RING_*` constants stay **MODULE-PRIVATE** and `@/lib/phaseState` is imported as a **TYPE only** (D-14), with the "moving `arc` is pure geometry, not a derivation" argument written into the docblock and the warning sign named. `NodeIconWell.tsx` (**146 L**) = `:764-794`, carrying the tree's ONLY mention of the clipping utility. **Each file exports exactly TWO names — the component and its props type — so eslint is unmoved at 5/0 (R2); one `export const` would have taken it to 6 and undone 188.1's only measurable lint win.** All three assembled BY SCRIPT (`git show HEAD:<file> \| sed -n 'A,Bp'`), wrapper written to sit at the card's own indentation so no re-indentation was needed. **ADDITIVE: `PhaseNodeCard.tsx` byte-unchanged at 797 L at all three commits; Plan 03's five baselines untouched — zero re-capture.** **C-2 observed RED THREE times, one plant per fence, each in a REAL destination file and each run showing EXACTLY TWO failures (plant + the expected N15):** N1 (`onClick=` as a real JSX prop in `NodeCornerMarks.tsx`), N5 (the graph-package scope in `NodeRunOverlay.tsx`), and **N10 (`dangerouslySetInnerHTML=` substituted at the seal's own authored-string render site) — which converts 188.2-01's T-188.2-02 mitigation from a re-scope ASSERTED into a guard OBSERVED**, ASVS V5. **C-5 RED TWICE** — the value AND the type-only back-import both caught, no regex widening needed under `verbatimModuleSyntax: true`. **C-8 RESOLVED HONESTLY: the rendered walk did NOT go red** — the card still renders its inline copies so the module is not mounted; the SOURCE half (the `onClick=` fence) covers it today and **the RENDERED half is OWED to Plan 06**. **⚠ Rule 2 deviation — BOTH membership guards removed, not the one the plan named**: there are two guarded loops (`:549` subtree fence, `:611-616` cycle fence), all five destinations now resolve, and a guard over a live file reads nothing and reports success. **⚠ Rule 3 deviation — the plan's two docblock instructions are mutually exclusive** (lift the ring/chip/well prose into each second docblock AND diff the spans EMPTY); the empty diff won per D-01's own "moved with the code that it explains", and each design docblock instead names the in-place argument and adds only what the card never had to say. Gates: `tsc` **33** / 0 in workflows, eslint **5/0**, count gate per-file `PhaseNodeCard.test.tsx 107→124 (+17)`, total **2502**, no per-file decrease, pin untouched, **0 packages installed**. **⚠ ONE EXPECTED RED, recorded not worked around:** fence N15 (clip-utility count) reads **2** while both the card and `NodeIconWell.tsx` hold the sentence — that IS the proof the re-anchored fence reaches the new file; **Plan 06's cut returns it to 1, re-verify green, do NOT edit the pin.** **⚠ FOUR inherited claims measured FALSE (18 of 22 held), every one a TEST-file anchor rotted by Plans 01+03 growing the file 2063→3675**: `sealJsxBlock` is at `:1251` not `:1046-1064`; the no-focusable-control walk at `:249-265` not `:195-216`; fence N10 at `:502-503` not `:423-428`; fence N2's narrowing at `:430-454` not `:359-366` — every CONTENT description was accurate · **188.2-06 (Wave 5) — THE CUT — DONE** `7ca0ec1b`·`a4e60a04`·`579504de`: **`PhaseNodeCard.tsx` 797 → 274 L**, and the phase's FIRST CONTRACT HELD ON THE FIRST RUN — `CARD_HTML_BASELINE`'s three whole-`innerHTML` comparisons and all three Wave-2 geometry matrices are green with **ZERO re-capture**, so the rendered DOM is byte-identical to the tree as it shipped. **ALL THREE D-03 TARGETS MET, none by rounding**, with a classifier written to the scratchpad and VALIDATED against the pre-cut file's known figures BEFORE any after-number was trusted (`total=797 comment=518 code=249 blank=30`, reproduced exactly): **total 797 → 274** (≤ 380, 106 under) · **CODE 249 → 100** (≤ 120, 20 under) · **BODY 427 → 169** (≤ 175, 6 under), the body pinned as a RANGE — `grep -n "── The card ──"` = **:106** through EOF at **:274**. `git diff --numstat` on the card reads **16 insertions / 499 deletions**, so the diff reads as a MOVE. Seven ranges deleted; three TERSE call sites (`<NodeCornerMarks verdict grounded />`, `<NodeRunOverlay reading />`, `<NodeIconWell icon tint />`) rendered at byte-identical JSX positions; the cut assembled BY SCRIPT behind a **27-entry line-anchor table that throws on drift** — which caught its own span bug immediately. **D-06 holds mechanically:** both re-export greps return **0**, both importers re-pointed as SEPARATE statements (`FlowEdge.test.tsx:80-85` convention), and the one-direction fence — green BY VACUITY since 188.2-01 — now carries **four import assertions plus the no-shim half anchored on the export FORM** rather than on any type name, **both observed RED** (a renamed `NodeIconWell` import; a REAL planted `export type { BadgeSlot, BadgeSlots } from …`). **Fence N15 returned 2 → 1**, measured per file (only `NodeIconWell.tsx` holds the clip-utility sentence) — the pin was NOT edited. **T-188.2-23 / V3: both WR-04 falsifications proven to still BITE their now-separated sinks** — each `own(…)` guard removed from its NEW home one at a time, site 4 RED (`expected '' to contain '?'`) and site 5 RED (`expected 'object' to be 'string'`), each run showing exactly one failure, both reverted. **C-8's RENDERED half, owed by 188.2-05, DISCHARGED**: a planted `<button>` inside `NodeCornerMarks.tsx` drove **5 failures** including three no-focusable-control walks (it could not go red in 188.2-05 because nothing mounted the module). Docblock **110 → 75 L** with the **five fenced paragraphs byte-identical** (proved as contiguous blocks + an EMPTY `diff` on the XSS span against `git show`), six false-after-move paragraphs relocated — **three of them SPLIT** because each described both what left and what stayed. `nodePresentation.ts`'s alias sentence corrected in the SAME COMMIT that made it false. Gates: vitest **124/124** card + **139/139** adapter+canvas, `tsc` **33** / 0 in workflows, eslint **5/0**, `count gate OK` **failed 0** (total **2502**, pin untouched), 0 packages, no file deleted. **⚠ SEVEN inherited claims measured FALSE (16 of 23 held), and one matters a lot:** the plan's **subtree estimate of `~940 L / +18 % / "comparable to 188.1's +8 %"` is FALSE — measured **1332 L, +67.1 %**, roughly EIGHT TIMES 188.1's relative growth; the card's own docblock now carries the measured figures, and **Plan 07 must not restate the estimate**. Also: `nodePresentation.ts` is `:144-145` not `:145-146` (188.2-04 had already corrected this and the plan re-typed the stale figure); the 2-importer grep returns **5** (three are the cycle fence's own positive-control STRING literals — the property, two real imports, holds); ALL FOUR Task-2 `read_first` fence anchors were stale and the WR-04 anchor rotted for the THIRD time this phase (they are at `:2287` / `:2306`); the must-not-touch paragraph starts at `:11` not `:12`; and `:33-35`'s quoted sentence is at `:34-35` and says "still names a slot the component now FILLS", not "no longer holds". **⚠ Rule 1 deviation — the `:54-57` badge-tuple paragraph was DELETED rather than relocated**: `phaseNodeCardContract.ts:97` already holds that argument verbatim (moved by 188.2-04), so following `<interfaces>` literally would have shipped the second copy these headers exist to prevent. **OWED TO PLAN 07:** the pin move (107→124, 58→61, total 2482→2502), the CLAUDE.md ledger flip to *satisfied (188.2)* carrying **797→274 L (−65.6 %), body 427→169, CODE 249→100, subtree 1332 L (+67.1 %)**, the three stale doc pointers, and Board B's driven `elementFromPoint` row | - |
| 189. Governed External-Action Node Model | 12/16 | **EXECUTING — THE NODE IS NOW PLACEABLE, AND THE BACKEND WAS ALREADY COMPLETE END TO END (2026-08-07).** **`189-12` LANDED THE CLIENT'S 7TH `phase_type` (SC#1)** (`dcfdfb38`, ONE commit covering both tasks because Task 2's own action mandates it — the 7th type reds five suites the instant it lands; **7 files / 495 insertions / 49 deletions**, no deletions of tracked files, backend and `supabase/` diffs EMPTY). The step-type picker offers seven choices with the shipped six in their shipped positions, asserted as a **PREFIX** because a bare count passes an insert. **`tsc` 33 → 37 → 33**, measured in three steps: the two SILENT edits raised **four** errors — `SLUG_BASE`'s `satisfies`, a **knock-on `TS7053` at `slugForType` that no planning document lists**, `requiredConfigFor`'s `never` guard, and `REQUIRED_CONFIG_KEYS` in a TEST file — while **`PHASE_TYPE_ORDER` raised NOTHING**, confirming the plan's named risk rather than assuming it. `SLUG_BASE` is **`act`** (keyed by TYPE, one token for three capabilities; nothing is sent in 189). ⚠ **`requiredConfigFor` emits a REAL `capability`** — the one departure from the empty-string register, because a closed `Literal` has no empty member and `""` would 422 the whole definition on the first save, making the step placeable and then unstorable. **⚠ D-23 IS NOW MECHANICAL**: the suite reads `harness.py` through the `?raw` loader and asserts the client mirror equals the backend union IN ORDER, with positive controls on all three extractors; the docblock pointer `harness.py:157-167` → `:288-299` was stale TWICE over. **Plants V / W / X each observed RED**, and only the D-23 fence can see V (a client member the server lacks). ⚠ **RESEARCH's "sixteen six-count pins" is WRONG BOTH WAYS — 5 missing, 6 that do not move**; four of the five are SHAPE assertions a grep cannot see, found by RUNNING the suite. **19 sites moved across 5 suites, 12 of them counts, all DERIVED from `PHASE_TYPE_ORDER.length`**; the subtree-path, contract-export and icon-tint six-counts UNMOVED with a zero-line diff. Count gate **2546 → 2560**, both pins in-commit. Two FALSIFIABLE placeholders ship — one tripwire each for 189-13 and 189-14 — and until they land, a placed node reads `external_action` with the `•` fallback (the shipped TOTALITY floor, never a fabrication). ⚠ **UI-SPEC §7b's *"nothing chosen yet"* state is no longer reachable from the picker; 189-13 owns finishing that choice.** *(prior record:)* **V20, THE PHASE'S HEADLINE GATE, IS GREEN.** **`189-11` CLOSED THE D-05 SEAM: an `external_action` workflow PUBLISHES** (`7ce83c0d` the fifth write · `61084eec` the seam; **5 files / 616 insertions / 1 deletion**, no tracked-file deletions, `frontend/` and `supabase/` diffs EMPTY). `record_phase_not_sent` is the FIFTH phase-keyed status write — ONE `pool.execute`, ONE `UPDATE`, `complete_phase`'s atomic shape, deliberately **not** `fail_phase`'s payload merge (a human approved and work was recorded, so there is no failure reason; the identifier is unspelled even in the denial, because 189-09 learned a grep cannot tell a denial from a use). The engine gains a **THIRD branch** beside 101.1's emit-failure sentinel, consuming the **imported** `phase_types.RECORDED_INTENT_KEY` rather than a re-typed literal, at **+78 insertions / 0 deletions** — no shipped line changed. **“The run CONTINUES” is bought by PLACEMENT and PROVEN BY THE NEXT PHASE RUNNING**, never by the status alone: a branch that wrote the row and returned satisfies every status assertion, which is 189-05's PLANT E and 189-09's PLANT H exactly. **THE RECEIPT QUESTION IS ANSWERED EXPLICITLY** — a `recorded_not_sent` phase writes **no** `phase_completed` receipt (the ledger would say the step completed; consequence is not receipt), and the row that IS written rides the **existing** `phase_transition` kind carrying `via: recorded_not_sent`, so **D-09 holds with `test_audit_event_registration.py` PASSING UNCHANGED** and the `harness_audit` literal set carrying no diff. ⚠ **AND NO SSE IS EMITTED EITHER — A DECISION WITH A STATED COST.** `phase_completed` maps to a “✓ Complete” card in `StreamsProvider.onPhaseCompleted`, so emitting it would paint the exact lie the branch exists to prevent; but emitting nothing lets `finalizeAllPhasesForThread` sweep the card to `done` at `run_completed`, so a live viewer sees **“Complete” until a reconcile fetch**. Logged as **`D-189-DEF-03`** with Phase 190 as its trigger (the fix needs a new SSE + a client handler + a sweep exclusion in ONE commit — a plan, not a deviation), and it is the one thing a UAT row can catch that no unit test will. **⭐ THE STRONGEST RESULT: BOTH OF V20's ORIGINAL REDs WERE RE-OBSERVED THROUGH V20 ITSELF.** Quoting a prior plan's RED beside a green proves two things happened, not that they are connected — so each upstream fix was UNDONE in production source and V20 re-run: reverting **189-05**'s golden-run branch → `blocked_stage='golden_run_error'` naming *“the ask channel was subscribed with timeout_seconds=None”* (CONFLICT 1's mechanism verbatim), and reverting **189-04**'s `tool_names` union → `blocked_stage='grounding_fidelity'` with `{'code': 'unregistered_tool', 'phase': 'notify-the-customer', … 'send_email'}` (CONFLICT 2's shape). Both restored md5-identical, `grep -c REVERTPLANT` → 0. **That is why V20 mocks NEITHER stage 2.6 NOR the golden run** — only the leaf registry READS, the DB edges and the judge; the REAL union and the REAL armed checkpoint execute, and the file still finishes in **0.88 s** because the ask channel is the raising recorder and can never hang. **Four more plants, each observed RED**: **J** the record skipped via `skip_phase` (“nothing was sent, so nothing happened”), **K** the status written `completed`, **L** the completion receipt written anyway, **M** the sentinel renamed — ⚠ **under J the receipt fence stayed GREEN, and under L BOTH V20 and the D-05 core test stayed GREEN**, the fourth time this phase that the headline proof was blind to a real defect and exactly one test could see it. ⚠ **AND ONE DEFECT FOUND BY ACCIDENT RATHER THAN BY PLANTING: V20's first run passed every publish assertion over an EMPTY phase spine** (`load_run_phases` → `[]`, zero phases looped, `published: True`) — a publish test asserting only the verdict cannot tell a workflow that ran from one that did not, which is now recorded IN the test as the reason the `recorded_not_sent` write is asserted at all. Deviations: **`tests/test_publish_gate.py` is the Phase-136 SKILL gate for the THIRD plan running** (V20 landed in `tests/unit/test_publish_service.py`; the gate file was RUN and left unedited — 11 passed, unchanged), and `test_audit_event_registration.py` is listed as modified while its own criterion demands it be UNCHANGED (the criterion won). ⚠ **RESEARCH §A3's seam pointer `:1605-1623` is 68 lines STALE — the branch is at `:1673-1680`** (189-05 added 60 lines above it). Nine-file scope **2 failed / 181 passed** (+3, the two named `pool is closing` rows unmoved); `tests/unit` **62 failed / 1745 passed** — **+1 = V20, ZERO regressions**; SC#4 fence still **0**; V22 still green; `test_migration_115.py` **PASSES (3), not skips**. **Remaining work is entirely client-side (189-12/13/14/15) plus the docs/validation close (189-16).** ⚠ **CONN-01 stays `Pending`** — 5 plans owed; `requirements.mark-complete` deliberately NOT run. *Prior record follows.* **WAVE 2 OPERATOR GATE DISCHARGED (2026-08-07).** **`189-06` COMPLETE: THE SIXTH `workflow_phases.status` LITERAL IS LIVE, AND IT IS A SLUG** (`0e900cc8` authoring · `8b8c393e` regeneration; **2 files / 1 created / 1-line artifact diff**, zero tracked-file deletions). Migration 115 is applied to the local DB — `pg_get_constraintdef` returns **SIX** literals, `CHECK ((status = ANY (ARRAY['pending'::text, 'active'::text, 'completed'::text, 'failed'::text, 'skipped'::text, 'recorded_not_sent'::text])))` — and `supabase/full-schema.sql` was regenerated by `bash scripts/regenerate-full-schema.sh` in DEFAULT mode with **`--reset` NOT passed**, never hand-edited. **The diff is ONE LINE — 1 insertion / 1 deletion at `:1932`.** That is the payoff of Task 1's first deliberate departure from the 114 analog: writing the constraint as `= ANY (ARRAY[…])` with `::text` casts (the form `pg_dump` regenerates) rather than the more readable `IN (…)` that 114 used. Had it been `IN`, the dump would have reformatted the whole constraint and the diff would have been unreviewable against its own ~10-line guard — **the guard never had to fire, because the design anticipated it**, and there are no extra hunks to explain. **V13 FLIPPED FROM 3 GREEN-SKIPS TO 3 PASSES, BOTH CONTROLS DRIVEN:** the POSITIVE admits the slug on BOTH write paths (an INSERT at the status, and the `active` → `recorded_not_sent` UPDATE that `189-11`'s executor will actually perform); the NEGATIVE — **D-17 expressed as an executable test, not a comment** — proves D-16's display sentence `Not sent — recorded` (em-dash included) is REJECTED with SQLSTATE **23514** naming `workflow_phases_status_check`; and the third asserts all five shipped literals survive the DROP+ADD while `definitely_not_a_status` is still refused, so the CHECK was widened by one reviewed literal rather than turned into a free-text column. The negative control is as load-bearing as the positive: without it, a migration that put the SENTENCE in the constraint would make the positive green for entirely the wrong reason. Every test write occurs inside a transaction that ROLLS BACK, and the skip guard reads `pg_constraint` rather than probing with an INSERT — so the skip→pass transition is honest. `scripts/check-deploy-drift.sh` → **`RESULT: PASS`, exit 0**, the expected no-op (189 adds no env var and no seed-bearing migration); its two WARNs are pre-existing and **115 is correctly ABSENT from the seed-like list**, being a pure CHECK change with no DML. **⚠ THE FINDING — THE APPLY METHOD IS NOT THE ONE THE PLAN'S `must_haves.truths` ASSERTS, AND IT IS RECORDED RATHER THAN SMOOTHED.** The plan claims *"the operator applied the migration by pasting it into the Supabase SQL editor"*. It was operator-**authorised**, but executed by the orchestrator running the file verbatim in ONE transaction via `psycopg2` against `127.0.0.1:54322`. The substantive prohibition the rule exists to enforce — CLAUDE.md's outright ban on `supabase db push` / `db reset`, which protect local dev data — **held completely, and is proved by DATA rather than claimed**: `workflow_phases` **439 rows unchanged by the apply**, `workflow_runs` 197, `workflow_definitions` 191, `threads` 711, `documents` 39 all survive; a reset would have wiped every one. The SQL-editor instruction is the *mechanism* by which the ban is normally honoured, not the ban itself — but a `truths` entry is believed by every downstream reader and guarded by nothing, which is precisely the inherited-claim failure mode this project keeps paying for. (`test_migration_115.py`'s docstring and skip string still describe the SQL-editor protocol; accurate as the standing rule, not as a record of what occurred.) **The two commits are auditable as a pair via the DIRECTIONAL INVARIANT, verified rather than assumed** — `git ls-tree 8b8c393e` confirms the migration was ALREADY in the tree at the regeneration commit, and no commit anywhere carries a regenerated `full-schema.sql` without it. A single shared SHA was impossible: the blocking apply sits between authoring and regeneration, exactly as the shipped 114 precedent did. Between the two commits the tree held the migration and an UNCHANGED artifact — coherent, with `test_migration_115.py` green-skipping, so nothing ever claimed a widening that was not live. **⚠ CLOUD PARITY IS OWED — 115 IS NOT APPLIED TO CLOUD.** It joins the standing queue (migs 099 onward) and lands only in the operator-driven parity window, **in order**, at the next production push; T-189-20 records that its `DROP`+`ADD CONSTRAINT` takes a brief ACCESS EXCLUSIVE lock — immaterial on local dev, not something to run mid-traffic on cloud. **Execution is UNBLOCKED: all eight plans stalled behind this gate (`189-08/10/11/12/13/14/15/16`) are runnable; next is `189-08`** (wave 3), and `189-11` (wave 5) is also clear since `189-09` is done. ⚠ **CONN-01 stays `Pending`** — 8 plans owed; `requirements.mark-complete` deliberately NOT run. Also noted, not acted on: `supabase/snippets/` carries ~38 pre-existing modified/untracked `Untitled query *.sql` operator scratch files, out of scope and untouched. *Prior record follows.* **EXECUTION HALTED, WAITING ON THE OPERATOR — WAVE 4 STARTED (2026-08-07).** **`189-09` LANDED THE 7TH EXECUTOR AND TURNED SC#4 GREEN** (`b09bb361` · `d42b7533`; **5 files / 637 insertions / 8 deletions**, no tracked-file deletions, `frontend/` and `supabase/` both empty). `_exec_external_action` resolves `phase.config.capability` against the closed `EXTERNAL_ACTION_CAPABILITIES` and **RAISES on absence**, copying `_exec_programmatic`'s wording verbatim in shape (the raise names the phase slug, the offending value AND the closed collection — all three asserted); it drives no agent loop, requests no tool schema and makes no call of any kind. **All three of 189-01's V10 patched-transport REDs are GREEN with BOTH inertness controls intact**, and the `mcp` fence is still 0 over `backend/app`. The output is a plain dict carrying `text` plus a `recorded_intent` sentinel holding the capability + resolved inputs — **the ENGINE branch that reads it and writes `recorded_not_sent` is `189-11`'s**; the key is exported as `phase_types.RECORDED_INTENT_KEY` so producer and consumer cannot drift on a bare literal. The NOT-SENT body opens with the negation, closes from a CLOSED table and names what the record is NOT, fenced by `_assert_reads_as_not_sent` with a six-plant positive control. **⚠ THE `mcp` SOURCE FENCE FIRED ON MY OWN DOCSTRING** — *"no MCP client — there is ZERO MCP code in backend/app"* — and the plan's own body-grep read 2 for `httpx|requests|socket`. Both fences read the FILE, not the behaviour, so **a denial of a transport is indistinguishable from a use**; the executor now names NO transport at all and records why (a fence with a prose exemption is a fence somebody widens later). **⚠ THREE WRONG FIXES PLANTED INTO PRODUCTION SOURCE, EACH OBSERVED RED — AND UNDER TWO OF THEM THE THREE V10 CASES STAYED GREEN.** PLANT F (a receipt-shaped body: *"✓ Sends an email — done. Delivered successfully. Sent to …"*) and PLANT H (the record silently SKIPPED — D-05's rejected alternative arriving through the back door) both sailed past SC#4's headline proof, which only asserts a dict with a `str` text. **That is 189-05's PLANT E finding reproduced, two-for-two: the phase's headline test cannot tell "recorded" from "recorded nothing", or from a lie.** PLANT G (a default instead of the raise) swung the D-02 case; PLANT I (an external action needs no whitelist) swung the D-03 builder case. All removed, `grep -c PLANT` → **0**, md5-verified against pre-plant backups. **D-03 is PROVEN on the SHIPPED guard and NO second guard was built** — four DRIVEN cases: the capability reaches `phase_whitelist` through `_build_phase_tool_context` itself (the MAIN RUN's builder, not a hand-rolled frozenset), `dispatch_tool` refuses `execute_code` with the shipped `tool_not_available_in_phase` envelope, a whitelisted capability with no handler is a **SOFT** `"Unknown tool: send_email"` (asserted deliberately, with a comment forbidding a later "fix" into a raise — for a step whose point is sending nothing that is the safest failure mode), and `resolve_phase_available_tools` re-reads from the DEFINITION so a client submitting `["execute_code","search_documents"]` gets `["create_ticket"]`. `grep -c phase_whitelist tool_dispatcher.py` unmoved at **7**; `tool_dispatcher.py`, `runs.py`, `openai_service.py`, `harness_engine.py` and `threads.py` ALL have EMPTY diffs; **V22 still green** and D-09 holds (`test_audit_event_registration.py` green — no new audit event type). The NL generator now says **seven** phase types with one `external_action` bullet at **104 chars** against the six shipped bullets' **83.5 median** (measured, not eyeballed — an over-long bullet is a nudge that skews generation toward the type). Scope **2 failed / 177 → 2 failed / 178 passed** (the two named pre-existing `pool is closing` rows, unmoved), and the whole `tests/unit` tree **65→62 failed / 1733→1744 passed** — reconciled EXACTLY (−3 = the V10 flips; +11 = those 3 plus 8 new tests), so **zero regressions**. The pre-plan `tests/unit` figure was RE-MEASURED by reverting the three files to `b09bb361~1`, not inherited. **⚠ `D-189-DEF-02` is CARRIED, not closed: `189-13` must scope `PhaseFormPanel`'s generic rail AWAY from `external_action`, never widen `toolOptions`** — this plan strengthens that twice (V22 green, and `available_tools` is derived by total replacement so an author cannot legitimately edit it). ⚠ **THIS WAS THE LAST PLAN RUNNABLE WITHOUT THE OPERATOR.** `189-08/10/11/12/13/14/15/16` all depend on `189-06` directly or transitively, and **migration 115 is AUTHORED but NOT APPLIED** — the operator must paste `supabase/migrations/115_workflow_phases_recorded_not_sent.sql` into the Supabase SQL editor (never `db push`/`db reset`), then run `bash scripts/regenerate-full-schema.sh` with no `--reset`. ⚠ **CONN-01 stays `Pending`** — 9 plans owed; `requirements.mark-complete` deliberately not run. *Prior record follows.* **EXECUTING — WAVE 3 STARTED (2026-08-07).** **`189-07` LANDED THE 7TH `phase_type`, and its two governance properties are STRUCTURAL rather than conventional** (`22552460` · `7d3891a4`; **7 files / 509 insertions / 31 deletions**, no tracked-file deletions). `ExternalActionPhaseConfig` is appended as the SEVENTH `PhaseConfig` union member — discriminator, `extra='forbid'` and union structure UNTOUCHED, no new `ValidatorSpec.kind` (189 REUSES `action_risk_approval`), and a pre-189 row still validates. **ALL EIGHT of 189-01's REDs are GREEN and both of its controls stayed green.** `capability` is a closed `Literal` of exactly three, mechanically fenced EQUAL to `grounding.EXTERNAL_ACTION_CAPABILITIES`; `available_tools` is DERIVED from it by **TOTAL REPLACEMENT, not a merge** — a security decision, because a merge would let an author park `search_documents` on the step and detection is `available_tools ∩ KB_TOOLS`; `action_risk_armed` is **COERCED to True by a `model_validator` on `PhaseSpec`**, so it is pinned on every write path including a hand-edited JSONB row (both coercions log; the driven `model_dump` shows the lie gone). **⚠ THE FINDING: A SHIPPED 185 FENCE FORBADE THE MECHANISM D-04 REQUIRES.** `test_the_models_module_declares_no_model_validator_on_phase_spec` asserts `PhaseSpec` carries NO `@model_validator` — named by NOTHING (RESEARCH §A4 RECOMMENDS the very shape it forbids). It went RED and was **NARROWED, NOT DELETED**, to the property it was proxying (the only validator admitted is the NAMED D-04 pin, AND the class slice must store no grounding-cause token) — strictly STRONGER, since the old regex would have passed a cached cause in a plain field. The fence-preserving alternative (a `WorkflowDefinition` validator, outside the fence's class slice) was REJECTED as weaker. **THREE WRONG FIXES PLANTED INTO PRODUCTION SOURCE, EACH OBSERVED RED** — PLANT A (a 4th name in the Literal only) swung the new agreement fence, the ONLY test that can see the two spellings disagree; PLANT B (a DEFAULT instead of a coercion) swung V06's stored-false half; PLANT C2 (arm EVERYTHING) swung 189-01's negative control AND a shipped 185 zero-migration test. All removed (`grep -c PLANT` → 0/0, md5-verified). **⚠ D-26's TWO NAMED DOCBLOCK SITES WERE ACTUALLY FOUR** — grep found `GovernanceSection.tsx`'s MODULE docblock (12 lines above the named one) and `runs.py:resolve_phase_available_tools`; all four corrected in the SAME commit, comment-only, `GROUNDING_DIAL_TYPES`/`DIAL_TYPES` byte-identical, ZERO changed lines of executable frontend code. Also measured: **`llm_emit` was never added to the shipped strict-parse matrix at 101.1** — both it and the 7th are now there, with a union-membership test so the next omission fails. Scope **13 failed / 191 passed → 5 failed / 205 passed**, and the whole `tests/unit` tree moved by the IDENTICAL delta (**73→65 / 1719→1733**) — **zero regressions**; `tsc` **33** and the count gate **2508 / 45** both unmoved; `openai_service.py`, `tool_dispatcher.py`, `harness_engine.py`, `threads.py` all EMPTY diffs and V22 still green. **No client rollout was started** — the enumeration sites belong to 189-08/10/12/13/14/15. ⚠ **`189-06` REMAINS PAUSED at its blocking operator checkpoint** (migration 115 authored `0e900cc8`, NOT applied); **the next autonomous plan is `189-08`**. ⚠ **CONN-01 stays `Pending`** — 10 plans owed. *Prior record follows.* **EXECUTING — WAVE 2 UNDER WAY (2026-08-07).** **`189-05` RESOLVED CONFLICT 1 (D-19) — D-06's stage-3 blocker is gone and the gate did NOT loosen** (`a38c3198` · `3383dda8`; **3 files / 172 insertions / 5 deletions**, all backend, `frontend/` empty, `threads.py` untouched at `grep -c is_golden_run` = **0**). `is_golden_run` occurred **ZERO times in `harness_engine.py`**; it is now threaded from the ONE ctx builder that holds it (`publish_service._drive_golden_run`'s `SimpleNamespace`) and read as `getattr(ctx, "is_golden_run", False)`, so every other ctx builder stays byte-identical. On a golden run the armed action-risk checkpoint **skips the PAUSE and still runs the STEP** — no approval sentence, no `action_risk_pending` row **or** emit, no ask-channel subscribe. **Both 189-02 REDs flipped GREEN** (subscribe count `1 → 0`; the `timeout_seconds=None` indefinite wait gone) and **all three named controls plus `test_187_armed_checkpoint_property.py` (30) stayed green**. Shape note: an `if/elif` on a hoisted `_armed` local, NOT a re-indent — so `git diff` SHOWS the live path unchanged rather than asking anyone to trust it; `_interactive_phase_failures` and `_is_armed_action_risk` have **no diff hunk at all**. **⚠ THE BEST RESULT IS THE 189-04 TRAP, REPRODUCED: four wrong fixes were planted into production source and each observed RED — and under PLANT E (`return PhaseOutcome("completed", …)`, i.e. skip the pause AND the step) BOTH ORIGINAL REDs STAYED GREEN.** The plausible wrong reading of "auto-continue" would have satisfied the plan's headline criteria completely while shipping Phase 188's fail-open shape on every publish; **exactly one test in the tree could see it** — the executor-reached fence this plan added. PLANT B (Option B, in `_interactive_phase_failures`) swung the not-a-validator fence + the stage-3 composition; PLANT C (a synthetic `validator_ask_user_approved`) swung the new receipt test; PLANT D (skip the pause on every run) swung the **D-04 live-run fence**. All removed, `grep -c PLANT` → **0/0/0**, md5sums match pre-plant backups. **Options B and C explicitly DECLINED with reasons** (`grep -c external_action publish_service.py` → **0** — the fix is type-agnostic; the golden run's only trace is a `logger.info`, deliberately not a ledger row, per D-09 + `consequence ≠ receipt`). Scope figures now **10-file = 2 failed / 199 passed** — **the only remaining failures are the two named pre-existing `pool is closing` rows**; `test_publish_service.py` is **27 passed in 0.30 s**, so **no test awaits the 7200 s budget**. ⚠ **CONN-01 STAYS `Pending`** — 11 plans owed; `requirements.mark-complete` deliberately not run. *Prior record follows.* **`189-04` RESOLVED CONFLICT 2 (D-20): D-06's stage-2.6 blocker is gone and the governance hole stayed shut** (`ac51449a` · `44092d43`; 2 files, 109 insertions, 11 deletions, all backend). `EXTERNAL_ACTION_CAPABILITIES` is a closed `frozenset[str]` beside `KB_TOOLS` with **no `_SORTED` wire twin** and a module-level disjointness assertion; `assemble_grounding_bundle` now builds **two** named sets, so **`GroundingBundle.tool_names` is a strict SUPERSET of `GroundingBundle.tools` — and that difference IS the boundary**. V21 flipped RED → GREEN, **V22 stayed GREEN**, both negative controls hold and `_unregistered_tools` is **byte-identical** (`phase_type` in its body = 0 — the set widened, the shared rule did not weaken). **⚠ Proved by two plants, and the second one is the finding: under `tools=sorted(fidelity_tool_names)` the ENTIRE fidelity suite stayed green (11 passed) while the D-20 hole was open — V22 is the ONLY test in the estate that can see that leak, so it may never go red.** **⚠ The same-commit prose clause covered FOUR sites, not the one D-20 / CONTEXT / PATTERNS / `189-02-SUMMARY.md` all name** (`:107`, `:111`, `:446-447`, `:802`); the NL prompt stays byte-identical *precisely because* `tools` was not widened. Scope figures now **9-file = 2 failed / 172 passed** (the two named pre-existing `pool is closing` rows) and **10-file = 4 failed / 195 passed** — **CONFLICT 1's two REDs remain and are `189-05`'s**. Two deferrals logged in `189-DEFERRED`-style `deferred-items.md`: **D-189-DEF-01** (`test_182_extraction_parity.py`'s NL-gen count pin has been RED since 189-02 — dated by `git show fe7bd092~1` → 2 vs `fe7bd092` → 6 — and sits in NEITHER phase-scope command) and **D-189-DEF-02** (`PhaseFormPanel`'s rail paints an unregistered name struck-through and still pressable, so **189-09 must scope the rail away from `external_action`, never widen `toolOptions`**). ⚠ **CONN-01 stays `Pending`** — 12 plans owed. *Prior record follows.* **`189-03` SHIPPED SUCCESS CRITERION #3 IN FULL, and SC#3 now depends on no 189 code** (`d280f1c2` · `7b0e3e10`; 3 files, 152 insertions, 0 deletions, **no source and no test file**): `docs/CONNECTOR-ARCHITECTURE.md` (137 L, new) records the **MCP-first, first-party-thin, Open-Platform-sequenced** verdict with its five-source basis (`deep-dive/{BEAM,GLEAN,N8N}.md` + SEED-013/014), a `What it does NOT decide` section handing credentials / the unconditional SSRF-egress guard / the MCP spec-version pin to CONN-02/03, and a **dated 2026-08-07 re-open trigger whose three triggers each carry an OBSERVABLE**; `.planning/prd-reset/DECISIONS.md` gains **`## D-v3.6-01 — Connector-Architecture verdict (records CONN-01's decision half)`** at `:1411`, strictly between `D-v3.4-01` (`:1400`) and `## Cross-references` (`:1424`), as a POINTER closing *"the full text lives in `docs/CONNECTOR-ARCHITECTURE.md`"*; `CLAUDE.md` +1 line. **D-10 and D-11 are guarded by counts, not by editorial care:** `^### ` inside the new section = **0** (not `D-PRD-15`'s full-ADR shape), `^\|` in the doc = **0 table rows** (no re-validation matrix). **SC#4 constrained the prose:** the doc states the measured zero-MCP-code fact and that **190 is STRETCH**, naming the real path where the decision stands recorded and no connector ships. ⚠ **CONN-01 STAYS `Pending`** — only its decision half shipped; `requirements.mark-complete` was deliberately not run. ⚠ **Measured out-of-scope finding: `docs/DEPLOYMENT-PIPELINE.md` is on disk but UNTRACKED by git** (`git ls-files docs/` → 4 files, not ignored), so `CLAUDE.md:41` points at a doc a fresh clone lacks — a one-line `/gsd:fast`, not fixed here. ⚠ **`roadmap.update-plan-progress 189` returned `{"summary_count": 3, "status": "In Progress"}` and ticked the plan checkbox but left THIS ROW at `2/16` — the count in this cell was corrected by hand, exactly as memory predicts for this verb.** Prior record follows. `189-02` proved **D-06 FALSE AT HEAD, TWICE, BY DRIVEN TESTS** (`86290a97` · `fe7bd092` · `a5af250d`, 693 insertions, **zero source files touched**). **CONFLICT 1 (D-19) RED:** an armed phase on a golden run awaited `subscribe_for_response` **1 time (expected 0) with `timeout_seconds=None`** — the "wait indefinitely" value — inside a request bounded at `harness_publish_max_seconds` = **7200** (asserted from config). **CONFLICT 2 (D-20) RED:** stage 2.6 rule 2 emits an `unregistered_tool` finding for **all three** D-15 capabilities (`{'code': 'unregistered_tool', 'phase': 'research', 'message': "… non-registered tool 'send_email'"}`). **V22 authored GREEN and proved non-vacuous by PLANTING `send_email` into `get_tools()` and observing the guard go RED**; the plant is not committed. Both conflicts were captured with **SHIPPED phase types**, which is what proves the golden-run hang is **pre-existing** rather than introduced by 189 — and no test awaits the 7200 s path (the recorder RAISES a sentinel; whole surface runs in **1.41 s**). Seven controls fence the three REJECTED fix shapes (Option B, Option C, disabling rule 2). ⚠ **THE PLAN'S NAMED FILE FOR CONFLICT 1 WAS MEASURED WRONG** — `tests/test_publish_gate.py` is the Phase-136 **SKILL** publish gate and does not import `publish_service` at all, so its "use its judge mock" instruction was unsatisfiable; the falsification lives in **`tests/unit/test_publish_service.py`** and `189-VALIDATION.md`'s V20 pointer was corrected in the same wave. **The 9-file backend baseline no longer covers the phase — append that TENTH file.** ⚠ **`state.update-progress` DELETED `stopped_at` + `resume_file` and re-derived `last_activity` while reporting `{"updated": false}`** — a NEW false-completion behaviour beside `state.advance-plan`'s; both repaired by hand, and `state.record-metric` / `state.record-session` were deliberately not run. Prior record follows. `189-01` shipped the three backend falsification suites (`d8926830` · `47d0a988` · `41e289f8`, 964 insertions, **zero source files touched**): **11 assertions OBSERVED RED before any 189 source existed** (V06, V09, D-02's closed-set raise, D-15's set + `KB_TOOLS` disjointness, and V10 ×3), **7 controls green today**, and `test_migration_115.py` green-skipping with a reason that names **189-06**. The 189-scope backend baseline is unmoved at **166 passed / 2 pre-existing failures** (re-derived). ⚠ **The plan's own `mcp` source fence was FALSIFIED BY ITS OWN MANDATED POSITIVE CONTROL** — `re.compile(r"\bmcp\b", IGNORECASE)` does not match `MCPClient`, so SC#4's only mechanical proof would have reported green while missing the likeliest shape of MCP code; the matcher now fires on camel/Pascal segment starts too and `backend/app` is still **0**. ⚠ `requirements.mark-complete` was run per the executor contract and marked **CONN-01 Complete after a tests-only plan with 15 plans still owed** — reverted; do NOT re-run it until the phase actually delivers. ⚠ `state.advance-plan` again deleted `stopped_at` + `resume_file`; repaired. Prior record follows. Not started, CONTEXT REFRESHED + READY TO DISCUSS/PLAN — **UNBLOCKED 2026-08-07.** **Update 2026-08-07 (`04f9920a`, `267f6347`, `e4304b59`):** discuss-phase was RE-RUN against the shipped tree and D-01..D-13 were kept LOCKED (no decision changed); **D-14 added** routing `BUG-260807-01` OUT of this phase. Six load-bearing measurements re-derived and HELD — the 5-value `workflow_phases_status_check` (`full-schema.sql:1932`), **migration 115 still free** (live head 114), the 6-member `PhaseConfig` union, the badge slot-1 reservation (now asserted in 5 files + guarded by 2 test suites), the max-2 badge tuple (now `@ts-expect-error`-controlled, RED at 34 / green at 33), and the 185 seal's claim on top-right. **TWO measured FALSE, and one was wrong BEFORE 188.2 moved anything: `PHASE_GLYPHS` lives at `soulData.ts:43` (6 entries), NOT `phaseVocabulary.ts`** — resolver is `lib/phaseGlyph.tsx` and the two must be swapped in ONE commit; the other was the expected 797 L / G-5 figure. **`BUG-260807-01` no longer rides into this phase** — its code fix shipped standalone under G-3 (`267f6347`: `own()` on `verticalOffsetFor`, an INLINE guard on `lib/providerLogo.tsx`, LEAF fence re-read at 52/52, both rows observed RED first, pin 61→63) and it stays `open` ONLY for its driven browser row. ⚠ **THREE manual rows are owed and all three want the SAME live workflow run** — that row (`elementFromPoint` with a `constructor`-slugged phase), 188.2 UAT **A2** ⛔ (greyscale/animation arc distinguishability) and **A1**'s subjective visual half. **189 launches live runs under D-06: author ONE `189-VALIDATION.md` row that drives all three rather than staging a run three times.** ⚠ **`state.record-session` REGRESSED `STATE.md`'s frontmatter** at `115f7266` while reporting `updated: ["Resume File"]` — reverted `last_activity` a day, wound `last_updated` backwards and DELETED `stopped_at`; repaired at `e4304b59`. Treat that verb as unsafe on a populated frontmatter. Prior record follows. The G-5 gate that held it is discharged: The G-5 gate that held it is discharged: 188.2 shipped the `PhaseNodeCard.tsx` refactor (797 → 274 L) and the `CLAUDE.md` hot-file ledger row now reads *satisfied (188.2 — 2026-08-07)*, so a discuss-phase scan of 189's `files_modified` no longer matches a G-5-firing row on this file. 188.2 is complete on all four gates. **Two items ride into 189's discuss-phase:** `BUG-260807-01` (the unguarded WR-04 sink in `editAffordance.ts`, filed by 188.2's secure-phase) and 188.2's two owed manual UAT rows (A2 ⛔ needs a live run; A1's visual half) — neither blocks planning. `189-CONTEXT.md` is already locked. | - |
