     ...
     THREAT MODEL MANDATORY             ONE family, proving the seam
        +------------+---------------+
              ...
              233  THE PREVIEW                       no migration - G-2 SKETCH
                     v
              234  THE WATCH LOOP + FIRST REAL SYNC  migs 157-160
                     |  RETIRES THE CLAUDE.md MANUAL-UPLOAD-ONLY RULE, SAME COMMIT
                     |  THREAT MODEL MANDATORY
                     v
              235  THE SOURCE SAYS WHAT IT DID       mig 161 - G-2 SKETCH
                     v
              236  THE CORPUS UNDER ATTACK           no migration - GA gate
                     v
              237  ONE RULE ENGINE                   mig 162
        +------------+---------------+
        v                            v
238  MICROSOFT GRAPH             239  ANY MCP SERVER WITH FILES
     if not SMALL, 232 was wrong      mig 163 (reserve) - rows-not-code proof
        +------------+---------------+
                     v
              240  MAIL IS A SHAPE                   mig 164 - own discuss-phase, LAST
                     v
              241  RECALL AT CORPUS SCALE            mig 165 (reserve) - HARD
```
...
**UI hint**: yes
**Migrations**: **157-160** — `157_connector_watches.sql` · `158_connector_watch_items.sql` · `159_documents_source_state.sql` · `160` reserved (audit action types / freeze state). ⚠ **Shifted +2 at 231's close** — 231 consumed 154/155/156, not the one number reserved for it.
...
**UI hint**: yes
**Migrations**: **161** — `161_connector_sync_runs.sql` (per-source run history: counts, errors, completeness). ⚠ Shifted +2 at 231's close.
...
| 232 | — | none expected |
| 233 | — | none expected |
| 234 | **157 · 158 · 159 · 160** | `connector_watches` · `connector_watch_items` · `documents_source_state` · reserve (audit action types / freeze state) |
| 235 | **161** | `connector_sync_runs` |
| 236 | — | none expected |
| 237 | **162** | `classification_rules_scope` |
| 238 | — | none expected |
| 239 | **163** (reserve) | MCP tool binding on the watch row |
| 240 | **164** | `documents_thread_key` |
| 241 | **165** (reserve) | filtered-search index parameters |
