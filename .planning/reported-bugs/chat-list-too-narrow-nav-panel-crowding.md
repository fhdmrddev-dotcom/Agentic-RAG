...
- **CLOSED → Phase 156 (2026-07-16, verify-work):** the shipped 58px permanent icon rail + dedicated full-height `ChatHistoryColumn` verifiably relieve the crowding. Live Chrome-DevTools UAT at real scale (399 threads / 7 folders): three independent regions — thin rail · dedicated 399-row history column · chat area · workspace panel — the rail can no longer starve the history list; zero horizontal overflow at 390/820/1280/3440px. The later collapsible-layout refinement (`8486e0c3`) further lets the operator fold the history column fully away to widen the conversation. `status: folded → closed`, `verified_closed_by: 156`. See `.planning/phases/156-everyday-ux-polish-stretch/156-VERIFICATION.md` (SC#1 + human_verification item 1) and `156-UAT.md` (test 5).
