---
seed_id: SEED-269
title: "id SEED-269 was claimed by TWO seeds — this is the disambiguation, not a seed"
...
renumbered_because: >
  ...
  `created`-else-`planted` date. `SEED-269-explanations-are-noise-in-the-form-move-them-behind-an-info-affordance.md` kept SEED-269;
  `SEED-284-one-home-for-the-elapsed-formatter.md` moved to SEED-284. This file is the redirect stub
...
# SEED-269: this id named TWO different seeds — read this before following the citation

## What happened

Two unrelated seeds each claimed `SEED-269`. Nobody was careless: the register had no written contract and
...
⭐ **KEPT `SEED-269`** — `.planning/seeds/SEED-269-explanations-are-noise-in-the-form-move-them-behind-an-info-affordance.md`

> The app's explanations are good but dumped inline as noise — move them behind an info affordance with a guided popup, app-wide.

➡ **MOVED to `SEED-284`** — `.planning/seeds/SEED-284-one-home-for-the-elapsed-formatter.md`

> Three file-local elapsed-time formatters now ship (`RunCard.tsx`, `MessageList.tsx`, `ThinkingBlock.tsx`) — one home is owed, and the third was accepted deliberately rather than overlooked.

## Which one does your citation mean?

A citation about form copy, inline explanation or an info affordance means the KEEPER; one naming `RunCard.tsx`, `MessageList.tsx`, `ThinkingBlock.tsx` or an elapsed/duration formatter means `SEED-284`.

## How it was decided

`created: 2026-09-10` (explanations) against `created: 2026-09-11` (elapsed formatter) — one day apart, no tie-break needed.
