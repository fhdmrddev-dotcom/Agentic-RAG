...
last_activity: 2026-08-08 -- **Plan 189-15 COMPLETE — badge slot 1, the LAST free word-badge on the card, is SPENT on the muted `Not connected` label.** ⚠ **THE FINDING IS THAT TWO GATES RENDER IDENTICALLY.** D-12 requires the badge conditional on the projection STATE (`data.notConnected`), never on the TYPE — so 190 retires it by DATA and re-opens no file. But nothing in this app can be CONNECTED to anything until 190, so a type-conditional gate is byte-identical today: under PLANT 1 **all four RENDER assertions stayed GREEN and only the SOURCE fence went red**. A DOM-only suite would have shipped the coupling D-12 exists to prevent. Four plants, all RED (type-conditional gate · slot-2 order · a third badge, tsc 33→34 naming the line · the card rendering only `badges[0]`, RED on 7 cases across 3 suites). The shipped `@ts-expect-error` max-2 control was **RE-OBSERVED swinging 33 → 34 → 33** rather than trusted. ⚠ **TEN reservation sites were re-derived by grep — CONTEXT lists five, UI-SPEC six, 189-13 found a seventh, and this plan found THREE MORE** (`phaseVocabulary.ts:771` fourteen lines below the block 189-13 corrected in the same file; two test comments). Every one REWRITTEN, never deleted — the seven grep hits that remain are all former wordings quoted inside a "used to read" correction. Both test guards rewritten in place with positive controls; the card suite gained the two-badge control it never had (one-badge→one-chip was equally satisfiable by a card that renders at most one). **D-21 discharged as PROSE** — the unarmed-edge docblock no longer names 189 as claimant of a state D-04 makes unreachable forever, `checkpointOnTarget` re-read at `:303-305` (NOT the `:274-276` both the plan and UI-SPEC name), and `FlowEdge.tsx`'s diff is EMPTY. Three fenced-subtree files edited COMMENT-ONLY, verified line by line (the six JSX-comment lines PROVED inside their `{/* … */}` block by delimiter containment); `CARD_SUBTREE_PATHS` still 6, pin unmoved; no file added. tsc **33 → 33**; gate **2619 → 2627**, 47/47, 0 failing; `src/components/workflows` **2203 passed / 0 failed**; `vite build` ✓. Commits `4defa427`, `c842d9dd`. **NEXT: `189-16`** — the docs/validation plan, and the ONLY place the badge's rendered legibility, occlusion and height cost can be proved (U1-U4, Chrome MCP; jsdom paints nothing).
