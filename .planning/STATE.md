...
status: executing
last_updated: "2026-08-07T11:34:17.819Z"
last_activity: 2026-08-07 -- **Phase 189 EXECUTING — plan 189-03 (Wave 1) COMPLETE: SUCCESS CRITERION #3 IS SHIPPED IN FULL, and it now depends on no 189 code at all.** 2 commits (`d280f1c2` · `7b0e3e10`), **3 files / 152 insertions / 0 deletions, no source file and no test file**. **`docs/CONNECTOR-ARCHITECTURE.md` (137 L, NEW)** records the own-framework-vs-Open-Platform verdict — **MCP-first, first-party-thin, broad catalog sequenced with Open Platform (SEED-013/014)** — with its five-source basis (`deep-dive/{BEAM,GLEAN,N8N}.md` from the 2026-07-24 crawl + the two seeds), a `What it does NOT decide` section handing credentials / the SSRF-egress guard / the MCP spec-version pin to CONN-02/03, and a **DATED 2026-08-07 re-open trigger with THREE named triggers each carrying its own one-line OBSERVABLE**. **`.planning/prd-reset/DECISIONS.md` gains `## D-v3.6-01 — Connector-Architecture verdict (records CONN-01's decision half)`** at `:1411` — strictly between `D-v3.4-01` (`:1400`) and `## Cross-references` (`:1424`) — as a **POINTER** in `D-v3.4-01`'s shape, closing *"This is a pointer; the full text lives in `docs/CONNECTOR-ARCHITECTURE.md`"*. **D-10 is honoured MECHANICALLY, not editorially:** `grep -c '^### '` inside the new section = **0** (no `D-PRD-15` full-ADR subheads), and `grep -c '^|'` on the doc = **0 table rows** (D-11 forbids a re-validation matrix, so the basis is a bullet list — a reader cannot tell a "basis matrix" from a "scoring matrix" by shape). **SC#4 constrained the PROSE too:** the doc states the measured zero-MCP-code fact (`grep -rni '\bmcp\b' backend/app --include=*.py` → 0) and that **190 is STRETCH**, so it names the real path where the decision stands recorded and no connector ever ships. `CLAUDE.md` moved by **exactly 1 inserted line** (T-189-09 cap held). **⚠ MEASURED OUT-OF-SCOPE FINDING: `docs/DEPLOYMENT-PIPELINE.md` is on disk but NOT TRACKED BY GIT** (`git ls-files docs/` → 4 files; not gitignored) — so "all five shipped `docs/` files" is true on disk and FALSE in the repo, and `CLAUDE.md:41` points at a doc a fresh clone does not have. Not fixed here (pre-existing, another author's file); it is a one-line `/gsd:fast`.
stopped_at: Plan 189-03 (Wave 1) COMPLETE — summary `189-03-SUMMARY.md`; **WAVE 1 IS NOW DONE (189-01, 189-02, 189-03)**. **NEXT is `189-04`** (D-20 — `EXTERNAL_ACTION_CAPABILITIES` + the fidelity-only union + the falsified `tools == sorted(tool_names)` prose at `grounding.py:446-447`, corrected IN THE SAME COMMIT). ⚠ **`189-04` and `189-05` have executable acceptance criteria from 189-02's RED captures and MUST NOT break its controls:** 189-04 turns `test_an_external_capability_in_available_tools_produces_no_unregistered_tool_finding` green by unioning into `tool_names` **ONLY** — never into `get_tools()`/`GroundingBundle.tools` (V22 is the fence) and never as a `phase_type` exemption inside rule 2; 189-05 threads `is_golden_run` into ctx (auto-record-and-continue) while `test_a_live_non_golden_run_still_pauses_on_an_armed_phase` and `test_the_armed_checkpoint_is_not_a_validator` stay green. ⚠ **THE 9-FILE BACKEND COMMAND NO LONGER COVERS THE PHASE — append `tests/unit/test_publish_service.py` (a TENTH file).** ⚠ **TWO WAVES STILL NEED THE OPERATOR:** `189-06` is `autonomous: false` — migration `115_workflow_phases_recorded_not_sent.sql` is applied by PASTING INTO THE SUPABASE SQL EDITOR (CLAUDE.md forbids `db push`/`db reset`), then `bash scripts/regenerate-full-schema.sh` with NO `--reset`; and `189-16` drives ONE live browser session discharging all three owed manual rows. **D-17 remains the trap: the CHECK stores the SLUG `recorded_not_sent`; "Not sent — recorded" is a CLIENT-RENDERED word.** ⚠ **CONN-01 MUST STAY `Pending` — `requirements.mark-complete` was deliberately NOT run by 189-03.** This plan discharged only CONN-01's recorded-decision half; the governed node is 13 unshipped plans. ⚠ **THE TWO HOSTILE GSD VERBS MISBEHAVED AGAIN, EXACTLY AS PREDICTED — third consecutive plan:** `state.advance-plan` reported `{"advanced": true}` while DELETING `stopped_at` + `resume_file` and gutting `last_activity` to a bare date, and `state.update-progress` reported `{"updated": false, "reason": "Progress field not found"}` while ALSO gutting the body's `Last activity:` line. Both repaired by hand here; `state.record-metric` and `state.record-session` were **deliberately not run** (185/188/189-02 precedent).
resume_file: .planning/phases/189-governed-external-action-node-model/189-04-PLAN.md
progress:
  total_phases: 21
  completed_phases: 10
  total_plans: 142
...
Last activity: 2026-08-07 -- Phase 189 plan 189-03 (Wave 1) executed: SC#3 shipped — `docs/CONNECTOR-ARCHITECTURE.md` + the `D-v3.6-01` POINTER entry + one `CLAUDE.md` line
