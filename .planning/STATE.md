...
stopped_at: "2026-08-15 — **Phase 193.1 Template-First Authoring: ALL 11 PLANS EXECUTED AND MERGED. NOT COMPLETE — blocked on three human-only UAT rows (U2/U3/U4), not on code. NEXT = drive them with a person who has not read the source.** ⚠ **THE PHASE'S CENTRAL ASSUMPTION WAS MEASURED FALSE IN WAVE 1 and required a plan that did not exist at plan time (`193.1-11`, D-26): sending `template_placeholders` does NOT flip the DELIVERABLE RULE branch (0 `render_template` on 3 of 3 real calls) — the model must ALSO be told a template was PROVIDED. Fixed and re-measured: 3/3, key coverage 3-4/8 -> 8/8, control unchanged at 0.** ⚠ **`AUTH-03` DELIBERATELY NOT TICKED: no test observes a real run FILLING a template end-to-end, and the rewritten requirement demands exactly that — ticking on unit evidence is the trap Phase 193 fell into.** ⚠ **D-19 corrected in one direction and confirmed in the other; its `no_template_bound` hazard went LIVE once the branch fired, so plan 07 landed the wire AND the bind in ONE commit (`02606a08`).** ⚠ **FOUR fences were found that could not fire, every one caught by PLANTING a failure rather than by reading — a fence is only real once you have watched it fail.** ⚠ **The GSD SDK `state.*` verb corrupted this file AGAIN while reporting `\"updated\": true` — hand-edit only, and re-parse the YAML after.** ⚠ **Wrong-base bug 9 for 9 on this phase; the `merge-base` assertion caught every one.** ⚠ **`GSD_VITEST_MAX_WORKERS=2` is no longer deterministic either.** **Guardrail overrides: NONE — G-5 fired on THREE files (one more than CONTEXT or the ROADMAP named) and all three were honoured.** Gates at HEAD: tsc **33** unmoved across all eleven plans · count gate **exit 0 / total 3892 / failed 0 / 75-75 pinned** · backend **62 failed (SEED-056 rot baseline) / 2092 passed** · deploy-drift **PASS** · **G-7 CLEAR** (11 plans, 0 gap-closure). ⚠ **Phase 193 STILL owes its own U1/U2 — nothing here discharges them.**"
resume_file: .planning/phases/193.1-template-first-authoring/193.1-UAT.md
---
...
**Phase:** **193.1 Template-First Authoring — ALL 11 PLANS EXECUTED AND MERGED (2026-08-15). NOT COMPLETE: blocked on three human-only UAT rows, not on code.**
