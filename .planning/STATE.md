...
last_activity: 2026-08-09 -- **Phase 190 plan 16 EXECUTED - SETTINGS -> CONNECTIONS, and the blocking D-190-DEF-07 contradiction RESOLVED rather than passed on.** Branch **(b)** taken: the 190-09 write gate STAYS and the COPY moved - UI-SPEC 2h's second sentence, the rendered `CONNECTIONS_BANNER_BODY`, AND the structural half a copy-only fix would have missed (while `live_connectors` is off, every write affordance is REMOVED, not disabled) - all in ONE commit `5af38925`. Branch (a) was refused ON MEASUREMENT: it would delete a security gate from a security phase inside a Settings-table plan and turn a shipped plant-driven test RED (`test_190_connectors_api.py` case 9). The four-edit reversal cost is written in three places, and **9's panel notice inherits the same false sentence and is OWED BY 190-17.** **The `Used by` count was MEASURED before it was chosen** (the plan's own OQ#6): 204 workflow_definitions, **125 published**, **48 kB** of definition JSONB, 2.21 phases avg / 5 max, server-side scan **1.6 ms** - so the LIVE count is taken rather than the on-demand-expand fallback, and it costs **zero new wire** because `GET /workflows/published` already returns `definition` inline. The read is OWNER-scoped, so the count is a FLOOR: zero renders `none you can see`, never a bare `0 steps`, with the scope stated ONCE at the table's foot. Sketch 155-C is IMPLEMENTED, NOT REINTERPRETED - five locked columns on the shipped 068-A divide-y roster (no `<table>`, no shadcn primitive, ZERO installs), the filter bar + live count, four states as GLYPH AND WORD, TWO DISTINCT empty states, graded guards (Delete always sheets; Disable sheets only above zero; Enable flips direct at nine victims) with the receipt idiom and NO toast mounted anywhere. Connections is the sixth tab at routing key `5`, rendered visually fourth, with 0 renumbered keys and 0 deep-link hits, both measured against HEAD. **34 cases, SEVEN driven RED against real plants in production source**, each restored md5-identical - and the one to read is the Add button planted as `disabled` instead of REMOVED, because a `toBeDisabled()` assertion PASSES on exactly that defect. BOTH count-gate knobs moved in the same commit (`src/components/settings/` was inside NEITHER, so the suite would never have RUN); pinned at 34 from the script's own `actual` across two agreeing runs, then PROVED to catch a deletion. Worth inheriting: this suite's first gate run red SIX UNRELATED shipped suites as bare STACK_TRACE_ERROR timeouts, purely because `userEvent.setup()` defaulted to a per-keystroke delay - `delay: null` cleared all eight. `tsc` 33 -> 33 unmoved; `eslint src/components/settings/` 5 -> 5 (a sibling `connectionsCopy.ts` takes 190-12's named fix rather than repeating its 5 -> 10 debt); count gate OK 50/50 `failed 0`; all eight fences blob-identical at BOTH `de122b9a` and HEAD; package diffs empty. New **D-190-DEF-09**: `live_connectors` has no operator card in the Control Room and `GovernedFeature` does not name it - widening costs `tsc` 33 -> 38 across five exhaustive maps, TWO inside `/admin` which D-25 fences, so both halves are owed by one later commit.
last_activity_prev: 2026-08-09 -- **Phase 190 plan 14 EXECUTED - the standing fences, and SIX plants driven RED in real production source.** Two test files, 19 tests: `test_190_connector_source_fence.py` (D-05's no-transport walk over `connectors/**`, the D-04 closed-registry proof, the capability-resolution promise 190-08 left with a date on it, and the SCOPED credential fence) and `test_190_ssti_fence.py` (SC#3 + D-32's scope tripwire). **THE HEADLINE IS AN HONESTY CLAIM, NOT A BUILD: SC#3 WAS PROVED, NOT BUILT** - 190 added no evaluator, `_adapter_args` is a closed two-column lookup, and the shipped `SandboxedEnvironment(autoescape=True)` sites were written by Phase 101; the module docstring opens with that sentence, per D-09. **D-05's six literal substrings were REPLACED by property matchers because the substring form was measured wrong in BOTH directions** - `import requests` contains none of the six (under-fires) and a docstring sentence ending '...the number of requests.' contains one (over-fires); the control then proves all six contract tokens still fire inside real source lines. **PLANT F is the most useful thing in this plan: it caught the fence being INERT** - `assert "SandboxedEnvironment(autoescape=True)" in text` still PASSED with the real construction swapped for a bare `Environment(`, satisfied by the DOCSTRING eighteen lines above it (190-02's lesson verbatim); the matcher was strengthened to require an assignment and re-driven RED. **PLANT B aborted the entire pytest session at COLLECTION** - a 4th registry key is an `AssertionError` while `conftest` imports `app.main`, so `test_the_registry_key_set_is_derived_not_retyped` did NOT report RED, and that is recorded as what it was rather than dressed up. Plants C/D/E are one per WALK ROOT (`phase_types.py`, `connectors/**`, `egress.py`) so the walk's reach is proved by observation, not by its own non-vacuity assertion. Credential fence SCOPED per 190-11 (`Authorization` appears exactly twice under the package, both in `slack_adapter.py` - tree-wide would have been RED on correct code from its first commit), with the replacement drives asserted to EXIST. Full suite measured BOTH ways: **211 failed/3542 passed without the new files, 211 failed/3561 passed with - +19 passed, ZERO new failures.** The 189 acronym fence untouched and green, and no production prose was written, so the prose-vs-fence conflict did not recur a fifth time. WARNING - **a latent import cycle was MEASURED and deliberately NOT fixed**: `import app.services.connectors.registry` in a cold interpreter raises `ImportError: cannot import name 'get_adapter' from partially initialized module` (registry -> harness.grounding -> harness/__init__ -> phase_types -> registry). Unreachable today for exactly one measured reason - `phase_types.py:94` is its ONLY importer under `backend/app`. Created by 190-08+190-13, and this plan is test-only under D-32, so it is logged as **D-190-DEF-08** and given a TRIPWIRE (`importers == ["app/services/harness/phase_types.py:94"]`) rather than a fix.
stopped_at: "Phase 190 plan **16 of 19 EXECUTED** - Settings -> Connections ships whole and D-190-DEF-07 is RESOLVED (branch b: the gate stays, the copy AND the structure moved in one commit). Commits `5af38925` (the table + the copy module + the resolution), `34221a41` (the sixth tab at routing key 5), `307b4099` (34 cases + both count-gate knobs). ⚠ **190-15 IS STILL OPEN - 16 shipped OUT OF ORDER**, which its own frontmatter licenses (`depends_on: [190-09, 190-12]`, both landed). The consequence is on the surface rather than hidden: `Check credential` is REMOVED, not inert, until 190-15 lands the client function beside its endpoint. **NEXT: `190-15`** (the credential check + Gate 2), then `190-17` - which MUST amend UI-SPEC 9's panel notice in the commit that builds the panel, because that sentence is false under branch (b) for the same reason 2h's was. Owed and named, not discovered: **D-190-DEF-09** (no Control Room card for `live_connectors`; the banner names the right home before that home has a row in it)."
...
Phase: 190 (live connector slice + connector security — stretch) — **EXECUTING, plan 16 of 19**
⚠ This heading read *"plan 13 of 19"* while 14 and 16 had shipped — corrected on MEASUREMENT at 190-16 (`ls .planning/phases/190-*/*-SUMMARY.md | wc -l` → **15**), not incremented from the previous value. **And the count is 15 rather than 16 because `190-15` IS STILL OPEN: plan 16 shipped OUT OF ORDER**, which its own frontmatter licenses (`depends_on: ["190-09", "190-12"]`, both landed) — 190-15 is an independent wave-5 sibling, not a prerequisite.
