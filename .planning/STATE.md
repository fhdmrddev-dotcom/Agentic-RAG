...
status: executing
last_updated: "2026-08-08T04:05:00.000Z"
last_activity: 2026-08-08 -- **Plan 189-14 COMPLETE — the author can CHOOSE a capability, and the arming switch tells the truth about being unmovable.** **THE HOT-FILE LEDGER ROW IS HONOURED BY CONSTRUCTION AND MEASURED, NOT ASSERTED**: git diff --numstat on PhaseFormPanel.tsx = **11 insertions / 0 deletions**, of which the render body takes 5 lines and **THREE are JSX** — one under Phase 185’s four-insertion benchmark. Everything else lives in ExternalActionSection.tsx (185 L), GovernanceSection.tsx’s whole-file shape with all six contract properties copied AND each one guarded. **D-23’s CAPABILITY half is now MECHANICAL** — 189-12 built the phase-type half; this suite reads harness.py’s capability Literal through the same ?raw loader and asserts the client mirror matches EXACTLY, with a falsifiable extractor and a non-empty-haystack guard so two empty lists cannot agree forever. The mirror is Object.keys(EXTERNAL_CAPABILITY_SENTENCES) — **ONE constant read twice, DERIVED not re-typed**. ⚠ **UI-SPEC §7b IS RESOLVED AS OPTION (a)** — the half 189-13 assigned in writing: the stored capability renders pre-selected because the component reflects its prop; no “unset” representation was invented (the D-15 fence forbids making the field optional). The *no row selected* state stays REACHABLE and DRIVEN for an absent, unrecognised or INHERITED value. ⚠ **THE RAW CAPABILITY ID NEVER REACHES THE DOM, AND THAT WAS FORCED RATHER THAN CHOSEN**: 189-13’s own D-189-DEF-02 guard asserts the whole external-action panel’s innerHTML contains none of the three names, so the obvious data-capability={name} spelling would have turned a shipped governance guard RED. Rows are addressed by accessible name. Same class of reason: the mount keeps the INLINE literal pt === "external_action", because a named constant would have made the rails suite’s <ToolsField proximity fence silently vacuous. **D-04 ON SCREEN**: ARM_PINNED_TYPES (a SEPARATE list from DIAL_TYPES, with a docblock forbidding the merge) renders the switch ON, disabled, aria-disabled, cursor-not-allowed, with **exactly ONE** new sentence — ACTION_RISK_LOCKED_REFUSAL (D-24), export const count **28 → 29** — as real DOM text in the shipped amber refusal block wired by aria-describedby. **NOT struck through** (asserted, with a positive control proving line-through really does render where 142-B puts it) and **NOT removed**: the 185 rule is *replace a DEAD control with the sentence that states the fact*, and this control is pinned, not dead. ⚠ **A REAL DEFECT NO PLANNING DOCUMENT NAMES**: minimalPhaseFor("external_action") emits NO action_risk_armed, so the shipped code would have painted the switch **OFF** on a freshly-placed step — the exact lie D-04 exists to prevent, in the interval an author is actually looking at it. The rendered state is now armPinned || actionRiskArmed, and the guard supplies actionRiskArmed: false ON PURPOSE. **SIX wrong fixes planted into production source, every one RED**: **A** a fourth client capability (6 failures — only the cross-language fence can see a rename) · **B** “never leave the picker looking unset”, the one a reviewer waves through (4, T-189-39) · **C** a hand-typed mirror copy (1, attributable to one claim) · **D** a switch pinned only in its ATTRIBUTES (1 — caught by the DRIVEN CLICK, which an attribute read cannot be) · **E** the struck-through switch (1) · **F** the reason ALSO as a title (2, on two independent nets). All restored md5-identical, grep -c PLANT → 0 across four production files. **The six-type NEGATIVE CONTROL is written FIRST**, because the whole block would otherwise pass on a switch disabled everywhere. DIAL_TYPES byte-identical to HEAD; the 189-07 D-26 docblock untouched; **V22 re-run GREEN** with a zero-line backend diff; the six fenced card-subtree modules have a zero-line diff; no file deleted in either commit. ⚠ **THREE INHERITED CLAIMS MEASURED FALSE**: the plan’s gate baseline (2508, 45/45) was **STALE at 2587, 46/46** — the FIFTH consecutive plan in this phase to inherit one; the required **TARGETS entry does not exist and must not** (src/components/workflows is already a DIRECTORY entry, so the new suite RAN on creation — only the BASELINE pin was owed, added in-commit per 188-12); and UI-SPEC’s “eleven governance strings at :450-502” is **THIRTEEN at :456-541**. One blocking constraint found by LINT, not by planning: react-refresh/only-export-components **forbids exporting the mirror beside a component**, so it is module-private and the suite reads it from its one home. tsc **33 → 33 unmoved**; vite build ✓; the whole src/components/workflows sweep **36 files / 2195 tests, 0 failed**; count gate **2587 → 2612 → 2619**, pinned files **46 → 47** (ExternalActionSection.test.tsx new at 25, GovernanceSection.test.tsx 42 → 49), both pins in-commit, no per-file decrease. 2 commits (0024e03e, 368ef797). **CONN-01 stays Pending — 2 plans owed.** Full detail: 189-14-SUMMARY.md. *(prior record follows)* **Plan 189-13 COMPLETE — the 7th step type HAS A FACE: one 3D mark, one tint, three words, a capability-derived sentence and the predicate its badge will read.** ⚠ **THE TWO INVARIANTS THAT COULD SILENTLY BREAK ARE NOW MACHINE-CHECKED PROPERTIES, NOT DOCBLOCK SENTENCES.** soulData.PHASE_GLYPHS and lib/phaseGlyph.PHASE_GLYPH_MARKS both gained external_action -> outbox-tray in ONE COMMIT (f5ba6c5a lists both files), and the same-commit rule is now asserted as **IDENTICAL KEY SETS** via a new PHASE_GLYPH_MARK_KEYS export — so a NINTH type inherits the guard rather than six comparisons quietly ceasing to cover; **PLANT Y** (the key added to one map only) produced 3 failures. ⚠ **The KEYS are exported and the MAP is not, deliberately**: handing out the map would let a caller bypass phaseGlyph()'s own-property guard, which is the [Function Object] React child that hard-crashed a node face at 188.1-04. **The slug was VERIFIED, not trusted** — the installed @iconify-json/fluent-emoji@1.2.7 icons.json was read directly (3174 icons: outbox-tray PRESENT, bare outbox ABSENT) — and *“a missing slug FAILS THE BUILD”* is now a MEASUREMENT in this repo: planting outbox produced `Error: Icon fluent-emoji/outbox not found`, and the restored build succeeds. ⚠ **Presence is NOT visibility** — jsdom paints nothing, the 168.4 luminance figure is an unweighted palette estimate, and **UAT row U1 (189-16) remains the only real check**, said in the test's own comment. **D-13's tier is (4), an INTEGER on a NAMED gate**, sitting directly above HUMAN INPUT because tiers 1-3 read injected NAMES and can fabricate while 4 and 5 read the config alone and cannot; the tiers below were RENUMBERED, never decimalised, and the integer numbering is asserted on the SOURCE with a positive control. **EXTERNAL_CAPABILITY_SENTENCES is ONE exported constant** for 189-14's picker to read — never a second copy — and its lookup is own-property guarded in the **INLINE** form, because phaseVocabulary.ts still has **ZERO imports** (asserted mechanically, not claimed). **TWO WR-04 SINKS ARE NOW GUARDED WHERE THERE WAS ONE UNGUARDED**: the new capability lookup, and canvasModel's PHASE_TYPE_SUBTITLES read — the only read of that map outside its declaration — whose shipped nullish fallback did NOT fire for an inherited key. **FOUR wrong fixes planted into production source, every one RED and each attributable**: **Z1** the plausible bare lookup returned **[Function Object]** verbatim, the measured crash value; **Z2** the ungated tier let llm_single claim *“Sends an email”*; **Z3** a fabricated face for an unknown capability; **Z4** the fused one-liner that would force Phase 190 to re-open the adapter and the card. All restored md5-identical, PLANT residue 0 across five production files. **BOTH tripwires 189-12 armed FIRED and are DISCHARGED** — and one was **MIS-ASSIGNED**: TYPES_AWAITING_A_VOCABULARY_ENTRY was recorded as 189-14's but is 189-13's, because THIS plan owns all three vocabulary maps while 189-14 owns the PICKER (the phaseVocabulary.corpus.test.ts pin moved here for the same reason). Both cases were **REWRITTEN IN PLACE, never deleted** — the gate refuses a per-file decrease, and deleting would drop the only assertion that no row sits on the fallback floor. ⚠ **189-12's OPEN QUESTION IS RESOLVED, AND ITS UNNAMED HALF WAS FOUND BY RUNNING THE SUITE**: the **picker row** for external_action now reads **Sends an email**, not UI-SPEC §6d's predicted *Reach outside*, because minimalPhaseFor emits the real default capability so tier 4 fires on the preview. That is a DECISION, not a drift — the binding property is WR-03, *the row previews the card that LANDS*, and it does. The tier-3 floor (*Reach outside*) stays REACHABLE and driven for a stored capability that is absent / unrecognised / inherited; **the PICKER-SELECTION half of §7b is EXPLICITLY ASSIGNED TO 189-14** with a recommendation (render send_email pre-selected). ⚠ **D-189-DEF-02 IS CLOSED — and its PREMISE was measured FALSE first**: the author-facing rail is already unreachable for this type BY CONSTRUCTION (six mutually exclusive pt === branches, no default arm), so **no render code changed**; what was missing was the GUARD, and five cases now fence it, **four of them RED under PLANT Z5** (the exact D-20 wrong fix: give the type the generic rail and widen its options with the three capability names). toolOptions NOT widened, **V22 re-run GREEN**, backend and supabase diffs EMPTY. ⚠ **A RED TEST SHIPPED IN TASK 1'S COMMIT AND THE REASON MATTERS MORE THAN THE FIX**: the count gate printed `failed 1` and I matched it to model-info.test.ts — **which the gate does not run at all**. The real failure was panel/__tests__/PhaseTimeline.test.tsx, inside TARGETS but outside my hand-run scope. **A failure COUNT is worthless until you have named WHICH file produced it.** Fixed in Task 2 by NARROWING the case: two different tables answer there — the panel's DECLINED PHASE_TYPE_LABEL (still declined, still pinned) and the SHARED glyph map (which 189-13 did fill) — so byte-identity to an unknown type was never the property, only a coincidence of two empty tables. 14 canvas snapshots regenerated and **AUDITED rather than blessed**: the diff is **36 insertions of "notConnected": false and NOTHING else**, zero deletions. tsc **33 to 33, unmoved**; count gate **2560 to 2587**, five pins moved IN-COMMIT, no per-file decrease, 46/46, and five further suites changed ASSERTIONS but not COUNTS so their pins were deliberately left alone. Every count-bearing assertion written or edited is **DERIVED** (PHASE_TYPE_ORDER.length, Object.keys(PHASE_GLYPHS).length, a >= 7 floor) rather than re-pinned at 7. The six fenced card-subtree modules have a **zero-line diff**; CARD_SUBTREE_PATHS unmoved at 6; **no file deleted in any commit**. ⚠ **The plan's own context baseline was STALE** (2508, 45/45 vs the measured 2560, 46/46) — the fourth consecutive plan in this phase to inherit a stale gate figure. 3 commits (f5ba6c5a mark+tint, 6712f314 words+tier+predicate, 5963ba4b D-189-DEF-02). **CONN-01 stays Pending — 3 plans still owed.** Full detail: 189-13-SUMMARY.md. *(prior record follows)* **Plan 189-12 COMPLETE — the 7th `phase_type` is PLACEABLE: the picker offers seven choices, the shipped six unmoved, and `slugForType` + `minimalPhaseFor` build a phase the backend union accepts (SC#1).** `tsc` **33 → 37 → 33**, measured in three steps: the two SILENT edits (`PhaseTypeId` + `PHASE_TYPE_ORDER`) raised **four** errors — `SLUG_BASE`'s `satisfies`, a **KNOCK-ON `TS7053` at `slugForType` that no planning document lists**, `requiredConfigFor`'s `never` guard, and `REQUIRED_CONFIG_KEYS` in the TEST file — while **`PHASE_TYPE_ORDER` raised NOTHING**, exactly as predicted (`satisfies` admits a subset, so omitting the 7th would have compiled cleanly and the picker would simply never have offered the type). **`SLUG_BASE` is `act`, not `send`** — the token is keyed by TYPE, one for all three capabilities, so `send-` would be false on a `create_ticket` step, and nothing is sent in 189 anyway. ⚠ **`requiredConfigFor` emits a REAL capability (`send_email`), the one place the switch departs from its empty-string register, and the reason is structural**: `prompt`/`fn` are plain `str` so an empty one PARSES, but `capability` is a closed `Literal` (D-15) so `""` is a `ValidationError` that 422s the WHOLE definition on the first save — the step would be placeable and then unstorable, satisfying and falsifying SC#1 in one act. `available_tools` is EXCLUDED twice over (backend default + D-03 server-side derivation). **⚠ D-23 IS NOW MECHANICAL RATHER THAN EDITORIAL:** `definitionOps.test.ts` reads `backend/app/models/harness.py` through the `?raw` loader — the shipped cross-language idiom at `PublishGauntlet.test.tsx:46` — and asserts the client mirror equals the backend union's `phase_type` literals **IN ORDER**, plus that the emitted capability is a member of the server's closed `Literal`, with **positive controls on all three extractors** so an empty read cannot make it vacuous. The stale docblock pointer was corrected (**`harness.py:157-167` → `:288-299`**, stale TWICE over), and the fence is what survives the next time that number rots. **Three wrong fixes planted into production source, every one observed RED** (**V** an 8th client-only member → 3 failed, and **only the D-23 fence can see it** · **W** `capability: "wire_transfer"` → 1 failed · **X** `capability: ""`, the plausible wrong fix → 2 failed); restored md5-identical, `grep -c PLANT` → 0. **THE PIN LIST WAS RE-DERIVED AND `189-RESEARCH.md` IS WRONG IN BOTH DIRECTIONS: 5 sites it does not list, 6 it lists that do not move here.** Four of the five are invisible to a `toHaveLength(6)` grep because they are SHAPE assertions (an `undefined` subtitle concatenated into an expectation → `'external_actionundefined'`, a missing `<svg>`, a sentence read from a map with no entry) — **they were found by RUNNING the suite, not by grepping**, and the plan's own warning sign (a green `tsc` and a red vitest) is exactly what appeared. 19 sites moved across 5 suites, **12 of them counts, every one DERIVED from `PHASE_TYPE_ORDER.length` rather than re-pinned at 7** (189-10's lesson). The subtree-path, contract-export and icon-tint six-counts are **UNMOVED with a zero-line diff**, and `definitionOps.test.ts:1650` + `phaseVocabulary.corpus.test.ts:221` were measured to be **phase counts / 189-14's**, not this plan's. ⚠ **TWO FALSIFIABLE PLACEHOLDERS SHIP**: until 189-13/189-14 land the mark and the words, a placed node reads **`external_action`** with the **`•`** fallback — the shipped TOTALITY floor, never a fabrication — and each exclusion ASSERTS the entry is genuinely missing, so the owing plan's arrival is a RED test rather than a silent skip. Count gate **2546 → 2560**, both pins moved IN-COMMIT (`definitionOps.test.ts` 232 → 243, `StepTypePicker.test.tsx` 43 → 46), no per-file decrease, `TARGETS` untouched. ONE commit (`dcfdfb38`) covering both tasks, because Task 2's own action mandates it — the 7th type reds five suites the instant it lands. `toolOptions` NOT widened, V22 green, backend/`supabase/` diffs EMPTY. **CONN-01 stays `Pending` — 4 plans still owed.** Full detail: `189-12-SUMMARY.md`. *(prior record follows)* **Plan 189-11 COMPLETE — the D-05 loop is CLOSED and V20, the phase's headline gate, is GREEN: an `external_action` workflow PUBLISHES.** `record_phase_not_sent` is the FIFTH phase-keyed write — ONE `pool.execute`, ONE `UPDATE`, `complete_phase`'s atomic shape, **not** `fail_phase`'s payload merge (`grep -c "_failure_reason"` inside the body → **0**, and the identifier is unspelled even in the denial, because 189-09 learned a grep cannot tell a denial from a use). The engine gains a **THIRD branch** beside 101.1's emit-failure sentinel, consuming the **imported** `phase_types.RECORDED_INTENT_KEY` rather than a re-typed literal — **+78 insertions / 0 deletions**, so no shipped line changed. **"The run CONTINUES" is bought by PLACEMENT and PROVEN BY THE NEXT PHASE RUNNING**, never by the status alone: a branch that wrote the row and returned satisfies every status assertion, which is 189-05's PLANT E and 189-09's PLANT H exactly. **THE RECEIPT QUESTION IS ANSWERED EXPLICITLY** — a `recorded_not_sent` phase writes **no** `phase_completed` receipt; the ledger row rides the EXISTING `phase_transition` kind (`via: recorded_not_sent`), so **D-09 holds with `test_audit_event_registration.py` PASSING UNCHANGED (6)** and the literal set carrying no diff. ⚠ **AND NO SSE IS EMITTED EITHER, which is a DECISION WITH A STATED COST**: `phase_completed` maps to a "✓ Complete" card in `StreamsProvider.onPhaseCompleted`, so emitting it would paint the exact lie the branch exists to prevent — but emitting nothing lets `finalizeAllPhasesForThread` sweep the card to `done` at `run_completed`, so a live viewer sees "Complete" until reconcile. Logged as **`D-189-DEF-03`** with Phase 190 as its trigger (the fix needs a new SSE + a client handler + a sweep exclusion in ONE commit — a plan, not a deviation). **THE STRONGEST RESULT: both of V20's original REDs were re-observed THROUGH V20 ITSELF.** Quoting a prior plan's RED proves two things happened, not that they are connected — so each upstream fix was UNDONE in production source and V20 re-run: reverting 189-05 → `blocked_stage='golden_run_error'` naming *"the ask channel was subscribed with timeout_seconds=None"* (CONFLICT 1's mechanism verbatim); reverting 189-04 → `blocked_stage='grounding_fidelity'` with the `unregistered_tool` finding on `send_email` (CONFLICT 2's shape). Both restored, md5-identical, `grep -c REVERTPLANT` → 0. **This is why V20 mocks NEITHER stage 2.6 NOR the golden run** — only the leaf registry reads, the DB edges and the judge; the REAL union and the REAL armed checkpoint run, and the ask channel is the raising recorder so the file finishes in **0.88 s** and can never hang. Three wrong fixes also planted and observed RED (**J** the record skipped via `skip_phase` · **K** the status written `completed` · **L** the completion receipt written anyway) plus **M** the sentinel renamed; ⚠ **under J the receipt fence stayed GREEN, and under L BOTH V20 and the D-05 core test stayed GREEN** — the fourth time this phase that the headline proof could not see a real defect. ⚠ **A REAL DEFECT FOUND BY ACCIDENT, NOT BY PLANTING: V20's first run passed every publish assertion over an EMPTY phase spine** (`load_run_phases` → `[]`, zero phases looped, `published: True`) — a publish test asserting only the verdict cannot tell a workflow that ran from one that did not, which is now recorded IN the test as the reason the `recorded_not_sent` write is asserted at all. Deviations: **`tests/test_publish_gate.py` is the Phase-136 SKILL gate for the THIRD plan running** (V20 landed in `tests/unit/test_publish_service.py`; the gate file was RUN and left unedited — 11 passed, unchanged), and `test_audit_event_registration.py` is listed as modified while its own criterion demands it be UNCHANGED (the criterion won). ⚠ **RESEARCH §A3's seam pointer `:1605-1623` is 68 lines STALE — the branch is at `:1673-1680`** (189-05 added 60 lines above it). Nine-file scope **2 failed / 181 passed** (+3, the two named `pool is closing` rows unmoved); `tests/unit` **62 failed / 1745 passed** — **+1 = V20, ZERO regressions**; SC#4 fence still **0**; V22 still green; `test_migration_115.py` **PASSES (3), not skips**. `frontend/` and `supabase/` diffs are EMPTY. 2 commits (`7ce83c0d` the write · `61084eec` the seam). **CONN-01 stays `Pending` — 5 plans still owed.** Full detail: `189-11-SUMMARY.md`. *(prior record follows)* **Plan 189-10 COMPLETE — the canvas has its eighth word and its eighth ring, and the compiler-generated worklist is CONSUMED TO EXHAUSTION.** `tsc -p tsconfig.app.json` **40 → 38 → 33 — the baseline**; all seven armed `TS2741` are closed and every one of 189-08's cited line numbers was re-derived by symbol and **HELD**. The word is **`Not sent — recorded`** (D-16, byte-exact, em dash asserted by **CODEPOINT** — a hyphen or en dash is invisible in a diff), the clause is `null` with its reason at the table, and `RUN_READING_BORDER`'s **FIFTH deliberate absence** is recorded in the docblock AND pinned by a test. The ring is **UI-SPEC §4b IMPLEMENTED, not re-designed** — dash .15 / gap .10 / **repeats 4** / gapCentre .125 / **spinning FALSE** — and every number it predicted was independently recomputed before it was trusted: `4 × (.15 + .10) = 1` exactly, gaps at .125/.375/.625/.875 so **NONE at 12 o'clock**, dashoffset **16.022** and positive. **THREE WHOLE-TABLE INVARIANTS THAT EXISTED NOWHERE NOW DO** — exact tiling, four-arc uniqueness, pairwise-distinct geometry — each stated over the table so the **NINTH** reading inherits it, and **five literal `7`s were DERIVED from `ALL_READINGS.length` rather than re-pinned at 8** (a literal makes growth read as regression). **Colour spent NOTHING**: the stroke-set size is unmoved at **5** while the reading set grew to 8, and that unmoved number is now an assertion. **Six wrong fixes planted into production source, every one observed RED** (P the word reads `Complete` — *"the not-sent word must not be the success word"* · Q an EN dash · R the ring SPINS · S a gap at 12 o'clock, which also produced the negative dashoffset the UI-SPEC predicted · T `repeats: 2` colliding with `failed`, which fired the brand-new tiling invariant · U a stroke claiming success); all reverted, md5-verified, `grep -c PLANT` → 0. `CARD_READING_SHAPES`' 8th row is **CAPTURED** from the rendered DOM (observed TWICE, both dumps md5-identical) and spliced programmatically so no value was retyped; it pins three **ABSENCES** nothing else reaches — the DEFAULT border branch (`RUN_READING_BORDER` declining, rendered), the arc's missing spin class (`spinning: false` as an absent attribute), and no pause chip. ⚠ **A REAL DEFECT the plants did not find, the tests did**: the captured row first spliced into `CARD_BORDER_SHAPES`, because the three shape matrices declare readings → verdicts → **borders** and my anchor matched the last object before the target `describe`. Caught by `to deeply equal undefined`, relocated with a guard on the destination. ⚠ **`CARD_SUBTREE_PATHS` UNMOVED at 6** with a zero-line diff; only `NodeRunOverlay.tsx` touched inside the fence, at **6 insertions**. ⚠ **jsdom CANNOT prove greyscale distinguishability — U3 (189-16) is still OWED**; this plan proves the ATTRIBUTE half only. New suite `runVocabulary.test.ts` (the module's first, 23 cases); count gate **2521 → 2546**, all pins moved IN-COMMIT, no per-file decrease, `TARGETS` NOT edited (`src/components/workflows` is a DIRECTORY entry, so the new suite ran on creation — measured, not assumed). 2 commits (`d240836f` the word · `5b2cc2e1` the ring); 6 files, 1 of them new. Deviations, all recorded: `runVocabulary.test.ts` **did not exist** (a NEW file listed as modified); `PhaseNode.test.tsx` is a **7th file absent from `files_modified`** yet non-optional (one of the seven `TS2741`); `WorkflowRunPage.test.tsx` **was listed and needed NO edit** (it has no `Record<CanvasReading, …>`); an **unmeasured `tsc` claim reached commit 1's message and was amended** with the correction stated rather than smoothed; and the `BASELINE_TOTAL` note was found **stale at 2508 vs the gate's own 2521** — the very drift its own comment block already records, recurring one plan later. **CONN-01 stays `Pending` — 6 plans still owed.** Full detail: `189-10-SUMMARY.md`; every earlier plan's detail lives in its own SUMMARY, in Current Position and in the ROADMAP progress cell.
stopped_at: **Plan 189-14 is COMPLETE — the authoring surface is built and the arming switch refuses.** 14 of 16 plans done. **Next: 189-15** (wave 8) — the badge: slot 1 spent on “Not connected”, the reserved-slot prose flips, and D-21. ⚠ **FOUR THINGS 189-15 INHERITS, each named so it is not discovered.** **(1) ONE OF YOUR SIX PROSE SITES IS ALREADY DONE AND A SEVENTH EXISTS** — 189-13 flipped canvasModel.ts’s grounded docblock and phaseVocabulary.ts’s badge-slot section header in the commit that falsified them; the phaseVocabulary.ts site appears in NEITHER CONTEXT’s list of five NOR UI-SPEC §2e’s list of six. **Re-derive the list; do not transcribe it.** D-21’s canvasModel.ts:201-203 sentence is UNTOUCHED and is yours. **(2) notConnectedOf ALREADY EXISTS** (phaseVocabulary.ts, 189-13) and PhaseNodeData.notConnected is already on the interface and in the 13 canvas snapshots. What is owed is the TUPLE in PhaseNode.tsx:214-220 — **EXPLICIT BRANCHES, NEVER A SPREAD** ([...a, ...b] widens BadgeSlots to BadgeSlot[] and silently retires the max-2 typecheck guard). **(3) THE CARD SUBTREE IS FENCED AND MUST STAY AT 6.** CARD_SUBTREE_PATHS is unmoved; 189-14 touched none of the six. A third badge must remain a TYPECHECK ERROR and no focusable control may enter the card. **(4) NOTHING IN 189-14 TOUCHES YOUR FILES** — the panel, the picker, definitionOps and GovernanceSection are disjoint from PhaseNode.tsx / canvasModel.ts / PhaseNodeCard.test.tsx. ⚠ **BASELINES, MEASURED TODAY**: npx tsc --noEmit -p tsconfig.app.json = **33**; count gate **2619, 47/47, 0 failing** (the pinned-file count moved for the first time since 188-01). ⚠ **A NEW SUITE NEEDS ONLY A BASELINE PIN, NOT A TARGETS ENTRY** — src/components/workflows is a DIRECTORY entry and the gate runs a new file the moment it exists; 189-14 measured this rather than inheriting the plan’s claim, and the pin is still owed in the SAME commit (188-12). ⚠ **A FAILURE COUNT IS STILL WORTHLESS UNTIL YOU NAME THE FILE** (189-13’s lesson, honoured here: every gate run was read by suite name). ⚠ A pre-existing failure lives at src/lib/model-info.test.ts (costTier ‘mid’ vs ‘high’) — NOT in the gate’s TARGETS, unrelated to this phase, last touched by 47a6294b. ⚠ **UAT rows U1-U6 REMAIN OWED to 189-16** — jsdom applies no CSS and paints nothing, so nothing in 189-14’s green says the amber refusal block is legible on Deep Midnight or that the selected picker row reads as selected without colour. Prefer evaluate_script attribute/geometry reads; take_screenshot times out in this estate. Remaining: 189-15/16 (2 plans). ⚠ Carry forward: **cloud parity on migration 115 is OWED** (standing queue, migs 099 onward, applied in order at the next operator-driven production push — NOT during this phase; brief ACCESS EXCLUSIVE lock, T-189-20). *(prior record follows)* **Plan 189-13 is COMPLETE — the 7th step type has its face, and D-189-DEF-02 is closed.** 13 of 16 plans done. **Next: 189-14** (wave 7) — the authoring surface: ExternalActionSection (the capability picker, its OWN file plus ONE gated line in PhaseFormPanel, per the hot-file ledger), the ACTION_RISK_LOCKED_REFUSAL constant, and the arming switch rendering ON-and-NON-INTERACTIVE with its reason as real DOM text wired by aria-describedby — **never a title attribute**. ⚠ **THREE THINGS 189-14 INHERITS, each named so it is not discovered.** **(1) EXTERNAL_CAPABILITY_SENTENCES ALREADY EXISTS** — exported from phaseVocabulary.ts, closed at three, own-guarded. **IMPORT IT; do not create a second home.** D-23's *“ONE constant read twice”* is satisfied the moment the picker reads this one, and 189-12's note that the constant was still owed is now discharged. **(2) The §7b “nothing chosen yet” PICKER STATE IS 189-14's TO DECIDE, IN WRITING.** A placed node arrives carrying capability: send_email (189-12: a closed Literal has no empty placeholder), so *no row selected* is unreachable from a placed step. 189-13's recommendation: **render send_email PRE-SELECTED**, the honest reading of the stored config. The alternative — inventing an “unset” representation — is forbidden from being made optional by the D-15 fence in test_189_external_action_model.py. The §7b *no-row-selected* rendering stays correct for the case it was written for: an **unrecognised stored value**. **(3) The D-23 CAPABILITY-mirror fence is still owed** — 189-12 built the PHASE-TYPE half against harness.py via ?raw; the capability half is 189-14's. ⚠ **D-189-DEF-02 IS CLOSED and its premise was FALSE**: the generic tool rail was already unreachable for external_action by construction. Five guards now fence it, four RED under a plant. **189-14 MUST NOT widen toolOptions** — doing so re-opens the D-20 hole and will now fail four named tests plus V22. ⚠ **189-15 INHERITS A CORRECTION**: one of its six reserved-slot prose sites (canvasModel.ts's grounded docblock) was flipped HERE, in the commit that falsified it, and a **SEVENTH site exists that neither CONTEXT nor UI-SPEC lists** (phaseVocabulary.ts's badge-slot section header, also flipped here). **D-21's canvasModel.ts:201-203 sentence was left UNTOUCHED — it is 189-15's.** Re-derive the list. ⚠ **BASELINES, MEASURED TODAY — the plan's own context block was stale (2508, 45/45)**: tsc --noEmit -p tsconfig.app.json = **33**; count gate **2587, 46/46, 0 failing**. ⚠ **A FAILURE COUNT IS WORTHLESS UNTIL YOU NAME THE FILE.** 189-13 shipped a RED test in its first commit by matching the gate's `failed 1` to model-info.test.ts — **which the gate does not run**. The real failure was panel/__tests__/PhaseTimeline.test.tsx, inside TARGETS but outside the hand-run scope. Run the gate AND read which suite failed. ⚠ **A pre-existing failure lives at src/lib/model-info.test.ts** (costTier 'mid' vs 'high') — NOT in the gate's TARGETS, unrelated to this phase, last touched by 47a6294b. ⚠ **UAT rows U1-U6 remain OWED to 189-16** — jsdom applies no CSS and paints nothing, so nothing in 189-13's green says the outbox-tray mark is VISIBLE on Deep Midnight, that the badge is not occluded, or that the 8th ring survives greyscale. Remaining: 189-14/15/16 (3 plans). ⚠ Carry forward: **cloud parity on migration 115 is OWED** (standing queue, migs 099 onward, applied in order at the next operator-driven production push — NOT during this phase; brief ACCESS EXCLUSIVE lock, T-189-20). *(prior record follows)* **Plan 189-12 is COMPLETE — the 7th step type is PLACEABLE and its silent count debt is paid.** 12 of 16 plans done. **Next: `189-13`** (wave 5) — the authoring surface: `ExternalActionSection` (the capability picker), the `outbox-tray` 3D mark and its `ICON_TINT` entry, and the arming switch rendering ON-and-refusing. ⚠ **THREE TRIPWIRES 189-13/189-14 WILL TRIP BY DESIGN**, and they are the mechanism, not defects: `TYPES_AWAITING_A_3D_MARK` in `StepTypePicker.test.tsx` goes RED the moment the `outbox-tray` slug lands (189-13, which also owns `PhaseNodeCard.test.tsx:800`'s `ICON_TINT` pin, deliberately unmoved here); `TYPES_AWAITING_A_VOCABULARY_ENTRY` goes RED the moment `PHASE_TYPE_SENTENCES.external_action` lands (189-14, which also owes `phaseVocabulary.corpus.test.ts:221`'s pin, measured to be 189-14's rather than 189-12's). Each exclusion asserts the entry is genuinely MISSING, which is what forces it to be emptied rather than to outlive its reason. ⚠ **`tsc` is back at 33 and NOTHING is left armed** — a rise from here is a real regression. ⚠ **UI-SPEC §7b's *"nothing chosen yet"* picker state is NOT reachable from the picker any more**: a placed node arrives carrying `capability: "send_email"`, because a closed `Literal` has no empty placeholder and an unstorable step would falsify SC#1. **189-13 owns finishing that choice** — accept a pre-selected first option, or find a representation of "unset" that survives `model_validate` (the D-15 fence forbids making it optional). Not blocked, just named. ⚠ **A pre-existing failure lives at `src/lib/model-info.test.ts`** (`costTier 'mid' vs 'high'`) — it is NOT in the count gate's `TARGETS` and is unrelated to this phase. *(prior record follows)* **Plan 189-11 was COMPLETE — V20, the phase's headline gate, was GREEN.** 11 of 16 plans done. **Next was `189-12`** (wave 5) — the client's 7th step type: `PhaseTypeId`, the picker's seventh choice, the slug base and the required-config switch, plus the ~16 hard-coded six-count assertions across five suites that must move IN THE SAME COMMIT (a green `tsc` and a red vitest is that plan's signature). ⚠ **The backend half of 189 is now DONE end to end** — the node parses, arms structurally, publishes, runs, records and persists its own status. What remains is entirely client-side (189-12/13/14/15) plus the docs/validation close (189-16). ⚠ **`D-189-DEF-03` is NEW and it is the one thing a UAT row can catch that no unit test will**: during a live `external_action` run the run surface shows the governed step as **"Complete"** until a reconcile fetch replaces it with *"Not sent — recorded"*. It is a latency-of-honesty gap, not a persistent lie, and closing it is Phase 190's (new SSE + client handler + `finalizeAllPhasesForThread` exclusion, one commit). ⚠ **`test_publish_gate.py` is the Phase-136 SKILL gate — three plans have now been sent to it by mistake; the workflow gauntlet's tests live in `tests/unit/test_publish_service.py`.** ⚠ RESEARCH §A3's seam pointer is stale by 68 lines; re-derive by symbol as every plan in this phase has had to. Remaining: 189-12/13/14/15/16 (5 plans). ⚠ Carry forward: **cloud parity on migration 115 is OWED** (standing queue, migs 099 onward, applied in order at the next operator-driven production push — NOT during this phase; brief ACCESS EXCLUSIVE lock, T-189-20). *(prior record follows)* **Plan 189-10 was COMPLETE.** 10 of 16 plans done. Next was `189-11` (wave 5) — the D-05 seam: `record_phase_not_sent`, the run that continues, and **V20, the headline gate**. It is the plan that WRITES the slug this plan now words and draws. ✅ **`tsc` is back to 33 — treat 33 as the baseline again**; the seven-table worklist 189-08 armed is fully discharged, so a rise above 33 from here is a real regression rather than an armed reminder. ⚠ **`runVocabulary.test.ts` now EXISTS and is pinned at 23** — it is the canvas vocabulary's only suite; extend it rather than re-asserting table properties through a jsdom render. ⚠ **U3 (the DRIVEN greyscale row, 189-16) is OWED and is NOT discharged by any green unit run** — jsdom applies no CSS and paints nothing. ⚠ Five `ALL_READINGS.length` derivations now exist where literal `7`s were; a ninth reading will be covered by them automatically, but `EXPECTED_RING`, `CARD_READING_SHAPES` and the `RING_STROKE`/`RUN_READING_WORD`/`STATIC_CLAUSE`/`RING_GEOMETRY` tables will each force again — by design. Remaining: 189-11/12/13/14/15/16 (6 plans). ⚠ Carry forward: **cloud parity on migration 115 is OWED** (standing queue, migs 099 onward, applied in order at the next operator-driven production push — NOT during this phase; brief ACCESS EXCLUSIVE lock, T-189-20).
resume_file: .planning/phases/189-governed-external-action-node-model/189-15-PLAN.md
progress:
  total_phases: 21
  completed_phases: 10
  total_plans: 142
...
Phase: 189 (governed-external-action-node-model) — **EXECUTING, UNBLOCKED** (12 of 16 plans complete;
...
**Next: `189-13`** (wave 5 — the authoring surface: the capability picker, the `outbox-tray` mark
...
Last activity: 2026-08-07 -- Phase 189 plan 189-04 (Wave 2) executed: CONFLICT 2 resolved — `EXTERNAL_ACTION_CAPABILITIES` unioned into `tool_names` for the fidelity check ONLY; `GroundingBundle.tools` stays narrow (V21 RED→GREEN, V22 still GREEN, proved by two plants)
