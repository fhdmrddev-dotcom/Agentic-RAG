...
stopped_at: "2026-08-17 — **Phase 195 PLANNED — 8 plans in 4 waves (`b2c3674f`), plan-checker `VERIFICATION PASSED` on the FIRST iteration with ZERO blockers and one non-blocking documentation warning (since fixed).** The operator's single-digit directive was MET AT THE CAP: 8 plans, against 187's 29, 194's 13, 193.1's 11 and 194.1's 8. ⚠ **D-16 IS ALREADY DISCHARGED AND ITS COMMIT PREDATES THE PLANS — `195-BASELINE.md` at `7d131463`, driven live through Chrome before `b2c3674f` was written.** Measured on run `833e8e85` / thread `1189a1a3` / file `d4598e01`: the region renders, the row reads `Northwind-QBR-Template.docx` + `38.7 KB` (39,660 B / 1024 ⇒ KiB), and a real `button` carries `aria-label='Download Northwind-QBR-Template.docx (38.7 KB)'`. **SC#1 CONFIRMED PRE-CHANGE — scope does NOT grow.** ⚠ **THREE BASELINE FINDINGS RESEARCH COULD NOT HAVE KNOWN.** (1) **THE WHOLE RUN SURFACE IS BEHIND A KILL SWITCH WHOSE COLD DEFAULT IS `off` FOR EVERYONE, OPERATORS INCLUDED** — `ChatLayout.tsx:752` gates it on `canvasEnabled` and `user_settings.py:1197-1203` defaults `visual_workflow_canvas` to `off`; the live local DB reads `everyone` only because the operator flipped it, so **RUN-02's surface does not exist on a fresh install.** This is the identical trap 194.1's D-09 amendment hit. (2) ⚠ **CHAT RENDERS NO FILE CARD FOR A WORKFLOW DELIVERABLE AT ALL** — the only chat trace is plain markdown, *'Produced the filled deliverable: /Northwind-QBR-Template.docx'*, and `querySelector('[data-testid=output-file-card],[data-dead]')` returns **null**. It corroborates D-13 and D-14, and it **REFINES D-20: the three-surface side-by-side CANNOT use this file for the chat row** — `OutputFileCard` must be exercised with a sandbox `execute_code` file, or the evidence recorded as two live surfaces plus one fixture. (3) **Four measured deltas the extraction must reconcile, none stylistic:** the panel uses a DIFFERENT design token (`text-panel-muted-foreground`, a deliberate 088-05 AA-contrast choice) vs the page's `text-muted-foreground`; full `font-mono` path vs basename; `px-2.5` vs `px-2`; `div[role=option]` with roving tabindex vs `button`. **RESEARCH refuted ZERO of the 20 CONTEXT decisions** — the D-01 histogram reproduced EXACTLY (0→161 · 1→60 · 20→1), `run_claim` is 0/86 non-null re-confirming it is Phase 141 template-asset isolation and NOT the attribution field, and all six D-17 triples re-derived with zero drift. **What changed is blast radius, in eleven findings.** ⚠ **F1 — A SHIPPED FENCE ASSERTS THE EXACT DUPLICATION THIS PHASE EXISTS TO REMOVE:** `WorkflowRunPage.test.tsx:1467-1468` requires `function formatBytes` AND `function iconFor` to be PRESENT; the suite is pinned at **102 EXACT with zero slack**, so it is INVERTED IN PLACE (the 194.1-07 precedent) because deleting it silently forces a pin LOWERING. ⚠ **F2 — THE ARCHITECTURE WAS ALREADY MECHANICALLY CONSTRAINED BEFORE ANYONE DESIGNED IT:** two RAW-source absence arms (`not.toMatch(/useViewingThread/)` `:1464`, `not.toMatch(/FilePreview/)` `:1478`) force the shared row to be PURELY PRESENTATIONAL ⇒ D-09's 'parameterise the activation' is a CALLBACK PROP, never a preview import. ⚠ **F5 — `supersedes`, one of D-08's two must-survive states, has ZERO coverage repo-wide** and `OutputFileCard.test.tsx` does not exist anywhere: verbatim the Phase-194 `ActiveRunsTray.test.tsx` trap. It needs THREE plants, because the subline is written TWICE. ⚠ **F7 — D-12's SORT KEY IS ABSENT ON EXACTLY THE FILE D-12 EXISTS TO SURFACE:** the live SSE carries no `created_at` and the store APPENDS, so a naive DESC puts the just-produced deliverable LAST ⇒ the comparator must treat a MISSING key as NEWEST, and **PATTERNS measured that NO comparator in this tree does that** (`PendingAskCard.tsx:608-614` returns 0 ⇒ newest LAST, the exact inverse). New ground. ⚠ **F9 — A FIFTH FILE PRESENTATION EXISTS AND CONTEXT DOES NOT NAME IT:** `panel/SeamCard.tsx:72-80`, whose own docblock FALSELY claims it reuses `OutputFileCard`'s shape. Declared an explicit boundary so a future SC#2 dispute is pre-answered. ⚠ **F10 + PATTERNS — 'ONE ICON PATH' IS NOT A NO-OP AND WOULD HAVE SHIPPED A REAL PANEL REGRESSION:** `FilesSection`'s `iconFor` is MIME-FIRST while `fileIcon` is EXTENSION-ONLY, and `EXT_MAP` omitted **nine** extensions (`sh bash sql yml yaml css jsx mjs`) plus four glyph categories differ — invisible to all 11 shipped cases, which only use docx/pptx/xlsx. ⚠ **PATTERNS also found TWO INCOMPATIBLE `codeOf()` IMPLEMENTATIONS SHIP** — the unanchored one (`ChatLayout.launch.test.tsx:459`) EATS `//` inside URL string literals, and the sweep targets `OutputFileCard.tsx`, which carries them ⇒ use the line-anchored form, and adopt PATTERNS' recommended **27th plant** (a stripper non-vacuity case), without which a mis-stripping `codeOf` returning the empty string makes all four sweep arms pass GREEN. **PATTERNS measured EIGHT expected analogs ABSENT**, including a shared non-primitive component directory (17 dirs, all feature-scoped) and `OutputFileCardProps` NOT being exported — so the prop-set fence must be a source sweep. **VALIDATION registers 26 plants across 10 claims where a naive reading names 10**, plus the 27th. **FOUR PLANNER DEVIATIONS worth knowing, each measured rather than argued:** (a) the `downloadFrom` dispatcher RESEARCH sketched was **DECLINED** because `WorkflowRunPage.test.tsx:1450-1455` pins `downloadWorkspaceFile(runThreadId` as a **TENANCY** property — download from the RUN's thread, never the viewed one — so routing it through a util would force inverting a fence guarding a real security property; (b) `FileRow` takes `density: 'chat' | 'panel' | 'run'` with all class strings in ONE record, because one skin would restyle three surfaces and D-18 declined a sketch — explicitly NOT the retired hero/working `cva` shape; (c) `fileIcon` gains `tone: 'inherit'`, not `'muted'`, since `'muted'` would hardcode a token the page is fenced against; (d) `StopControl.baseline.test.tsx` is ADOPTED into the gate — it greps raw `WorkflowRunPage.tsx` and was ungated, and a pin in an ungated suite is a pin nothing checks. ⚠ **THE COUNT GATE DOES NOT COVER THIS PHASE: `TARGETS` has NO entry for `src/components/chat` at all**, nor `src/lib`, nor `src/components/panel` — only 1 of 5 in-scope suites is gated, so a green gate says nothing about four fifths of the work and the grand total reads IDENTICALLY before and after. Both knobs run every wave. **Fresh baselines: gate `OK · total 3972 · failed 0 · pinned 3898 · 75/75` (⚠ CLAUDE.md's 3918/3868 is STALE — a growing total is the gate WORKING) · `tsc -p tsconfig.app.json` = 33 unmoved · suites 102/11/11/11/15.** ⚠ **THE DECISION-COVERAGE GATE IS A FAR WEAKER INSTRUMENT THAN ITS NAME: it parsed only 1 of 20 CONTEXT decisions as trackable** (`total: 1`), initially failed on D-20 for lacking a literal citation in `must_haves`, and passed once one line was labelled. All 20 were verified by the plan-checker instead. **G-5 fires on five files and is HONOURED BY CONSTRUCTION — the extraction IS the requirement; an override was OFFERED AND DECLINED for the SIXTH consecutive phase, so the absence of a 195 entry under `Guardrail overrides` is a MEASUREMENT.** G-2 declined (D-18, no sketch); G-7 not in play. ⚠ **THREE ENVIRONMENT FINDINGS THAT EACH PRODUCE WRONG READINGS SILENTLY:** vite binds **`[::1]` ONLY**, so an IPv4 probe of `127.0.0.1:5173` reports the frontend DOWN while it is running (this produced a wrong claim the operator corrected); **the Chrome extension's coordinate clicks do NOT reach this React app** (two correctly-measured clicks were no-ops, `element.click()` worked immediately); and **a 700ms settle manufactured a false empty spine on the very thread being looked for** — use 2.5s+. Word 2016 is installed and `.docx` is associated, so D-20's open-the-file bar is achievable, not aspirational. NEXT: `/gsd:execute-phase 195` — dispatch worktree agents ONE MESSAGE AT A TIME, ≤2 concurrent test-running agents, `GSD_VITEST_MAX_WORKERS=2`, and ⚠ **195-01 and 195-08 are `autonomous: false` — their checkpoints need the ORCHESTRATOR's browser, not an executor.** ⚠ STATE frontmatter HAND-EDITED (not `state.record-session`, which has corrupted this file before). · PRIOR: 2026-08-17 — **Phase 195 CONTEXT GATHERED (`99bdd152`) — 20 decisions locked in `195-CONTEXT.md`, and THE PHASE'S SHAPE CHANGED because its own ROADMAP flag (*'first task is measurement'*) was executed at discuss-phase instead of deferred to planning.** ⚠ **SEED-148 RECORDS THE WIRE QUESTION AS 'EXPLICITLY UNMEASURED' AND FRAMES 195 AS POSSIBLY-BACKEND. IT IS NOT. Measured against source + the live local DB: a workflow `llm_emit` deliverable IS persisted as a thread-scoped workspace file (`tool_dispatcher.py:3550-3578`) AND emits `workspace_file_written` on the PRODUCER stream (`_ProducerStreamCtx`, `harness/phase_types.py:1161-1192`); 86 `workspace_files` rows hold real 37-40 KB `.docx` deliverables through 2026-08-16.** ⇒ **195 writes NO Python and ships NO migration.** ⚠ **SC#1 IS MEASURED ALREADY-SATISFIED: `WorkflowRunPage.tsx` has listed and downloaded a run's files since Phase 188-10 (`783daab5`, *'the deliverable region — listed and downloadable, never previewed'*), `useWorkspaceFiles(run.thread_id)` at `:432`, rendered `:1015-1090`.** ⚠ **SEED-148's `grep output_files` WAS TRUE AND MISLEADING — IT SEARCHED THE WRONG NOUN**; the run surface reads WORKSPACE files, not sandbox `output_files`, and the region already existed two days before the seed was planted. ⚠ **`BUG-260816-05`'s capability table row *'see produced output files | canvas ❌'* IS REFUTED and corrected BESIDE the original in the report itself.** **The real work is RUN-03: there are FOUR file presentations and the run page's OWN shipped region is one of them** — `lib/fileIcon.tsx` (the DECLARED single source, consumed by `OutputFileCard` only), `FilesSection`'s inline map + a `formatBytes` commented *'copied verbatim from OutputFileCard'*, `WorkflowRunPage`'s hand-rolled `iconFor()` + rows, and `lib/fileIcons.tsx` on the documents side (explicitly NOT swept). ⚠ **THE BLOCKER THAT KILLS A NAIVE REUSE, AND IT IS NOT STYLISTIC: `OutputFileCard` identifies a file as `{filename,url}` and treats a MISSING `url` as a DEAD FILE (red, `aria-disabled`, *'Download unavailable'*, `:92-119`); a workspace file has NO `url` at all — so feeding workflow deliverables into it unchanged renders EVERY ONE AS A BROKEN LINK.** ⇒ D-05 extracts ONE shared row on a discriminated source (`{kind:'sandbox',url}` | `{kind:'workspace',threadId,fileId}`), homed on NEUTRAL ground (**not** `components/chat/`), with all three surfaces converted in-phase. ⚠ **SC#3 NAMES A RETIRED PATTERN AND CANNOT BE SATISFIED AS WRITTEN — Phase 095.1 (D-095.1-06) REVERSED the hero/working split by an operator-approved decision (`variant` INERT, `is_hero` written-but-unread), while `sketch-findings-agentic-rag/references/chat-tool-card-unification.md` STILL records sketch 016-A 'Hero block' as the WINNER and lists a no-hero list under ❌ anti-patterns. BOTH records are corrected BY this phase (D-11), beside the original, never over it — otherwise the next UI phase builds a hero block from the skill.** **Scoping decided against the DB rather than argued: 226 runs / 222 distinct threads, files-per-thread histogram 0→161 · 1→60 · 20→1, so thread-scope is EXACT for 60 of 61 file-bearing runs; the one 20-file thread is an agent CHAT (19 matplotlib PNGs) that also ran a workflow.** ⚠ **`run_claim` LOOKS like the attribution field and IS NOT — 100% NULL across all 86 rows, it is Phase 141 COLL-02 template-asset context isolation; `kind` is NULL on 84/86. No migration, no `run_id`, and a time-window filter was REJECTED as an inference.** **D-14 records a MEASURED LIE the phase DECLINES: `RunCard`'s file badge parses `output_files` out of `tool_calls` and a workflow emit returns `path`, so it reads 0 for a real deliverable; `ThreadRunLine.tsx` has ZERO file references.** ⚠ **D-07 ('convert all three') and D-13 ('chat unchanged') are COMPATIBLE and MUST NOT BE CONFLATED — presentation converts, capability does not.** ⚠ **G-5 FIRES ON FIVE FILES AND TWO ARE ABSENT FROM THE LEDGER** — re-derived 2026-08-17 with the CLAUDE.md recipe: `WorkflowRunPage.tsx` **12/3/1101** (present, at threshold — 195 makes it the 4th phase), **`OutputFileCard.tsx` 7/6/187 — ABSENT**, **`FilesSection.tsx` 6/3/277 — ABSENT**, `MessageItem.tsx` 57/29/856 (extraction due), `RunCard.tsx` 21/9/608. **G-5 is HONOURED BY CONSTRUCTION — the extraction IS the requirement (RUN-03) and ships as the structural first move; an override was OFFERED AND DECLINED for the SIXTH CONSECUTIVE PHASE (193, 193.1, 193.2, 194, 194.1, 195), so the absence of a 195 entry under `Guardrail overrides` is a MEASUREMENT.** Ledger rows + `docs/HOT-FILE-LEDGER.md` sections for both absent files (same-commit rule) are a PHASE DELIVERABLE. ⚠ **G-2 FIRES AND WAS DELIBERATELY DECLINED — no sketch (D-18), recorded as a decision, not skipped silently**: RUN-03 forbids new file UI and the one genuinely-open visual question was settled in discussion instead. **G-7 not in play (zero rounds).** **Bug routing (FRONTMATTER UPDATED, not just decided): `BUG-260816-05` — its `re_open_trigger` named `/gsd:discuss-phase 195` explicitly, the arm FIRED, and the operator ruled FILE HALF ONLY; the ask-responder half stays Phase 198 / NODE-02. Status left `deferred`, DELIBERATELY NOT `folded`, because a `folded` record is invisible to the open-report routing scan (the `feedback_bug_status_frontmatter_is_the_index` lesson).** `BUG-260813-01` reviewed, offered, NOT folded — stays `open`. ⚠ **D-16: the phase's FIRST task is to PROVE SC#1 on a LIVE run before changing anything — a baseline only proves something if it PREDATES the change.** ⚠ **D-04 records an ACCEPTED BLIND SPOT, not a proven absence: 161 of 222 workflow-run threads produced no file and this phase does NOT check whether any of them had an emit phase.** **G-4 acceptance bar (DRIVEN, not owed): launch a real template workflow → watch it on the run surface → download the `.docx` → OPEN IT; plus a three-surface side-by-side. Wire + screenshot explicitly insufficient. DO NOT locate the row by `getElementById` (192.1 D-27).** ⚠ **THIS FILE'S PREVIOUS ENTRY PREDATES PHASE 194.1's CLOSE — the ROADMAP records 194.1 COMPLETE + VERIFIED 2026-08-16 (8 plans / 6 waves, UAT 5 of 5 PASS, `194.1-VERIFICATION.md`) while `stopped_at` below still reads PLANNED. Recorded rather than silently overwritten; RUN-01 remains UNTICKED and L-01 STILL HAS NO ROADMAP HOME.** NEXT: `/gsd:plan-phase 195` — `/clear` first. ⚠ STATE frontmatter HAND-EDITED (not `state.record-session`, which has corrupted this file before). · PRIOR: 2026-08-16 — **Phase 194.1 PLANNED — 8 plans, 6 waves** (`5f1df776` … `c66e234d`), plan-checker verdict **PASS** with zero CRITICAL and zero MAJOR. **The operator's efficiency directive was MET AT THE CAP, NOT OVER IT: 8 plans against a single-digit target, where 193.1 ran 11, 194 ran 13 and 187 ran 29.** The two named levers were both taken — R4+R6 share ONE read, R4+R5 are ONE component in two states (D-15). ⚠ **D-09 IS AMENDED BY MEASUREMENT AND THE OPERATOR DECIDED IT AT PLAN TIME, with the original left standing beside it, not overwritten.** D-09 locked *'one owner-scoped GET on the existing `workflow_runs` router'* on the stated premise that *'the stop NULLs the anchor, so no cheaper construction exists'* — **both halves measured FALSE.** The `workflow_runs` router is canvas-gated (`middleware/canvas_gate.py:85-93`, `workflow_runs.py:169`) and `visual_workflow_canvas` **ships off** (`models/user_settings.py:1197-1203`), so an R4 **chat**-surface receipt fed from it would have been **invisible at the shipped default** — a trap neither the SPEC nor CONTEXT named. And a cheaper construction DOES exist: `GET /threads/{id}/workflow` is **ungated** and already resolves anchor-then-latest via `last_workflow_run_id`, added by **Phase 188 CR-03 for the identical problem**. ⇒ widen the shipped `ThreadWorkflowState` with three additive-optional fields — **no new route, no `CANVAS_GATED_PATHS` edit, no migration, ONE round trip**. ⚠ **R6 needs NO backend work at all (D-09b)** — the chain is shipped end to end (`last_workflow_run_id` + `194-11`'s dual-id fallback on `DELETE /runs/{id}`). **THREE FURTHER SPEC CORRECTIONS, each measured and now binding as D-22…D-25:** (1) **the composer Stop does NOT go through `stopThread`** — it goes through `stopStream` (`ChatArea.tsx:361` → `useMessages.ts:118`) and the false-cause string exists **TWICE** (`StreamsProvider.tsx:2394` **and** `:2457`), so the SPEC's `grep == 0` acceptance was **unsatisfiable as written**; (2) **R5's kickoff gate must carry a REPAIR or it ships a NEW silence** — the placeholder it deletes is the sole carrier for the **403 kill-switch amber notice** (the harness path is the ONLY path that raises it) and the network-failure flip; (3) **R1's slot fence CANNOT be geometric** — jsdom returns 0 for all layout, so the honest fence is class-token identity on a fixed-size reservation wrapper. **38 named plants pre-registered in `194.1-VALIDATION.md`, every one required RED before its fix.** ⚠ **G-5 honoured BY CONSTRUCTION for the FIFTH consecutive phase (193, 193.1, 193.2, 194, 194.1) — an override was OFFERED AND DECLINED, so the absence of a 194.1 entry under `Guardrail overrides` below is a MEASUREMENT, not an omission.** All six ledger rows are fixed in plan 08 (3 ADD, 2 UPDATE, 1 CORRECT-BESIDE). ⚠ **L-01 / CR-04 STILL HAS NO ROADMAP HOME; RUN-01 STAYS UNTICKED**, and plan 03 carries a fence that reds against *fixing* the `⊘ Stopping this run…` → `✓ Complete` sequence by suppressing the terminal reading. ⚠ **One plan-checker MINOR was CONFIRMED BY MEASUREMENT AND FIXED IN PLACE** (`c66e234d`): plans 04 and 06 both write `ChatArea.tsx` while 06 declared only 02 and 03, so an orchestrator computing readiness from `depends_on` could have dispatched them concurrently into one file in two worktrees. Every other file intersection was already serialized (`WorkflowRunPage.tsx` 06→07; `scripts/vitest-count-gate.cjs` 05→07). NEXT: `/gsd:execute-phase 194.1` — `/clear` first, dispatch worktree agents ONE MESSAGE AT A TIME, ≤2 concurrent test-running agents, `GSD_VITEST_MAX_WORKERS=2`. ⚠ STATE frontmatter HAND-EDITED (not `state.record-session`, which has corrupted this file before). · PRIOR: 2026-08-16 — **Phase 194.1 CONTEXT GATHERED** (`bc5c73da`) — SPEC's 7 requirements locked, 4 gray areas settled, ~7 plans targeted. **G-5 fired on FIVE files and was honoured BY CONSTRUCTION — an override was OFFERED AND DECLINED, so the absence of a 194.1 entry under `Guardrail overrides` below is a MEASUREMENT.** ⚠ The SPEC named four G-5 files and MISSED `StreamsProvider.tsx` (**77 commits / 33 phases / 3660 L**), which 194.1 touches twice and whose ledger row has read *'5+ phases · satisfied (075.7)'* for 28 phases — a row that is present and WRONG is worse than one that is absent. `MessageInput.tsx` (24/12/446) and `MessageList.tsx` (18/7/234) are ABSENT from the ledger entirely; all rows are fixed inside this phase (CONTEXT D-04). All four claimed bug reports have `status: folded` / `folded_into: 194.1` in FRONTMATTER (two PARTIAL and labelled so). ⚠ **L-01 / CR-04 STILL HAS NO ROADMAP HOME and 194.1 does NOT close it — operator decision: state it as a boundary, insert a phase AFTER 194.1. RUN-01 STAYS UNTICKED.** ⚠ **194.1's honest UI makes the L-01 lie MORE visible, not less** — on the affected runs the surface will show `⊘ Stopping this run…` and then land on `✓ Complete`; no plan may 'fix' that by suppressing the terminal reading. NEXT: `/gsd:plan-phase 194.1`. ⚠ STATE frontmatter HAND-EDITED (not `state.record-session`, which corrupted this file at `/gsd:discuss-phase 189`). · PRIOR: 2026-08-16 — **Phase 194 EXECUTED 13/13 AND VERIFIED `gaps_found` — 2 of 3 success criteria. ⛔ SC#2 IS FAILED AND THE PHASE IS NOT COMPLETE. RUN-01 stays UNTICKED.** ⚠ **L-01 / CR-04 — AT THE SHIPPED DEFAULT `WORKER_COUNT=2`, ROUGHLY HALF OF ALL STOPS END WITH THE RUN REPORTING `completed`.** `RUN_TASKS` is per-process; when the producer lives on the other worker the Stop correctly writes `cancelled` and the still-running producer then calls `finish_run(run_id,'completed')` (`harness_engine.py:2012`) and **overwrites it**. Measured at verification: `finish_run` is an unconditional `UPDATE workflow_runs SET status = $2 WHERE id = $1` with **NO terminal guard** (`db/workflows.py:1458-1463`); `publish_cancel_sentinel` publishes ONLY onto `ask_user:*` channels so it cannot reach a producer that is not paused at a prompt; the engine has no in-loop status re-read; and `WORKER_COUNT=2` is the default in `backend/Dockerfile:43`, `backend/.env.example`, `deploy/onebox.env.example` and `scripts/restart-backend.ps1`. SC#2's wording is *'not failed, **not silently complete**'* ⇒ **a FAILED criterion, not a bounded exception**. ⚠ **The limitation is INHERITED; what 194 added is the CLAIM to have closed it.** ⛔ **IT HAS NO ROADMAP HOME — phases 195-198 cover none of it.** Routing per G-7: **INSERT A PHASE** (gap 1 is a NEW MECHANISM, which a closure round may not add) — cheaper repair first: an in-loop `workflow_runs.status` re-read before `mark_phase_active` raising `CancelledError` when terminal; Redis cancel channel as fallback. ⚠ **DO NOT route to `/gsd:plan-phase 194 --gaps`.** ⚠ **A-1 — FOUND AT VERIFICATION, IN NO PHASE ARTIFACT, AND CR-01's FIX DOES NOT GUARD IT: the client-side twin of the sub-agent defect.** `api/threads.py:475-482` selects active runs with `status='streaming'` and **no `parent_run_id IS NULL`**, sub-agent runs carry the same `thread_id`/`user_id`/status (`task_service.py:551-562`), the reconcile loop stamps a `runStatus:'streaming'` placeholder per run (`StreamsProvider.tsx:1607-1638`), and `stopThread` picks by BUCKET ORDER — so a sub-agent id reaches `DELETE /runs/{id}` via the **PRIMARY Step-1 path**, which CR-01's narrowing never touches. Ordering-dependent ⇒ **UNCERTAIN**, which is exactly why the undriven reload-mid-run row matters. ⚠ **L-02 — `api/workflows.py:1483-1490` carries the IDENTICAL unnarrowed join CR-01 fixed**, so `delete_workflow_cascade` can cancel a sub-agent; pre-existing (`git diff` EMPTY on that file for all of 194), owner-scoped so **no cross-tenant exposure**; trigger = the next phase naming that file. ⚠ **THE ORCHESTRATOR WROTE A FALSE CLAIM AND THE VERIFIER CAUGHT IT — recorded rather than quietly fixed, because it is the exact failure this phase spent thirteen plans warning about.** `194-VALIDATION.md` asserted L-01/L-02 were *'written here, in `STATE.md` and on the RUN-01 row so that none of them lives only in a transcript'*; measured, **neither STATE.md nor REQUIREMENTS.md contained them** — the entry violated the lesson it cited in the same sentence. Corrected at this commit: both are now genuinely on the RUN-01 row and here. **CODE REVIEW: 4 Critical / 8 Warning / 5 Info.** CR-01/CR-02/CR-03 + WR-01 FIXED (`c601aeb0`, `2c614ebf`, `f97aa5c5`, `06f40aca`); CR-01 and CR-02 were **independently confirmed in source by the orchestrator before the fix**. ⚠ **CR-01's RED is the sentence that justifies the whole pass: the sub-agent case failed on `assert '54cd6f46…' != '54cd6f46…'` — THE STOP CANCELLED THE SUB-AGENT.** Reachable, not theoretical; the shipped suite structurally could not see it (`_FetchPool` seeded with ONE row, answering regardless of the SQL). ⚠ **CR-02 — a CRASHED phase was being persisted as `cancelled`**, i.e. the canvas would read *'Stopped by you'* for a crash; the arm was `except BaseException:` with its own comment conceding *'a crash escapes the same way'*. Fixed by scoping to `isinstance(_escape, asyncio.CancelledError)`. ⚠ **Writing `fail_phase` on the crash path was REJECTED** — `harness_engine.py:15-16` states the shipped crash-leaves-active RESUME CONTRACT and a terminal write would repeal it from inside a cancel fix; the crash arm writes nothing (pre-194 behaviour) and the residual `active`-under-`failed` row is named as needing its own vocabulary decision + migration. **WHAT GENUINELY HOLDS: SC#1 and SC#3, both proved rather than claimed.** Mount 4's D-16 descope rests on a measured premise and does NOT weaken SC#1 (mount 3 already ships per-run Stop + Stop-all). SC#3 re-queried on live data at verification: healed phase `d751095b…` still reads `completed` while its sibling reads `cancelled`; **0 active runs / 0 active phases / 0 orphan anchors**; the constraint carries all seven literals. ⚠ **MIGRATION 119 IS APPLIED TO THE LIVE LOCAL DB** (`33eeb270`) by the orchestrator at the operator's explicit instruction via the SQL-editor path — **never `db push`/`db reset`** — verified LITERAL BY LITERAL (all six shipped literals individually present, `ADDED=['cancelled']`, `REMOVED=[]`, 496 rows byte-identical); `full-schema.sql` regenerated with no `--reset`, diff exactly ONE line; `test_migration_119.py` 3 skipped → **3 passed**, discharging 194-02's own caveat. ⚠ **THE FIVE-ROW HEAL IS DONE AND THE PLAN'S 'EXACTLY FOUR ROWS' WAS WRONG — corrected BEFORE the write, not after.** The 3-vs-2 `active`-phase discrepancy resolved in D-17's favour: the third row was the interrupted phase OF a stuck run, so healing it was **SC#3 working on live data**. ⚠ **THE HTTP ROUTE WAS NOT DRIVEN IN THE HEAL and the receipt says so PER ROW** — no JWT was obtainable, auth was left intact (`403 Not authenticated`, wrote nothing), rows healed via `cancel_workflow_run_internals`; **the two `204`s DO NOT EXIST AND ARE NOT CLAIMED.** ⚠ **NINE OF THIRTEEN PLANS FOUND AT LEAST ONE OF THEIR OWN FENCES UNABLE TO FIRE — NONE CAUGHT BY READING, EVERY ONE BY PLANTING**: a needle matching the author's own docblock (5×, incl. the orchestrator's migration pre-flight guard firing on the word UPDATE in a COMMENT); four required plants all redding on ONE clause (194-03); a fence VACUOUS on its own RED run because a nine-phase-old shipped READ carried the predicate byte-identically — **read WHICH cases failed, never just how many** (194-06); a plant that fails to red being a FINDING (194-08, 194-11); `assert` SHORT-CIRCUITING so six plants were needed where two were named (194-10) and ELEVEN where two were named (194-12); a plant producing a **SKIP rather than a RED**, which would have shipped a fence with no RED observation while the receipt claimed one (194-12); and **194-05 finding the PLAN'S OWN ACCEPTANCE GREP passing over a genuinely broken ledger table**, because a substring match cannot tell a row from a suffix. ⚠ **194-12 also found F-7's plant IMPOSSIBLE on a populated DB** (Postgres refuses the constraint outright, `violated by some row`) — a real defence that **exists only where live rows exist and therefore VANISHES on a greenfield DB restored from `full-schema.sql`**. ⚠ **7 OF 7 WORKTREES FORKED FROM THE WRONG BASE**, every one returning `3781a3fe`, one HEAD on a 'Merge develop into master' commit — all self-corrected because the assertion runs before anything is written. **This is now DEFAULT behaviour here.** ⚠ **The by-name unit comparison needed its PARSER fixed before it could be believed, in TWO spellings** — truncated `- ImportE...` suffixes (8 PHANTOM regressions AND 8 phantom disappearances, same names both sides, 194-06) and an interleaved Windows path glued onto a node id (one test reading as NEW and DISAPPEARED at once, 194-09). ⚠ **THE COUNT GATE DOES NOT EXECUTE `src/components/chat` OR `src/lib`** — `TARGETS` covers `src/components/workflows` + four named Builder files, so several new suites are invisible to it and the total reads identically before and after; run them explicitly (138 passed / 15 files). Deferred with a trigger rather than widening TARGETS mid-phase. ⚠ **The count gate is NON-DETERMINISTIC at cap 2 and it can present as `[missing-file]` WITH `failed 0`, which reads like a pass at a glance** — measured this session as failed **4 → 1 → 0** on an identical tree, the one failure being a Phase-184 test on a file 194 never touched that rendered an EMPTY body and passed **3/3 in isolation**. ⚠ **The orchestrator broke the capture-filenames-first rule on the first run** (piped through `tail -4`) and recovered by capturing full output to disk. ⚠ **194-01 Task 2 is a recorded DEFERRAL, NOT a verdict** — the duplicate-icon root cause was never measured; no Chrome was reachable and both operator samples were the SAME thread, disqualified twice (no `workflow_runs` row ⇒ an ordinary Deep chat, and `runStatus:'completed'` ⇒ the symptom is a streaming-time condition). **Evidence that MAY be cited but is NEVER the verdict: harness runs leave `runs.message_id` NULL in 587 of 607 rows while the sampled Deep run sets it and it points EXACTLY at the rendered row.** The report's named prior art (`BUG-260521-01`) is the WRONG one. ⚠ **The plan's own console expression THROWS as written** (`useStreamsStore` is not on `window`); the working form is `await import('/src/stores/streamsStore.ts')`. ⚠ **RESEARCH's UAT roster command is WRONG** — `ms[-1]` is insertion-order-last, not newest, stale on 4 of 8 providers; transcribing it would have scored the board on `o1` and `moonshot-v1-8k`. ⚠ **CONTEXT D-01 is FALSE**: `RunCard.tsx` and `WorkspacePanel.tsx` do not *'occur only inside other rows prose'* in `CLAUDE.md` — neither appeared AT ALL. ⚠ **SIX HOT FILES WERE ABSENT FROM THE LEDGER and now have rows** — `harness_engine.py` **45/16/2536**, `api/runs.py` **33/16/1376**, `run_lifecycle.py` 4/3/437, `WorkspacePanel.tsx` 14/9/580, `RunCard.tsx` **21/9/608** (re-derived AFTER 194-07 staled 194-05's row **the same day it was written**), `db/workflows.py` **34/18/1582**. **NO GUARDRAIL OVERRIDE FOR PHASE 194 — G-5 fired on six files, every one honoured BY CONSTRUCTION with a measured test; that absence is a measurement.** Gates at close: `tsc -p tsconfig.app.json` **33** unmoved all phase · count gate `OK` **3954** · failed **0** · 75/75 · backend cancel path **123 passed, 0 skipped** (110 → 123 across the fix pass) · `tests/unit` **62/2242 set-identical BY NAME**. **NEXT = (1) INSERT the cross-worker-Stop phase — SC#2 has no home; (2) drive `194-UAT.md`'s reload-mid-run row FIRST (cheapest probe for A-1), then the 8-row roster; (3) `/gsd:fast` the WR-03/04/05/07/08 findings.** Prior: 2026-08-16 — **Phase 194 EXECUTING — 11 of 13 plans complete and merged; TWO PLANS BLOCKED ON AN OPERATOR MEASUREMENT, and the phase is deliberately NOT verified and NOT marked complete.** Merged: 194-02 (mig 119 authored), 194-03 (panel Stop mount), 194-04 (the frontend `cancelled` phase-status union), 194-06 (`db/workflows.py` cancel writers), 194-08 (composer+tray Stop pins), 194-09 (`_cancel_run_internals` cross-heal, exporting `cancel_workflow_run_internals`), 194-10 (engine cancel arm), 194-11 (the DELETE dual-id fallback), 194-12 (mig 119 APPLIED to the live local DB), 194-13 (the five-row heal + 4 new ledger rows). ⛔ **194-01 is 2/3 — its Task 2 `checkpoint:human-verify` (the duplicate-assistant-icon root cause) needs a LIVE run and the Chrome extension was not connected; Task 3 was executed BEFORE it under a recorded orchestrator deviation (`194-01-DEVIATION.md`), because Task 3 has zero dependency on Task 2. ⛔ 194-05 and 194-07 are UNSTARTED — both read `194-MEASUREMENTS.md`, which Task 2 owes.** ⚠ **MIGRATION 119 IS APPLIED TO THE LIVE LOCAL DB** (`33eeb270`) by the orchestrator at the operator's explicit instruction, via the SQL-editor path (the file's own statements executed against 127.0.0.1:54322, its BEGIN/COMMIT governing) — **never `db push`/`db reset`**. Verified LITERAL BY LITERAL, not by counting: all six shipped literals individually present, `ADDED=['cancelled']`, `REMOVED=[]`, 496 rows byte-identical. `full-schema.sql` regenerated (no `--reset`), diff exactly ONE line. `test_migration_119.py` 3 skipped → **3 passed**, which discharges 194-02's own caveat that its F-8 green proved little while the OLD constraint also refused the display sentence. ⚠ **THE FIVE-ROW HEAL IS DONE AND THE PLAN'S 'EXACTLY FOUR ROWS' WAS WRONG — corrected before the write, not after.** The 3-vs-2 `active`-phase discrepancy RESOLVED IN D-17's FAVOUR: the two orphans are `961c290d`/`9e6acf54` (both `confirm`, under already-`failed` runs); the third, `0e0cf57c` (`summarize`), is the interrupted phase OF stuck run `4b0feda7` — so healing that run also terminalizes it, which is **SC#3 working on live data** and the best evidence in the phase. Live DB now: `workflow_runs` active 2 → **cancelled 2**; `workflow_phases` active 3 → **cancelled 3**, total still 496; thread anchors 2 → **0**; `runs` histogram byte-identical. ⚠ **THE HTTP ROUTE WAS NOT DRIVEN AND THE RECEIPT SAYS SO PER ROW** — `DELETE /runs/{id}` needs a Supabase JWT that could not be obtained; the executor REFUSED to forge or weaken auth (verified live at `403 Not authenticated`, wrote nothing) and healed via `cancel_workflow_run_internals`, which is what the route's no-live-producer arm calls. **The two `204`s the plan's acceptance criteria ask for DO NOT EXIST AND ARE NOT CLAIMED.** Read-only forward-resolution confirmed `producer_id IS NULL` on both ids, so the arm that would have run is the arm whose writer was called. ⚠ **NINE FOR NINE: EVERY PLAN THAT PLANTED ITS FENCES FOUND AT LEAST ONE THAT COULD NOT FIRE — none caught by reading.** 194-02's own docstring tripped its grep twice; 194-03's FOUR REQUIRED PLANTS ALL RED ON ONE CLAUSE (a fifth, unplanned, was needed to prove clause (b) non-redundant) and its empty-sweep control left clauses GREEN with a third of the union invisible; 194-06 found a fence VACUOUS ON ITS RED RUN, passing on a tree where neither writer existed because a nine-phase-old shipped READ carried the predicate byte-identically (**a count-only read of a RED run would have shipped it** — read WHICH cases failed); 194-08's plant C could not fire in TWO independent ways; 194-10 shipped SIX plants where its plan named two, FOUR load-bearing, because `assert` SHORT-CIRCUITS; 194-11 found a clause that reds under NO realistic cross-user world; **194-12 needed ELEVEN plants and found one that produces a SKIP rather than a RED — it would have shipped with no RED observation while the receipt claimed one**; and the orchestrator's own migration pre-flight guard fired on the word UPDATE inside mig 119's COMMENT block. ⚠ **194-12 also found F-7's plant is IMPOSSIBLE on a populated DB** (Postgres refuses the constraint outright, `violated by some row`) — a real defence the fence does not provide, **which exists only where live rows exist and therefore vanishes on a greenfield DB restored from `full-schema.sql`**; re-driven with `NOT VALID` to reproduce the greenfield case. ⚠ **7 OF 7 WORKTREES FORKED FROM THE WRONG BASE**, every one returning `3781a3fe`, one HEAD on a 'Merge develop into master' commit — all self-corrected because the base assertion runs before anything is written. This is now DEFAULT behaviour here, not a Phase-192 curiosity. ⚠ **The by-name unit comparison needed its PARSER fixed before it could be believed, in TWO spellings**: 194-06 hit truncated `- ImportE...` suffixes (8 PHANTOM regressions AND 8 phantom disappearances, same names both sides) and 194-09 hit an interleaved Windows path glued onto a node id (one test reading as NEW and DISAPPEARED at once). Final: **62/62 set-identical, 0 new, 0 disappeared.** ⚠ **The count gate does NOT execute `src/components/chat`** — `TARGETS` covers `src/components/workflows` + four named Builder files, so 194-08's two new suites are invisible to it (total 3954 before and after); run explicitly, **16 passed**. Deferred with a trigger rather than widening TARGETS mid-phase. ⚠ **A count-gate run VIOLATED with `failed 0` and two `[missing-file]` entries, then passed on re-run** — cap-2 non-determinism in a NEW spelling: it can present as MISSING FILES rather than as failures, and `failed 0` on a violated gate reads like a pass at a glance. ⚠ **CONTEXT D-01 is FALSE and corrected beside it: `RunCard.tsx` and `WorkspacePanel.tsx` do not 'occur only inside other rows prose' in `CLAUDE.md` — `grep -o` returns 0 for both; neither appears AT ALL.** ⚠ **RESEARCH's UAT roster command is subtly WRONG** — its third column is `ms[-1]`, dict-insertion-order-last rather than NEWEST, stale on 4 of 8 providers; transcribing it would have scored the board on `o1` and `moonshot-v1-8k` instead of `gpt-5.6-sol` and `kimi-k2.6` (the SEED-040/SEED-135 failure). ⚠ **FOUR MORE HOT FILES WERE ABSENT FROM THE LEDGER and now have rows** — `harness_engine.py` **45/16/2536** (16 phases, G-5 never once fired), `api/runs.py` **33/16/1376** (`grep` → 0 occurrences), `run_lifecycle.py` 4/3/437, `WorkspacePanel.tsx` 14/9/580; `db/workflows.py` re-derived **32/17/1447 → 34/18/1582**. ⚠ **`RunCard.tsx` got NO row: `git log 743965a1..HEAD` is EMPTY — this phase never touched it**, so its triple `20/8/550` is carried in the heal receipt with 194-05/194-07 named as owing it. ⚠ **The ledger cells 194-13 wrote WILL go stale when 194-05 and 194-07 land, and the cells SAY SO** — 194-05's `files_modified` includes `CLAUDE.md`. **NO GUARDRAIL OVERRIDE FOR PHASE 194 — G-5 fired on six files and every one was honoured BY CONSTRUCTION with a measured test; that absence is a measurement.** Gates at this checkpoint: `tsc -p tsconfig.app.json` **33** unmoved all phase · count gate `OK` **3954** · failed **0** · 75/75 · backend cancel path **110 passed, 0 skipped**. **NEXT = drive the 194-01 Task 2 console dump on a LIVE streaming harness run (`await import('/src/stores/streamsStore.ts')` — the plan's bare `useStreamsStore` is not a global and throws), then `/gsd:execute-phase 194` to run 194-05 + 194-07, then verification.** Prior: 2026-08-16 — **Phase 194 PLANNED (`67e87b88`) — 13 plans in 8 waves; plan-checker VERIFICATION PASSED on the FIRST iteration, zero blockers, four warnings (one fixed, one judged correct-as-is, two close-out items).** ⚠ **RESEARCH REFUTED EIGHT CONTEXT.md CLAIMS BEFORE ONE LINE WAS PLANNED, and three of them changed what ships — recorded as operator rulings D-16/D-17/D-18 with the originals STRUCK THROUGH, never overwritten.** **D-16 — mount 4 (the Workflows-page library-row Stop) is DESCOPED because its premise was measured FALSE:** `LibraryRow` carries no run field, all three feeds are DEFINITION feeds, and a grep across the whole `library/` directory returns ONE hit which is prose inside a docblock — so *'using a run id the library already has'* named an id that does not exist on the row or on the wire; delivering it needs a new backend read + a new row field + a poll, i.e. a genuinely SECOND concern on `WorkflowsPage.tsx`, whose ledger row already owes a G-5 refactor recommendation FIRST (`34/12/1176`). ⚠ **SC#1 is NOT weakened, and that is measured rather than argued: mount 3 (`ActiveRunsTray`) ALREADY ships per-run Stop + Stop-all, which IS the 'stoppable from outside its thread without navigating in' property mount 4 was reached for.** Trigger: *the first phase that puts any live-run state on the library row*. **D-17 — the heal covers FOUR rows, not two:** beyond D-12's two stuck `workflow_runs`, research found **two orphan `workflow_phases` rows stuck `active` under runs that already FAILED** (both `confirm` phases, 2026-07-18) — SC#3's exact failure mode already sitting in the live data. ⚠ One of the two run rows is `is_golden_run = True` (permanently unresumable since Phase 190's A4 gate); if it needs distinguishing that is an audit note, **NEVER a new status literal**. **D-18 — the banner fix ships and crosses PANEL-09 with the cost STATED:** the mechanism is `hasAnyTools = tool_calls.length > 0` and a harness run writes NO `tool_calls`, so `MessageItem.tsx:635` holds the pre-tools banner for the entire run; the re-render is **per-PHASE, not per-token**, so the cost PANEL-09 was protecting against is not the cost incurred. ⚠ **THE SINGLE MOST IMPORTANT FINDING, which nobody had named: `WorkflowLock.runId` CARRIES TWO ID TYPES** — two of its four write sites store a `workflow_runs.id`, two a producer `runs.run_id`, its own JSDoc asserts only the first, `DELETE /runs/{id}` accepts only the second, and `cancelRun` **SWALLOWS 404** ⇒ a Stop wired to the lock **silently succeeds while doing nothing**, the exact dishonesty SC#2 forbids. ⚠ **`WorkspacePanel.tsx:161-165` already recorded this in Phase 188 and it was NOT carried forward** — a measurement that lives only in one file's comment is invisible to the next phase. The fix already ships TWICE (`continue_run`, `ask_user_response`); `DELETE` is the only route on that prefix without it. ⚠ **D-09/D-10's whole backend fix is ALREADY WRITTEN, in the wrong file** — `delete_workflow_cascade` (`api/workflows.py:1494-1518`) already composes `_cancel_run_internals` + `finish_run(wf_id,'cancelled')`; 194's work is largely MOVING that composition into the shared writer so all three callers get it. ⚠ **THE PLANNER FOUND A GAP NEITHER RESEARCH NOR PATTERNS MAPPED, and SC#3 fails without it:** the panel spine reads a DIFFERENT, CLOSED union — `Phase['status']` (`types/index.ts:1035-1043`, eight members, **no `cancelled`**) fed by a TOTAL `phaseStatusFromDb` that resolves anything unrecognised to `unknown` — **so a stopped run's interrupted phase would render 'Unknown' on the spine.** Fail-closed, so not a lie, but not D-13's *'reads stopped, not failed'* and a guaranteed G-4 failure ⇒ plan `194-04`. **Also corrected: a FOURTH shipped mark for the cancelled concept research missed (`⊘ Cancelled`, `WorkflowRunPage.tsx:299`), and `ActiveRunsTray.test.tsx` DOES NOT EXIST** (VALIDATION marked V-08 *extend*; measured, the tray has no dedicated suite anywhere — it is a Wave-0 CREATE). ⚠ **BUG-260815-07's *'permanent delete blocker'* claim measured FALSE** (the cascade already terminalizes them; neither definition is `is_system_global`) — recorded, and it does not remove the reason to heal the rows. **Definitively answered: do NOT add `is_app_shutting_down()` to Step 3b** (three independent proofs; resumability is already forfeit there) **while the F2 terminalize MUST keep its gate** (096-09, Phase 096 UAT Test 2) — both arms are fenced. **12 fences, each naming the REAL production-source plant it must be driven RED against; F-4/F-6/F-10 carry TWO plants each and F-1 additionally needs an empty-sweep RED ⇒ 24 required RED observations, each with a post-revert md5** — because this project shipped FIVE inert fences in 193.2, four in 193.1, three in 192.1 and five in 190, every one caught by planting and none by reading. **All 20 VALIDATION rows V-01..V-20 covered and cited; RUN-01 in all 13 plans.** ⚠ **G-5 FIRES ON THREE FILES — `RunCard.tsx` (8 phases), `WorkspacePanel.tsx` (8), `db/workflows.py` (17) — honoured BY CONSTRUCTION with a measured test rather than an argument, each plan carrying a STOP condition if the measurement disagrees. A G-5 override was OFFERED AND DECLINED for the FOURTH CONSECUTIVE PHASE; this file records NO guardrail override for Phase 194, and that absence is a measurement.** ⚠ **D-01's claim that both G-5 filenames *'occur only inside other rows prose'* is FALSE — `grep -o` returns 0 for both; neither filename appears in `CLAUDE.md` AT ALL.** Waves 7 and 8 are ⛔ **SERIALIZED** (rule 4): the mig-119 apply takes an ACCESS EXCLUSIVE lock on `workflow_phases`, and the four-row heal mutates real, irreplaceable rows — both `autonomous: false`. **NEXT = `/gsd:execute-phase 194`.** Prior: 2026-08-16 — **Phase 194 CONTEXT GATHERED (`99b6a5b4`) — 15 decisions locked in `194-CONTEXT.md`, and the phase was RE-SCOPED against BOTH planning artifacts that describe it.** ⚠ **The ROADMAP's scope flag and STATE.md's own correction of it are BOTH half wrong, and both are recorded rather than overwritten.** ROADMAP:494-495 says *'mostly UI over an endpoint that already exists'*; the pre-discuss finding below says *'194 is a NEW runtime path'*. Measured at HEAD: a harness run is driven by **the same producer task as a Deep run**, so `DELETE /runs/{id}` (`api/runs.py:1155`) already cancels it, and the F2 block (`run_producer.py:254-277`) already calls `finish_run(workflow_run_id, 'cancelled')` — whose own comment names it the *'v2.8-audit cancel-honesty fix'*. `workflow_runs.status` has admitted `cancelled` since mig 057. **The finding's four individual measurements are each CORRECT; only its INFERENCE is wrong** — it searched the workflow-side modules for a cancel verb and concluded none exists, when the cancel lives on the `runs` side by design. ⇒ 194 is not *'build cancel'* but *'is the shipped cancel REACHABLE, HONEST and SAFE from the workflow run surface?'*, and **three gaps were measured, one per success criterion**: **G-A/SC#1** `WorkspacePanel.tsx` has **no Stop control of any kind**, and every shipped Stop keys off a streaming assistant message's `runId` that `BUG-260815-04` may defeat; **G-B/SC#2** `_cancel_run_internals` Step 3b heals **only** the `runs` row — `workflow_runs` stays `active`, the anchor is never cleared, the thread wedges, and with `WORKER_COUNT=2` + a per-process `RUN_TASKS` that is roughly **half of all missed Stops**, not an edge case (it is measurably the two rows stuck `active` since **2026-06-14** and **2026-08-01**); **G-C/SC#3** `finish_run` writes the run row + anchor only, so the interrupted `workflow_phases` row stays `active` forever — and `workflow_phases_status_check` (mig 115) admits **no `cancelled` literal**. **Decisions: migration `119` widens that CHECK 6 → 7 in the mig-115 shape** (reusing `failed`/`skipped` was offered and REJECTED as dishonest — the phase did not fail and was not skipped, it ran and was interrupted); **four Stop mounts, ONE mechanism** (panel spine PRIMARY · composer · ActiveRunsTray · library row), no second cancel path; **the zombie arm is extended across to `workflow_runs` AND the two historically stuck rows are healed**, with their ids + before/after captured because *a data repair with no receipt is indistinguishable from a claim*; a stopped run **keeps its completed phases, marks the interrupted one, and is TERMINAL** (resumability rejected — `workflow_runs.status` already carries a distinct `paused`). ⚠ **G-5 fires on `RunCard.tsx` (20 commits) and `WorkspacePanel.tsx` (13), both ABSENT from the ledger; honoured BY CONSTRUCTION and writing the two rows is a PHASE DELIVERABLE (D-01/D-02). A G-5 override was OFFERED AND DECLINED for the FOURTH consecutive phase — this file records NO guardrail override for 194, and that absence is a measurement.** **Bug routing (frontmatter UPDATED, not just decided): `BUG-260815-07` FOLDED — reproducible half only, the non-reproducible delete failure stays open; `BUG-260815-04` FOLDED — ⚠ its `\"Starting workflow…\"` string is DELIBERATE and byte-pinned, the defect is that the surface never ADVANCES; `BUG-260808-02` FOLDED with a scope fence — ⚠ its OWN trigger fired, since it bundles SEED-140 which IS RUN-01; `BUG-260815-03` ⚠ FOLDED IN THE FIRST PASS AND UN-FOLDED ON MEASUREMENT → DEFERRED to 195** with the trigger *'the first phase that renders a completed workflow run's phases outside the chat thread'* — 194 makes a LIVE run stoppable from the library row, but the complaint was about a COMPLETED run's history, which this phase structurally cannot close, and marking it `folded_into: 194` would have put a bug in the *claimed* state against a phase that cannot address it. ⚠ **All four reports previously read `folded_into: null` while STATE.md recorded the routing — a routing that lives only in prose is invisible to every audit that reads frontmatter.** **NEXT = `/gsd:plan-phase 194`.** Prior: 2026-08-15 — **Phase 193.2 ✅ CLOSED — 10 of 10 plans, all four success criteria DRIVEN by the operator, and `AUTH-03` SATISFIED END TO END for the first time.** The evidence is the row no test could stand in for (`193.2-UAT.md` U1): run **`b021c7b0` COMPLETED**, definition `93a86e21` / slug `northwind-qbr-fa65a43c`, all five phases, **zero gate failures** — `/Northwind-QBR-Template.docx` at **39,698 B** against a 37,424 B template, **10 of 10 fields filled** each with a real `source_doc`, **0** residual `{{ }}`, **0** literal `None`, **branding verified against an operator screenshot** (navy band, Georgia title, amber account-health block, teal headings 1-6, grey summary panel, navy footer rule), and **all eight planted facts grounded** (68% utilisation · 412 of 605 seats · INC-4471 · a 6h12m outage on 12 Aug · £284,000 ARR · term to 31 Jan 2027 · Freight Analytics ~£62,000 · AMBER). **A template attached AT AUTHORING TIME and a real run that FILLS THAT SAME TEMPLATE — both halves observed in one sitting**, which is exactly what 193.1 declined to tick on unit evidence alone. ⚠ **The 193.1 ATTRIBUTION REGRESSION DID NOT RECUR — Marcus Feld is named TWICE in the rendered document**, a second data point in the right direction, so the *'a second sighting earns a seed'* trigger is NOT fired and no seed is owed. **M1 the requirement arrived pre-filled, durable and NOT an echo of describe** (so 193.2-07's anti-echo predicate EARNED the mark rather than defaulting to it), `business_requirement_seeded_by_ai: true`. **M2 published with the canvas UNTOUCHED.** **M3 found without searching at rendered position 4 of 112 — the predicted index `starters.length` held EXACTLY — and the post-publish Run CTA appeared and NAMED the workflow**; recency is proved against a near-identical OLDER row (the new QBR at 14:17 above the old Q3 at 02:07), not by alphabetical luck. ⏸ **CLOSED WITH TWO UAT ROWS NOT DRIVEN, BY DECISION: U5** (the composer Harness picker — the one user-visible consequence of the sort that lands OUTSIDE the library, and nobody has looked at it; thirty seconds) **and U2** (the rendered interactive refusal — ⚠ **it could NOT be driven: no interactive step ever appeared, so the surface never rendered, and *the defect not occurring* is NOT the same as *its message reading well*. The rewritten two-arm copy has still never been read by a person**). ⚠ **ONE HONESTY CAVEAT ON M1: the operator confirmed *'I think all pass'* broadly and did NOT specifically confirm the VISIBLE mark — the DURABLE half is measured, the VISIBLE half rests on a general confirmation, and it is NOT recorded as a specifically-observed mark.** ⚠ **THREE PUBLISH ATTEMPTS WERE BLOCKED FIRST AND NONE WAS A PRODUCT DEFECT ON THE PUBLISH PATH**: the OpenAI credit balance was **exhausted** (`429 insufficient_quota`, confirmed by a live `embeddings.create`), and since every document is embedded with `text-embedding-3-small` **every search must embed its query** — retrieval returned 0 sources, `citations_required` failed 3× per run, and publish **correctly refused**. All three rows read `blocked_stage: structural_gate` (14:02:21 / 14:07:42 / 14:10:29), read through the **C-7 accessor** `metadata #>> '{}'`. Ruled out by measurement: 5 docs / 18 chunks / 0 null embeddings / matching `org_id`. **Four bugs and three seeds were filed from that session** — `BUG-260815-05` (blocking: a provider outage is reported as *'nothing was retrieved (0 sources)'*), `BUG-260815-06` (major: the structural-gate refusal names a stage, not a cause — ⚠ **the same failure class this phase fixed for the interactive gate, surviving on the gate next door**), `BUG-260815-07` (major, **NOT reproducible**, stays open; two runs stuck `active` since 2026-08-01 and 2026-06-14 are a permanent delete blocker), `BUG-260815-08` (minor, Workflows header — **G-2 fires, sketch it with `SEED-155`'s card density**), **`SEED-159` — trigger FIRED and its PREDICTION WAS WRONG** (not a silent blank but a verbose internal disclaimer, *'Not explicitly stated in the KB.'*, rendered in the front-page header of a document footed *'Commercial in confidence'* — the model behaved correctly and the real defect is that **a deliverable has no register distinct from an internal answer**), `SEED-165` (52 out-of-gate backend failures, triaged 34 stale / 18 undiagnosed), `SEED-166` (settings/operator/admin IA). **Code review: 0 Critical / 0 Warning / 5 Info**, all six Warnings fixed post-review. **G-7 CLEAR** — 10 plans, 0 gap-closure rounds. Gates at close: count gate **`OK` · 3918 · failed 0 · 75/75** · backend workflow suites **210 / 0** · full `tests/unit` **62 failed / 2221 passed** (the 62 is the baseline rot set) · `tsc -p tsconfig.app.json` **33**, unmoved all phase. NEXT = drive **U5** when someone next opens the composer, then `/gsd:discuss-phase 194`. *(Superseded, kept for the record: this entry previously read 'EXECUTED … ALL FIVE UAT ROWS ARE OWED … NEXT = drive U1'. U1 was driven hours later.)* Gates re-run at HEAD by `193.2-10` against `193.2-BASELINE.md`: backend `tests/unit` **62 failed / 2154 passed** (failures IDENTICAL to baseline — the SEED-056 rot set; passes grew from 2092) · six backend workflow suites **110 / 0** (was 81) · five frontend workflow suites **359 / 0** (was 337) · count gate **`count gate OK` · total 3918 · failed 0 · pinned 3868 · 75/75** (was 3892) · `tsc -p tsconfig.app.json` **33, unmoved across all ten plans**. **NO NEW FAILURE.** ⚠ **NO GUARDRAIL OVERRIDE FOR PHASE 193.2 — offered and declined for the THIRD consecutive phase (193, 193.1, 193.2); that absence is a measurement.** G-5 fired on SEVEN files, all honoured by construction. ⚠ **FIVE `CLAUDE.md` ledger rows were owed, not the three D-03 planned** — `db/workflows.py` **31/17/1405** (the SECOND-HOTTEST BACKEND FILE IN THE TREE, absent from the table for seventeen phases), `publish_service.py` **17/7/1158**, `workflow_authoring.py` **11/6/540**, `models/harness.py` **17/16/611**, and `builderStore.ts` **11/5/837**, which appeared in NO prior artifact of this phase at all. ⚠ **THE MEASURED RESULT, and the D-08 rule binds every word of it: `business_requirement` 5/5 on all four arms (pre-fix 0/N); `llm_human_input` 0/5 on all four (pre-fix 2/2 with a template); the SC#4 `render_template` control 5/5 LIKE-FOR-LIKE and it DID NOT FALL; `external_action` displacement 0/5. A MEASURED REDUCTION, NEVER AN ABSENCE — the publish gate STAYS.** ⚠ **D-10's escape hatch is recorded NOT TRIGGERED: neither candidate fact was false — BOTH are true. The false premise was the unstated third one, that a 400 was ever received.** Prior: **Phase 193.2 PLANNED (`2ddac519`) — 10 plans in 6 waves; plan-checker VERIFICATION PASSED first iteration, zero blockers. NEXT = /gsd:execute-phase 193.2.** Research REFUTED D-10 premise before any code: harness_audit measured 1 publish_attempted / 1 publish_succeeded / 0 publish_blocked, so the publish endpoint never refused anything and the refusal came from an already-greyed control — rewriting the string IS the fix, Wave 1 is confirm-and-record. A shipped green fence (test_no_feed_orders_by_updated_at) FORBIDS D-15/D-16 and is rewritten in place, never deleted, driven RED against the pre-change source. FOUR ledger rows owed not three — a seventh hot file, backend/app/models/harness.py (16/15/578), was absent from the table. BUG-260815-02 arithmetic was over the wrong feed: rendered 17 of 109 (scope=mine), landing at 4 of 109 after the sort, not 1. The post-publish Run CTA has had ZERO automated coverage since it shipped. NO guardrail override recorded — third consecutive phase to decline one. Prior: CONTEXT GATHERED (`4d1d9374`). 26 decisions locked in `193.2-CONTEXT.md`. ⚠ Two scouting findings reshaped the phase and both are recorded as DELIVERABLES rather than assumptions: (1) `/validate` ALREADY mints an `interactive_phase` verdict, it is registered route-assigned + incomplete, it ALREADY greys the Publish control with the server message verbatim, and there is a PINNED test — so the operator could not have clicked Publish and got a 400, and Wave 1 must establish which of those two facts is false on the live path (D-10); (2) the post-publish Run CTA ALREADY names the workflow and offers Run (`WorkflowsPage.tsx:897-908`), so 'the product never told me the name' is at least partly already closed and the operator was still lost (D-19). ⚠ **G-5 fires on SIX files and THREE are ABSENT from the `CLAUDE.md` hot-file ledger** — `db/workflows.py` **30 commits / 16 phases**, `publish_service.py` 16/6, `workflow_authoring.py` 9/5 — the same invisibility failure `WorkflowsPage.tsx` had for 10 phases. Honoured by construction; writing the three missing ledger rows is a phase deliverable (D-01/D-03). ⚠ **A G-5 override was OFFERED AND DECLINED for the THIRD consecutive phase, so this file records NO guardrail override for 193.2 — that absence is a measurement.** ⚠ `WorkflowsPage.tsx`'s ledger cell measured STALE for the 4th time (`33/12/1160` → **34/12/1176**). Prior: **Phase 193.1 ✅ CLOSED, AUTH-03 satisfied on a real end-to-end run.** 193.2 bundles three items the operator hit in ONE sitting on 193.1s own headline path: SEED-163 (the AI leaves business_requirement blank), BUG-260815-01 (BLOCKING — the draft grows an llm_human_input step the publish gate refuses; caused by 193.1s OWN D-26 fix, measured 2 for 2), BUG-260815-02 (BLOCKING — a just-published workflow is unfindable; ORDER BY name at three call sites, no recency anywhere, position 129 of 146). ⚠ **I routed all three to 197 and the operator OVERRULED it; the original reasoning is left visible rather than overwritten.** They are one phase because they share one root: authoring makes decisions the author is never shown and does not know what publish requires. ⚠ G-2 fires on the library half and must NOT absorb the sort bug. ⚠ G-1 does not fire but a THIRD 193.x would trip it. Also filed and routed to 194: BUG-260815-03 (run history unreachable from canvas) and BUG-260815-04 (chat stuck on Starting workflow — the string is DELIBERATE and pinned, do not reword). ⚠ Phase 193 still owes U1/U2; 193.1 closed with U2/U3/U4 owed by decision."
resume_file: .planning/phases/195-show-the-deliverable/195-01-PLAN.md
---
