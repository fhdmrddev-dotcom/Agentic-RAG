...
**Phase:** 192 Workflow Library IA — **context gathered** (no plans yet)
**Plan:** —
**Status:** Planning
**Next action:** `/gsd:plan-phase 192` — **G-2 SATISFIED** (sketches 157/158/159, winners 157-B · 158-A · 159-C) and **G-5 HONORED** (`514c8e64`). Downstream MUST read `.planning/phases/192-workflow-library-ia/192-CONTEXT.md`; its **D-12 corrects the sketch MANIFEST** on the `onTweak`/`onUseStarter` fork mechanics, and its **D-04 records that 192 is not frontend-only**.
**Last activity:** 2026-08-10 — v3.7 opened; v3.6 phases archived; SEED-147..150 planted; v2.9 STRETCH carry-forward guide written; **ROADMAP v3.7 repaired** (`2313329c`) so the SDK can resolve 192-198; **192-CONTEXT.md + DISCUSSION-LOG.md written** (`514c8e64`)
