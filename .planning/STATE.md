...
status: needs_uat
last_updated: 2026-08-09T23:55:00.000Z
last_activity: 2026-08-09 -- **Phase 190 CODE REVIEW FIXES applied (`190-REVIEW-FIXES.md`, commits `f64a0ebb` + `50d058e3`) - ALL FOUR CRITICALS CLOSED, three warnings closed or half-closed, six findings DEFERRED with written triggers, ZERO skipped.** **CR-01 was WORSE than the review stated and it was MEASURED before anything was changed:** both `authenticated` AND `anon` held column-level SELECT on `connector_connections.secret_ciphertext` (plus INSERT/UPDATE/DELETE), so an UNAUTHENTICATED role held the tenant credential column - closed by **migration 118**, which revokes the blanket grants and re-grants SELECT column by column, omitting exactly one. ⚠ **The fix FORCES a code change and that was driven against live PostgREST, not reasoned about:** with the column grant in place `SELECT *` is `42501` for `authenticated`, and PostgREST's DEFAULT projection is `select=*` - so every read AND every `return=representation` write on the user-JWT client now names its columns (`_SELECTABLE_COLUMNS`, derived from the response model, never retyped). Measured as `authenticated` after 118: `?select=*` -> 403 42501 - `?select=secret_ciphertext` -> 403 42501 - `?select=<safe>` -> 200 - `POST` with representation and no select -> 403 42501 - `POST` with `select=<safe>` -> reaches the ROW gate. **CR-02** the kill switch read the SYNC settings cache with no staleness bound (the only kill switch in the tree doing so, and the one whose false negative sends real email) - `await ensure_settings_fresh()`, with the drive asserting the ORDER `["fresh","audience"]` because an outcome assertion reads identically on a warm cache. **CR-03** the switch failed OPEN for 3 of its 4 legal audiences - an operator piloting live sending for super-admins only (`audience: "role"`) would have had every published workflow in every org start sending for real; the test is now POSITIVE (`!= "everyone"` records), which is fail-closed against a hand-edited row AND against any fifth audience, per the 185 lesson. **CR-04** blocking `getaddrinfo` on the event loop from three `async def` sites - the SMTP one is the sharp one, where the CONNECT had been wrapped since 190-07 and the lookup that PRECEDES it was left on the loop; all three now go through `run_in_threadpool` and `validate_destination` stays a SYNC module attribute so the D-06 ordering fence still binds what the executor calls - only the THREAD moved. Driven by THREAD IDENTITY (`assert 96204 != 96204`), the only instrument that tells "off the loop" from "fast enough today". **WR-02** `update_connection` skipped the capability<->config validator (a valid PATCH silently destroyed an SMTP destination and answered 200) - driven by a 3x3 matrix whose MATCHING diagonal must still succeed. **WR-03** half-fixed: a stranded `connection_id` now reaches D-17's `recorded_not_sent` instead of an uncaught `ValueError`; the UI half is `D-190-DEF-11`. **WR-05 (+IN-03)** half-fixed: the models now refuse an empty connection and a `port` outside 1..65535 (which also closes IN-03's silent 465/587 substitution); the panel mirror is `D-190-DEF-12`. ⚠ **WR-01 is DEFERRED (`D-190-DEF-10`) BUT ITS FALSE ✅ IS CORRECTED** - `190-VALIDATION.md`'s W0-3 row, its G-6 row 6 and its "six of eight fully driven" count (now FIVE of eight) were all corrected on measurement: `ExternalActionPhaseConfig` carries no `base_url`, so GATE 3's pre-credential guard is structural for `post_message` ONLY and is never called for `create_ticket` / `send_email`. No reachable bypass (both binders validate first), so the CODE is deferred and only the SCORE moved - a score that only ever improves is not a score. **WR-04 / WR-06 / IN-01 / IN-02 / IN-04** deferred as `D-190-DEF-13` / `-14` / `-15`; ⚠ **WR-06 stated plainly rather than softened: with the switch OFF an OPERATOR can still create/edit/delete connector connections via curl**, because `require_visible`'s operator no-op runs BEFORE the `"off"` test - deferred only because that dependency is shared by four other governed features, and bounded because it cannot cause a SEND (GATE 2 is independent and now requires a positive `"everyone"`). ⚠ **ONE FINDING THE REVIEW DID NOT MAKE, found while regenerating the artifact and closed in the same commit:** `full-schema.sql` carries **NO ACLs AT ALL** (`pg_dump --no-privileges`; `grep -c "^GRANT|^REVOKE"` -> **0**), so migration 118 would have been INVISIBLE to every greenfield bootstrap - the grant is mirrored into `scripts/full-schema-supplement.sql` and the general case (nothing fences the NEXT privilege-narrowing migration) is `D-190-DEF-16`. ⚠ **APPLY DEVIATION, recorded as a deviation and never as a gate met:** D-21 asks for the Supabase SQL editor; this session is unattended, so 118 was applied to the LOCAL db via `psycopg2`, autocommit, **no reset**, dev data preserved - the fallback D-21 names in as many words, with 189-06 and 190-03 as precedent. **`supabase db push` / `db reset` were NOT run; the ban is untouched**, and `full-schema.sql` was regenerated by the sanctioned script with no `--reset` and never hand-edited. **CLOUD PARITY GREW: 099 -> 117 becomes 099 -> 118, and 118 is SECURITY-BEARING** - until it is applied, CLOUD STILL HAS THE CR-01 DEFECT; it must be applied in the same operation as the `connector_service.py` deploy, because the grant WITHOUT the code breaks every connector read with a 42501. Suites: **314 passed** across all sixteen 190/189 files plus the three new ones; full backend **211 failed / 3617 passed** - failures UNCHANGED against the recorded baseline (211/3575), +42 passes, and ZERO failures name a 190 file. Frontend UNTOUCHED (no file under `frontend/` is in either commit), so the eight D-23/D-24 fences, the 189 acronym fence, the count gate and `tsc` at 33 are all trivially unmoved. NEXT = `/gsd:secure-phase 190`, which should read `190-REVIEW-FIXES.md` first. || PREVIOUS: 2026-08-09 -- **Phase 190 plan 19 EXECUTED - THE PHASE IS CODE COMPLETE AND CLOSES WITH TWELVE UAT ROWS OWED, stated as a DECISION and never as a claim that everything ran.** All 19 plans done; **51/51 tasks** carry an automated verify with the sampling-continuity rule stated and ZERO exceptions; **14/14** falsification rows carry a plan, a test, a plant and a VERBATIM RED; **7/7** source fences green with their controls named. The eight D-23/D-24 fences are EMPTY **phase-wide** against BOTH candidate bases (`83a93c9a`, `de122b9a`) and the worktree, existence confirmed at each rev first - the only comparison that catches a file opened in one plan and closed in another. **Every baseline was RE-MEASURED from the base commit in a throwaway worktree rather than quoted:** backend `211 failed / 3372 passed` -> **`211 / 3575`** (failures UNCHANGED, **+203** passes, FAILED set-diff showing **0** disappeared), `tsc` **33 -> 33**, count gate `2727/2714/48 files` -> **`2851/2838/51 files`** with drift `+13 -> +13`, and ZERO installs across the whole phase. ⚠ **A MEASUREMENT ERROR IN MY OWN INSTRUMENT was caught and corrected rather than caveated:** the first base run used a sparse worktree that omitted `supabase/`, reported `221 failed`, and would have let the close claim *"the phase fixed 11 pre-existing failures"* - false, because those tests read `supabase/full-schema.sql` and failed on MY checkout, not on the base commit. With `supabase` added the "gone" list collapses to **0**. Both numbers are recorded so the correction is auditable; a corrected number nobody can re-derive is just a different assertion. The only **2** new failures are in `test_114_explain_index.py`, a file the phase NEVER opened, whose two tests pass twice in isolation and which swapped WHICH case failed between runs - a live-DB EXPLAIN-planner flake, named rather than absorbed. **THE SC#10 SCOREBOARD: the roster is DERIVED by grouping `MODEL_CAPABILITIES`** (61 ids / 8 providers; the command AND its output pasted above the table so the next reader re-derives rather than trusts), and **all 8 rows are PRESENT and ⛔ with a named reason plus the blocking id `BLOCK-190-UAT-01`** - a scoreboard that lists only what passed is not a scoreboard. The block is MEASURED, not assumed: **0** `connector_connections` rows, **0** workflow definitions of ANY status containing an `external_action` phase, and `live_connectors` ABSENT from `feature_visibility` (⇒ cold default off, an operator action by D-26, still with no Control Room card - D-190-DEF-09). ⚠ **VALIDATION's own claim that Row B is "runnable today, no operator dependency" is CORRECTED on that measurement** - its CREDENTIAL half is true, its ENVIRONMENT half is false; the mechanism Row B tests IS proved (D-17 on three axes across two real engine runs, three plants), and what is owed is the lived read of the card word. ⭐ **The one cheap unblock was VERIFIED before being written down, and REFUTED TWICE:** `supabase_inbucket`'s web UI answers on 54324 but NO SMTP port is reachable (54325 / 2500 / 1025 refuse; 54327 opens TCP and never speaks SMTP), AND - the stronger reason - `egress.py` REFUSES it by design, driven: `smtp://127.0.0.1` -> `scheme_not_tls`, `smtps://127.0.0.1` -> `address_not_public`, `smtp+starttls://127.0.0.1` -> `address_not_public`, `smtps://localhost` -> `host_not_allowed`. **So `send_email` needs a PUBLICLY-ROUTABLE TLS SMTP host, strictly LARGER than D-30's "throwaway mailbox"** - and the guard refusing the one destination that would have made the row cheap is the guard WORKING. `check-gap-closure-rounds.cjs 190` -> **G-7 clear, exit 0, ZERO gap-closure rounds**; `folded_into: 190` -> **0**; parity queue re-derived at **104 -> 117**; `check-deploy-drift.sh` -> PASS. **CONN-02 / CONN-03 settled in REQUIREMENTS.md as NOT complete with the reason and the closing condition written out** - CONN-03 because its own text names `/gsd:secure-phase (threats_open: 0)` and that gate has not run; CONN-02 because its word is RUN and no message, ticket or email has left this application.
last_activity_prev: 2026-08-09 -- **Phase 190 plan 18 EXECUTED - the REFUSAL COPY, the CHECK MOMENTS and the GRADED GUARDS, and the plan's own falsification found that the fence for 14's most-named failure condition DID NOT WORK.** Sketch 156 drew ONE refusal; `egress.py` refuses for SIX reasons and 4d keeps three further outcome shapes apart - all ten are now authored ONCE in `connectionRefusalCopy.ts` and rendered from the guard's own reason code, never improvised at a call site. **THE HEADLINE IS A FENCE THAT MEASURED NOTHING:** 14 states the condition as *a refusal renders the word "failed"*, so the first draft asserted exactly that - and a plant that made the REFUSED branch render 5b's sentence stayed **GREEN at 63/63**, because that sentence carries `failing` and `fail` but never the whole word `failed`. The exact flattening 14 exists to prevent had happened in real rendered output and nothing saw it; the fence was widened to the STEM (`fail` / `refus`), which is strictly stronger and costs nothing because every string each branch can render is measured free of it. **A SECOND plant also stayed green, and it is the OPPOSITE finding** - renaming the `aria-describedby` id moved BOTH ends together, so the wiring still resolved and the test was RIGHT to pass: there the PLANT was wrong, not the fence. Both are recorded in the count-gate pin comment, because *ten plants went red* is a weaker claim than *two did not, and here is which half was wrong each time*. **The 5c negation was READ OUT OF `_EXTERNAL_ACTION_NEGATION` at HEAD** - never typed from this plan's task text, which still quotes the string 190-15 corrected away - and PINNED against that source through Vite's `?raw` loader with the parser falsified on synthetic input BEFORE it was trusted on the tree. **4b's asymmetry ships in both directions**: an egress refusal leaves Save ENABLED with no `aria-describedby`; `no_encryption_key` DISABLES it - the ONE place on this surface where `disabled` is right, because the control is meaningful and its refusal IS the message - and the wiring is asserted to **RESOLVE**, not merely to exist, since an id pointing at nothing passes both `toBeDisabled()` and `getByText()`. 4c's `{ip}` and `{allowed}` are rendered in a MEASURED shorter form because the wire carries neither (190-15 deviation 3), and re-typing the guard's permitted-host list is what 4c forbids in the same breath as it names it; the reversal is purely additive and BOTH forms are already asserted. A PLATFORM condition gets a FOURTH block shape deliberately outside 4d's three. 32 new cases (31 -> **63**), TEN plants driven RED with every file restored md5-identical. Measured: count gate **OK 51/51, failed 0, total 2851** (pin 31 -> 63, PROVED to catch a decrease at 64/63); `tsc` **33 -> 33**; `eslint src/components/settings/` **5 -> 5**; the eight D-23/D-24 fences `0 0` with existence confirmed at the base first; `npm test` 4654 -> **4686 passed**, the SAME 8 D-190-DEF-05 files failing, **zero new failures**; zero installs.
### Previous last_activity - Phase 190 plan 17 EXECUTED (the add/edit panel), demoted from frontmatter to keep exactly ONE last_activity_prev key (the 735d1033 convention)

2026-08-09 -- **Phase 190 plan 17 EXECUTED - the add/edit PANEL, and D-190-DEF-07 is now CLOSED IN FULL.** Sketch 156-A ships whole: a 400px push/split panel that KEEPS THE LIST VISIBLE (the whole reason A beat a dialog - *when a check fails the honest next action is to look at the list*), with the FOCUS TRAP and FOCUS RESTORE a `Dialog` would have given free written as real code and asserted through `document.activeElement` **with a control rendered OUTSIDE the panel**, so the assertion cannot pass vacuously. Exactly **4 / 5 / 3** fields per capability and nothing beyond them (`tls` is DERIVED from the port - 465 implicit, else STARTTLS - which is what keeps `send_email` at the four 3b binds; the choice is never silent because the footer renders the mode on every keystroke). The 🔒 destination footer is UNCONDITIONAL, present on the FIRST render reading *nothing yet - fill the fields above* rather than disappearing, and derived during render, never from state. The secret is TEXT on edit - a `font-mono` `<span>`, never an input carrying a fake value - and is PROVEN absent from every response shape reaching the panel by enumerating each declared interface field-by-field (the one `key` hit is Jira's `project_key`, a non-secret). **THE HEADLINE IS THE OWED FIX, AND IT WAS LARGER THAN 190-16 NAMED:** UI-SPEC 9's panel notice promised *You can save this connection* while `require_visible("live_connectors")` refuses the write 403 - and directly beneath it sat a `font-mono` footer reading **`will save . will not send`**, the SAME false promise one line down, which 190-16's pointer did not mention. Both corrected in the commit that builds the panel, same branch (b), plus THE STRUCTURAL HALF: while the switch is off the panel renders no Save, no Replace and static text - REMOVED, not disabled. That half was driven, not assumed: 190-16's plant-C shape applied here goes RED on **both** absence cases (`renders static text, no Replace and no Save - ABSENT, not disabled` and `every WRITE affordance is REMOVED while the switch is off`), which a `toBeDisabled()` assertion would have passed twice. Reversal cost is now SIX edits, recorded in four places. **A SECOND locked sketch string was corrected on MEASUREMENT:** 3d's `stored 3 Aug` is not shipped - `ConnectorConnection` carries NO secret timestamp and `updated_at` moves on a RENAME, so dating a credential from it is the fabricated-timestamp error `credentialLabel` refuses one column away. Shipped: `stored since 3 Aug` plus a line saying what is not known. 31 cases, **TEN plants driven RED** against real production source and every file restored md5-identical. Measured: gate **OK 51/51, failed 0, total 2819**; tsc **33 -> 33**; eslint settings **5 -> 5**; the eight D-23/D-24 fences blob-identical at base, HEAD and worktree; zero installs.
stopped_at: "Phase 190 is CODE COMPLETE, SECURED (`threats_open: 0`) and LIVE-PROVEN on ONE connector. **A REAL SLACK MESSAGE WAS SENT** through the full governed path — approval gate, ordered guards, real send, honest `completed` status, and migration 117's `external_action_sent` audit receipt. The phase's own sentence *\"no message, ticket or email has left this application\"* is no longer true. **NEXT = `/gsd:complete-milestone` (v3.6).** ⚠ **SC#1 IS 1-of-2-3, NOT 5/5** — an earlier note this session claimed 5/5 and was corrected; the criterion's words are *2-3 first-party live connectors* and exactly one was driven. **THREE DEFECTS WERE FOUND BY RUNNING THE REAL THING, ALL INVISIBLE TO 19 PLANS OF RED-FIRST TESTING, A CODE REVIEW, A SECURITY AUDIT AND A VERIFICATION PASS:** (1) **FIXED `3aedf2fc`** — `workflow_kickoff.build_harness_run_context` never set `org_id`, so `_exec_external_action`'s D-14 guard failed closed on EVERY live run and **no bound connection could ever send, on any provider, with any destination**; SC#1 was scored *owed, not broken* and was BROKEN. (`_build_resume_context` HAS carried org_id since Phase 163 — only the live builder lacked it; the compiler refuted the first diagnosis as a duplicate-keyword SyntaxError. `publish_service`'s golden-run ctx DELIBERATELY still withholds org_id as a second fence behind D-16, documented + pinned.) (2) **⛔ BLOCKER `D-190-DEF-17`, NOT FIXED** — only **1 of 3 capabilities is drivable from a workflow AT ALL**. Measured by composing the shipped `_adapter_args` against the adapters' declared required sets: `post_message` MISSING=None · `create_ticket` MISSING=[`summary`] · `send_email` MISSING=[`subject`,`to`]. The executor auto-fills exactly ONE field (`_BODY_ARG_FOR_CAPABILITY`); **Slack works only because its required set equals that one field — a coincidence, not a design.** Jira raises *'summary' must be a non-empty string*; email has no way to name a recipient. So SC#1's 2-3 is **unreachable by construction** and no operator destination changes it. **NOT a gap-closure fix (G-7): there is nowhere for an author to supply the values** (`ExternalActionPhaseConfig` is `extra='forbid'`; the panel picks capability + connection only) and deriving them from content is COMPOSITION, which **D-09 forbids**. It needs new authoring surface = A PHASE. (3) connectors send **TEXT ONLY** — the `emit` phase produced a real 37 KB `.docx` the connector beside it cannot attach. **ALL THREE SHARE ONE ROOT CAUSE: each side was tested in isolation and NOTHING COMPOSED THEM.** Every org test injected a `SimpleNamespace` ctx carrying `org_id`, and every adapter test passed `args=` explicitly — both are shapes the shipped path never produces (the code review saw the pattern as WR-01 on `base_url` and did not generalise it). The new fence `test_190_ctx_org_scoping.py` therefore asserts over the REAL SOURCE of the real builders via AST and was driven RED against the shipped defect. **TWO LIVE CONNECTIONS EXIST AND BOTH CHECK GREEN:** Slack `post_message` (channel `C0BNK1QCF8F`) and Jira `create_ticket` (`fhdmrdautomation.atlassian.net`, project `KAN`) — both Fernet `enc:v1:`, credential absent from DB, logs and git. A seeded published workflow `slack-connector-uat-190` (`e7c68d09`) is the live proof artefact; do NOT delete it. **PARALLEL EXECUTION IS NOW ENABLED** (`use_worktrees: true`, `d08a492d`) — Phase 190 took 10.8 h serial vs ~5.7 h parallel; `scripts/bootstrap-worktree.sh` junctions the 1.7 GB venv + 541 MB node_modules, `scripts/teardown-worktree.sh` MUST be used (a plain `rm -rf` FOLLOWS the junction and destroys the real venv), and `GSD_VITEST_MAX_WORKERS=4` is MANDATORY in parallel runs. Rules in CLAUDE.md § *Parallel execution*. **THREE NEW SEEDS capture the operator's integration direction — read `SEED-146` FIRST, it is the umbrella:** 146 = the full capability surface (⭐ its headline: **EVERY capability is a WRITE; there is NO read/search/list capability at all**, which for a retrieval product is the wrong half to have built first — and reads are cheaper AND safer than the writes already shipped, though they add a genuine prompt-injection surface); 144 = provider-shaped accounts + OAuth (mig 116 puts `capability` ON the row, so 3 Slack actions = 3 copies of one token; adding capability #4 costs ~41 files + a migration); 145 = connections are PLATFORM assets usable from CHAT (⚠ **NEVER wire an outbound capability into `_TOOL_REGISTRY` before the approval model exists** — 189/190's armed checkpoint is a CANVAS construct and chat has none of it; Phase 085's `ask_user` is the primitive to build on). **Recommended next milestone = 144 + 145 + 146 + D-190-DEF-17 as ONE piece**, first cut: schema-driven input forms (closes DEF-17, same primitive a catalog needs) → ONE read capability end to end → provider accounts + OAuth → attachments. **Phase 191 (STRETCH) should be SKIPPED** — its condition *\"only if real scale exceeds the small default\"* is unmet at 222 definitions / max 5 phases; record the decision rather than leaving it dangling. ⚠ **CLOUD PARITY OWED: migrations 116, 117 AND 118 — and 118 is SECURITY-BEARING** (it revokes a `secret_ciphertext` SELECT grant that `anon` and `authenticated` both held). **118 and the `connector_service.py` deploy MUST land in the SAME operation** or every connector read 42501s. ⚠ Stale bookkeeping `/gsd:complete-milestone` should fix: the ROADMAP checkboxes for 181/184/185/188/190 still read `[ ]` though complete, and CONN-02/CONN-03 are still unticked in REQUIREMENTS.md. **`develop` was pushed to origin at `d88e8486`** after 1,557 unpushed commits. ⚠ The SDK `state.*` verbs corrupted this file FIVE times this phase — every write since is hand-edited."

### Previous stopped_at — Phase 190 secure+verify close, superseded by the block above

"Phase 190 is **CODE COMPLETE (19/19), REVIEWED, FIXED, SECURED and VERIFIED at 4 of 5 success criteria** — the fifth is a stated operator dependency, not a defect. Full chain ran unattended: plan-phase (19 plans, plan-checker PASSED) -> execute-phase (19/19) -> code-review -> fixes -> secure-phase -> verify-work. **`/gsd:code-review 190` found 4 Critical / 6 Warning / 4 Info, and the guard was NOT among them** — the reviewer could not break `egress.py`. **CR-01 was a REAL credential exposure and I confirmed it against the live DB before acting: BOTH `authenticated` AND `anon` held column-level SELECT on `secret_ciphertext`, with the RLS SELECT policy admitting any org member — so any authenticated org member could read the encrypted credential straight from PostgREST, making SC#4 FALSE as shipped.** The RLS shape was copied from `sso_configs`, **a table with no secret column**; the precedent did not carry. Closed by **migration 118** + explicit column projections (PostgREST default-projects `select=*`, so `SELECT *` now 42501s and the service had to name columns). Re-verified by me post-fix: `secret_ciphertext` -> `authenticated` INSERT/UPDATE only, **`anon` NOTHING**; the 11 safe columns still read. Also fixed: **CR-02** kill-switch read an unbounded-stale cache (one `await ensure_settings_fresh()`), **CR-03** the switch failed OPEN for 3 of its 4 legal audience values (now a positive `!= \"everyone\"` test, fail-closed against a fifth), **CR-04** blocking `getaddrinfo` on the event loop from three `async def` sites (D-v2.5-01 red line), plus WR-02/WR-03/WR-05. **`190-SECURITY.md`: `status: verified`, `threats_open: 0`** over a 115-threat register (102 closed / 13 accepted-risk); all fourteen D-28 minimum threats have a mitigation at a read file:line and a test observed RED against a real plant. **NEXT = the owed live UAT. RUN SLACK `post_message` FIRST** — smallest SSRF surface (code-constant host), falsifies **T13**, retires assumption **A3** in one run. ⚠ **A FALSE CLOSING MEASUREMENT WAS FOUND AND CORRECTED, and it CHANGES WHAT IS OWED:** the close recorded *0 workflow definitions of any status containing an `external_action` phase*. **There are 5, one PUBLISHED** (`ff3c6ca3` \"Weekly Status Report\", `send_email`, authored at Phase 189 UAT). Two compounding errors: `definition->'phases'` on a jsonb column holding a **JSON string scalar** (194 of 222 rows) silently returns zero rows rather than erroring — correct form is `((definition #>> '{}')::jsonb)`; and **`phase_type` lives inside `phase[\"config\"]`**, not at the phase top level (my own first correction attempt made that second mistake and returned 0 too). **So `BLOCK-190-UAT-01` is TWO links, not three — the owed rows need a destination and the flag, NOT authoring.** ⚠ **LOUDEST OPERATIONAL ITEM: cloud parity is `099 -> 118` and **118 is SECURITY-BEARING — cloud still has CR-01**. Migration 118 and the `connector_service.py` deploy MUST land in the SAME operation: the grant without the code breaks every connector read with `42501`.** ⚠ Two NEW accepted risks the audit added (`AR-190-09`), neither a blocker: `require_visible` gates four endpoints and the fourth is **`/check`, an egress surface**; and `require_visible` has **no `ensure_settings_fresh()`** at HEAD (CR-02 root cause fixed in the executor, not here). No send can result — GATE 2 is independent, freshness-bounded and positively tested. ⚠ `send_email` needs a **publicly-routable TLS SMTP host**, not just a mailbox: `supabase_inbucket` was tested and REFUSED by the phase own guard (`scheme_not_tls` / `address_not_public`), which is the guard working correctly. ⚠ **The SDK state verbs corrupted this file FIVE times this phase** (`state.record-session`, `state.planned-phase`, `state.begin-phase`, `state.advance-plan`, `update-progress`) — deleting `stopped_at`/`resume_file` and truncating `last_activity` while REPORTING SUCCESS. All writes since are hand-edited. 190-06 corrected the diagnosis: the failing parse is the markdown **BODY**, not the frontmatter."

### Previous stopped_at — Phase 190 plan 19 close, superseded by the block above

"Phase 190 is **CODE COMPLETE at 19 of 19 plans** and **CLOSES WITH TWELVE UAT ROWS OWED — a stated DECISION, never a claim that everything ran.** Plan 19 commits: `4afd1ba7` (the filled VALIDATION — the phase-wide fence sweep, 14/14 falsification rows with verbatim REDs, 7/7 fences, the 51-row per-task map, the derived SC#10 roster) and the close commit for ROADMAP / STATE / REQUIREMENTS. Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-19-SUMMARY.md`. **NEXT = `/gsd:secure-phase 190`** — mandatory, `threats_open: 0`, and it is the LAST thing standing between CONN-03 and complete. ⚠ **IT MUST DISPOSITION TWO SURFACES NO PLAN'S REGISTER COVERS:** (1) the **request-time SSRF / outbound-latency-amplifier** surface the credential check introduced — the FIRST synchronous outbound network call made from a user HTTP request in this codebase, with no rate limit (D-32 fences one) and an audience bounded only by `require_org_manage`; and (2) the **resume-path blind spot** A4 exposed — the sweep re-drives runs on boot through a SECOND ctx builder that carries no `is_golden_run`, closed at the root for golden runs, but any future per-run suppression flag inherits the same blind spot. **THE OWED ROWS, NAMED AND PRIORITISED (12):** the 8 SC#10 provider rows · the 3 live-send rows (D-30) · Row B's lived read — plus the kill-switch lived toggle (already automated in BOTH directions), the two sketch-visual rows, the long-message axis, and 190-17's still-owed **375px rendered-geometry** row, which 190-18's refusal blocks make MORE pressing since they are now the tallest content this surface has. ⭐ **RUN `post_message` (Slack) FIRST** — it needs only a bot token and a channel id, its host is a code constant (the smallest SSRF surface of the three), it falsifies **T13** (the likeliest shipped defect), and it retires assumption **A3** (`auth.test`'s reply shape, never driven live) in the same run. **THE BLOCK IS MEASURED, NOT ASSUMED — `BLOCK-190-UAT-01`:** 0 `connector_connections` rows, 0 workflow definitions of ANY status containing an `external_action` phase, and `live_connectors` absent from `feature_visibility` (cold default off — an operator action by D-26, and **D-190-DEF-09** records that the Control Room still has no card for it). So the artefact every SC#10 row needs does not exist, and building one is authoring + a global-setting change in the operator's environment, which is what D-32 and G-7 fence a closing documentation plan out of. ⚠ **VALIDATION's own claim that Row B is 'runnable today' is CORRECTED on that measurement** — its credential half is true, its environment half is false. WARNINGS TO CARRY, each measured rather than remembered: (a) **`send_email`'s dependency is LARGER than D-30 recorded** — it needs a publicly-routable **TLS** SMTP host, because `supabase_inbucket` has no reachable SMTP listener AND `egress.py` refuses a loopback plaintext destination by design (four variants driven: `scheme_not_tls` / `address_not_public` ×2 / `host_not_allowed`); that refusal is the guard working. (b) **RESIDUAL-190-01's stated trigger CANNOT FIRE** — `requirements.txt:38` is `httpx>=0.28.0` with **no upper bound**, so httpx can move on any fresh install or container rebuild with nobody editing the file; what protects the DNS pin is that `test_190_residual_fence.py` runs against the **installed** library on every test run. Re-open on any Python or httpx version move. (c) **Assumptions A1 (Jira's error envelope, never driven live) and A2 (an unreadable reply after a POST leaves the outcome genuinely unknown, and D-18 forbids the retry that would resolve it) are open**, both with triggers. (d) **D-18's *'halts the run'* is NOT the shipped engine semantic** — at-most-once IS shipped and grep-checked, but a failure fails the PHASE and the engine continues by its own explicit decision; a wording reconciliation, not unshipped work. (e) **D-190-DEF-08's import cycle is tripwired, not fixed** — `phase_types.py:94` is the registry's only module-scope importer and the fence now classifies module-scope vs function-local. (f) **Cloud parity owed: migrations 116 and 117 join the queue, re-derived at `104 -> 117` + `SECRETS_ENCRYPTION_KEY`** (`bash scripts/pending-cloud-migrations.sh`, never quoted). (g) ⚠ Do NOT read `npm test` as this phase's regression signal (D-190-DEF-05/-06) — 21 failures in 8 files, re-attributed at close by running ONLY the named rot set and reproducing 21. (h) **G-7 is clear at ZERO gap-closure rounds** (`check-gap-closure-rounds.cjs 190`, exit 0), so a later `--gaps` routing starts from a measured number."

### Previous stopped_at - Phase 190 plan 18 EXECUTED (the refusal copy / check moments / graded guards), superseded by the block above

Phase 190 plan **18 of 19 EXECUTED** - the refusal copy, the three check moments and the graded destructive guards all ship. Commits `1a9b0b08` (the closed copy module), `9eab21c2` (the two asymmetric refusals + the check + the guards) and `53928e41` (32 cases by character-identity + ten plants + the count-gate re-pin). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-18-SUMMARY.md`. **NEXT = plan 190-19**, the last of the phase. WARNING - carry forward, each measured rather than asserted: (a) **the stem-not-word fence on 4d is load-bearing and must NOT be narrowed back** - the whole-word `failed` version was MEASURED to stay green while a refusal rendered 5b's sentence, so restoring it restores a fence that is green forever. (b) **the `cannot` acceptance grep contradicts UI-SPEC 11d copy rule 5** and the rule wins: rule 5 bans an absolute verb only for a guarantee a CLIENT gate provides, and 4b block 9's locked heading names a server-enforced 503. The residual is exactly ONE line (`connectionRefusalCopy.ts:390`); the shipped fence is the three phrases 14 actually names, asserted over EVERY string the module can produce, and it is 0 in both files. (c) **do not re-type the guard's permitted-host list in the client** - 4c forbids it in the same sentence that names `{allowed}`; add `ip` + `allowed` to `ConnectorCheckResponse` and pass them, and 4c's literal wording renders with no copy change (both forms are already asserted). (d) **the negation table is pinned against `_EXTERNAL_ACTION_NEGATION` through `?raw`** - `send_email` is `No email was sent.`; this plan's own task text still quotes the corrected-away string, and UI-SPEC 5c carries the correction. (e) **a `failed 1` with no nameable file has now been seen THREE times across two plans** (once here, twice in 190-17) and never reproduced - re-run the gate before attributing it to any suite. (f) unchanged and still owed: the parity queue is `099 -> 117` + `SECRETS_ENCRYPTION_KEY`; `live_connectors` is OFF by cold default in EVERY environment so every surface this plan ships is correctly absent until an operator flips it, and **D-190-DEF-09** (no Control Room card for it) is still open; **nothing in this plan contacts a real host** - every drive is stubbed, so D-30's operator credentials are still owed, along with 190-17's named RENDERED-GEOMETRY row at 375px, which the refusal blocks make MORE pressing since they are now the tallest content this surface has. ⚠ Do NOT read `npm test` as a regression signal (D-190-DEF-05/-06)."

### Previous last_activity - Phase 190 plan 15 EXECUTED (the credential check + Gate 2), demoted from frontmatter to keep exactly ONE last_activity_prev key (the 735d1033 convention)

2026-08-09 -- **Phase 190 plan 15 EXECUTED - the credential CHECK and GATE 2, and the plan's own discipline caught a defect in its own source documents.** `POST /connectors/connections/{id}/check` ships as a DEDICATED org-admin action on the STORED connection: it takes an id and NOTHING ELSE (asserted three ways - the signature's exact parameter list, `requestBody` ABSENT from the OpenAPI schema, and a live POST carrying `secret`+`password` that is accepted, ignored and never echoed), so no plaintext credential ever crosses the wire for a non-storage purpose, and it resolves through the SAME org-scoped resolver a RUN uses - a green verdict is evidence about the row the engine will actually resolve. **THE HEADLINE FINDING IS A DOCUMENT CORRECTION, NOT A BUILD:** UI-SPEC 5c, this plan's own interfaces block AND plan 190-18 all quote the closed negation table's `send_email` row as *'No mail was delivered to anyone.'* - in the sentence that FORBIDS improvising it. The table 189 actually ships says **'No email was sent.'**, confirmed four independent ways (`phase_types._EXTERNAL_ACTION_NEGATION`, `189-UI-SPEC.md:761`, `189-UAT.md:104` read straight out of `workflow_phases.output`, `189-SECURITY.md:92`). Found by RUNNING the assertion, not by reading; the CODE was not bent to the document (that would be a 189-surface change invalidating 189's own UAT evidence) - 5c was corrected and all three rows are now pinned against production source. **GATE 2's reach is exactly org + `is_enabled`**: measured `grep -c last_check_verdict phase_types.py` -> **0**, `grep -c is_enabled` -> **3**, and door (b) is asserted as a **POSITIVE** (the server ACCEPTS a `connection_id` write naming a failing connection) plus a tree-wide ABSENCE (the column name appears nowhere outside its ONE writer, its response model and the router's prose) - so a later 'tightening' fails loudly instead of silently contradicting 5b's shipped sentence. **'And nothing was sent' is PROVED per capability with the transport recorded**: no `chat.postMessage`, no POST to `/rest/api/3/issue`, SMTP verbs exactly `['login', 'quit']` and no request body on any exchange, with the non-vacuity halves (`ok` AND a named identity) asserted FIRST. 14 backend cases + 2 frontend container fences; **EIGHT plants driven RED, each on the case it was aimed at**, every file restored md5-identical - and plant D is the one to read, going red on exactly ONE of three parametrised rows. The registry import tripped 190-14's sole-importer fence, correctly: the fence was made **STRONGER** (it now classifies module-scope vs function-local rather than counting) and driven RED against a real module-scope plant. `Check credential` now RENDERS - the container passes `onCheck`, and two `?raw` fences close the gap that let 34 view-level cases stay green for all of 190-16 while the item was absent from the running app. Full backend suite **211 failed / 3575 passed - 211 is UNCHANGED from 190-13's recorded figure, ZERO new failures**; `tsc` 33 -> 33; `eslint src/components/settings/` 5 -> 5; count gate OK 50/50 `failed 0` (pin 34 -> 36, PROVED to catch a decrease at 37/36); all eight D-23/D-24 fences `0 0` with existence confirmed at the base first; zero installs.

### Previous stopped_at - Phase 190 plan 17 EXECUTED (the add/edit panel), superseded by the block above

Phase 190 plan **17 of 19 EXECUTED** - the add/edit panel ships and D-190-DEF-07 is CLOSED IN FULL. Commits `f84a45e6` (the panel + its copy module + the ConnectionsTab seam + the owed UI-SPEC 9 fix) and `c83c82ab` (31 cases + both count-gate knobs, the pin proved to catch a deletion). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-17-SUMMARY.md`. **NEXT = plan 190-18** - the refusal copy (4c's six closed sentences + 4b's two blocks), the check moment (5c) and the destructive guards. Two things it inherits: the save path already surfaces `ConnectorApiError.reasonCode` VERBATIM so the closed map keys off the server's own code, and 4b's `no_encryption_key` branch is the ONE place 11d asks for `disabled` rather than removed - because there the control is meaningful and its refusal IS the message. **ONE ROW IS OWED TO MANUAL UAT and is named rather than implied:** a RENDERED-GEOMETRY check at 375px. jsdom computes no layout, so the shipped test asserts the CAUSES of horizontal overflow (no 400px track beside a 375px viewport, the tallest notice inside the `overflow-y-auto` body, `max-h-[85vh]`) - not overflow itself. ⚠ Do NOT read `npm test` as a regression signal (D-190-DEF-05/-06): 8 files / 21 tests fail and they are the recorded SEED-056 rot, attributed BY NAME, with the +33 delta fully accounted (4642 + 190-15's 2 + this plan's 31 = 4675). The honest instrument is `node scripts/vitest-count-gate.cjs`."

### Previous stopped_at - Phase 190 plan 15 EXECUTED (the credential check + Gate 2), superseded by the block above

Phase 190 plan **15 of 19 EXECUTED** - the credential check and Gate 2 both ship, and UI-SPEC 5c was corrected in the same commit as the mechanism it describes. Commits `889f5409` (the dedicated check action + `record_check_verdict` + the strengthened import fence), `c486ff9c` (Gate 2's exact reach, door (b) as a POSITIVE, and the 5c copy correction) and `1db22a24` (the client function + `Check credential` actually appearing). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-15-SUMMARY.md`. **NEXT: `190-17`** - which MUST amend UI-SPEC 9's panel notice in the commit that builds the panel, because that sentence is false under branch (b) for the same reason 2h's was. WARNING - carry forward: (a) **190-18 QUOTES THE CORRECTED-AWAY STRING** in its own task text; it must read the closing negation from `_EXTERNAL_ACTION_NEGATION` (`send_email` -> `No email was sent.`), never from its plan. (b) **do not re-litigate door (b)** - reversing it is a FOUR-EDIT change (the gate, 5b's absolute verb, the deliberate deletion of two named tests, and either giving members the check or recording the "ask an admin" dead end), and no subset of it is legitimate. (c) **the `is_enabled` branch cannot sit beside the unbound branch** - the executor holds no row until it resolves, so a second check there is dead code; the property was already shipped by 190-13 and only its legibility was missing. (d) **`connectors.registry` must be imported FUNCTION-LOCALLY outside `phase_types`** - a module-scope import opens a real cycle, and the fence now says so mechanically rather than by counting. (e) **a SECOND latent cycle was measured and is inherited, not introduced**: `import app.models.connector` first in a cold interpreter raises ImportError (models.connector -> harness.grounding -> harness/__init__ -> phase_types -> connector_service -> models.connector); `app.main` boots fine. (f) **NEW threat surface for secure-phase**: the check is the FIRST synchronous outbound network call made from a user HTTP request in this codebase - a request-time SSRF surface (mitigated by the pinned binders) and an outbound-latency amplifier with no rate limit (D-32 fences one), bounded to org admins by `require_org_manage`. (g) unchanged: the parity queue is `099 -> 117` + `SECRETS_ENCRYPTION_KEY`; `live_connectors` is OFF by cold default in EVERY environment, so `Check credential` is unreachable until an operator flips it and **D-190-DEF-09** (no Control Room card for it) is still owed; **nothing in this plan contacts a real host** - every drive is stubbed at the transport seam, so D-30's operator-provided credentials are still owed before any live UAT row can run.

### Previous last_activity - Phase 190 plan 16 EXECUTED (Settings -> Connections), demoted from frontmatter to keep exactly ONE last_activity_prev key (the 735d1033 convention)

2026-08-09 -- **Phase 190 plan 16 EXECUTED - SETTINGS -> CONNECTIONS, and the blocking D-190-DEF-07 contradiction RESOLVED rather than passed on.** Branch **(b)** taken: the 190-09 write gate STAYS and the COPY moved - UI-SPEC 2h's second sentence, the rendered `CONNECTIONS_BANNER_BODY`, AND the structural half a copy-only fix would have missed (while `live_connectors` is off, every write affordance is REMOVED, not disabled) - all in ONE commit `5af38925`. Branch (a) was refused ON MEASUREMENT: it would delete a security gate from a security phase inside a Settings-table plan and turn a shipped plant-driven test RED (`test_190_connectors_api.py` case 9). The four-edit reversal cost is written in three places, and **9's panel notice inherits the same false sentence and is OWED BY 190-17.** **The `Used by` count was MEASURED before it was chosen** (the plan's own OQ#6): 204 workflow_definitions, **125 published**, **48 kB** of definition JSONB, 2.21 phases avg / 5 max, server-side scan **1.6 ms** - so the LIVE count is taken rather than the on-demand-expand fallback, and it costs **zero new wire** because `GET /workflows/published` already returns `definition` inline. The read is OWNER-scoped, so the count is a FLOOR: zero renders `none you can see`, never a bare `0 steps`, with the scope stated ONCE at the table's foot. Sketch 155-C is IMPLEMENTED, NOT REINTERPRETED - five locked columns on the shipped 068-A divide-y roster (no `<table>`, no shadcn primitive, ZERO installs), the filter bar + live count, four states as GLYPH AND WORD, TWO DISTINCT empty states, graded guards (Delete always sheets; Disable sheets only above zero; Enable flips direct at nine victims) with the receipt idiom and NO toast mounted anywhere. Connections is the sixth tab at routing key `5`, rendered visually fourth, with 0 renumbered keys and 0 deep-link hits, both measured against HEAD. **34 cases, SEVEN driven RED against real plants in production source**, each restored md5-identical - and the one to read is the Add button planted as `disabled` instead of REMOVED, because a `toBeDisabled()` assertion PASSES on exactly that defect. BOTH count-gate knobs moved in the same commit (`src/components/settings/` was inside NEITHER, so the suite would never have RUN); pinned at 34 from the script's own `actual` across two agreeing runs, then PROVED to catch a deletion. Worth inheriting: this suite's first gate run red SIX UNRELATED shipped suites as bare STACK_TRACE_ERROR timeouts, purely because `userEvent.setup()` defaulted to a per-keystroke delay - `delay: null` cleared all eight. `tsc` 33 -> 33 unmoved; `eslint src/components/settings/` 5 -> 5 (a sibling `connectionsCopy.ts` takes 190-12's named fix rather than repeating its 5 -> 10 debt); count gate OK 50/50 `failed 0`; all eight fences blob-identical at BOTH `de122b9a` and HEAD; package diffs empty. New **D-190-DEF-09**: `live_connectors` has no operator card in the Control Room and `GovernedFeature` does not name it - widening costs `tsc` 33 -> 38 across five exhaustive maps, TWO inside `/admin` which D-25 fences, so both halves are owed by one later commit.

### Previous stopped_at - Phase 190 plan 16 EXECUTED (Settings -> Connections), superseded by the block above

Phase 190 plan **16 of 19 EXECUTED** - Settings -> Connections ships whole and D-190-DEF-07 is RESOLVED (branch b: the gate stays, the copy AND the structure moved in one commit). Commits `5af38925` (the table + the copy module + the resolution), `34221a41` (the sixth tab at routing key 5), `307b4099` (34 cases + both count-gate knobs). ⚠ **190-15 IS STILL OPEN - 16 shipped OUT OF ORDER**, which its own frontmatter licenses (`depends_on: [190-09, 190-12]`, both landed). The consequence is on the surface rather than hidden: `Check credential` is REMOVED, not inert, until 190-15 lands the client function beside its endpoint. **NEXT: `190-15`** (the credential check + Gate 2), then `190-17` - which MUST amend UI-SPEC 9's panel notice in the commit that builds the panel, because that sentence is false under branch (b) for the same reason 2h's was. Owed and named, not discovered: **D-190-DEF-09** (no Control Room card for `live_connectors`; the banner names the right home before that home has a row in it).

### Previous stopped_at - Phase 190 plan 14 EXECUTED (the standing fences), superseded by the block above

Phase 190 plan **14 of 19 EXECUTED** - the standing fences are live and every one of them has been observed FAILING. Commits `8d3ff9d0` (the D-05 property walk + D-04 + capability resolution + the SCOPED credential fence) and `d191dd64` (SC#3 proved-not-built, D-32's scope tripwire, the subprocess lazy-import fence, and the import-cycle deferral). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-14-SUMMARY.md`. **Next: plan 15.** WARNING - carry forward: (a) **the credential-header fence is SCOPED and must STAY scoped** - `slack_adapter.py` is exempt BY NAME because a bearer header is Slack's own auth mechanism; 'completing' the fence by deleting the exemption makes it RED on correct code, and a fence that is RED on correct code gets deleted, taking the Jira half with it. (b) **SC#3 has NO mitigation artefact, by design** - if verify-work or secure-phase expects one, D-09 says there is none and the honest sentence is *proved, not built*. (c) **D-190-DEF-08 is NEW: the connector registry's latent import cycle** - do not add a second importer of `connectors.registry` under `backend/app` without breaking the cycle in the SAME commit; the tripwire in `test_190_connector_source_fence.py` is what will tell you, and `deferred-items.md` names two cuts. (d) **the D-18 per-line rule now covers the whole package**: a line naming retry/backoff/sleep/queue must cite `D-18`; `idempot` is deliberately NOT in the token set (three shipped prose lines carry it on a sentence continuation) and adding it would make the fence RED on correct code. (e) **six of the fourteen falsification rows now have a standing fence with a driven RED behind it** - D-05, D-04, T9 source AND behaviour, plus D-32; `/gsd:secure-phase` does not need to re-drive them, the verbatim REDs and md5 pairs are in the summary. (f) unchanged: **D-190-DEF-07 is STILL OPEN** and still blocks the Settings banner (190-16/17 move copy and gate in ONE commit); the parity queue is `104 -> 117` + `SECRETS_ENCRYPTION_KEY`; the kill-switch is off by default in EVERY environment; **nothing in this plan sends anywhere real** - it is test-only, `git diff --numstat -- backend/app/` is EMPTY and `requirements.txt` is untouched, so D-30's operator-provided destinations are still owed before any live UAT row can run.

### Previous stopped_at - Phase 190 plan 13 EXECUTED (the ordered send path), superseded by the block above

Phase 190 plan **13 of 19 EXECUTED** - the send is LIVE behind six ordered gates. Commits `57c16808` (the executor + D-16 RED->green + the SUPERSEDED docblock), `108e2dbe` (A4 closed at the root + the docblock-retirement fence) and `d6b26d9d` (W0-3's meaningful RED, the sentinel's bound case green, D-17 on three axes, the receipt). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-13-SUMMARY.md`. **Next: plan 14.** WARNING - carry forward: (a) **`/gsd:secure-phase` gains a threat entry the plan did not anticipate** - `T-190-13-A4`'s disposition moved from *assert unreachability* to *WAS reachable, closed at the root*; the resume sweep re-drives runs with a SECOND ctx builder, and any future per-run suppression flag has the same blind spot - make the path unreachable, never thread the flag. (b) **D-18's wording needs reconciling at verify-work, not more code**: at-most-once is shipped and grep-checked (32 -> 33 hits, the one new match DENIES retry), but *'halts the run'* is NOT the shipped engine semantic - a `failure` output fails the PHASE and the run continues to the next one by the engine's own explicit decision, and changing that would need an engine edit the acceptance forbids. (c) **do not re-litigate the gate order**: the guard sits BELOW the kill-switch (D-26's *exactly as today*) and ABOVE the unbound branch (D-06 itself); moving it below the unbound branch IS n8n #28218 and the transcript of what that looks like is recorded in `test_190_egress_ordering.py`. (d) **the pre-credential guard is real for `post_message` only** (Slack's host is a code constant); `create_ticket`/`send_email` have no step destination because 190-06 settled it onto the connection row, and their socket-time guard is the binder's - 190-14's D-05 source fence is what keeps that true. (e) **the 190-01 `model_validate` debt is SETTLED, not deferred**: the sentinel's bound builder is model-validated again, and `test_190_egress_ordering.py`'s three original drives stay duck-typed PERMANENTLY because their `base_url` lives on the step - a new model-validated ordering case covers what they cannot. (f) unchanged: **D-190-DEF-07 is STILL OPEN** and still blocks the Settings banner (190-16/17 move copy and gate in ONE commit); the parity queue is `104 -> 117` + `SECRETS_ENCRYPTION_KEY`; the kill-switch is off by default in EVERY environment; **nothing in this plan sends anywhere real** - every drive is stubbed at DNS, at the resolver or at the transport sentinel, so D-30's operator-provided destinations are still owed before any live UAT row can run.

### Previous last_activity - Phase 190 plan 12 EXECUTED (ConnectionPicker + the slug seam), demoted from frontmatter to keep exactly ONE last_activity_prev key (the 735d1033 convention)

2026-08-09 -- **Phase 190 plan 12 EXECUTED - `ConnectionPicker` + the slug seam, and `PhaseFormPanel.tsx` is MEASURABLY `0 0`.** The picker ships as a NET-NEW SIBLING CHILD owning the `useEffect`, the API read AND the store write, so `ExternalActionSection` keeps its **THREE** props (CORRECTION #3 re-confirmed at HEAD, `:88-98`) and its whole cost is **`2 0`** - one relative sibling import, one gated JSX line, the `:222-231` note untouched. **THE SIX SOURCE-PURITY FENCES WERE RE-RUN AFTER THE MOUNT RATHER THAN TRUSTED** (research assumption A3, which both the plan and UI-SPEC 1b said to run rather than believe): all four purity `it()` blocks green, file unmoved at **34 tests**. A precision worth carrying: the "six fences" are six `expect().not.toMatch()` calls across **THREE** `it()` blocks, not six blocks - the fourth is their shared positive control. **FOUR plants driven RED in real production source**, each restored with an empty `git diff`: the planted `smtp_password` in the patch object (`expected [ 'connection_id', 'smtp_password' ] to deeply equal [ 'connection_id' ]`), Gate 1 short-circuited (`expected "patchConfig" to not be called at all, but actually been called 1 times`), the effect's disconnected guard deleted (a real `TypeError` inside a provider-less render), and the `aria-disabled` attribute removed. WARNING - **four findings worth carrying:** (1) **the control is a NATIVE `<select>` + the shadcn `<Label>`, not `@/components/ui/select`** - a Radix `SelectItem disabled` is UNSELECTABLE, which would make UI-SPEC 5a's mandated inline refusal unrenderable dead copy; U-09's recorded substance is *a select, not a typeahead* and that is satisfied. The reversal (Radix + `aria-disabled` NOT `disabled`, plus the two jsdom pointer shims the four existing Radix suites carry) is written in the summary. (2) **`aria-disabled` is set WITHOUT the hard `disabled` attribute, deliberately** - announced unavailable, write declined by the handler, refusal reachable and driven. (3) **the effect is gated on the SAME two nulls the write is** - guarding only the write still opens an unmocked request inside `ExternalActionSection.test.tsx`; plant 3 is that difference being observed. (4) **`eslint src/components/workflows/` went 5 -> 10**, all five new being `react-refresh/only-export-components` on the copy identifiers the plan explicitly requires - the same trade-off `BuilderStoreProvider.tsx` already ships three times in this directory; the one-line alternative (a sibling `connectionPickerCopy.ts`) is named rather than left implicit.



### Previous stopped_at - Phase 190 plan 12 EXECUTED (ConnectionPicker + the slug seam), superseded by the block above

Phase 190 plan **12 of 19 EXECUTED** - `ConnectionPicker.tsx` + `SelectedPhaseSlugContext.tsx`, and the six purity fences RE-RUN rather than trusted. Commits `b06df317` (the slug seam - children-only, no DOM node, non-throwing accessor), `e12f05eb` (the picker - seven states, Gate 1, ONE key written) and `04764bdf` (the 2-line mount + 20 pinned cases). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-12-SUMMARY.md`. **`ExternalActionSection.tsx` numstat is exactly `2 0`; all EIGHT D-23/D-24 fenced files are EMPTY against HEAD **and** against the phase base `de122b9a`, with their paths confirmed to exist first (190-05's lesson - an empty numstat and a typo are indistinguishable). Count gate OK, 49/49, pinned total 2719 -> 2739; `tsc -p tsconfig.app.json` unmoved at 33.** **Next: plan 13 (the send).** WARNING - carry forward: (a) **190-15 must build Gate 2 and ONLY its stated reach** - validate the `connection_id` write against the row's **org** and **`is_enabled`**, and deliberately NOT `last_check_verdict` (U-07a door (b)); the picker's copy is already worded to exactly that, and restoring an absolute verb without the server gate is the U-07a defect UI-SPEC 14 names as the single most likely regression here. (b) **190-18 must REUSE `CONNECTION_PICKER_FAILING_REFUSAL`** - it is exported and asserted by character-identity; a second wording is a second truth-teller. (c) **do not re-litigate the control** - a Radix `SelectItem disabled` is unselectable, so 5a's mandated inline refusal would be unrenderable dead copy; the shipped control is a native `<select>` + the shadcn `<Label>`, matching the plan's own named analog `ProviderPicker.tsx:184-195`, and the reversal is written out in the summary. (d) **`npm test` was RE-MEASURED a third time rather than quoted** - 23 failed / 10 files at HEAD (D-190-DEF-05 said 21/8, D-190-DEF-06 said 26/9), and it was DRIVEN: rolling this plan's changes out and re-running the identical 10-file set gives `8 failed | 21 failed` BOTH ways. **Zero new failures.** verify-work must still use `node scripts/vitest-count-gate.cjs`. (e) unchanged: **D-190-DEF-07 IS STILL OPEN and still blocks the Settings banner** (190-16/17 move copy and gate in ONE commit); 190-13 inherits three exception routes that are NOT `AdapterError` subclasses; the parity queue is `104 -> 117` + `SECRETS_ENCRYPTION_KEY`; the kill-switch is off by default in EVERY environment; 190-15 owes `checkConnectorConnection` in `api.ts`.

### Previous last_activity - Phase 190 plan 11 EXECUTED (the Slack post_message adapter), demoted from frontmatter to keep exactly ONE last_activity_prev key (the 735d1033 convention)

2026-08-09 -- **Phase 190 plan 11 EXECUTED - the `post_message` adapter, and T13 CLOSED.** `slack_adapter.py` POSTs `chat.postMessage` to the CODE-CONSTANT Slack API base through `send_pinned_http` and nothing else (zero banned transport tokens, zero `https://slack.com` spellings - the one spelling stays in `egress.py`), with the `xoxb-` bot token in an `Authorization: Bearer` header and NEVER in the body. **THE HEADLINE IS THE TRAP, AND IT IS A MEASUREMENT:** with the `ok` gate removed and the status gate left standing, the adapter logged `post_message: one message posted (channel=C0123ABCDEF ts=None)` and returned SUCCESS for a reply that said `{"ok": false, "error": "channel_not_found"}` - **the SAME log line a real success writes, except Slack's own timestamp is `None` because there is no message** (13 failed / 12 passed). That is D-31's *Complete for a send that did not leave the app* verbatim, on the vendor UI-SPEC 8c names as the likeliest place this phase ships it. Restored byte-identically (`e0474eca...`, `grep -c PLANT` -> 0). **25 passed / 0 failed** (18 `def test_`; the plan asks >= 9). WARNING - **five findings worth carrying:** (1) **the verdict is an IDENTITY check** - `payload.get("ok") is True`, never truthiness, because `{"ok": "false"}` is a non-empty string; driven across `"false"`/`"true"`/`1`/`0`/`[]`/`{}`/`None`/`"yes"`. (2) **there is deliberately NO third gate** - the sibling needs the issue-key gate because Jira's status line says nothing about existence, but `ok: true` IS Slack asserting the message posted, and demanding more would start failing sends that succeeded. (3) **the plan's "ignore a stored base_url" is UNCONSTRUCTABLE** - `PostMessageConfig` is `extra='forbid'` with `default_channel` only, so it is REFUSED (stronger); D-02 is instead asserted four ways, the strongest being that `_endpoint(method)`'s SIGNATURE cannot see stored data. (4) **the trap is closed on `check()` too** - `auth.test` answers `200 {ok:false}` for a REVOKED token, and a status-line check would paint a dead credential GREEN, which is worse than a failed send because it is a light a person acts on later. (5) **the plan's module-header text contradicts its own acceptance greps** (it asks the docstring to name `raise_for_status()` and `https://slack.com` while requiring both at count 0) - rewritten per 190-07's rule, the FOURTH occurrence of that prose-vs-fence conflict this phase. T14 also driven for this capability across five hostile destinations with a raw-string positive control.


### Previous stopped_at - Phase 190 plan 11 EXECUTED (the Slack `post_message` adapter), superseded by the block above

Phase 190 plan **11 of 19 EXECUTED** - the Slack `post_message` adapter, and **T13 is CLOSED with a driven falsification**. Commits `b336e047` (the drive: 18 cases, RED first at **24 NAMED failures + 1 green control**) and `6473603d` (`slack_adapter.py`, 561 L, **25 passed / 0 failed**, the plant observed and restored byte-identically). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-11-SUMMARY.md`. **Next: plan 12.** WARNING - carry forward: (a) **190-14 must SCOPE the credential-header fence, not apply it tree-wide** - 190-10 asked for the encoding vocabulary matched case-insensitively, which is correct for the ticket adapter and WRONG for this one, where a bearer header IS the vendor auth; fence this file on token-in-body and sentinel-in-refusal instead, both already driven. (b) **all three capabilities now resolve to a real importable adapter** whose `CAPABILITY` equals its registry key - the promise 190-08 left with a date on it is keepable at 190-14. (c) **190-13 must not "normalise" the two HTTP adapters' verdict functions** - they are fenced from BOTH sides now (the Jira-side fence shipped at 190-10, the Slack-side one here), matchers proved non-inert. (d) **A3 is new for SECURITY.md** - `auth.test`'s call shape and reply fields are NOT in R11 and not driven live; the verdict fails CLOSED in every direction, so a wrong guess yields a check that refuses to go green, never one that goes green wrongly. Trigger: the first live `post_message` UAT row, which VALIDATION already names as the recommended FIRST row. (e) unchanged from plan 10: **D-190-DEF-07 IS STILL OPEN and still blocks the banner**; 190-13 inherits three exception routes that are NOT `AdapterError` subclasses (`EgressRefused`, `EgressResponseTooLarge`, `EgressResponseUndecodable`) and will let them escape a bare `except AdapterError`; the parity queue is `104 -> 117` + `SECRETS_ENCRYPTION_KEY`; the kill-switch is off by default in EVERY environment; 190-15 owes `checkConnectorConnection` in `api.ts`; and verify-work must use `node scripts/vitest-count-gate.cjs` rather than `npm test` (D-190-DEF-06).

### Previous last_activity - Phase 190 plan 10 EXECUTED (the Jira `create_ticket` adapter), demoted from frontmatter to keep exactly ONE `last_activity_prev` key (the `735d1033` convention)

**Phase 190 plan 10 EXECUTED - the `create_ticket` adapter: Jira Cloud REST v3.** `jira_adapter.py` POSTs to `/rest/api/3/issue` at the org-configured atlassian.net site through `send_pinned_http` and nothing else (zero banned transport tokens), hands the credential pair to the library as `auth=(account_email, api_token)` with **zero** encoding-vocabulary or credential-header tokens in the file, and builds the ADF description PROGRAMMATICALLY. **THE HEADLINE IS THAT `ok=True` MEANS THE TICKET EXISTS, and it is a measurement rather than a claim:** with the error-envelope and issue-key gates removed (PLANT B) the adapter logged `create_ticket: one issue created (project=OPS key=UNKNOWN)` and returned SUCCESS for a reply carrying Jira's own `errorMessages` and no key at all - D-31's *Complete for a send that did not leave the app* in the Jira vendor's clothes (3 failed / 14 passed). PLANT A (the plan's) made `_plain_text_to_adf` accept a dictionary and the captured log shows the author-supplied rich document **filed as OPS-17** (1 failed / 16 passed). Both restored byte-identically (`6bd7c768...`, `grep -c PLANT` -> 0). WARNING - **four findings worth carrying:** (1) **both plants were checked for REACH, not just for RED** - 190-08 warned that a plant can leave its target cases green when an earlier step refuses first, so `send` hands the builder the RAW `description` deliberately; a string check in front of it would move the fence off the real path and make PLANT A unreachable. (2) **`ok=True` needs THREE gates, not one** - a 2xx status, no error envelope whatever the status line said, and an issue KEY; the plan's own text asked for the status-line-only check, and taking it literally would have left the plan's own success criterion unmet. (3) **the plan's `issuetype` "from config if present" is UNCONSTRUCTABLE** - `CreateTicketConfig` is `extra='forbid'` with only `base_url`/`project_key`/`account_email`, so it is the constant `DEFAULT_ISSUE_TYPE = "Task"` with a named re-open trigger. (4) **A2 is a NEW named residual** - `EgressResponseTooLarge`/`Undecodable` after a POST leave the outcome genuinely unknown and D-18 forbids the retry that would resolve it, so both propagate unchanged rather than wearing a §4d heading that would be false. 17 passed / 0 failed; `test_harness_engine.py` unmoved at 46; the 190/189 group measured WITH and WITHOUT the new file (151 + 17 = 168, same 4 known REDs); zero installs, zero migrations.

### Previous stopped_at - Phase 190 plan 10 EXECUTED (the Jira `create_ticket` adapter), superseded by the block above

Phase 190 plan **10 of 19 EXECUTED** - the Jira `create_ticket` adapter. Commits `68a24aea` (the drive: 17 cases, RED first at 17 NAMED failures, resolved through a `_require`-shaped helper so no collection error could silence the file) and `aeaab93f` (`jira_adapter.py`, 631 L, suite green, both plants observed and restored). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-10-SUMMARY.md`. **17 passed / 0 failed.** **Next: plan 11 (the Slack adapter).** WARNING - carry forward: (a) **190-11 must NOT import anything from `jira_adapter`** - Slack's contract is the OPPOSITE (a 200 with `ok:false` is a FAILURE; `raise_for_status()` shape is WRONG there), and `test_jira_and_slack_do_not_share_a_response_checker` will go red with its matcher already proved non-inert. (b) **D-190-DEF-07 IS STILL OPEN and still blocks the banner** - the write endpoints carry `require_visible(\"live_connectors\")` while UI-SPEC 2h promises a non-operator can save; 190-10 touched no router and no copy, so whoever renders it must still move BOTH halves in one commit. (c) **190-13 inherits three exception routes that are NOT `AdapterError` subclasses** - `EgressRefused`, `EgressResponseTooLarge` and `EgressResponseUndecodable` pass straight through the adapter and will escape a bare `except AdapterError`. (d) **190-14 should lift the credential-encoding fence and match it CASE-INSENSITIVELY** - a lowercase header key is the same hand-built header and a case-sensitive walk passes over it. (e) **A1 (Jira's error-envelope shape, not driven live) and A2 (an unreadable reply after a create) are drafted verbatim for SECURITY.md** in the 190-10 summary, each with its trigger. (f) Unchanged from plan 09: the parity queue is `104 -> 117` + `SECRETS_ENCRYPTION_KEY`, the kill-switch is off by default in EVERY environment, 190-15 owes `checkConnectorConnection` in `api.ts`, and verify-work must use `node scripts/vitest-count-gate.cjs` rather than `npm test` (D-190-DEF-06).

### Previous stopped_at - Phase 190 plan 08 EXECUTED (the connectors package: adapter seam + registry + SMTP adapter), superseded by the block above

Phase 190 plan **08 of 19 EXECUTED** - the connectors package. Commits `563fde70` (the package + `protocol.py` + `registry.py`), `b2fb6f49` (the T12 suite RED first - **12 NAMED failures / 1 passed**, and the 1 that passed is the POSITIVE CONTROL, which is green by design because it proves the attack using nothing but the stdlib) and `8ed7d22b` (`smtp_adapter.py`, 13 passed, both plants observed). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-08-SUMMARY.md`. **13 passed / 0 failed**; the 190/189 regression group **160 passed** with only the known 190-13 RED; `test_harness_engine.py` still **46 passed**; zero deps installed. **Next: plan 09.** WARNING - carry forward: (a) **the contract for 190-10 / 190-11** is `class Adapter` with `CAPABILITY` + `INPUT_SCHEMA` class attributes, returning `AdapterResult` / `AdapterCheckResult` and subclassing `protocol.AdapterError`; the registry CHECKS `CAPABILITY` against the key it resolved. **Do NOT add a shared response helper** - `AdapterResult.ok`s docstring says why, one plan before T13 bites. (b) **190-13 calls** `registry.get_adapter(capability).send(args=..., credential=..., config=...)`; `capability=` is accepted and checked but deliberately NOT required, because 190-13s own task text describes a three-argument call. `config` is the CONNECTION ROWs non-secret dict, never the steps JSONB. (c) `jira_adapter` / `slack_adapter` are DECLARED in the registry now and raise `ModuleNotFoundError` when resolved - intentional, so the D-04 assert is meaningful from the first commit; 190-14 turns that gap into a promise. (d) The same **four RED tests still belong to 190-13** (`test_a_bound_connection_under_the_armed_sentinel_MUST_attempt_egress` + the three `test_190_egress_ordering.py` cases) - unchanged, not a regression. (e) The banned-token tuple and its non-vacuity control are written in `test_190_smtp_header_injection.py` and can be lifted verbatim by **190-14** - and note that PROSE is swept too. (f) The cloud-parity queue is still **104 -> 117** + `SECRETS_ENCRYPTION_KEY`, and D-30s three live-send rows remain BLOCKED on an operator-supplied mailbox / Jira project / Slack channel.


### Previous stopped_at - Phase 190 plan 07 EXECUTED (the two DNS-pinned transport binders), superseded by the block above

Phase 190 plan **07 of 19 EXECUTED** - the two DNS-pinned transport binders. Commits `2c437a69` (the six binder cases RED-first - **6 failed / 71 passed, six NAMED symbol failures**, deliberately not the collection-error shape that would have silenced the 71 inherited validator cases), `31eeae5a` (`send_pinned_http` + `open_pinned_smtp`) and `673599b9` (the RESIDUAL-190-01 fence, driven RED against a SIMULATED library change). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-07-SUMMARY.md`. **82 passed / 0 failed**; `test_harness_engine.py` still **46 passed**; zero deps installed. **`redirected` now HAS a raise site** - the code 190-02 declared and left unraised - so all six UI-SPEC 4c codes are live; a 3xx RAISES rather than being returned to the caller, because handing a bare 302 to an adapter leaves the bypass one adapter line away. **Next: plan 08** - the adapter protocol + registry + SMTP adapter. WARNING - carry forward: (a) `190-06`'s hand-off predicted *plan 07 = the connectors router*; **that was wrong, the plan file is authoritative**, and the router is not yet claimed by a plan number. (b) The same **four RED tests still belong to 190-13** (`test_a_bound_connection_under_the_armed_sentinel_MUST_attempt_egress` + the three `test_190_egress_ordering.py` cases) - unchanged by this plan, not a regression. (c) `send_email` callers MUST pass `allowed_host=` or the guard fails closed. (d) `PinnedResponse` interprets NOTHING - Slack success is `200 AND json()[\"ok\"] is True` (T13), Jira uses real status codes; a shared success flag IS the T13 defect. (e) The standing `connectors/**` D-05 fence is **190-14's**; the banned-token tuple and its non-inertness control are written in `test_190_residual_fence.py` and can be lifted verbatim. (f) The cloud-parity queue is still **104 -> 117** + `SECRETS_ENCRYPTION_KEY`, and D-30's three live-send rows remain BLOCKED on an operator-supplied mailbox / Jira project / Slack channel.


### Previous stopped_at - Phase 190 plan 06 EXECUTED (the credential layer / the D-14 leak), superseded by the block above

Phase 190 plan **06 of 19 EXECUTED** - the credential layer. Commits `09808f6d` (the Pydantic models + D-13's ONE additive-optional `connection_id`, with the standing no-optionals refusal block extended in the SAME commit), `e1694006` (`connector_service` - the leak observed, then the two-gate close, D-11 both ends, D-15 answered PER CALL SITE) and `994d8028` (eight credential cases, seven plant-driven RED). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-06-SUMMARY.md`. **Next: plan 07 - the connectors router** (inject the USER-JWT client per D-15 case 1; map `ConnectorNotFound` -> 404 and `ConnectorCipherUnavailable` -> 503, sketch 156 moment 9). WARNING - carry forward: **four tests in the 189/190 slice are RED and they are exactly what 190-13 owes** - `test_a_bound_connection_under_the_armed_sentinel_MUST_attempt_egress` plus the three `test_190_egress_ordering.py` cases, which IMPROVED from a collection error to a meaningful `AttributeError: module app.services.harness.phase_types has no attribute resolve_connection / validate_destination` - precisely the namespace import three drives depend on. `base_url` is SETTLED: it lives on the connection ROW (`CreateTicketConfig`), never on the step config, and the two duck-typed builders 190-01 flagged are now UNBLOCKED (re-measured: `model_validate` accepts `connection_id`, and an old row without it still validates). No router and no `delete_connection` exist yet - deliberate, not stubbed. The cloud-parity queue is still **104 -> 117** + `SECRETS_ENCRYPTION_KEY` (190-19 should RE-RUN `scripts/pending-cloud-migrations.sh`, never quote it), and D-30's three live-send rows remain BLOCKED on an operator-supplied mailbox / Jira project / Slack channel."


### Previous stopped_at - Phase 190 plan 04 EXECUTED (the D-01 ROADMAP amendment), superseded by the block above

Phase 190 plan **04 of 19 EXECUTED** - the paper half of D-01. Commits `31cb7cea` (the appended amendment, +86/-0), `0fc1c2b8` (`D-v3.6-02` + SC#1 + the D-32 fence + the measured parity queue) and `00b682da` (`phase_types.py` onto the G-5 hot-file ledger, +1/-0). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-04-SUMMARY.md`. **Next: plan 05 - the `Not connected` badge retires by DATA, one line at `phaseVocabulary.ts:812`** (`PhaseNode.tsx` / `PhaseNodeCard.tsx` and the six fenced card-subtree modules must have EMPTY diffs - D-24). WARNING - carry forward: the cloud-parity queue is **104 -> 117** + `SECRETS_ENCRYPTION_KEY`, and 190-19 should RE-RUN `scripts/pending-cloud-migrations.sh` rather than quote that number. D-30's three live-send rows remain BLOCKED awaiting an operator-supplied mailbox / Jira project / Slack channel - Slack `post_message` runs first.
### Previous stopped_at - Phase 190 plan 03 EXECUTED (migrations 116 + 117 live), superseded by the block above

Phase 190 plan **03 of 19 EXECUTED** — the schema half of the connector slice is LIVE. Commits `ed53a909` (both migrations + the Python mirror + the moved G2 control + the regenerated `full-schema.sql`, all four plan files in ONE commit as Task 3's acceptance criterion requires) · `0e5a62a9` (the Rule-3 lockstep count bump). **Migration head re-derived, never inherited:** `ls supabase/migrations/ | tail -1` → `115_…`, 109 files ⇒ 116 and 117 are the next two free slots, both matching `^[0-9]+_[a-z0-9_]+\.sql$` (a letter suffix is silently skipped by the CLI and would ship a table that does not exist). **The 23 inherited audit literals were RE-DERIVED from the live artifact** — `sed -n '1124p' supabase/full-schema.sql | grep -o …` — and that output is what 117 lists, plus `external_action_sent` last. **`org:manage` reused, not a new `connectors:manage` key** (already seeded to super-admin + org-admin at `104:416-426`, and it is exactly UI-SPEC §2b's audience). **Two schema comments carry decisions that read as bugs without their reasoning** and are worth knowing before touching this table: `secret_ciphertext` records the fail-CLOSED inversion of `get_cipher()`'s fail-OPEN polarity AND that the column is deliberately NOT in `SECRET_COLUMNS`; `last_check_verdict` records that it is a QUALITY HINT and that server Gate 2 deliberately does not read it (U-07a, door (b)). ⚠ **The apply is a RECORDED DEVIATION** — `psycopg2` against local, not the SQL editor, because the session is unattended; D-21 + the 189-06 precedent; the destructive `supabase db` subcommands were never run. ⚠ **Out of scope, logged as D-190-DEF-04:** `backend/tests/test_dual_mode_wiring.py` has **15 pre-existing failures**, PROVED not assumed to be unrelated — its four audit-vocabulary tests are green (`-k 'audit or event_type'` → 4 passed), it pins no count, and the histogram is dominated by `app.api.threads does not have the attribute insert_run`, which moved out at Phase **162.5**. Do not read a whole-suite number off that file. **Unblocked downstream:** the org-scoped resolver (D-14) has its table and its `(org_id, capability)` index; the cipher path (D-11) has its column; the send receipt has its literal at BOTH layers. **NEXT: `/gsd:execute-phase 190` continues at plan 04.**

### Previous stopped_at — Phase 190 plan 02 EXECUTED (the egress destination validator), superseded by the block above

Phase 190 plan **02 of 19 EXECUTED** — `backend/app/security/egress.py` exists, and it is defensible because of a **29-address corpus** rather than because of its predicate. Commits `d67bb34b` (corpus RED-first, exit 2 `ModuleNotFoundError`) · `36a2bb14` (the validator) · `388032b4` (the standing fence) · `fe07aa0a` (SUMMARY). **71 passed, 0 failed**; zero deps (`git diff --numstat backend/requirements.txt` empty); `backend/app/security/` is exactly three tracked files. **SIX falsifications driven in PRODUCTION source and each observed RED individually** — `ipv4_mapped` (3 failed) · `_V4_TRANSLATED`, **the hole that would have shipped** (3 failed) · `_NAT64` (4 failing tests) · `_SITE_LOCAL` (2 failed) · T1, the `refuse_reason` call deleted from the resolve step (3 failed, `169.254.169.254` stopped refusing) · T14, raw-string host matching (13 failed, **all four classic bypasses ACCEPTED** — userinfo, userinfo+port, query, fragment). Restored byte-identical. ⚠ **THE PLAN'S OWN PLANT-1 EXPECTATION WAS MEASURED FALSE AND CORRECTED:** CPython's `IPv6Address.is_global` **already unwraps IPv4-mapped**, so deleting that clause does NOT flip `::ffff:127.0.0.1` to allow — it flips the **audited address** (`::ffff:7f00:1`, which an operator cannot act on) and the **multicast** case. Written as planned, that falsification round would have proved nothing while reading as though it had. Fixed two ways: the corpus's 29th address is `::ffff:224.0.0.1` (the plan enumerated 28 and asserted 29 — the gap is closed with the one MEASURED flip, not filler), and a new test makes `ipv4_mapped`/`sixtofour`/`teredo` falsifiable via the reason string. §R14's 29/29 was independently reproduced before any of it was trusted. ⚠ **`md5sum` IS NOT A VALID IDENTITY MEASURE ACROSS A `git checkout` ON THIS PLATFORM** — `core.autocrlf` restores CRLF, so the raw hash changes while the content does not. Use the LF-normalised md5 (`74cad9c0007273a685493aa3ac4acf3c`, identical before and after) or `git hash-object`; `git diff --numstat` empty corroborates. Any future plan asserting md5 identity across a restore should say so. ⚠ **190-01's stated concern is DISCHARGED, not dodged:** `test_190_egress_ordering.py` did NOT merely turn green when the module landed — it now imports cleanly and fails on BEHAVIOUR (`DID NOT RAISE EgressRefused`, 3 failed), i.e. the shipped executor calls no guard at all. **190-13 still owes the other half**: resolver-before-guard observed producing a credential-shaped error, recorded, then reordered. That file was deliberately NOT edited here (D-32 / not in this plan's `files_modified`). **Also carried:** `redirected` is declared in `REFUSAL_REASONS` and unraised — 190-07 owns the raise site; `send_email` callers MUST pass `allowed_host=` or the guard fails closed on every SMTP send; `PinnedDestination(ip, hostname, port, scheme)` is the contract 190-07 consumes — connect to `ip`, verify `hostname`, **never re-resolve**. ⚠ `state.advance-plan` + `state.update-progress` + `state.record-metric` corrupted this file AGAIN (errored on stdout AND still wrote): deleted `stopped_at`, deleted all three 'Previous stopped_at / Previous session' narrative blocks, dropped `resume_file`, truncated `last_activity` to a bare date, set `status: planning` on an executing phase, and flipped `.planning/config.json` `_auto_chain_active` true→false. Reverted + hand-edited; only the ROADMAP checkbox and `completed_plans` were kept. **NEXT: `/gsd:execute-phase 190` continues at plan 03** (migrations 116 + 117 — carries a BLOCKING operator apply step).

### Previous stopped_at — Phase 190 PLANNED (19 plans in 8 waves), superseded by the block above

Phase 190 PLANNED — 19 plans in 8 waves, committed `9f005d53` (+ this session's pattern map and RESEARCH open-questions closeout). `gsd-plan-checker` returned **VERIFICATION PASSED**, 0 blockers, 1 non-blocking doc-hygiene warning (RESEARCH `## Open Questions` unmarked) which is now FIXED — all six questions carry an inline RESOLVED pointer + a resolution table naming the implementing plan. NEXT: `/gsd:execute-phase 190`. ⚠ **`state.planned-phase` corrupted this file exactly as `state.record-session` has before** — it deleted `stopped_at`, deleted the `resume_file` frontmatter key, and truncated a 5-paragraph `last_activity` to the string 'Phase 190 planning complete', while reporting `updated: [Status, Last Activity]`. Reverted + hand-edited. **The false-completion verb list is now SIX, not five.** **Gate results:** decision coverage `passed: true` but **`skipped: true` — reason `no trackable decisions`**, i.e. the SDK parser does not recognise this CONTEXT's `- **D-NN — ...**` bullet form, so the gate did NOT actually verify anything; coverage of all 32 D-IDs was instead confirmed by GREP, twice (planner self-sweep + plan-checker re-derivation) — treat the green gate as a NO-OP, not as evidence. Post-planning gap analysis reads `21 of 24 items not covered`, which is EXPECTED and not a defect: it sweeps the whole milestone, and 190's own two requirements **CONN-02 + CONN-03 are both ✓ Covered** (13 and 17 plans respectively). **Planner corrections carried into the plans (measured at HEAD, not inherited):** `notConnectedOf` is `phaseVocabulary.ts:810-813` ⇒ the edited line is **812**; `_exec_external_action` is `phase_types.py:1810-1885`; `live_connectors` joins **`_VISIBILITY_FEATURES`** (`admin.py:104`) ⇒ **two lines, ZERO migrations, no 118**; `secret_cipher` exports `encrypt_secret`/`decrypt_secret` (`encrypt_value`/`decrypt_value` DO NOT EXIST) and `sso_provider_service.py:82-92` is the copy source — **NOT `:93-94`, which are the fail-OPEN half**; migration head 115 ⇒ 116/117, `harness_audit_event_type_check` 23 ⇒ 24 literals. **Three net-new findings the plans act on:** `_exec_external_action`'s own docblock (`:1814-1829`) asserts *'this executor performs NO network I/O'* and *'NOTHING IN THIS FUNCTION MAY NAME A TRANSPORT'* — both become FALSE on the send commit and `190-13` Task 2 rewrites them **in that same commit**; `db/workflows.py`'s hand-maintained `(059 + 069 + 070 + 114)` provenance appears **TWICE** (`:1161`, `:1194`) and `190-03` appends `+ 117` to both; `sso_configs` (mig `104:153-163` + `:372-390`) is a closer RLS analog than the `skills` policy. **Two things owed before the phase can CLOSE (neither blocks execution):** the `/gsd:fast` sweep of BUG-260807-01 / BUG-260808-01 (WR-04, security-tagged) so `/gsd:secure-phase 190` inherits no findings it did not cause — 260807-01's CODE FIX ALREADY LANDED, the DRIVEN ROW is what is owed, do not re-fix it; and D-30's three throwaway destinations (mailbox, Jira project, Slack channel). `190-19` is a `checkpoint:decision` presenting close-with-owed-rows / hold / Slack-only, and names Slack `post_message` as the first row — the one that falsifies T13.

### Previous stopped_at — Phase 190 RESEARCH + VALIDATION + UI-SPEC complete, superseded by the block above

Phase 190 RESEARCH + VALIDATION + UI-SPEC complete (2026-08-08) — `190-RESEARCH.md` + `190-VALIDATION.md` committed `2fc6223b`, `190-UI-SPEC.md` committed `34829db8` (ui-checker APPROVED 6/6 after ONE revision round). **`/gsd:plan-phase 190` STOPPED at the UI-SPEC safety gate and planned nothing; re-run it now that the gate is satisfied.** ⚠ **RESEARCH MEASURED SIX INHERITED CONTEXT CLAIMS FALSE, four of which change the plan's shape before a task is written:** (1) **D-26 names the WRONG allowlist** — Phase 181's `visual_workflow_canvas` lives in `_VISIBILITY_FEATURES` (`admin.py:104`, JSONB, ZERO migration), not `_FLAG_HUMAN_NAMES`; a true `/admin/flags` entry needs an `app_settings` boolean column ⇒ **a third migration 118**. Recommended: use `_VISIBILITY_FEATURES` and stay at two migrations. (2) **D-23 is structurally wrong** — `ExternalActionSection` has **THREE** props, not six; the "six" are six SOURCE-PURITY FENCES that an inline picker turns RED, and `onChange` is key-bound so a `connection_id` write needs a 4th prop ⇒ `PhaseFormPanel.tsx` goes non-zero, **breaking D-23**. Resolution: a **net-new `ConnectionPicker.tsx` child**, which is why the UI-SPEC exists. (3) **`is_global` is NOT the SSRF property predicate** — 29 driven addresses found **four holes**: multicast, NAT64 `64:ff9b::/96`, IPv4-**translated** SIIT `::ffff:0:0:0/96` (distinct from IPv4-*mapped*; `ipv4_mapped` returns `None`), and `fec0::/10`. Corrected unwrap-then-test predicate in §R14, driven 29/29. (4) **D-29's caution is INVERTED — per-request routing WORKS**: there is no workflow-run POST route; `MessageCreate` carries `model`, `provider` AND `workflow_definition_id` on one request, so SC#10's 8 rows mutate **no global setting**. Cost: the per-thread anchor 409s (`workflow_kickoff.py:146-173`) ⇒ **8 rows need 8 DISTINCT THREADS**. Also measured: **D-16 is a ONE-LINE gate** (`ctx` already carries the flag at the call site — no signature change, no engine edit); **DNS pinning works for BOTH transports** so CONTEXT's SMTP escape hatch is not needed (the real residual is RESIDUAL-190-01: both recipes depend on non-public attributes, fenced); **`EmailMessage` ALREADY raises** on `
` so the defence is architectural, not a regex (add `.sendmail(` to the D-05 source fence); `smtplib` is BLOCKING ⇒ `run_in_threadpool` (D-v2.5-01); zero new deps, zero new env vars. ⚠ **TWO NET-NEW THREATS for D-28: T13** Slack returns **HTTP 200 with `ok:false`** — `raise_for_status()` reads `channel_not_found` as SUCCESS, which is D-31's *"reads Complete for a send that did not leave the app"* verbatim and is the likeliest defect this phase ships; **T14** host bypasses (`slack.com@evil.com`) — match `httpx.URL(u).host`, never the raw string, and require `.isascii()` (httpx IDNA-DECODES). ⚠ **Line numbers wrong three ways: `notConnectedOf` is `phaseVocabulary.ts:810-813`, the edited line is `812`** (CONTEXT's `:811-814` and `<canonical_refs>`'s `:788-814` are BOTH wrong); `_exec_external_action` is `1810-1886`, not `1667-1903`. ⚠ **Count-gate pins already drifted +13** (`ExternalActionSection.test.tsx` pins 25, runs 34); measured HEAD **2727 running / 2714 pinned / 0 failed / 48 files**, `tsc -p tsconfig.app.json` **33**. **UI-SPEC decisions needing operator ratification at plan-phase: U-02** (org-admin gate on Settings → Connections — a plain member creating a connection binds colleagues to a destination; Add button REMOVED not disabled, API-enforced) and **U-07a** (took door **(b)** on sketch 156's "two refusals" — the picker *will not offer* a failing connection, a client-only Gate 1; Gate 2 validates **org + `is_enabled` only**, NOT `last_check_verdict`, because check is admin-only while bind is org-wide, so a server gate would dead-end a member holding a stale verdict they cannot clear. Reversal cost recorded: door (a) must land the server gate AND restore the absolute verb in ONE commit). **Still owed before 190 executes:** `/gsd:fast` sweep of BUG-260807-01 / BUG-260808-01 (WR-04 prototype-pollution, security-tagged) so `/gsd:secure-phase 190` inherits no findings it did not cause; and D-30's blocking operator dependency — a throwaway mailbox, Jira project and Slack channel (planning does not need them, CLOSING the phase does; first row to run is Slack `post_message`, the one that falsifies T13). ⚠ `state.record-session` corrupted this file AGAIN (deleted `stopped_at`, truncated `last_activity`, dropped the frontmatter `resume_file` key, rewrote an unrelated *"Resume file:"* line ~2000 lines down inside a Phase-186 narrative) while reporting `updated: ["Resume File"]`. Reverted + hand-edited. **Always `git diff .planning/STATE.md` after that verb.**

### Previous session — Phase 190 discuss-phase (2026-08-08), retained as evidence, superseded by `stopped_at` above

Phase 190 context gathered (2026-08-08, unattended/autonomous) — `190-CONTEXT.md` + `190-DISCUSSION-LOG.md` committed `1ca202f3`. 32 decisions locked across transport, egress, credentials, the executor seam, migrations, the author surface and UAT. **TWO ROADMAP AMENDMENTS ARE PROPOSED IN THE OPEN AND NEED AN OPERATOR YES-OR-NO AT PLAN-PHASE.** **D-01 (material):** SC#1's *"(MCP-backed action nodes)"* cannot hold alongside CONN-03 and red line D-14 — a remote MCP server makes the outbound call **inside its own process**, so our unconditional egress guard would cover only the hop to it (SC#2 unprovable) and the credential doing the sending would live in its config rather than our org-scoped Fernet store (SC#4 satisfied for the wrong credential); running MCP servers locally is a **second runtime**, which D-14 forbids in as many words. So the slice ships as **three first-party adapters whose sockets we own** (SMTP · Jira REST · Slack `chat.postMessage`, the last on a **code-constant host**) behind an intentionally MCP-shaped `ConnectorAdapter` protocol. `docs/CONNECTOR-ARCHITECTURE.md`'s verdict is untouched in substance — MCP-first stays the direction, first-party-thin **is** this three-capability slice, breadth still defers to Open Platform; only the clause implying an MCP client on day one moves, and that doc already records **measured** that zero MCP code exists in `backend/app`. **D-20 (minor):** TWO migrations, not "a migration likely" — **116** `connector_connections` + **117** one literal on `harness_audit_event_type_check`, which is **measured at 23 literals** (`full-schema.sql:1124`; the column is `event_type`, NOT `kind`) and is what 189's D-09 deferred to 190 *"where it would describe a real consequence"*. Next free slot re-derived as **116**. ⚠ **TWO LATENT DEFECTS THAT THIS PHASE'S OWN COMMIT CREATES, both non-discretionary and both belonging in Wave 0, RED, before any adapter exists: (1) `D-189-DEF-04` — without a `ctx.is_golden_run` gate INSIDE the executor, PUBLISHING a workflow performs the external action, nobody asked, once per publish attempt; its trigger is already armed as a check (`test_a_golden_run_of_an_external_action_performs_no_egress` goes RED on the commit that adds a real send — drive it RED then green, never route around it). (2) the cross-org credential leak — a definition in org A carrying a `connection_id` owned by org B; the resolver must scope by the RUN's org, never by the id alone, and an id-only SELECT passes every ordinary test (SEED-124/mig 110 and SEED-125/mig 112 are the two precedents).** Also locked: the guard runs BEFORE credential resolution (the n8n CVE inversion, asserted directly with a no-credential drive); **zero new phase statuses and zero `workflow_phases` migration** — `recorded_not_sent` becomes the honest terminal for an UNBOUND step rather than being retired; at-most-once, no retry; static tokens, no OAuth; no 4th capability; **no new expression language** (CONN-03 SC#3 is a fence over the shipped `SandboxedEnvironment`, and the plan must SAY it proved rather than built); `PhaseFormPanel.tsx` diff must be **`0 0`** and the card subtree EMPTY — the `Not connected` badge retires by DATA, one line in `notConnectedOf`, which 189 deliberately wrote across two lines for exactly this. ⚠ **`get_cipher()` fails OPEN when unkeyed (D-150-01); 190 INVERTS that for tenant credentials and refuses to store** — a decision, not a bug. ⚠ **G-2 FIRES and is honoured, not skipped** — `/gsd:sketch 190` scoped to the Settings → Connections page only (~3 actions) is the recommended next command; overriding it is legitimate but must be recorded under Guardrail overrides. **Blocking operator dependency named NOW rather than at UAT: a throwaway mailbox, Jira project and Slack channel** — planning does not need them, closing the phase does. **BUG-260808-02 (approval hands off to chat) DEFERRED not folded** — 190 raises its stakes (the approval you leave the run surface to give becomes the one that actually SENDS), but it is a new capability the operator said not to fold, and it belongs with SEED-139 + SEED-140 as one workflow-surface phase; frontmatter now carries that trigger. **BUG-260807-01 + BUG-260808-01 (WR-04 prototype-pollution class) should be swept by `/gsd:fast` BEFORE 190 executes**, so `/gsd:secure-phase 190` does not inherit findings this phase did not cause. ⚠ **`state.record-session` corrupted this file again and was reverted** — it deleted `stopped_at`, truncated `last_activity`, dropped the frontmatter `resume_file` key entirely and instead rewrote an unrelated *"Resume file:"* line ~2000 lines down inside a Phase-186 narrative. Hand-edited. **Always `git diff .planning/STATE.md` after that verb.**

Still open from 189: `D-189-DEF-04` — **now OWNED and MANDATORY as decision D-16 in `190-CONTEXT.md`**. `D-189-DEF-01` CLOSED by `/gsd:fast` `1b4599fb`; `D-189-DEF-02` CLOSED at 189-13 (premise measured FALSE at HEAD); `D-189-DEF-03` CLOSED by CR-02. **`BUG-260807-01`'s CODE FIX ALREADY LANDED** (both `own()` sinks guarded, `editAffordance.ts:244-247` + `providerLogo.tsx:107`) — its `status: open` means the DRIVEN ROW is owed, not that code is missing; do not re-fix it. Three authoring defects found during the 189-16 UAT and NOT folded: **⑂ Tweak fails with a silent 409** (console only, no toast, no state change); **the LOOSE "Describe & run" door never persists `business_requirement`**, so nothing it creates can ever be published and the POWER door has no editor for that field; and the approve control reads **"Approve and run this step"** — flagged for Phase 190 specifically, because that is the phase in which the button WILL send.

### Previous session — Phase 189 close (2026-08-07/08), retained as evidence, superseded by the block above

Still open: `D-189-DEF-04` (Phase 190). **`D-189-DEF-01` CLOSED** by `/gsd:fast` `1b4599fb` — the fidelity count pin went `== 2` → `== 8` (RED `assert 8 == 2` → `6 passed`), with every step of `2 → 6 → 7 → 8` dated inside the test's own docstring; kept an exact `==` rather than relaxed to `>=`, since a floor cannot catch the silent SWAP the Phase-177 lesson was about. ⚠ **Its BLINDNESS is mitigated, not fixed, and that is stated rather than smoothed:** there was no scope command to edit (`backend/pytest.ini` is `testpaths = tests`, so the file is already in the DEFAULT collection — what excluded it was a hand-typed per-plan list in archived planning docs no future phase reads), so the durable mitigation is a ⚠ block in the test's docstring naming the three scopes that omit it. `D-189-DEF-02` CLOSED at 189-13 — its premise measured FALSE at HEAD. `D-189-DEF-03` CLOSED by CR-02. **`BUG-260807-01`'s CODE FIX ALREADY LANDED** (both `own()` sinks guarded, re-read at `editAffordance.ts:244-247` + `providerLogo.tsx:107`); its `status: open` means the DRIVEN ROW is owed, not that code is missing — do not re-fix it. ⚠ **THE OWED LIVE-UAT ROWS ARE NOW DRIVEN (2026-08-08) — U1…U7 ALL PASS, and the last owed row FAILED INTO A NAMED SUCCESSOR.** The publish blocker is discharged: **`weekly-status-report-wfzqel` v1 is PUBLISHED** with `retrieve · act · emit`, `act = external_action / send_email` at phase **2 of 3** (deliberately not last, the only arrangement in which the CR-02 mid-run sweep can fire). The step was placed **BY KEYBOARD** (`End` then `Enter`), a path that did not exist before BUG-260807-02 shipped. **CR-02 is now proven on the PAINT as well as the wire:** after `emit` both started and completed — firing `finalizeEarlierPhasesForThread` AND `finalizeAllPhasesForThread` — the spine still read `retrieve ✓ Complete ▸ act ↛ Not sent ▸ emit ✓ Complete`; `act` never flashed "Complete". **U3** is confirmed on a REAL rendered run — four arcs (`32.044 21.363` ×4), `dashoffset 16.022` exactly as 189-10 predicted from a synthetic DOM, muted stroke, `animationName: none` (still), byte-identical under `grayscale(1)`, and the only ring on the canvas. **U5** (the 188.2 debt) passes: running arc genuinely animates, a no-reading card is still. **D-25**'s rendered half passes. The live armed checkpoint **paused and refused to continue** — D-04/189-14, the correct counterpart to D-19. ⚠ **`BUG-260807-01`'s driven row FAILED, and that is a result:** its `own()` guard HOLDS (`transformHasNaN: false` everywhere), but a phase slugged `constructor` still renders wrong via a DIFFERENT sink — **no transform written at all**, card stacked on phase 1 at rect `58,176`, control swung both ways against `ordinaryslug` → `translate(320px,0px)`. Filed as **`BUG-260808-01`** (seventh WR-04 sink); 260807-01 stays open pending it. ⚠ **Three authoring defects found en route, none a 189 regression:** **⑂ Tweak fails with a silent 409** (console only, no toast, no state change); **the LOOSE "Describe & run" door never persists `business_requirement`**, so nothing it creates can EVER be published, and the POWER door has no editor for that field — 87/193 definitions carry it and every one traces to a STARTER template, which is why forking a starter was the only route that worked; and the approve control reads **"Approve and run this step"** on a node whose whole contract is that it never runs anything outward (wording, flagged for Phase 190 when that button WILL send). ✅ **One strong result: the AI seed placed `external_action` unprompted** from a plain-language "…then email that answer to the requester", with a seed receipt explaining its grounding choice — 189-09 holds on a real request.
resume_file: .planning/phases/190-live-connector-slice-connector-security-stretch/190-VERIFICATION.md
progress:
  total_phases: 21
  completed_phases: 11
  total_plans: 161
...
### Previous stopped_at - Phase 190 plan 05 EXECUTED (the `Not connected` badge retired by DATA), superseded by the block above

Phase 190 plan **05 of 19 EXECUTED** - CONN-02 / D-24 delivered by a one-line data edit. Commits `fc994b4a` (the bound/unbound pair + the zero-import fence, RED first), `41a9ead2` (the `:812` edit + the amended seam case, 117/117 green) and `24c0a639` (the count-gate pin 112 -> 117 + `D-190-DEF-05`). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-05-SUMMARY.md`. **Next: plan 06.** WARNING - carry forward: **`cd frontend && npm test` is NOT green (21 failed / 8 files) and was NOT green before this plan** - PROVED by resetting the three changed files to `de122b9a` and reproducing the identical 21/8, then restoring md5-identical; all eight sit OUTSIDE the count gate's TARGETS (SEED-056 chat/ingestion rot). **`/gsd:verify-work 190` must not read `npm test` as this phase's regression signal** - use `node scripts/vitest-count-gate.cjs` (48 files, failed 0) plus the per-suite run. Logged as `D-190-DEF-05`. The cloud-parity queue is still **104 -> 117** + `SECRETS_ENCRYPTION_KEY` (190-19 should RE-RUN `scripts/pending-cloud-migrations.sh`, never quote it), and D-30's three live-send rows remain BLOCKED on an operator-supplied mailbox / Jira project / Slack channel.
...
**Current focus:** Phase 190 — live connector slice + connector security — stretch
...
Phase: 190 (live connector slice + connector security — stretch) — **CODE COMPLETE + CODE-REVIEW FIXES APPLIED (2026-08-09). NEXT = `/gsd:secure-phase 190`, which must read `190-REVIEW-FIXES.md` first.**

### ⚑ Code-review fix round — 2026-08-09 (`190-REVIEW-FIXES.md`, commits `f64a0ebb` + `50d058e3`)

**All four Criticals CLOSED. Nothing skipped.** 8 of 14 findings fixed (two of them half-fixed
with the remainder named and triggered), 6 deferred with written triggers, 0 skipped.

| | |
|---|---|
| **CR-01** | ⚠ **WORSE than the review stated, and MEASURED before anything changed:** both `authenticated` **AND `anon`** held column-level SELECT on `connector_connections.secret_ciphertext` — an *unauthenticated* role holding the tenant credential column. **Migration 118** revokes the blanket grants and re-grants SELECT column by column, omitting exactly one. ⚠ **It FORCES a code change, driven against live PostgREST rather than reasoned about:** with the grant in place `SELECT *` is `42501` for `authenticated`, and PostgREST's DEFAULT projection is `select=*`. Every read AND every `return=representation` write on the user-JWT client now names its columns. |
| **CR-02** | the only kill switch in the tree reading the settings flag raw — `await ensure_settings_fresh()`, with the drive asserting the ORDER `["fresh","audience"]` because an outcome assertion reads identically on a warm cache. |
| **CR-03** | the switch failed OPEN for 3 of its 4 legal audiences. The test is now POSITIVE (`!= "everyone"` records) — fail-closed against a hand-edited row **and** against any fifth audience. |
| **CR-04** | blocking `getaddrinfo` on the event loop from three `async def` sites. The SMTP one is the sharp one: the CONNECT had been wrapped since 190-07 and the lookup that PRECEDES it was left on the loop. Driven by THREAD IDENTITY (`assert 96204 != 96204`). `validate_destination` stays a SYNC module attribute so the D-06 ordering fence still binds — only the THREAD moved. |
| **WR-02 · WR-03 · WR-05 (+IN-03)** | fixed / half-fixed. Remainders: `D-190-DEF-11` (clear the stranded `connection_id` in the UI), `D-190-DEF-12` (disable Save for an incomplete panel). |
| **WR-01** | ⚠ **DEFERRED (`D-190-DEF-10`) — but its FALSE ✅ is CORRECTED.** `190-VALIDATION.md`'s W0-3 row, its G-6 row 6 and its *"six of eight fully driven"* count (**now five of eight**) were corrected on measurement: `ExternalActionPhaseConfig` carries no `base_url`, so GATE 3's pre-credential guard is structural for `post_message` ONLY. No reachable bypass (both binders validate first), so the CODE is deferred and only the SCORE moved. |
| **WR-06** | ⚠ **stated plainly rather than softened: with the switch OFF an OPERATOR can still create / edit / delete connector connections via curl**, because `require_visible`'s operator no-op runs BEFORE the `"off"` test. Deferred (`D-190-DEF-14`) only because that dependency is shared by four other governed features; **bounded** because it cannot cause a SEND — GATE 2 is independent and now requires a positive `"everyone"`. |
| **WR-04 · IN-01 · IN-02 · IN-04** | deferred — `D-190-DEF-13`, `D-190-DEF-15`. |

⚠ **ONE FINDING THE REVIEW DID NOT MAKE, found while regenerating the artifact and closed in the
same commit:** `full-schema.sql` carries **NO ACLs AT ALL** (`pg_dump --no-privileges`;
`grep -c '^GRANT\|^REVOKE'` → **0**), so migration 118 would have been **invisible to every
greenfield bootstrap**. The grant is mirrored into `scripts/full-schema-supplement.sql`; the
general case — nothing fences the NEXT privilege-narrowing migration — is `D-190-DEF-16`.

⚠ **APPLY DEVIATION, recorded as a deviation and never as a gate met.** D-21 asks for the
Supabase SQL editor; this session is unattended, so 118 was applied to the LOCAL DB via
`psycopg2`, autocommit, **no reset**, dev data preserved — the fallback D-21 names in as many
words, with 189-06 and 190-03 as precedent. **`supabase db push` / `db reset` were NOT run; the
ban is untouched.** `full-schema.sql` was regenerated by the sanctioned script with no
`--reset` and never hand-edited.

⚠ **CLOUD PARITY GREW: `099 → 117` becomes `099 → 118`, and 118 is SECURITY-BEARING.** Until it
is applied, **cloud still has the CR-01 defect**. It must be applied in the **same operation**
as the `connector_service.py` deploy: the code without the grant is harmless, but the **grant
without the code breaks every connector read with a 42501**.

**Gates:** 314 passed across all sixteen 190/189 suites plus the three new files; full backend
**211 failed / 3617 passed** — failures **UNCHANGED** against the recorded baseline (211 / 3575),
+42 passes, and **zero failures name a 190 file**. Frontend **untouched** (no file under
`frontend/` is in either commit), so the eight D-23/D-24 fences, the 189 acronym fence, the
count gate and `tsc` at 33 are all trivially unmoved. **D-32 held: no new capability** — seven
defect repairs, one privilege-narrowing migration, one grant mirrored into an existing
bootstrap supplement.

---

**Pre-fix position, preserved:** **CODE COMPLETE, 19 of 19 plans; CLOSED WITH TWELVE UAT ROWS OWED (a stated DECISION).**
⚠ Re-derived on MEASUREMENT at 190-19, not incremented: `ls .planning/phases/190-*/*-SUMMARY.md | wc -l` → **19**, names contiguous `01`…`19` (listed, not assumed), and `grep -c "<task type=" 190-*-PLAN.md` sums to **54** across the 19 plans ⇒ **51** tasks in plans 01–18, every one of which carries an automated verify. The out-of-order gap 190-16 opened was closed at 190-15 and stayed closed.
⚠ **THE PHASE DOES NOT READ "COMPLETE", AND THAT IS THE POINT.** Every automatable property is proved — 14/14 falsification rows with verbatim REDs, 7/7 source fences, the eight D-23/D-24 fences empty phase-wide against BOTH candidate bases, four gates re-measured from the base commit (`211 failed` unchanged, `+203` passes; `tsc` 33 → 33; count gate 2727/2714/48 → 2851/2838/51; zero installs). **What is owed is every row that requires a real destination or a real artefact:** all 8 SC#10 provider rows, the 3 live-send rows, Row B's lived read, the kill-switch lived toggle, the two sketch-visual rows, the long-message axis, and the 375px rendered-geometry row. **`BLOCK-190-UAT-01` is MEASURED, not assumed** — 0 connector connections, 0 workflow definitions of any status containing an `external_action` phase, `live_connectors` absent from `feature_visibility`. **First row to run: Slack `post_message`** (falsifies T13, retires A3). **CONN-02 / CONN-03 are settled in REQUIREMENTS.md as NOT complete**, each with its reason and its closing condition written out rather than left silently unticked.
⚠ **A note the previous two Current Position blocks did not carry, recorded because it costs nothing now and a re-derivation later:** the plan COUNT (19) comes from `190-*-PLAN.md`, and the summary count is what says how many have RUN. They agree at 18 of 19 today; if they ever disagree, the PLAN files are the denominator and the SUMMARY files are the numerator — never the reverse.
(Waves 0-1 done: `190-01` the RED drives, `190-02` `security/egress.py`, `190-03` migrations 116+117,
`190-04` the D-01 amendment, `190-05` the `Not connected` badge retired by DATA at `phaseVocabulary.ts:812`
with all eight fenced files measured `0 0`. **Wave 2: `190-06` the credential layer — the D-14
cross-org leak REPRODUCED (org A holding org B's decrypted token) and then closed by two gates;
`190-07` the two DNS-pinned transport binders — the socket now goes to the VALIDATED IP, proved by
deleting `copy_with(host=ip)` and watching the suite name `slack.com`.**
⚠ `190-06`'s hand-off said *"next: plan 07 — the connectors router"*. **That prediction was wrong and
the PLAN FILE is authoritative:** `190-07-PLAN.md` is the transport binders. The router is not yet
claimed by a plan number — whoever needs it should check `190-09`..`190-19` rather than inherit that
sentence. **Wave 3: `190-08` the connectors package — `ConnectorAdapter` + the closed registry keyed
off `EXTERNAL_ACTION_CAPABILITIES` + the SMTP `send_email` adapter, with T12 driven by TWO plants;
the deeper one returned `ok=True` over a wire carrying `Bcc: attacker@evil.com`; **`190-09` the
router — `/connectors/connections` with org-WIDE reads, API-ENFORCED org-admin writes and one
indistinguishable 404 for every miss, plus the `live_connectors` kill-switch in the allowlist
CORRECTION #6 names, for two lines and ZERO migrations (count measured unchanged at 111).** The
router question the note above left open is now ANSWERED: it is `190-09`, and it is done.
⚠ **`190-09` escalated a REAL document conflict rather than smoothing it — D-190-DEF-07.** The plan
directs `require_visible("live_connectors")` onto the WRITE endpoints; UI-SPEC §2h's approved banner
says *"Connections below can be saved and bound to a workflow"*, which is FALSE for a non-operator
while the cold default `off` stands. The PLAN was followed (the more restrictive of the two errors)
and the contradiction is written into the router header, the SUMMARY and `deferred-items.md` with the
one-line fix named for BOTH branches. **190-10 / 190-11 cannot render the banner without resolving
it, and must move the copy and the gate in the SAME commit.** ⚠ **`190-10` did NOT resolve it and did
not claim to** — it opened no router file and no copy file, so D-190-DEF-07 is **still open** and moves
intact to whoever renders §2h. **Wave 4: `190-10` the Jira `create_ticket` adapter — REST v3 `POST
/rest/api/3/issue`, an ADF description BUILT rather than accepted, basic auth handed to the library,
and `ok=True` gated on three things (a 2xx, no error envelope, an issue KEY) rather than on the status
line alone. PLANT B removed the last two gates and the adapter logged `one issue created (key=UNKNOWN)`
for a reply carrying `errorMessages` and no key; PLANT A made the document builder accept a dictionary
and the author's rich document was filed as `OPS-17`.** **`190-11` the Slack `post_message` adapter —
and **T13 is CLOSED with a driven falsification, not an assertion**. The verdict is `status_code == 200`
AND `payload.get("ok") is True` — an IDENTITY check, because `{"ok": "false"}` is a non-empty string and
every truthiness test says yes to it. With the `ok` gate removed and the status gate left standing, the
adapter logged `post_message: one message posted (channel=C0123ABCDEF ts=None)` for a reply that said
`channel_not_found` — **the SAME line a real success writes, except Slack's own timestamp is `None`
because there is no message** (13 failed / 12 passed, restored byte-identically). The destination is a
CODE CONSTANT and `_endpoint(method)`'s SIGNATURE cannot see stored data, so config cannot move it; the
token rides an `Authorization` header and never the body; nothing retries on `rate_limited`; and the
same trap is closed on `check()`, where `auth.test`'s `200 {ok:false}` would otherwise paint a REVOKED
credential green.** ⚠ **There is deliberately NO third gate here** (no `ts` requirement): the sibling
needs its issue-key gate because Jira's status line says nothing about existence, but `ok: true` IS
Slack asserting the message posted, and demanding more than the vendor's own assertion would start
failing sends that actually succeeded. **All THREE capabilities now resolve to a real, importable
adapter** whose `CAPABILITY` equals its registry key.
⚠ This heading read *"plan 11 of 19"* while plans 12 and 13 had shipped — corrected on
measurement at 190-13, not inherited. **Wave 4 cont.: `190-12` the `ConnectionPicker` +
`SelectedPhaseSlugContext` write seam, with `PhaseFormPanel.tsx` measurably `0 0` and the six
purity fences RE-RUN rather than trusted. Wave 5: `190-13` THE SEND — `_exec_external_action`
behind six ordered gates (golden-run → kill-switch → EGRESS GUARD → unbound → org-scoped resolve
→ adapter), zero new statuses, the engine diff empty and zero installs.** All four standing REDs
are green: the three D-06 ordering drives and the sentinel's bound-connection case, which now
proves the send is genuinely live rather than accidentally still inert.
⚠ **190-13 found a BLOCKER it was not sent to fix, and closed it: A4 was REACHABLE.** The golden
run is anchored on `threads.active_workflow_run_id` like every other run, `find_resumable_runs`
filtered `is_golden_run` not at all, and `_build_resume_context` does not carry the flag — so a
publish killed by a restart mid-phase was re-driven on the next boot **as a live run, and would
have SENT**. Closed at the root (`AND wr.is_golden_run = false` plus a post-fetch re-check),
deliberately NOT by threading the flag: *a golden run is not a thing to resume*.
⚠ **The already-armed D-16 fence was VACUOUS and it was measured, not suspected** — it bound no
connection and `live_connectors` is off by cold default, so it could not have gone RED on the
very commit it was armed for. Both preconditions are now set AND asserted inside it, and only
because of that does the recorded RED (`httpx.AsyncClient.send was called - outbound egress
attempted`, on the publish path) exist at all. Next: `190-14`.)
**WAVES 0-4 DONE; THE BACKEND IS COMPLETE END TO END**; the `189-06` operator gate is DISCHARGED)

🧩 **THE NODE IS PLACEABLE.** `189-12` landed the client's 7th `phase_type`: the step-type picker
offers seven choices with the shipped six in their shipped positions (asserted as a PREFIX, because
a bare count passes an insert), and `slugForType` + `minimalPhaseFor` build a phase the backend
union accepts. `tsc` **33 → 37 → 33**; **`PHASE_TYPE_ORDER` raised nothing**, confirming the plan's
named risk — `satisfies` admits a subset, so the one edit nothing would have told us about is the
one that makes the type reachable at all.
**Next: `189-16`** — the last plan: docs/validation, and the ONLY home for the RENDERED checks (U1-U4, Chrome MCP). `189-13` / `189-14` / `189-15` have all landed; the paragraph below is kept as the 189-12 record.

⚠ **`189-15` SPENT BADGE SLOT 1 — there is no free word-badge left on the card.** Both slots are taken (slot 1 `Not connected`, slot 2 `Waits for you`) and a third is a TYPECHECK ERROR against `BadgeSlots`, control-tested and re-observed swinging 33 → 34 → 33. A later face that wants to say more must argue for a slot, not merely find one. CONN-01 stays **`Pending`** — one plan owed.

(historic) **`189-13`** (wave 5 — the authoring surface: the capability picker, the `outbox-tray` mark
and its `ICON_TINT` entry, and the arming switch rendering ON-and-refusing).

⚠ **`189-13` AND `189-14` EACH INHERIT A TRIPWIRE, BY DESIGN.** `189-12` ships two FALSIFIABLE
placeholders in `StepTypePicker.test.tsx`: `TYPES_AWAITING_A_3D_MARK` (189-13's `outbox-tray` slug)
and `TYPES_AWAITING_A_VOCABULARY_ENTRY` (189-14's `PHASE_TYPE_SENTENCES` / `_SUBTITLES` entries).
Each excluding case also ASSERTS the entry is genuinely missing, so the owing plan's arrival turns
it RED and the exclusion must be emptied rather than outlive its reason. Until then a placed node
reads **`external_action`** with the **`•`** fallback mark — the shipped TOTALITY floor firing,
never an invented sentence, and pinned as a decision rather than left as an oversight.
**189-13 also owns `PhaseNodeCard.test.tsx:800`'s `ICON_TINT` six-count** and **189-14 owns
`phaseVocabulary.corpus.test.ts:221`'s `RAW_PHASE_TYPE_TOKENS` six-count** — both were measured to
be theirs, not `189-12`'s, and were deliberately left unmoved.

⚠ **UI-SPEC §7b's *"nothing chosen yet"* state is no longer reachable FROM THE PICKER.** A placed
node arrives carrying `capability: "send_email"`, because `capability` is a closed `Literal` with no
empty placeholder and an unstorable step would falsify SC#1 in the same act as satisfying it. The
state survives only where 189-07 said it lives — a stored value the client does not recognise.
**189-13 owns finishing that choice** (accept a pre-selected first option, or find an "unset"
representation that survives `model_validate`; the D-15 fence forbids making the field optional).

⚠ **`189-RESEARCH.md`'s "sixteen six-count pins" is WRONG IN BOTH DIRECTIONS** — measured by
`189-12`: **5 sites it does not list** (four of them invisible to a `toHaveLength(6)` grep, because
they are SHAPE assertions) and **6 it lists that do not move**. Re-derive by RUNNING the suite, not
only by grepping; the same instruction that has now caught RESEARCH out in three plans running.

🏁 **V20 — THE PHASE'S HEADLINE GATE — IS GREEN (`189-11`).** It closed the D-05 seam: an approved
external action writes `recorded_not_sent` in one atomic UPDATE, **the run continues (proven by
the next phase running, not by the status)**, no completion receipt is written, and an
`external_action` workflow **PUBLISHES**. Both of V20's original REDs were re-observed *through
V20 itself* by reverting 189-04's union and 189-05's golden-run branch one at a time — so the
green measures those fixes rather than coinciding with them. Remaining:
`189-13` / `189-14` / `189-15` / `189-16` (4 plans), **all client-side plus the
docs/validation close**.

⚠ **`D-189-DEF-03` (NEW, `189-11`) — the live run surface has no honest wire signal for
`recorded_not_sent`.** The engine deliberately emits NO SSE for that terminal (`phase_completed`
would paint a "✓ Complete" card), so `finalizeAllPhasesForThread` sweeps the card to `done` at
`run_completed` and a live viewer sees **"Complete" until a reconcile fetch**. Latency-of-honesty,
not a persistent lie — but it is the one thing a UAT row can catch that no unit test will. Closing
it is **Phase 190's** (new SSE + client handler + sweep exclusion, one commit).

⚠ **`backend/tests/test_publish_gate.py` IS THE PHASE-136 SKILL GATE.** Three plans in a row
(189-02, 189-05, 189-11) were sent there by their own `files_modified`. The workflow publish
gauntlet's fixture, judge mock and every publish test live in
...
Plan 189-02 COMPLETE (`86290a97` · `fe7bd092` · `a5af250d`; summary `189-02-SUMMARY.md`) —
both publish-gauntlet conflicts captured RED at HEAD (**D-06 proved FALSE, twice**) plus the
D-20 leak guard, authored green and proved non-vacuous by an observed plant. Zero source files.
Plan 189-03 COMPLETE (`d280f1c2` · `7b0e3e10`; summary `189-03-SUMMARY.md`) — **SC#3 in full**:
`docs/CONNECTOR-ARCHITECTURE.md` (137 L) carries the MCP-first verdict, its five-source basis and
a dated 3-trigger re-open clause; `D-v3.6-01` points at it without restating it (0 `###` subheads,
0 table rows); `CLAUDE.md` +1 line. **CONN-01 stays `Pending` — only its decision half shipped.**
Plan 189-04 COMPLETE (`ac51449a` · `44092d43`; summary `189-04-SUMMARY.md`) — CONFLICT 2 resolved:
`EXTERNAL_ACTION_CAPABILITIES` is the ONE runtime home of the D-15 set and `GroundingBundle.tool_names`
is now a strict SUPERSET of `.tools` — that difference IS the governance boundary. V22 stayed green.
Plan 189-05 COMPLETE (`a38c3198` · `3383dda8`; summary `189-05-SUMMARY.md`) — CONFLICT 1 resolved:
an armed phase's checkpoint auto-continues on a golden run; the PAUSE is skipped, the STEP is not,
and no receipt is written for an approval that never happened.
Plan 189-06 COMPLETE (`0e900cc8` authoring · `8b8c393e` regeneration; summary `189-06-SUMMARY.md`) —
**the sixth `workflow_phases.status` literal is LIVE, and it is a SLUG.** The constraint reads
`pending, active, completed, failed, skipped, recorded_not_sent`; `full-schema.sql` was regenerated by
the script (**no `--reset`**, never hand-edited) into a **one-line diff** at :1932 — the `= ANY (ARRAY[…])`
form chosen in Task 1 over `IN (…)` is exactly why, so the plan's ~10-line review guard never fired.
**V13: 3 green-skips → 3 PASSES**, both controls driven — the slug admitted on INSERT *and* on an
`active` → `recorded_not_sent` UPDATE, while D-16's sentence `Not sent — recorded` is REJECTED with
SQLSTATE **23514** naming the constraint (**D-17 as an executable test**), plus all five shipped literals
re-asserted and a nonsense value still refused. `check-deploy-drift.sh` PASS/exit 0 (no-op as expected;
115 correctly absent from the seed-like list). The two commits are auditable as a pair via the
**directional invariant** — verified, not assumed: the migration was already in the tree at `8b8c393e`,
and no commit anywhere carries a regenerated artifact without it.
⚠ **DEVIATION, RECORDED RATHER THAN SMOOTHED:** the plan's `must_haves.truths` asserts the operator
pasted the SQL into the Supabase SQL editor. That is not what happened — the apply was operator-**authorised**
but executed by the orchestrator running the file verbatim in one transaction via `psycopg2` against
`:54322`. The ban the rule exists to enforce held completely and is **proved by data**: `workflow_phases`
439 rows unchanged, `threads` 711, `documents` 39 all survive ⇒ no `db push` / `db reset`. Noted because a
`truths` entry is believed thereafter and guarded by nothing. (`test_migration_115.py`'s docstring still
describes the SQL-editor protocol; accurate as the standing rule, not as a record of this apply.)
Plan 189-07 COMPLETE (`22552460` · `7d3891a4`; summary `189-07-SUMMARY.md`) — **the 7th `phase_type`.**
`ExternalActionPhaseConfig` appended as the SEVENTH union member (mechanism untouched, pre-189 rows
still validate); `capability` a closed `Literal` of three fenced EQUAL to the runtime frozenset;
`available_tools` DERIVED by total replacement (D-03); `action_risk_armed` COERCED to True on
`PhaseSpec`, so it is pinned on every write path (D-04). **All 8 of 189-01's REDs green, both controls
unmoved, 3 wrong fixes planted and each observed RED.** ⚠ **A shipped 185 fence forbade the D-04
mechanism outright** and was NARROWED to the property it proxied, not deleted. ⚠ **D-26's two named
docblock sites were actually FOUR.** Zero regressions tree-wide (`tests/unit` 73→65 failed / 1719→1733
passed — the identical delta to the 189 scope).
Plan 189-09 COMPLETE (`b09bb361` · `d42b7533`; summary `189-09-SUMMARY.md`) — **the 7th EXECUTOR, and
SC#4 is now GREEN.** `_exec_external_action` resolves the capability against the closed
`EXTERNAL_ACTION_CAPABILITIES` and RAISES on absence (copying `_exec_programmatic`'s wording), records
what it WOULD have done, and **provably makes no network call**: all three of 189-01's V10 REDs green
with both transport-inertness controls intact. Returns `text` plus a `recorded_intent` sentinel — the
101.1 `output["failure"]` mechanism one branch over; **the ENGINE branch is 189-11's.** D-03 is PROVEN
on the shipped guard by four DRIVEN cases and **no second guard was built** (`tool_dispatcher.py` and
`runs.py` diffs both EMPTY, `grep -c phase_whitelist` unmoved at 7); the NL generator now says seven
phase types with a length-bounded 7th bullet (104 chars vs the six shipped bullets' 83.5 median).
⚠ **The `mcp` source fence fired on my own docstring** — a denial of a transport is indistinguishable
from a use to a fence that reads the FILE, so the executor now names NO transport at all and records
why. **Three plants driven into production source, each observed RED — and under TWO of them (PLANT F,
a receipt-shaped body; PLANT H, the record silently skipped) the three V10 cases stayed GREEN.** That
...
Plan 189-11 COMPLETE (`7ce83c0d` · `61084eec`; summary `189-11-SUMMARY.md`) — **THE D-05 LOOP IS
CLOSED AND V20, THE HEADLINE GATE, IS GREEN: an `external_action` workflow PUBLISHES.**
`record_phase_not_sent` is the FIFTH phase-keyed write (ONE `pool.execute`, ONE `UPDATE`,
`complete_phase`'s shape, not `fail_phase`'s merge); the engine gains a **THIRD branch** beside
101.1's emit-failure sentinel consuming the **imported** `RECORDED_INTENT_KEY` — **+78 / −0**, no
shipped line changed. **"The run CONTINUES" is bought by PLACEMENT and proven BY THE NEXT PHASE
RUNNING.** The receipt question is answered explicitly — **no `phase_completed` row**; the ledger
rides the existing `phase_transition` kind, so **D-09 holds and `test_audit_event_registration.py`
passes UNCHANGED**. ⚠ **No SSE either, with the cost stated: `D-189-DEF-03`** — a live viewer sees
"Complete" until reconcile, because `finalizeAllPhasesForThread` sweeps a non-terminal card;
Phase 190 closes it. **⭐ BOTH ORIGINAL REDS WERE RE-OBSERVED THROUGH V20 ITSELF** by reverting
189-05 (→ `golden_run_error`, *"the ask channel was subscribed with timeout_seconds=None"*) and
189-04 (→ `grounding_fidelity`, the `unregistered_tool` finding) in production source — so V20
mocks NEITHER stage 2.6 NOR the golden run, only the leaf reads, the DB edges and the judge, and
still finishes in 0.88 s because the ask channel raises. Plants J/K/L/M each observed RED; ⚠ under
**L (a false completion receipt) BOTH V20 and the D-05 core test stayed GREEN** — the fourth time
this phase the headline proof was blind to a real defect. ⚠ **And a defect found by accident, not
by planting: V20's first run passed every publish assertion over an EMPTY phase spine.** Nine-file
scope 2 failed / 181 passed (+3); `tests/unit` 62 failed / **1745** passed (+1 = V20, zero
regressions); SC#4 fence 0; `frontend/` and `supabase/` diffs EMPTY.

Plan 189-12 COMPLETE (`dcfdfb38`; summary `189-12-SUMMARY.md`) — **THE 7TH `phase_type` IS
PLACEABLE (SC#1).** `PhaseTypeId` + `PHASE_TYPE_ORDER` + `SLUG_BASE` (`act`) + `requiredConfigFor`,
in ONE commit with the count debt, because Task 2's own action mandates it — the 7th type reds five
suites the instant it lands. `tsc` **33 → 37 → 33**, and the four forced errors are named
individually; **`PHASE_TYPE_ORDER` raised NOTHING**, which is the plan's named risk confirmed rather
than assumed. ⚠ **`requiredConfigFor` emits a REAL `capability`** — the one departure from the
switch's empty-string register, because a closed `Literal` has no empty member and a `""` would 422
the whole definition on the first save. **D-23 is now MECHANICAL**: the suite reads `harness.py`
through the `?raw` loader (`PublishGauntlet.test.tsx:46`'s shipped cross-language idiom) and asserts
the client mirror equals the backend union IN ORDER, with positive controls on all three extractors;
the docblock pointer `harness.py:157-167` → `:288-299` was stale TWICE over and is corrected.
**Plants V / W / X each observed RED** — and only the D-23 fence can see V (a client member the
server lacks). ⚠ **RESEARCH's pin list is wrong BOTH WAYS: 5 sites missing, 6 that do not move**;
four of the five are SHAPE assertions a `toHaveLength(6)` grep cannot see, found by RUNNING the
suite. 19 sites moved across 5 suites, **12 counts, all DERIVED from `PHASE_TYPE_ORDER.length`**;
the subtree-path / contract-export / icon-tint six-counts UNMOVED with a zero-line diff. Count gate
**2546 → 2560**, both pins in-commit, no decrease. Two FALSIFIABLE placeholders ship, one tripwire
each for 189-13 and 189-14. `toolOptions` NOT widened; V22 green; backend + `supabase/` diffs EMPTY.
...
re-pointed and **green against a completely unchanged production tree** (0 production files
touched). Gates at close: vitest `112 passed (112)` (107 → 112), `tsc -p tsconfig.app.json` **33**
with 0 in `src/components/workflows/`, eslint **5 errors / 0 warnings**, `count gate OK` (+5, an
increase over a pin is legal — the pin move is Plan 07's).

- **D-09's third-badge typecheck invariant is asserted for the first time in this repo** and was
...
`WorkflowCanvas.test.tsx` + `WorkflowCanvas.composition.test.tsx` now read a `canvasSubtreeSource`
whose `import.meta.glob` path list already names `./PlaneEditingLayer.tsx` and `./editAffordance.ts`
— the two modules plan 188.1-03 creates — so the 311-line extraction cannot narrow a fence while
leaving it green. Positives and the per-file `phase_index` count pin deliberately stay on
`workflowCanvasSource` (T-188.1-04b, accepted). SC#4's no-focusable-control walk was **vacuous**
w.r.t. the moved code — both shipped assertions render read-only, so `PlaneEditingLayer` never
mounted — and now has an editable variant proven by inversion (`expected 6 to be +0`). Measured:
suites **68 passed / 0 failed**, gate `count gate OK` at **2465/2465, zero slack** (pins 46→48 and
19→20, twice-measured, `TARGETS` untouched), `tsc -p tsconfig.app.json` **33**, zero production
files changed. **Deviation worth carrying:** the plan's non-vacuity control was blind to a
SUBSTITUTED path literal (`toHaveLength(3)` catches truncation only) — strengthened under Rule 2 and
observed RED. **⚠ Orchestrator note:** `state.record-metric` also bumped `completed_plans` and
...
|---|-------------|------|--------|--------|-----------|
| 260808-w7r | **BUG-260808-01 — guard the NODE POSITION lookup, the SEVENTH WR-04 sink.** Swept ahead of Phase 190 so `/gsd:secure-phase 190` does not inherit findings it did not cause. ⚠ **The report's own hypothesis paragraph was half wrong and it said so** — it pointed at `canvasModel.ts`, which keys everything through `Map` and was never exposed. The sink is the two-memo split in `WorkflowCanvas.tsx`, at **three** bare-index sites: `:674` `dragOverlay[node.id]` (**the measured one** — resolves the inherited `Object.prototype.constructor`, a function, so `!== undefined` passes; the node's `position` BECOMES that function with no `.x`/`.y`, and the library then writes **no transform at all**, painting the card at the origin stacked on phase 1), plus `:639` and `:866` `nudges?.[node.id] ?? 0` (the function is not nullish, so `??` passes it through and it is ADDED to `position.y`). `:674` fires on **every render** — `dragOverlay` is `useState({})`, so nothing need be dragged for the sink to be live. All three now route through `own()` from the zero-import `ownProperty.ts` leaf. **`SEED-143` planted** for the class fix (constrain the slug at the boundary, or stop keying plain objects by it) — the report's own routing said to plant it if the boundary fix was not taken, and it was not: `harness.py` + a CHECK constraint is schema + API surface, outside `/gsd:fast` under G-3. | 2026-08-08 | `f02b5f41` | **Verified — both rows observed RED against the pre-fix body (source stashed) BEFORE the pin moved**, and the RED reproduced the report's own measurement exactly: `slug "constructor" got position function Object() { [native code] }`. ⚠ **The report claimed jsdom cannot see this; that is TRUE of the computed style and FALSE of the cause** — the `nodes` array is a captured prop, so the position handed to the library is directly assertable, and the guard sits there. Both rows carry positive controls (an ordinary slug still lands on the lane-1 pitch; an OWN nudge is still applied), so a rewrite that positions nothing cannot pass. Count-gate pin **63 → 65 EXTENDED**, nothing deleted, no other pin moved. `WorkflowCanvas.test.tsx:968`'s positive control **re-spelled, not weakened** — it anchored on the literal `dragOverlay[node.id]` and now anchors on `own(dragOverlay, node.id)`. Both canvas suites **118/118**; **tsc 33 → 33** unmoved, 0 in touched files. ⚠ **The full frontend suite has 22 pre-existing failures** (SEED-056 rot: `model-info`, `IngestionPage`, `MessageItem`, `Plan04`, `useMessages`, `StreamsProvider`, …) — verified NONE references `WorkflowCanvas`, so none is attributable here; the count gate therefore still exits non-zero on `[failing-tests]` at HEAD, as it did before this commit. ⚠ **BOTH reports deliberately stay `open`** — their shared regression check is a DRIVEN browser row (author a phase slugged `constructor`, read the affordance's computed `transform` AND the node's `style.transform`, control swung both ways), and relaxing that on unit evidence is precisely the failure mode `BUG-260807-01` was held open to avoid. **One row now closes both.** Needs a seeded `workflow_definitions` fixture — the UI cannot author this slug (`D-184-11`). | — (inline `/gsd:fast`, no directory) |
| 260808-148 | **BUG-260807-02 — the KEYBOARD half. The report is now `closed`** (`verified_closed_by: 260808-148`). The picker was `role="menu"` with `role="menuitem"` children and **no keyboard contract at all**: menuitems carried no `tabindex`, `ArrowDown` left focus on the `＋`, and a real `Tab` jumped *past* the open menu to the next door — so the surface was inoperable by keyboard or screen reader. Implemented the APG vertical-menu contract: focus enters on open, roving `tabIndex`, `ArrowDown`/`ArrowUp` with wrap, `Home`/`End`, focus **returned to the opening `＋`** on dismiss. ⚠ **`disabled` → `aria-disabled` on refused rows, a deliberate behaviour change:** a natively-disabled button cannot be focused, so the component's own docblock intent — *"an option greyed out mutely is worse … the author is left to guess what they did wrong"* — was being delivered to sighted mouse users ONLY. Refused rows are now focusable and announced, with activation guarded in the handler; that guard is load-bearing where the browser used to swallow the click. | 2026-08-08 | `5562d669` | **Verified — DRIVEN with REAL `press_key`, and re-driven independently by the orchestrator.** Focus enters on open (`step-type-choice-programmatic`), roving `0,-1,-1,-1,-1,-1,-1`; `End` lands on **`step-type-choice-external_action`** — reachable by `elementFromPoint` — with the panel scrolled to **168.8** while **all four ancestors stayed `0,0`** and the viewport transform `matrix(1.91765,…)` was byte-identical; `ArrowDown` from row 7 **wraps to row 1**; `Escape` closes and returns focus to the `＋`. Gates re-measured: **tsc 33 → 33**, count gate **exit 0 on three consecutive runs**, 48/48 pinned, 0 decreases, eslint 5 → 5. ⚠ **Driving found a defect fifteen green jsdom cases could not** — StrictMode double-invokes the open effect, so the "opener" captured second was the picker's OWN row and `Escape` stranded focus on `document.body`. The first StrictMode test written for it **stayed GREEN against the pre-fix code**, because React double-invokes on MOUNT, not on a dependency change: *a StrictMode wrapper alone proves nothing.* ⚠ **PLANT B (`scrollIntoView`) stayed GREEN in every behavioural case** — only a source-text fence caught it. jsdom cannot see the ancestor-scroll cheat at all. ⚠ **The plan named one file for the `toBeDisabled()` rewrites; there were two** (the second in `WorkflowCanvas.editing.test.tsx:872`) — both rewritten in place, never deleted. ⚠ **Orchestrator's own probe error, recorded because it is the same class that produced a false 7/7 the day before:** a first Escape check reported focus stranded on `body`, which was an artifact of opening the menu with a synthetic `.click()` (which does not focus the button); re-driven with a real CDP click, focus returns to the `＋` correctly. ⚠ **One gate run in seven exited 1 at HEAD and its output was discarded, so the failing case cannot be named** — a real evidence gap; the other six printed identical columns at `failed 0`, and the orchestrator's own three runs all exited 0. Matches the variance D-188.2-DEF-01 already records; the COUNT columns never varied. | [260808-148-steptypepicker-keyboard-navigation-rovin](./quick/260808-148-steptypepicker-keyboard-navigation-rovin/) |
| 260807-x9p | **BUG-260807-02 — the CLIPPING half.** The `StepTypePicker` renders inside react-flow's `<ViewportPortal>`, i.e. inside `.react-flow`, which has `overflow-y: hidden`; rows past the container's bottom edge were clipped away with **no scroll path of any kind**, and row 7 is `external_action` — the type Phase 189 exists to ship. Found by DRIVING at 189-16 Task 3, unreachable from **all three** insertion doors. Fixed by `pickerPlacement()`, a pure helper in `editAffordance.ts` that derives side (below/above), flow y, an **x clamp** and the height budget from the live store `transform` + `width`/`height` — so "re-measure on resize and zoom" is reactive and free via the library's own ResizeObserver, and nothing is measured off the panel being adjusted (no feedback loop). `StepTypePicker` stays a leaf and receives a number. | 2026-08-08 | `bc0256de` | **Verified — DRIVEN, and re-driven independently by the orchestrator.** 1536×674, live draft, `.react-flow` `176..597`, canvas zoom **1.918** (a real zoom ≠ 1): **7/7 rows reachable on all 3 doors**, panel contained (`372..590`), `max-height` **217.382px**, scrollable. **Control observed swinging BOTH ways: 7/7 → 3/7 (bound stripped) → 7/7 (restored).** Wheel over the panel leaves the viewport transform byte-identical while `scrollTop` moves; wheel over the pane still zooms. Gates re-measured, not quoted: **tsc 33 → 33**, count gate **exit 0**, 48/48 pinned, **+13** tests, 0 decreases, eslint 5 → 5. ⚠ **Two corrections the executor made to its own plan, both load-bearing:** (1) `onWheel` `stopPropagation` **cannot** work — React delegates synthetic events to the root, so it fires *after* the native wheel already reached `.react-flow`'s d3-zoom listener; the real mechanism is xyflow's **`nowheel`** class (`@xyflow/system` `createZoomOnScrollHandler` short-circuits on `event.target.closest('.nowheel')`). (2) The prescribed probe **could cheat and did** — `row.scrollIntoView()` scrolls the `overflow:hidden` `.react-flow` ancestor, which is programmatically scrollable but which **no user gesture can move**; with it the control read a false 7/7. The probe now scrolls the panel alone and fences four ancestors at `0,0`. ⚠ **Door 1's unexplained all-7 failure in the earlier reverted attempt was HORIZONTAL, not vertical** — its wrapper carries `translate(238.765px,0)` and without the clamp the panel's left edge sat at **−172.8**, 231px outside the container. ⚠ **`BUG-260807-02` stays `open`** — only the clipping half is closed; the **keyboard** defect (focus never enters the menu; menuitems carry no `tabindex`) was explicitly out of scope and is untouched. | [260807-x9p-bound-steptypepicker-height-to-measured-](./quick/260807-x9p-bound-steptypepicker-height-to-measured-/) |
| 260807-w4k | **BUG-260807-01** — guard the SIXTH and SEVENTH WR-04 prototype-pollution sinks. `verticalOffsetFor` (`editAffordance.ts:214-224`) indexed two plain object literals with an author-supplied phase slug and no own-guard, so a slug naming an `Object.prototype` member resolved the INHERITED member — never nullish, so the `!== undefined` check PASSED — and returned **NaN**. That number is interpolated into `translate(...)`; a NaN term invalidates the whole declaration, so the browser drops the ENTIRE transform. **188.2 amplified it**: Plan 02 gave those exact three style objects `zIndex: AFFORDANCE_Z` (1002) to close BUG-260806-01, so a mispositioned affordance now lands **above** every card instead of behind it (that fix was correct and stays). Sink 7 is `lib/providerLogo.tsx:107-108`, whose `?? null` comment claimed a totality it did not have. Fixed asymmetrically ON PURPOSE, matching the house split: `editAffordance.ts` imports `own()` from the zero-import leaf `ownProperty.ts`, while `providerLogo.tsx` — a `lib/` module, which does not import from `components/` — carries an INLINE guard in the tree's one spelling, matching `phaseState.ts:77` / `phaseGlyph.tsx:92`. | 2026-08-07 | `267f6347` | Verified — **the LEAF fence was RE-READ, not assumed** (D-14's named constraint): `WorkflowCanvas.test.tsx:686-703` bans `/@\/components\/workflows\/[A-Z]/` and `ownProperty` is camelCase, so the new edge is permitted by construction, not by luck; that suite is green 52/52. Both guard rows **observed RED against the pre-fix body BEFORE the pin moved**, reproducing the report's own measurement exactly (`AssertionError: expected NaN to be +0`). `WorkflowCanvas.editing` 63/63, `providerLogo` consumers 17/17, tsc **33** unmoved with 0 in touched files, eslint workflows **5** unmoved / providerLogo 0, count gate pinned == actual on every line (total **2508**, pin 61→63 EXTENDED, no `[count-decrease]`). ⚠ **The gate's `failed` column is red on an UNMODIFIED tree** — measured **9** at clean HEAD and varying **9 / 19 / 14**, which is WIDER than D-188.2-DEF-01's recorded 0–1; all nine were NAMED and attributed (axe collisions in `WorkflowCanvas.test.tsx`, plus `PhaseNode`/`StepTypePicker`/`composition`/`WorkflowBuilderPage.canvas`), none is in a file this commit touches, and each passes in isolation. Two stale gate figures corrected on measurement: the `BASELINE_TOTAL` marker read `2502` while the gate printed `pinned total 2506` on an unmodified tree. ⚠ **BUG-260807-01 deliberately stays `open`** — its recorded closing condition requires a DRIVEN browser row and states a green unit suite may not close it; the cause is guarded, the rendered half is owed (jsdom applies no CSS and computes no stacking contexts). Drive it alongside 188.2's owed UAT row A2 on Phase 189's first live run. | — (inline `/gsd:fast`, no directory) |
...
|-------|------|----------|-------|
| Phase 190 P15 | ~55 min | 3 tasks | 11 files (1 created / 10 modified; 1503 ins / 29 del; 8 plants driven RED, each on the case it was aimed at) |
| Phase 190 P10 | ~58 min | 2 tasks | 2 files created (1350 ins / 0 del; 2 plants driven RED, both checked for REACH) |
| Phase 190 P09 | ~96 min | 3 tasks | 8 files (1074 ins / 1 del; 9 plants driven RED) |
| Phase 190 P05 | ~55 min | 3 tasks | 4 files (1 production LINE) |
| Phase 189 P09 | ~70 min | 2 tasks | 5 files |
...
| Phase 190 P01 | 42min | 3 tasks | 3 files |

## Decisions

- [Phase 190]: 190-15: **A DOCUMENT was corrected, not the code — UI-SPEC §5c improvised the one sentence it forbade improvising.** §5c, plan 190-15's own `<interfaces>` block and plan 190-18 all quote the closed negation table's `send_email` row as *"No mail was delivered to anyone."*, in the sentence saying it is *"picked from the same closed table 189 §9d already ships, never improvised."* The shipped table says **`No email was sent.`** — four independent readings agree (`phase_types._EXTERNAL_ACTION_NEGATION`, `189-UI-SPEC.md:761`, `189-UAT.md:104` read straight out of `workflow_phases.output`, `189-SECURITY.md:92`). The CODE was not bent to the document: that string is 189's shipped, UAT-evidenced, security-reviewed output, and editing it from inside a 190 plan that claims to be REUSING it would invalidate 189's own recorded evidence. **General rule this establishes:** when a plan quotes a shipped value, ASSERT the quotation against production source before building on it — a value re-typed across three documents is still one typo, and only the source can tell you which way it drifted.
- [Phase 190]: 190-15: **Door (b)'s permissiveness is asserted as a POSITIVE test, because a deliberate non-gate is invisible to every other kind of check.** Gate 2 validates a bound connection's org and its `is_enabled` flag and reads the stored check verdict nowhere (`grep -c` on the executor → **0**). Two halves: the phase config naming a FAILING connection must VALIDATE, and the column name must appear nowhere on any bind or run path in the whole `backend/app` tree (three allowed readers: the writer, the response model, the router's prose). Reversing to door (a) is a **four-edit** change — the gate, §5b's absolute verb, the deliberate deletion of two named tests, and either giving members the check or recording the "ask an admin" dead end — and **no subset of it is legitimate**.
- [Phase 190]: 190-15: **A standing fence was made STRONGER rather than loosened when correct new code tripped it.** 190-14's tripwire pinned `connectors.registry`'s importers to an exact one-element list; the check must dispatch through the registry. The fence was right and the import SHAPE was wrong — a module-scope import can open the cycle, a function-body import runs after every module is loaded and cannot. The fence now CLASSIFIES (module-scope pinned by file+line; function-local pinned by file) instead of counting, its classifier is falsified on synthetic input inside the test before being trusted on the tree, and it was driven RED against a real module-scope plant.
- [Phase 190]: 190-09: **CORRECTION #6 re-derived before editing, and it holds — `live_connectors` joined `_VISIBILITY_FEATURES`, not `_FLAG_HUMAN_NAMES`.** `grep -n "visual_workflow_canvas" backend/app/api/admin.py` -> **104**, inside `_VISIBILITY_FEATURES` (`:97-105`); `_FLAG_HUMAN_NAMES` ends at `:79`. The flags path writes an `app_settings` BOOLEAN COLUMN (= migration 118); the visibility path writes the mig-098 JSONB via `set_feature_visibility`'s atomic `||` merge (= zero). Measured after the edit: `ls supabase/migrations/ | wc -l` -> **111**, exactly 190-03's 109 + 116 + 117, and `git status supabase/migrations/` prints nothing. A test now asserts membership in one set AND **absence** from the other, so a later move fails loudly instead of reading `False` off a column nobody wrote.
- [Phase 190]: 190-09: **A document conflict was ESCALATED rather than resolved by silence (D-190-DEF-07).** The plan directs the kill-switch gate onto the write endpoints; UI-SPEC §2h's operator-approved banner promises connections *"can be saved"* while the switch is off. Both govern the phase and they disagree. The PLAN was followed — it is the executor's contract, it was authored with the UI-SPEC in its own context, and of the two possible errors it is the MORE RESTRICTIVE one on a live-credential surface. The contradiction is written into `api/connectors.py`'s header, `190-09-SUMMARY.md` and `deferred-items.md`, each naming the one-line fix for BOTH branches. **General rule this establishes:** when two governing documents disagree, follow the contract, then write the disagreement in three places — never pick silently, and never edit an approved spec from inside an executor.
- [Phase 190]: 190-09: **A raw failure count is not a regression baseline.** `pytest tests/unit` read **66** failed here against **68** at the parent commit — *fewer*, which looks like an improvement and is not: the two that "improved" need `SECRETS_ENCRYPTION_KEY` from `backend/.env`, which the throwaway worktree does not carry. The claim was re-derived as a **set-diff of the FAILED lists** (`comm -13 base now` -> empty). Use the set-diff, never the count.
- [Phase 190]: 190-09: **A permission-gate test must forbid the SERVICE, not compare a status.** Case 2 monkeypatches every write verb in `connector_service` to raise, so dropping `Depends(require_org_manage)` produced *"a connector WRITE reached connector_service for a caller with no org:manage"* rather than a number that changed. A gate that refuses AFTER the row is written is not a gate, and only this shape can tell the difference.
