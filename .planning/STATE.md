...
status: executing
last_updated: 2026-08-09T00:45:00.000Z
last_activity: 2026-08-09 -- **Phase 190 plan 09 EXECUTED - the connector CRUD router, the `live_connectors` kill-switch and the API client.** `/connectors/connections` is live: org-WIDE reads (U-02 read+bind), org-admin writes enforced by the SHIPPED `require_org_manage` (mig 104's `current_user_has_permission` run AS THE CALLER), one indistinguishable 404 for every miss (`grep -cE "403|HTTP_403"` -> 0), and `ConnectorCipherUnavailable` -> **503 + the stable code `no_encryption_key`** (UI-SPEC 4b moment 9). **CORRECTION #6 RE-DERIVED AND UPHELD:** `visual_workflow_canvas` is at `admin.py:104` inside `_VISIBILITY_FEATURES` (:97-105), NOT `_FLAG_HUMAN_NAMES` (:67-79) - so `live_connectors` joined the ZERO-migration allowlist and `ls supabase/migrations/ | wc -l` is measured UNCHANGED at **111**. **Ten test cases, NINE driven RED against real plants in production source.** The one to read is case 2: dropping `Depends(require_org_manage)` did not merely change a status - it let a plain member's create REACH `connector_service`, because the test forbids the service rather than comparing a number. WARNING - **a REAL document conflict is escalated, not smoothed (D-190-DEF-07):** the plan directs the kill-switch gate onto the WRITE endpoints, but UI-SPEC 2h's operator-approved banner says *"Connections below can be saved and bound to a workflow"* - which is FALSE for a non-operator while the cold default `off` stands. The PLAN was followed (the more restrictive of the two errors) and the contradiction is written into the router header, the SUMMARY and deferred-items with the ONE-LINE fix named for BOTH branches; 190-10/190-11 must resolve it and move the copy and the gate in the SAME commit. Zero migrations, zero installs, `tsc` unmoved at 33, count gate OK (48/48, failed 0).
last_activity_prev: 2026-08-08 -- **Phase 190 plan 08 EXECUTED - the connectors package: the adapter seam, the closed registry, and the SMTP adapter.** `app/services/connectors/` now exists with `protocol.py` (the two-method `ConnectorAdapter` + frozen `AdapterResult` / `AdapterCheckResult` + the NAMED `AdapterError` base), `registry.py` (lazy import, key set asserted `== EXTERNAL_ACTION_CAPABILITIES` at module scope - D-04's SIXTH consumer) and `smtp_adapter.py`. **THE HEADLINE IS PLANT B, and it is stronger than any assertion:** with the headers built by string concatenation instead of `EmailMessage`, `send()` returned `AdapterResult(ok=True, ...)` while the bytes on the wire read `Subject: Renewal` followed by a CRLF and then `Bcc: attacker@evil.com` - an RFC-5322 parser reads that as a real blind-copy recipient, and the phase would have shown **Complete**. 8 failed / 5 passed under it; restored byte-identically (`0c7ac735...`, `grep -c PLANT` -> 0). WARNING - **four findings worth carrying -** (1) **the plan's OWN plant is insufficient**: swapping only the raw-string send API leaves the three CR/LF cases GREEN, because composition refuses before the transport is reached - it falsifies the source fence, not T12. Run the deeper plant too. (2) **My first draft of `protocol.py` turned the 189 acronym fence RED** (19 hits) - that fence sweeps every line under `backend/app` including docstrings, and its green is part of D-01's OWN evidence. Resolved the 190-07 way: rewrite the prose, never the fence; the expansion is written out instead. (3) **`parseaddr('v@ex ample.com')` silently REWRITES the input into a different valid address**, so the implemented rule is *the parser must AGREE with the literal input*, not *the parser must return something*. (4) **A plant harness's restore belongs in a `finally`** - mine aborted on a subprocess error and left production source planted, with no `git checkout` fallback because the file is not yet tracked.
stopped_at: "Phase 190 plan **09 of 19 EXECUTED** - the connector router + the kill-switch + the client. Commits `6c91ebe6` (the router: 292 L, `grep -c \"connector_service.\"` -> 11, ZERO SQL, `grep -cE \"403|HTTP_403\"` -> 0, plus `delete_connection` which 190-06 named and did not author), `727d87b7` (the two-line kill-switch + the `include_router` registration) and `42932238` (five client functions + three TS types in `lib/api.ts`, house bare-fetch shape, NO generic wrapper). Summary: `.planning/phases/190-live-connector-slice-connector-security-stretch/190-09-SUMMARY.md`. **12 passed / 0 failed** in the new suite; **19 passed** across the 190 credential+router group. **Next: plan 10.** WARNING - carry forward: (a) **D-190-DEF-07 BLOCKS 190-10/190-11**: the write endpoints carry `require_visible(\"live_connectors\")` per the plan, so with the cold default `off` a non-operator org admin CANNOT create a connection - and UI-SPEC 2h's banner promises they can. Decide which half moves, and move BOTH in one commit. (b) **The allowlist is settled and fenced** - a test asserts membership in `_VISIBILITY_FEATURES` AND absence from `_FLAG_HUMAN_NAMES`; moving it costs migration 118 and buys nothing. (c) **The kill-switch is off by default in EVERY environment, cloud included** - turning it on is an operator `PUT /admin/visibility` (`feature=live_connectors`, `audience=everyone`), NOT a migration; the parity queue is unchanged at `099 -> 117` + `SECRETS_ENCRYPTION_KEY`. (d) **190-15 owes `checkConnectorConnection`** in `api.ts` in the SAME commit as its endpoint - deliberately absent here, and `api.ts`'s section header says so. (e) **A raw failure count is NOT a regression baseline** - `pytest tests/unit` reads 66 here vs 68 at the parent commit, which looks like an improvement and is not; the claim was re-derived as a set-diff of the FAILED lists -> **zero new failures**. (f) **`npm test` rot GREW to 26 failures / 9 files** (was 21 / 8): the extra file is `ChatHistoryColumn.test.tsx`, attributed by rolling `api.ts` back to its `HEAD~1` bytes and reproducing 4 failed / 16 passed BOTH ways - logged as D-190-DEF-06. `/gsd:verify-work 190` must use `node scripts/vitest-count-gate.cjs`, never `npm test`."
...
Phase: 190 (live connector slice + connector security — stretch) — **EXECUTING, plan 9 of 19**
...
it, and must move the copy and the gate in the SAME commit.** Next: `190-10`.)
