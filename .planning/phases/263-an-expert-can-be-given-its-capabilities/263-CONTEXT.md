...
⚠ **CORRECTION, same session, recorded beside the original rather than replacing it — the
paragraph above cites `087` and the LIVE text is `089`'s.** Measured: `088_skill_creator_eval_step_sequencing.sql`
and `089_skill_creator_file_attach_honesty.sql` **each perform a full `SET instructions = $INSTRUCTIONS$…# Phase 263: An Expert Can Be Given Its Capabilities - Context
...
rewrite** of row `…0010`. The interview-loop finding is UNCHANGED and was reached independently twice —
what is corrected is **which migration to read**. ⛔ **Consequence for D-263-13's Fence 1: derive the craft-bullet
token list from the LIVE DB ROW, never from a migration file**, and note that 089's 5th bullet mixes authoring
doctrine with agent-tool choreography, so a naive section slice carries choreography into the prompt.
