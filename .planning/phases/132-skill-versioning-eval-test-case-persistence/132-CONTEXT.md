...
- **D-03:** Each version row carries a **monotonic `version_number`** (v1, v2, … computed per-skill as max+1 inside the trigger), `created_at`, and a **`source`** provenance tag (`manual` / `import` / `tuner` / `self_improve` / `backfill`). The snapshot stores name + description + instructions as they were at save time. Append-only — no UPDATE/DELETE of version rows.
  - **D-03-R1 (RESOLVED, operator 2026-06-29 — research conflict C-1):** A pure zero-app-code trigger (D-01) cannot distinguish write paths — every write arrives as an identical PostgREST statement and the `SET LOCAL` GUC trick isn't reachable through supabase-py. So: the **trigger stamps `source='manual'`** for all runtime captures, the **migration backfill stamps `source='backfill'`** (D-11). KEEP the full 5-value `CHECK` enum on the column (`manual`/`import`/`tuner`/`self_improve`/`backfill`) for forward-compat — later phases can populate the others — but 132 does NOT distinguish import/tuner/self_improve. D-01 (zero-app-code trigger) is preserved.
  - **D-03-R2 (RESOLVED, operator 2026-06-29 — research conflict C-2):** "Append-only" = **immutability on UPDATE only** (block UPDATEs via the 067 published-immutability block-trigger pattern). DELETE is **allowed via FK `ON DELETE CASCADE`** so deleting a skill removes its version rows — do NOT add a BEFORE DELETE block trigger (it would make skill deletion 500). History is immutable while the skill lives.
  - **D-03-R3 (research, service-role reality):** The backend writes via the **service-role key which BYPASSES RLS** (`backend/app/dependencies.py:19`). Therefore (a) the trigger must set `user_id` from `NEW.user_id`, NOT `auth.uid()` (NULL under service-role); (b) RLS on the two new tables is **defense-in-depth only** — the app-code `.eq("user_id", …)` owner filter is the real runtime gate (same model as `077_tuner_runs.sql`); (c) add a UNIQUE constraint on `(skill_id, version_number)` to guard the max+1 race.
