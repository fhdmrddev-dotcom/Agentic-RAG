...
* **247-01 (Wave 1: Adapters & Path Resolution)**: Resolves full folder hierarchy in `SourceFile.path` and propagates to `metadata.source.path` for Google Drive (`WATCH-01`) and Microsoft Graph (`WATCH-02`), resolving `SEED-282`. Fixes `WR-04` (human label display name for Gmail user labels) and `WR-07` (recursive boundary fence traversal).
