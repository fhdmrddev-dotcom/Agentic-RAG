...
phase: 247
phase_name: "Sources & Watches — the surface you now live on"
author: gemini
reviewer: pending
reviewed_at: null
...
* **247-04 (Wave 3: Warnings Disposition, Ledger Sync & Verification)**: Formal paperwork deliverable for `WATCH-08` authoring `247-DISPOSITION.md` (WR-04, WR-07, WR-09 fixed in code; WR-02, WR-05, WR-06, WR-08 accepted as debt), closing `BUG-260910-02`. Updates `docs/HOT-FILE-LEDGER.md` with re-derived triples for 5 touched files and verifies the strict fence on `api/connectors.py`. Executes full verification across frontend and backend.
...
* **Action Honesty (Consequence $\neq$ Receipt):** `sourceHealthVocabulary.ts` labels the disabled connection action as `"Open ${connectionName} in Settings ↗"` (or `"Open Connection Settings ↗"`). The handler routes to `onNavigateToConnections?.()`, traveling to Settings without falsely claiming an in-place write.
* **Seam status: CLEAR.**
...
* **Hot File Ledger Warning:** `backend/app/api/connectors.py` currently stands at **44 commits / 27 phases / 2,140 lines**, with an extraction **OWED at its sixth landing**.
...
## 3 · Standing Red Baseline Assertion
...
* **Empirical Measurement at Base Commit (`987e7a68515d1c47dbc7b942551e1798393b6d0b`):**
  - Command: `$env:CI="1"; node node_modules/vitest/vitest.mjs run src/components/sources/sourceComposition.test.tsx`
  - **Result: 17 failed | 32 passed (49 total)** in 35.35s.
* **Contract:** This baseline is established before touching `src/components/sources`. The failure count must remain strictly at 17 failed | 32 passed (no regression).
...
| `backend/app/services/sources/adapters/microsoft_graph.py` | 6 / 4 / 457 | Fires ($\ge 3$ phases) | Touched in 247-01; safe-as-is, sync ledger |
...
| `frontend/src/components/sources/sourceHealthVocabulary.ts` | 6 / 2 / 560 | Clean at base (2 phases) | Touched in 247-03; advances to 3 phases, sync ledger |
| `backend/app/api/connectors.py` | 44 / 27 / 2,140 | **Extraction Owed** | **⛔ STRICTLY FENCED (0 lines touched)** |
...
| `247-03` | 2 | Yes | WATCH-03, WATCH-04, WATCH-05, WATCH-06, WATCH-07, WATCH-08 | `WatchRowCard.tsx` extracted (<1000L); Variant A dual pill; row-level sync spinner + inline tag; missing files with timestamps; WR-09 Microsoft mark; WATCH-06 "Open ... in Settings ↗"; WATCH-07 drop "on"; standing red remains 17/32 | `$env:CI="1"; node frontend/node_modules/vitest/vitest.mjs run frontend/src/components/sources/watchProductMark.test.ts frontend/src/components/sources/sourceHealthVocabulary.test.ts frontend/src/components/sources/WatchedFoldersSection.test.tsx` |
| `247-04` | 3 | Yes | WATCH-08 | `247-DISPOSITION.md` authored; BUG-260910-02 closed; `docs/HOT-FILE-LEDGER.md` updated for 5 hot files; connectors.py fence verified; standing red preserved | `python -m pytest backend/tests/unit/test_247_source_paths.py backend/tests/unit/test_247_watch_missing_lifecycle.py -v` |
...
- [x] Standing red baseline for `sourceComposition.test.tsx` empirically measured: **17 failed | 32 passed**.
...
- [x] Frontmatter accurately marks `reviewer: pending`, `kind: builder-self-audit`, `reviewed_at: null`.

Ready for independent preflight review by Claude Code before execution.
