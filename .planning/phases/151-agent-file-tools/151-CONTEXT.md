...
- **D-07 (filename collision — OVERWRITE IN PLACE):** On a filename that already exists on the skill, **replace** the existing `skill_files` bytes + row (upsert on `skill_id + filename`). Predictable for an authoring loop where the agent iterates on the same helper file; no orphan rows. Tool result reports `"updated"` vs `"created"`. **Amended (plan-phase 2026-07-13): a clean upsert requires the D-10 unique-index migration — `skill_files` has NO `(skill_id, filename)` unique constraint today, so a read-check-then-write would race under `WORKER_COUNT=2`. Storage `.upload()` must also pass `upsert:"true"` to overwrite the bytes.**
- **D-08 (reuse existing write path):** Reuse the `_upload_skill_files` mechanism (`skills.py:113`) — `skill-files` bucket + `skill_files` row at storage path `{user_id}/{skill_id}/{filename}`. Do NOT invent a new table/bucket. (The D-10 unique index is a constraint on the existing table — NOT a new table/bucket — so it does not violate this.)
- **D-10 (upsert unique-index migration — RESOLVED plan-phase 2026-07-13):** Ship a new numbered SQL migration under `supabase/migrations/` adding a **UNIQUE index on `skill_files(skill_id, filename)`** so the D-07 overwrite is a clean, race-free PostgREST upsert (`on_conflict=skill_id,filename`) correct under `WORKER_COUNT=2`. Apply to the live local DB via the Supabase SQL editor (never `db push`/`db reset`), then regen `full-schema.sql`. **Cloud parity: this migration is a pending cloud apply that MUST land on cloud Supabase before Phase 151 ships live** (add to the pending-cloud-migrations tracking, per the standing parity rule). Before writing the migration, check the live seed/migration number space for the next free `<digits>_name.sql`.
- **D-11 (FILE-01 capability gate — RESOLVED plan-phase 2026-07-13):** `attach_skill_file` is gated behind the **`self_improve_enabled`** capability flag (mirrors the existing `save_skill` skill-write tool) — it only appears in the LLM's tool list when self-improvement is enabled, registered in `_CAPABILITY_FLAG_TOOLS`. FILE-02 `fetch_document_file` is gated behind **`sandbox_enabled`** (it materializes bytes into the sandbox container; meaningless when the sandbox is off), registered in `_CAPABILITY_FLAG_TOOLS` alongside the other sandbox-materializing tools. FILE-02 arg shape mirrors the existing `read_document`/`analyze_document` document-identifier convention (no new arg vocabulary).
