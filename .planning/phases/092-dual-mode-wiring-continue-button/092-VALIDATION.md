...
| **RAG grounding — workflows operate on the user's documents** (092-07, operator concern 2026-05-31) | MODE-01 | The product is a RAG app; a workflow run must actually search the KB, not answer generically. Browser + panel + LangSmith only | Scope to the **DBA** folder. Harness **Doc Q&A** AND **Research → Summarize**, plus a **Deep** message, on an IN-KB question (e.g. "success factors for digital transformation" / "how the literature defines RPA"). Confirm: (a) `search_documents` is actually invoked (workspace panel tool call + LangSmith), (b) the answer is grounded in the user's documents (cites/uses them, not generic world knowledge), (c) it holds in BOTH Harness and Deep. **Binding gate.** |
| **Out-of-KB answer transparency** (092-07, operator concern 2026-05-31) | MODE-01 | The user must be able to tell a document-grounded answer from a general-knowledge one | Ask an OUT-of-KB question (not in the DBA folder) in Harness and Deep. Observe how it's reflected: does the agent make clear the answer is NOT from the user's documents (e.g. "no relevant documents found" / Research→Summarize falls to `web_search` and says so), rather than silently answering from world knowledge as if grounded? Operator judges acceptability; record the actual UX. **Binding gate.** |
