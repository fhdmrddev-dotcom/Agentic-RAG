...
4. **⭐ Operator sequencing preference (recorded 2026-07-18 — confirm at v3.4-close `/gsd:new-milestone`, NOT hard-locked yet):** prioritize the **UX / end-user-experience track RIGHT AFTER v3.4**, ahead of Open Platform + Automations. Two adjacent-but-separate milestones (operator-agreed split — do NOT bundle a Large build with a cleanup pass): **(a) UX Consolidation & Chat Polish** (SEED-045 UI/UX-polish pass + the parked post-v3.3 chat-polish bug backlog + new-org-surface polish + terminology/plain-language extensions), then **(b) Workflow Studio — Visual / No-Code Authoring** ([[SEED-123]] — drag-and-drop, business-friendly, Beam/Glean/n8n-class, built ON the existing governed engine). This pushes **Open Platform (API/MCP) → later, Automations → later**. Honest tradeoff: defers the platform/integration story in favor of end-user adoption + competitive UX — aligned with the operator's business-end-user product vision; the "paying customer forces integrations first" escape valve still applies.
