...
| Claim-stamp losing race (`UPDATE 0`) → re-SELECT foreign → honest error, never bytes | COLL-02 | T-141-03 | Conditional `UPDATE ... WHERE run_claim IS NULL` + rowcount re-check + fallthrough | integration (offline) | `...::test_resolver_claim_race_falls_through -x` |
...
|----------|-------------|------------|-------------------|
| Cross-provider SMOKE (D-141-07) | COLL-02 (SC#2) | Live provider render; not the SC#10 4-axis (141 not on the SC#10 headline list; model only emits the tool call — resolver is shared backend) | **CORRECTED 2026-07-08 (live Chrome UAT):** `render_template` is NOT a Deep-chat tool — `get_tools()` omits it; it is exposed to the LLM ONLY inside a workflow FILL phase whose whitelist admits `"render_template"` (`phase_types.py:265,275`; `openai_service.py:649-650`). So the render SMOKE must run **inside a WORKFLOW run** (a published workflow with a render_template fill phase consuming a thread template), NOT a Deep turn. Upload a `.docx` template to the thread, run the workflow with **ONE** representative model (OpenAI **or** Anthropic **or** Google) → filled deliverable produced, in-scope, byte-correct. Proves the resolver did not regress the render happy path. |
| Live cross-run repro (optional lived-glance) | COLL-02 (SC#1) | Confirms the offline repro matches live behavior | **CORRECTED 2026-07-08:** since `render_template` is workflow-only, stage the cross-context block as **W1 → W2** (two workflow runs on the SAME thread template): run W1's fill phase (stamps `run_claim=str(W1)`), then run W2's fill phase on the same template → W2 gets the honest "belongs to a different run" message, never W1's bytes/id. (A Deep-turn render cannot be the second leg — Deep chat has no `render_template`.) |
