...
|---|---|---|---|
| 1 | **Cloud application of migration 176** ⚠ **AND IT IS NOT ALONE — measured 2026-09-10, cloud is FIFTEEN migrations behind** (`153,154,155,156,166..176`), because cloud is still on v3.9 and every v4.0 migration is pending. `bash scripts/pending-cloud-migrations.sh` prints the set; apply them in NUMERIC ORDER, once each. Reading this row as *"only 176 is owed"* would under-apply the schema by fourteen files. | Not this phase's work. Cloud schema changes are a deploy-parity item under CLAUDE.md's standing rule that every prod push walks the DB + non-code checklist | At the **next operator-triggered deploy**. ⚠ Until then cloud has no `hnsw_ef_search` / `hnsw_iterative_scan` columns, so the Settings controls will read the `config.py` defaults there and the knobs cannot be changed in production |
