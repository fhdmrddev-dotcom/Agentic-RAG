...
| M-8 | Disconnect → watching freezes, nothing is deleted | ✅ **PASS.** ⚠ **HEADLINE FLIPPED 2026-09-13 (Phase 245, D-06) — the original assessment below is PRESERVED VERBATIM AS HISTORY, not deleted**, because the row contradicting itself IS the finding: its headline read HALF while its own body ended `BUG-260907-03` **closed**, and a reader believed whichever half they reached first. SC#1 admits only pass / blocked / retired. ⛔ **NOT re-driven** — that would mean disabling a live integration again for a result already in this row; the fix landed at the choke point, was driven both directions on this row, and the bug is closed. **↓ ORIGINAL, 2026-09-07 ↓** — ⚠ **HALF PASS — and the failing half is a DEFECT, `BUG-260907-03`.** *Nothing is deleted*: ✅ all three documents survive, including the `missing_at_source` one. *Watching freezes*: ✅ for the scheduled loop (`watch_service.py:205`). ⛔ **But the connection does not stop READING.** Driven at `is_enabled=False`: `browse()` minted a fresh OAuth token and returned 6 real OneDrive folders from `graph.microsoft.com`. Browse, preview and import were all unguarded. **Not Graph-specific — Google Drive had the same hole since Phase 232.** ✅ **FIXED THE SAME DAY** at the choke point (`SourceRegistry.get_adapter` raises `SourceConnectionDisabled`), driven both directions on the same row: disabled -> refused with no network call, enabled -> 6 folders. `BUG-260907-03` closed. |
| M-9 | A `path contains '/Finance/'` rule fires for a file that IS in that folder | ⛔ **BLOCKED — M-9's Microsoft/OneDrive arm not driven; no `/Finance/` folder exists in the OneDrive account (operator action). Trigger: that folder existing, or the next phase touching Graph ingestion.** ⚠ **CONVERTED 2026-09-13 (Phase 245, T1 a-bis) — the original below is PRESERVED VERBATIM AS HISTORY.** `HALF` is not one of SC#1's three admissible verdicts, and leaving it there would have left the criterion unmeetable with no plan owning the fix. **What `245-02` DID drive, and why it does not settle this row:** the rule was created through the real `RuleBuilderPanel` surface (`rule_scope: watch`, `path contains '/Finance/'`, persisted) and driven through **BOTH doors** per D-05 — but against **Google Drive**, where both doors wrote `metadata.source.path = null` and the rule never fired. **238 is a Microsoft Graph phase, so M-9's own provider is OneDrive — where `path` IS populated** (`/Attachments/…`, this table's own M-6/M-7 documents). So the observed failure is real but belongs to a **wider, different defect**, filed as **`BUG-260913-01`** (the Google Drive adapter never writes `metadata.source.path`, on either door — every path-based rule is silently inert for every Drive document). Evidence: `.planning/phases/245-…/245-UAT-RESULTS.md` § *DEBT-01 — Phase 238's residue*. **↓ ORIGINAL, 2026-09-07 ↓** — ⚠ **HALF PASS, and stronger than before.** The fact is now real **in the database**, not only in the adapter: a watched document carries `metadata.source.path = /Attachments/Practical_Project_Management_Guide_Recreated.docx`. ⛔ The *rule* was still not created, so end-to-end matching remains owed — and it must be re-driven through the **manual import** door too, since that arm was broken until Defect 3 was fixed. |
| **S-1** | Browse a SharePoint document library | ⛔ **RETIRED 2026-09-13 (Phase 245, D-07) — `SEED-256`.** **Ground: no M365 work/school tenant. An ACCOUNT BOUNDARY, not effort** — a personal account has no `/sites/` to address, and no scope unlocks it. ⚠ Confirmed LIVE rather than assumed: `check()` returned **`drive_type: personal`**. **Re-open triggers: `SEED-256`'s four `trigger_when` arms** (a free M365 Developer tenant · a customer/pilot org on M365 — ⭐ the real one · any paid M365 Business/Enterprise subscription · a phase proposing SharePoint as a source) — cited by reference, not restated. Retired in writing across three registers in one commit: this table, `REQUIREMENTS.md` DEBT-01, and `SEED-256`'s `status:`. **↓ ORIGINAL, 2026-09-07 ↓** — ⛔ **BLOCKED — `SEED-256`.** No M365 work/school tenant; a personal account has no `/sites/` to address. ⚠ The live drive CONFIRMS the account kind: `check()` returned `drive_type: personal`. |
| **S-2** | `Sites.Read.All` self-consent vs admin approval in an enterprise tenant | ⛔ **RETIRED 2026-09-13 (Phase 245, D-07) — `SEED-256`.** Same ground, same commit: **no M365 work/school tenant; `check()` returned `drive_type: personal`** — an account boundary, not effort. ⚠ **Retired ≠ answered.** `Files.Read.All` self-consenting on a *personal* account says nothing about `Sites.Read.All` in an *enterprise* tenant, where admin consent is a real possibility with a **sales** consequence. `SEED-256` names this as **the FIRST thing to drive when its trigger fires**, before any SharePoint UAT row. **↓ ORIGINAL, 2026-09-07 ↓** — ⛔ **BLOCKED — `SEED-256`.** Unresolved and un-softened. |

**ELEVEN rows: `M-1`…`M-9` (the "nine") plus `S-1` and `S-2`.** ⚠ **The count ambiguity is resolved HERE, in the place the number is read from** — "nine" means **the M rows only**; S-1/S-2 are two *additional* rows, never a subset. **9 driven · 8 ✅ PASS · 1 ⛔ BLOCKED with a reason, a blocking id and a named trigger (M-9) · 0 HALF · 0 owed · 2 ⛔ RETIRED (S-1, S-2 — `SEED-256`) · 0 claimed.**

⚠ **UPDATED 2026-09-13 by Phase 245 — the original footer is preserved, not deleted:**
> **9 rows driven (7 full pass, 2 half) · 0 owed · 2 blocked (S-1, S-2) · 0 claimed.**
M-8's headline flipped ✅ (D-06 — its body already read `BUG-260907-03` **closed**); M-9 converted to ⛔ BLOCKED after `245-02` drove it against the wrong provider (see its row); S-1/S-2 retired (D-07). **The table is now terminal: no row reads HALF.**
