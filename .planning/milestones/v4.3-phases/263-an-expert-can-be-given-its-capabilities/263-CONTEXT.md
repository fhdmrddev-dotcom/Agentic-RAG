...
and `089_skill_creator_file_attach_honesty.sql` **each perform a full instructions rewrite** of row
`…0010`. The interview-loop finding is UNCHANGED and was reached independently twice — what is
corrected is **which migration to read**.

⛔ **Consequence for D-263-13’s Fence 1: derive the craft-bullet token list from the LIVE DB ROW,
never from a migration file.** 089’s fifth bullet mixes authoring doctrine with agent-tool
choreography, so a naive section slice carries choreography into the prompt.
