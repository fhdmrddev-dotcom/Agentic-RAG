# Requirements Archive: v3.0 Document Management
...
- [x] **META-01**: User can define custom metadata fields (beyond the built-in title/author/date/type/topics/language/summary) that the system extracts on ingest.
- [x] **META-02**: User can see a per-field confidence score for each extracted metadata value.
- [x] **META-03**: Metadata extraction uses the user-selected (or an admin-configured) model — not the hardwired `gpt-4o`.
- [x] **META-04**: Metadata extraction reads beyond the first 3,000 characters (configurable/larger window or title-page + tail sampling) so late title-page/byline data isn't missed.
...
- [x] **EMBED-01**: Admin can select the embedding provider (OpenAI / Google / local Ollama / local LM Studio / other OpenAI-compatible `/v1/embeddings`) from Settings — embeddings are no longer effectively OpenAI-only.
- [x] **EMBED-02**: Selecting a local provider auto-fills its base_url (Ollama `http://localhost:11434/v1`, LM Studio `http://localhost:1234/v1`) and relaxes the API key to a dummy value.
- [x] **EMBED-03**: The existing embedding model / dimensions / base_url / key remain as advanced overrides, with a provider→default-model→default-dimensions auto-fill map.
- [x] **EMBED-04**: Chunk-embedding and query-embedding use the SAME configured model + credentials (fix `embed_chunks` dropping `user_settings`) so a configured non-default embedder never embeds queries and chunks in mismatched vector spaces.
- [x] **EMBED-05**: Changing the embedding model/dimension triggers a guarded, RLS-scoped re-embed of existing chunks (wire `resize_embedding_column` + a batched re-embed background job from preserved `content`); search recovers afterward.
- [x] **EMBED-06**: A destructive-change confirmation names the re-embed (and its cost) before any embedding model/dimension change commits.
...
- [x] **UX-02**: The three highest-risk net-new surfaces — the **document detail panel** (metadata + confidence + inline edit + relationships + classification), the **view/filter builder**, and the **relationship panel** — are sketched and operator-approved before implementation (G-2 sketch-before-plan).
...
| META-01 | Phase 111 — Metadata Enrichment (Extraction Backend) | Complete |
| META-03 | Phase 111 — Metadata Enrichment (Extraction Backend) | Complete |
| META-04 | Phase 111 — Metadata Enrichment (Extraction Backend) | Complete |
| EMBED-01 | Phase 111.1 — Configurable / Multi-Provider Embeddings | Complete |
| EMBED-02 | Phase 111.1 — Configurable / Multi-Provider Embeddings | Complete |
| EMBED-03 | Phase 111.1 — Configurable / Multi-Provider Embeddings | Complete |
| EMBED-04 | Phase 111.1 — Configurable / Multi-Provider Embeddings | Complete |
| EMBED-05 | Phase 111.1 — Configurable / Multi-Provider Embeddings | Complete |
| EMBED-06 | Phase 111.1 — Configurable / Multi-Provider Embeddings | Complete |
...
| UX-02 | Cross-cutting G-2 sketch (Phases 112, 114, 117) | Complete |
