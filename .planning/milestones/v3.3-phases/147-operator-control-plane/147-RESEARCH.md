...
> All five questions below were resolved before/at planning (2026-07-11). Inline **RESOLVED** markers state the binding answer; the option text is retained for the audit trail.

1. **Run-kind derivation strategy (D-01).** ✅ **RESOLVED (operator, 2026-07-11 — D-Q1): Option A, no migration.** ZRANGE `runs:active` → batch `runs` lookup (`run_id = ANY($ids)`); rows found = chat/workflow/eval (workflow split via `workflow_runs`/`threads.active_workflow_run_id`, eval via its marker); ids with NO `runs` row = tuner (generic "Tuner job" label, no Kill per D-01). NO `runs.kind` column. Implemented in plan 147-02 Task 2.
...
2. **Which GET endpoints are floor-exempt, and how is the visit-row written (D-07)?** ✅ **RESOLVED (planning, 2026-07-11):** floor-exempt = `GET /admin/backpressure` (floor REMOVED from it) + the new `GET /admin/runs`. The visit-row is a dedicated floor-attached `POST /admin/control-plane/record` with a server-owned `{event: "visit"|"refresh"}` Literal → hardcoded (label, action) mapping: visit → ("Opened the Control Plane", `control_plane.visit`), refresh → ("Viewed system health", `health.view`); unknown events 422. Implemented in plan 147-02 Task 2; the shell records "visit" once on mount and "refresh" on the manual ↻ (plan 147-09).
...
3. **Long-running (>8 min) + "not responding" (stalled-stream) thresholds (064-B card tags).** ✅ **RESOLVED (planning + checker revision, 2026-07-11):** long-running = pure CLIENT-side elapsed math (>8 min from `started_at`, no backend field — D-07). `not_responding` = a SERVER-derived boolean on each `GET /admin/runs` row, computed by reusing the reconciler's stream-age oracle (`run_reconciler.py:264` `_is_chat_orphan` — `run:{run_id}` stream last-write age with stale_timeout + start-grace; read-only reuse, no re-derivation). False for tuner/eval rows and young runs. Wired: plan 147-02 (derive) → 147-06 (`ActiveRun.not_responding` type) → 147-07 (tag render).
...
4. **Maintenance flag polarity on a COLD cache.** ✅ **RESOLVED (operator, 2026-07-11 — D-Q4): default OPEN.** A truly-cold/uninitialized `maintenance_mode` read yields `false` (platform usable); the last-known-good TTL cache absorbs transient blips. Capability `*_enabled` flags cold-read their migration default `true` (a cold read never silently disables a capability). Implemented in plan 147-01 (helpers) + 147-05 (middleware treats a cold/failed read as OPEN).
...
5. **Does "self-improve" have a single enforcement seam? (A1)** ✅ **RESOLVED (planning, 2026-07-11): TWO seams, both gated.** (a) the `save_skill` tool — hidden from `get_tools()` (SAVE_SKILL_TOOL moves from the always-on list at openai_service.py:1031 to a `self_improve_enabled`-conditional append) AND refused at `dispatch_tool` (capability_disabled ToolResult); (b) the proposer entry — `skill_proposer_service.propose()` (:328) gets a `self_improve_enabled()` early-return guard. Implemented in plan 147-04 Task 1.
