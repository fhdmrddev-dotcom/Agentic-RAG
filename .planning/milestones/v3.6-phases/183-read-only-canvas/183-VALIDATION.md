...
> **26 tasks across 9 plans** (183-01 ×3, 183-02 ×3, 183-03 ×2, 183-04 ×3, 183-05 ×3, 183-06 ×3,
> 183-07 ×3, **183-08 ×3 — gap closure**, **183-09 ×3 — gap closure**). **Every task carries an `<automated>` verify command** —
...
| 183-08-01 | 08 | GC | CANVAS-01 | — | RED gate: keyboard activation + ARIA-honesty assertions installed and PROVEN failing on unmodified source (the control CR-01 slipped through — the suite asserted node reachability, never activation) | component (jsdom, RED) | `cd frontend && npx vitest run src/components/workflows/WorkflowCanvas.test.tsx` — expect **exactly 3 failed** (Enter / Space / announced-affordance) | ✅ exists — 3 tests **added** | ✅ green |
| 183-08-02 | 08 | GC | CANVAS-01 | T-183-01, T-183-12 | Enter/Space fire the SAME `onSelectNode(slug)` contract as a click (D-183-05); cap + broken-ref stub stay inert; ARIA strings are static literals (no definition data interpolated); activation reaches `setSelectedSlug` only — no write, no fetch, no `onNodesChange` | component (jsdom, GREEN) | `cd frontend && npx vitest run src/components/workflows/WorkflowCanvas.test.tsx src/pages/WorkflowBuilderPage.canvas.test.tsx` | ✅ exists | ✅ green |
| 183-08-03 | 08 | GC | CANVAS-01 | T-183-01 | `citation_policy: "partial"` reads the MIDDLE face on BOTH governance surfaces, pinned cross-module against `deriveTier`; unknown/absent policy still falls through to `open` (totality preserved); no snapshot moved; `PhaseNode.tsx` lint-clean with byte-identical rendered output | unit + lint | `cd frontend && npx vitest run src/components/workflows && npx eslint src/components/workflows/PhaseNode.tsx` | ✅ exists — 2 tests **added**, 1 renamed | ✅ green |
| 183-09-01 | 09 | GC | CANVAS-01 | — | RED gate: the panel-dismissal, auto-repeat and inert-ARIA assertions installed and PROVEN failing on unmodified source, with the dismissal asserted in BOTH the Spine (flag OFF) and the Canvas view | component (jsdom, RED) | `cd frontend && npx vitest run src/pages/WorkflowBuilderPage.canvas.test.tsx src/components/workflows/PhaseFormPanel.test.tsx src/components/workflows/WorkflowCanvas.test.tsx` — expect **exactly 8 failed** | ✅ exists — 9 tests added | ✅ green |
| 183-09-02 | 09 | GC | CANVAS-01 | T-183-12, T-183-01, T-183-09 | ✕ / Escape / pane-click all dismiss in BOTH views; `onClose` + `onClearSelection` REQUIRED so an unclosable panel is unrepresentable; dismissal reaches `setSelectedSlug(null)` only — no create, no PATCH, no generate; the stale "IDENTICAL props" docblock corrected | component (jsdom, GREEN) | `cd frontend && npx vitest run src/pages/WorkflowBuilderPage.canvas.test.tsx src/pages/WorkflowBuilderPage.test.tsx src/components/workflows/PhaseFormPanel.test.tsx` | ✅ exists | ✅ green |
| 183-09-03 | 09 | GC | CANVAS-01 | T-183-01 | A held Enter/Space activates exactly once (`event.repeat` guard); the end cap and the unresolved-skip stub no longer inherit the activation description while real phase nodes still do; the fixture snapshot grows additively by exactly 15 `domAttributes` keys with zero deletions | unit + component | `cd frontend && npx vitest run src/components/workflows` | ✅ exists — 2 tests added | ✅ green |
...
| **U-4** | Flag off = yesterday's Builder | D-183-03, D-181-01 | Requires a real operator session + a live `app_settings` write; the vitest analogue proves the render branch, not the end-to-end flag path | Operator flips `visual_workflow_canvas` → **Off** in the Control Room, reloads. **Pass:** the toggle is gone and the graph column is exactly as before — **including on an operator account**. **Scope of "byte-identical" (read this before judging a failure):** it covers the *canvas* surface only — the `[≣ Spine] [⬡ Canvas]` toggle strip is absent and no `.react-flow` subtree mounts. It does **NOT** mean the Spine renders as it did before Phase 183 shipped. Per **D-183-13** the Spine intentionally and unconditionally gains the 3D phase marks, the plain-language step titles, and the ⌥ technical-names reveal — flag-independent, "accepted cost: a shipped surface is touched". Seeing the Spine look different with the flag OFF is the phase working as designed, **not** a U-4 failure. |
