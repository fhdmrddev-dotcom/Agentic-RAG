...
- 🔄 **v2.1 Stability & RAG Correctness** — Phases 18–25 (in progress)
...
### v2.1 Stability & RAG Correctness (Phases 18–25)
...
**Plans**: 1 plan
Plans:
- [x] 19-01-PLAN.md — Sub-agent content cap + APIError context detection
...
**Plans**: 1 plan
Plans:
- [x] 20-01-PLAN.md — Empty content fallback + finish_reason length guard + maybe_single hardening
...
**Plans**: 1 plan
Plans:
- [x] 21-01-PLAN.md — folder_ids parameter through full keyword search stack
...
**Plans**: 1 plan
Plans:
- [x] 22-01-PLAN.md — read_document cap + metadata case normalization at ingest and search
...
**Plans**: 1 plan
Plans:
- [x] 23-01-PLAN.md — Similarity confidence hedging + citation format guidance
...
**Plans**: 1 plan
Plans:
- [x] 24-01-PLAN.md — Settings TTL cache + sentence boundary fix

### Phase 25: Sub-Agent Intelligence & Model-Aware Context
**Goal**: Sub-agents auto-select the cheapest capable model per provider, context budgets are provider-aware, and JSON token estimation is corrected
**Depends on**: Phase 18
**Requirements**: CTX-06, CTX-07, CTX-08
**Success Criteria** (what must be TRUE):
  1. `analyze_document` sub-agent selects Haiku for Anthropic, GPT-4o-mini/nano for OpenAI, Gemini Flash for Google, cheapest for OpenRouter, default for Ollama
  2. Main agent context budget uses provider-aware limits (Anthropic 120k, OpenAI 200k, Google 180k, OpenRouter 100k, Ollama 80k)
  3. Tool call JSON token estimation uses chars/3 (corrected from chars/4) to account for JSON punctuation density
  4. Existing context trimming tests continue to pass with the new estimation constant
**Plans**: 1 plan
Plans:
- [x] 25-01-PLAN.md — Provider-aware budgets + sub-agent model selection + token estimation fix
