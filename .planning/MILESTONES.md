...
- **Two Phase-194 residuals, routed not hidden.** **L-01** — the far-worker producer KEEPS RUNNING; the run now reports honestly, the work does not stop (needs a Redis cancel channel or an in-loop status re-read). **WR-04** — three non-owner callers drive `_cancel_run_internals`, so the UI says *“Stopped by you”* about a stop the reader did not make.
...
- ⚠ **Cloud parity owed at migrations 119 → 123.** All five are applied to the local DB (verified: timing columns present, `definition_snapshot` `object` on 8 of 8 rows). None is applied to cloud.
- ⚠ **The `v3.7-phases/` directory archive is DEFERRED, deliberately.** `.planning/phases/` already holds `201-…` and `202-…`, which are v3.8's and were being actively written by other sessions at close time; moving 17 directories out from under them would have raced live work. `/gsd:complete-milestone v3.7` was NOT run for the same reason — it archives the whole set. Everything else in the close was done by hand. **Re-open trigger: the first moment no other session is working in `.planning/phases/`.**
