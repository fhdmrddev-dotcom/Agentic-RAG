"""Phase 118 Wave-0 (RED) — dismiss_classification clears the suggestion; Undo via existing move.

Proves the CLASS-03 dismiss endpoint (Plan 03 T2) against live :54322:

  * dismiss pops ``metadata._classification`` (own-scoped UPDATE) — NO move, NO audit
    (clone the metadata-merge shape, but ``meta.pop("_classification", None)``).
  * dismiss does NOT change ``folder_id``.
  * cross-user dismiss of another user's doc → 404, NEVER 403.
  * Undo is NOT a new endpoint — the frontend reverses an accept by calling the EXISTING
    ``move_document`` with the stamped ``prior_folder_id`` (reversible by construction,
    D-118-6). This file documents that contract via the stamped prior_folder_id on an
    accepted suggestion.

xfail until Plan 03 ships dismiss_classification; imports inside test bodies. Harness from
...
pytestmark = pytest.mark.skipif(
    not PG_AVAILABLE,
    reason=f"Local Postgres on {_POSTGRES_TEST_DSN} not reachable; skipping live 118 dismiss/undo tests",
)
...
async def doc_with_suggestion(pg_pool):
    ...
    await pg_pool.execute(
        ...
        user_id, f"phase-118-dismiss-{user_id}@test.local",
    )
    for fid, name in ((folder_id, "Inbox"), (target_folder, "Invoices")):
        await pg_pool.execute(
    ...
    await pg_pool.execute(
        ...
        doc_id, user_id, "to-dismiss.txt", f"{user_id}/to-dismiss.txt", 100, "text/plain", "completed",
        {
    ...
    ctx = {
        "user_id": user_id, "folder_id": str(folder_id),
...
async def test_dismiss_clears_classification_no_move(pg_pool, doc_with_suggestion):
    ...
    from app.api.documents import dismiss_classification

    sb = _supabase_or_skip()
    ctx = doc_with_suggestion
    caller = {"id": str(ctx["user_id"])}
    result = await dismiss_classification(ctx["doc_id"], current_user=caller, supabase=sb)
    row = result if isinstance(result, dict) else result.model_dump()
    assert "_classification" not in (row.get("metadata") or {}), "dismiss must clear _classification"
...
async def test_dismiss_writes_no_audit(pg_pool, doc_with_suggestion):
    ...
    ctx = doc_with_suggestion
    caller = {"id": str(ctx["user_id"])}
    await dismiss_classification(ctx["doc_id"], current_user=caller, supabase=sb)
    audit = await pg_pool.fetchrow(
        "SELECT count(*) AS n FROM audit_log WHERE user_id=$1 AND action_type LIKE 'classification%'",
        ctx["user_id"],
    )
...
async def test_dismiss_cross_user_404_not_403(pg_pool, doc_with_suggestion):
    ...
    with pytest.raises(HTTPException) as ei:
        await dismiss_classification(ctx["doc_id"], current_user=other, supabase=sb)
...
async def test_undo_reuses_existing_move_via_prior_folder_id(pg_pool, doc_with_suggestion):
    ...
    the EXISTING move_document(prior_folder_id) (reversible by construction, D-118-6)."""
    ...
    # The stamped prior_folder_id is what an Undo move targets — no dedicated Undo route.
    assert sugg.get("prior_folder_id") == ctx["folder_id"], (
        "accept must stamp prior_folder_id so Undo can reuse the existing move endpoint"
    )
