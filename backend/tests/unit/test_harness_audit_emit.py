...
def test_write_audit_accepts_emit_kinds():
    ...
    #   + 1 armed action-risk pause (114, Phase 185 / BUG-260731-02) = 23 total kinds.
    # The 069 emit kinds are still all present (the 070 and 114 ALTERs are additive);
    # this count is bumped in lockstep with the _AUDIT_EVENT_TYPES extension. The guard
    # that keeps the Python set equal to the SQL CHECK — so a kind can never again be
    # emitted while unregistered — is tests/unit/test_audit_event_registration.py.
