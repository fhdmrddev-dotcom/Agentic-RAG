"""Phase 115 Wave-0 — SC#1 tool dual-registration RED scaffold (Plan 03 target).

The inverse-Phase-101 dual-registration: the new agent tool
`query_documents_by_view` must be wired into BOTH
...
Phase 101's render_template bug was that the handler+registry landed but the
SCHEMA never reached `get_tools` → the model never emitted the call. This test
guards against the same gap in reverse.
...
def test_tool_registered_in_dispatcher():
    ...
    assert "query_documents_by_view" in td._TOOL_REGISTRY
...
def test_tool_schema_present_in_get_tools():
    ...
    assert "query_documents_by_view" in names, (
        "query_documents_by_view schema missing from get_tools() — "
...
def test_registry_and_schema_agree():
    ...
    assert "query_documents_by_view" in td._TOOL_REGISTRY
    assert "query_documents_by_view" in schema_names
