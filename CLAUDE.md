...
| [`backend/app/api/threads.py`](docs/HOT-FILE-LEDGER.md#backendappapithreadspy) | 238 / 78 / 1408 | **FIRES** | extraction TAKEN 2026-08-17 · honoured by construction (**214**) — one launch-inputs field on a request model it already owns |
...
| [`backend/app/api/workflows.py`](docs/HOT-FILE-LEDGER.md#backendappapiworkflowspy) | 41 / 21 / 2211 | **FIRES** | ⚠ **extraction still OWED** — 214-05 declined it again and said so; the named seam is unchanged |
...
| [`frontend/src/components/layout/ChatLayout.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentslayoutchatlayouttsx) | 41 / 22 / 911 | ⚠ **FIRES** | honoured by construction (**214**) — the launch form is a CHILD component, not a branch. ⚠ absent from BOTH until 200, at 21 phases |
...
| [`backend/app/services/harness/publish_service.py`](docs/HOT-FILE-LEDGER.md#backendappservicesharnesspublish_servicepy) | 23 / 10 / 1567 | **FIRES** | honoured by construction (193.2 / **214**) — 214-05 added the argument-satisfiability lint through the existing lint seam |
...
| [`frontend/src/components/chat/RunCard.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentschatruncardtsx) | 22 / 10 / 661 | **FIRES** | honoured by construction (194 / **214**) — it renders the shared step identity and resolves nothing |
...
| [`frontend/src/types/index.ts`](docs/HOT-FILE-LEDGER.md#frontendsrctypesindexts) | 71 / 57 / 1209 | ⚠ **FIRES** | no seam proposed — a barrel of wire types; ⚠ absent until 196, at 56 phases (214) |
...
| [`frontend/src/components/metadata/DocumentDetailPanel.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentsmetadatadocumentdetailpaneltsx) | 6 / 5 / 405 | ⚠ **FIRES** | ⚠ absent for its ENTIRE LIFE at **5 phases**; its whole directory is outside the count gate — row added SEED-227 |
| [`scripts/vitest-count-gate.cjs`](docs/HOT-FILE-LEDGER.md#scriptsvitest-count-gatecjs) | 127 / 25 / 4207 | ⚠ **FIRES** | honoured by construction (214-15 / 214.1-02 / **SEED-227**) — BASELINE pins + one TARGETS line, NO logic/threshold/check change; ⚠ absent until 196, at 16 phases |
...
| [`frontend/src/components/workflows/PublishGauntlet.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentsworkflowspublishgauntlettsx) | 16 / 8 / 1245 | ⚠ **FIRES** | honoured by construction (**214**) — the refusal became `PublishRefusalList`, a child; ⚠ absent until 199 |
...
| [`frontend/src/components/settings/ConnectionsTab.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentssettingsconnectionstabtsx) | 17 / 7 / 1477 | ⚠ **FIRES — EXACTLY AT THRESHOLD** | honoured by construction (212 / **214**) |
| [`frontend/src/components/settings/ConnectionFormPanel.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentssettingsconnectionformpaneltsx) | 9 / 5 / 2009 | ⚠ **FIRES** | honoured by construction (212) |
| [`frontend/src/components/settings/connectionsCopy.ts`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentssettingsconnectionscopyts) | 8 / 6 / 573 | ⚠ **FIRES** | no seam proposed — a vocabulary doing one thing many times is the right shape |
...
| [`backend/app/models/connector.py`](docs/HOT-FILE-LEDGER.md#backendappmodelsconnectorpy) | 6 / 3 / 468 | ⚠ **FIRES — EXACTLY AT THRESHOLD** | honoured by construction (212) |
...
| [`backend/app/api/connectors.py`](docs/HOT-FILE-LEDGER.md#backendappapiconnectorspy) | 7 / 3 / 781 | ⚠ **FIRES — EXACTLY AT THRESHOLD** | honoured by construction (212) |
| [`backend/app/services/connector_service.py`](docs/HOT-FILE-LEDGER.md#backendappservicesconnector_servicepy) | 7 / 3 / 1149 | ⚠ **FIRES — CROSSED THE THRESHOLD IN THE COMMIT THAT ADDED THIS ROW** | ⚠ named inside Phase 206's section with no row and no anchor (added 211) — honoured by construction (211) |
| [`backend/app/models/message.py`](docs/HOT-FILE-LEDGER.md#backendappmodelsmessagepy) | 15 / 8 / 120 | ⚠ **FIRES** | ⚠ absent from BOTH for its ENTIRE LIFE at **8 phases** — row added 214; honoured by construction (214-16) |
...
| [`frontend/src/lib/api/knowledge.ts`](docs/HOT-FILE-LEDGER.md#frontendsrclibapiknowledgets) | 2 / 2 / 775 | no (2 phases) | young (207 split, 214) — ⚠ **NOT covered by `lib/api.ts`'s row: that row is the BARREL** |
| [`frontend/src/lib/api/threads.ts`](docs/HOT-FILE-LEDGER.md#frontendsrclibapithreadsts) | 3 / 2 / 1600 | no (2 phases) | young (207 split, 214) — ⚠ **NOT covered by `lib/api.ts`'s row: that row is the BARREL** |
...
| [`frontend/src/components/ingestion/DocumentList.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentsingestiondocumentlisttsx) | 22 / 12 / 609 | ⚠ **FIRES** | ⚠ absent from BOTH for its ENTIRE LIFE at **12 phases** — row added at the SEED-224 sketch. The 7-column order is load-bearing: `IngestionPage` sheds cols 3–5 by `nth-child` |
| [`frontend/src/pages/IngestionPage.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrcpagesingestionpagetsx) | 26 / 9 / 601 | ⚠ **FIRES** | ⚠ absent for its ENTIRE LIFE at **9 phases**. ⭐ It is the host the five-tab Library shell lands in, and the file rename `IngestionPage`→`LibraryPage` is scoped here |
...
| [`frontend/src/components/metadata/DocumentDetailPanel.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentsmetadatadocumentdetailpaneltsx) | 6 / 5 / 405 | ⚠ **FIRES** | ⚠ absent at **5 phases**. ⚠ A CROSS-SURFACE shell — it reuses `WorkspacePanel`'s sheet, so a change here lands on chat too. 430px track is fixed by its host |
...
| [`frontend/src/pages/KnowledgeHealthPage.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrcpagesknowledgehealthpagetsx) | 11 / 5 / 571 | ⚠ **FIRES** | ⚠ absent at **5 phases**. ⚠ It is `ChatLayout`'s POSITIONAL FALLBACK (`:879`) — retiring it into the Library removes the app's default view unless a replacement ships in the same commit |
| [`backend/app/api/knowledge_health.py`](docs/HOT-FILE-LEDGER.md#backendappapiknowledgehealthpy) | 10 / 5 / 713 | ⚠ **FIRES** | ⚠ absent at **5 phases**. ⭐ It already derives retrieval analytics from `audit_log` `search.query` — refuting BUS-026's "nothing logs retrieval today". Service-role by classified exception |
