...
| [`backend/app/services/watch_service.py`](docs/HOT-FILE-LEDGER.md#backendappserviceswatch_servicepy) | 3 / 2 / 599 | no (2 phases) | ⚠ row was STALE at `2 / 1 / 458`. 235: a run row per tick + a NAMED cause. ⛔ `release_watch` SWALLOWS its INSERT — nothing yet proves a row was STORED |
| [`backend/app/db/watches.py`](docs/HOT-FILE-LEDGER.md#backendappdbwatchespy) | 2 / 2 / 696 | no (2 phases) | 235-14 adds `last_success_by_watch` — the ONLY UNBOUNDED read, run for stopped watches only; ⛔ the prune's bound is still proven against FIXTURES only, never live rows |
| [`backend/app/api/sources.py`](docs/HOT-FILE-LEDGER.md#backendappapisourcespy) | 5 / 1 / 677 | no (1 phase) | ✅ 235 fixed `/sync`: it REFUSES when the LIVE reader is absent (`BUG-260906-02`). 235-14 closes G2 — the polled window is UNCHANGED; the last-good instant is filled unbounded for stopped rows only |
...
| [`frontend/src/lib/api/sources.ts`](docs/HOT-FILE-LEDGER.md#frontendsrclibapisourcests) | 4 / 1 / 296 | no (1 phase) | ⚠ **NOT covered by `lib/api.ts`'s row: that row is the BARREL.** ⛔ The barrel does not re-export it, so every mounting suite mocks it SEPARATELY |
...
| [`backend/app/services/sources/failure_cause.py`](docs/HOT-FILE-LEDGER.md#backendappservicessourcesfailure_causepy) | 1 / 1 / 169 | no (1 phase) | young (235) — **THE ONE classifier of why a source stopped.** ⛔ Writes nothing, imports no DB, hard-vs-soft is DATA (D-235-10) |
...
| [`frontend/src/components/sources/sourceHealthVocabulary.ts`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentssourcessourcehealthvocabularyts) | 1 / 1 / 315 | no (1 phase) | young (235) — ⛔ `Object.keys(COPY)` is PINNED at 26, which now blocks FOUR plans' orphaned labels; re-baseline them in ONE move |
...
| [`frontend/src/components/sources/RunHistoryList.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentssourcesrunhistorylisttsx) | 1 / 1 / 160 | no (1 phase) | young (235) — ⚠ it mounts BEHIND A CLICK, which is why five of the composition fence's reds are a harness finding, not a missing surface |
