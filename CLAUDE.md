...
| `scripts/vitest-count-gate.cjs` | 242 / 55 / 6056 | ⚠ STALE a 9th time; the prior `58` COUNTED 3 dated quick tasks the recipe says to subtract. **262-03** adopts the 2 catalog suites into BOTH knobs — `components/experts` still has no bare dir entry |
...
| `frontend/src/components/experts/catalog/ExpertCatalogPage.tsx` | 1 / 1 / 189 | ⚠ `0/0/0` AT PLANNING; measured at **262-03**. Renders EXACTLY what the grant-aware read returned. ⛔ No grey-out, no second card variant: PACK-11's vanish is the server's list, never a filter here |
