...
| `backend/app/models/expert.py` | 3 / 3 / 93 | ⚠ **NOW FIRES at 3 phases and the row read `1/1/52` / `young`** — PRESENT and WRONG stops an audit. honoured by construction (**263-03**): ONE new request model; ⛔ `member_skills: … \| None` untouched — that None is the PATCH distinction |
| `frontend/src/lib/api/experts.ts` | 5 / 3 / 298 | ⚠ **NOW FIRES at 3 phases and the row read `2/2/186` / `no`** — the 2nd row to cross in THIS plan. ⛔ the 422 arm reads `detail.error`, never the bare status: FastAPI's own 422 is a LIST and a naive arm swallows it |
...
| `backend/app/api/experts.py` | 8 / 3 / 560 | ⚠ row STALE a 2nd time ONE PLAN later (`6/3/398`). honoured by construction (**263-03**): 1 helper, 2 calls, 1 route. ⛔ the refusal sits ABOVE `create_expert`'s try — inside it, `HTTPException` is caught and the 422 ships as a 400 |
...
| `frontend/src/components/skills/SkillFormDialog.tsx` | 12 / 8 / 635 | ⛔ **FIRES, absent from BOTH registers its ENTIRE LIFE at 8 phases — row added at 263 PLANNING**, before a plan named it (the gate was driven RED). ⛔ The ONE human skill-authoring dialog; D-263-01 REUSES it pre-filled — a second dialog is the second engine PACK-15 refuses |
