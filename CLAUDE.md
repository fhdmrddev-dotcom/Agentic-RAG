...
| [`backend/app/api/documents.py`](docs/HOT-FILE-LEDGER.md#backendappapidocumentspy) | 71 / 31 / 2535 | ⚠ **FIRES** | ⚠ absent for its ENTIRE LIFE — row added 217.1-08, which wrote `embedded_at` at its main-ingest chunk INSERT (`:2296`) — one of the FOUR write sites |
| [`scripts/vitest-count-gate.cjs`](docs/HOT-FILE-LEDGER.md#scriptsvitest-count-gatecjs) | 128 / 28 / 4227 | ⚠ **FIRES** | honoured by construction (214-15 / 214.1-02 / **SEED-227**) — BASELINE pins + one TARGETS line, NO logic/threshold/check change; ⚠ absent until 196, at 16 phases |
...
| [`frontend/src/components/ingestion/DocumentList.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentsingestiondocumentlisttsx) | 22 / 12 / 609 | ⚠ **FIRES** | ⚠ absent from BOTH for its ENTIRE LIFE at **12 phases** — row added at the SEED-224 sketch. The 7-column order is load-bearing: `LibraryPage` sheds cols 3–5 by `nth-child` |
| [`frontend/src/pages/LibraryPage.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrcpageslibrarypagetsx) | 28 / 10 / 724 | ⚠ **FIRES** | ✅ **the named seam is TAKEN (217-09)** — one reducer, each tab body a CHILD. ⚠ re-derive with `git log --follow`, else it reads `1` |
...
| [`backend/app/api/knowledge_health.py`](docs/HOT-FILE-LEDGER.md#backendappapiknowledgehealthpy) | 10 / 5 / 737 | ⚠ **FIRES** | honoured by construction (**217.1-11**) — adds `could_not_search`; `retrieval_count` byte-unchanged. ⚠ absent at **5 phases**. Audit-analytics from `audit_log`. Service-role by exception |
| [`backend/app/services/tool_dispatcher.py`](docs/HOT-FILE-LEDGER.md#backendappservicestool_dispatcherpy) | 66 / 29 / 4336 | ⚠ **FIRES** | ⚠ absent for its ENTIRE LIFE — row added 217.1-11 (BE-4 error-path `search.query` write + BE-5 `similarities` key). Seam: `_handle_search_documents`'s audit write is the extraction leaf |
