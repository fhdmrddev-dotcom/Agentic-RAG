...
| [`frontend/src/components/workflows/PhaseFormPanel.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentsworkflowsphaseformpaneltsx) | 21 / 10 / 1289 | **FIRES** | honoured by construction ×4 (185 / 193 / 193.1 / 199) — ⚠ row read `19 / 9 / 1216`, stale BEFORE 199 started. ⚠ **`PhaseFormPanel.test.tsx` pins `useState`/`useMemo`/`useEffect` at an ABSOLUTE ZERO over the panel's own source** — a disclosure switch is state, so `199-06` put it in a new leaf (`FieldGuidance.tsx`) and the pin passed UNTOUCHED |
