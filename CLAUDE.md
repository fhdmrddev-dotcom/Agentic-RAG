...
| [`backend/app/services/watch_service.py`](docs/HOT-FILE-LEDGER.md#backendappserviceswatch_servicepy) | 4 / 2 / 608 | no (2 phases) | 235: a run row per tick + a NAMED cause; 235-13 makes seam 2 the ONE writer of `connection_disabled`. ⛔ `release_watch` SWALLOWS its INSERT — nothing proves a row was STORED |
