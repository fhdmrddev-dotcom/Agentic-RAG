...
| [`frontend/src/pages/WorkflowBuilderPage.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrcpagesworkflowbuilderpagetsx) | 49 / 15 / 2762 | **FIRES** | honoured by construction ×3 (193.1 / 193.2 / **197**) — ⚠ row read `42 / 13 / 2398`, stale by **5 commits / 1 phase / 258 L**; `197-10` FLAGGED it and correctly declined to edit a shared artifact mid-wave |
...
| [`frontend/src/lib/api.ts`](docs/HOT-FILE-LEDGER.md#frontendsrclibapits) | 178 / 101 / 6547 | ⚠ **FIRES HARDEST** | ⚠ **still the HOTTEST file in the repository** (row read `177 / 100 / 6521`; before that `171 / 98 / 6174`). ✅ **THE 197 DECLINE HOLDS A THIRD TIME AND ITS TRIGGER DID NOT FIRE — MEASURED:** 200.1's whole change is `+26 / −0` with **ZERO** lines matching `^\+export (const\|function\|class\|let\|var) ` over the real diff — a TYPE only, so `196-08`'s mock-factory failure mode again could not fire. Trigger VERBATIM: **the next phase adding a RUNTIME export or a second concern here**. ⚠ **A CLAIM ABOUT THIS ROW IS REFUTED, RECORDED BESIDE IT:** `200.1-02-PLAN.md` says the row *"was ALREADY STALE at this phase's base"* at `177 / 102 / 6521`. **Re-measured at `e0c57ef4`: `177 / 100 / 6521` — the row was CORRECT.** The `102` comes from **not subtracting the six-digit quick-task buckets** (`260405`, `260814`), which the recipe requires. ⚠ **A false staleness report ends an audit with a wrong number exactly as a stale row does** — the subtraction step is not optional bookkeeping. ✅ **The 197 DECLINE HOLDS and its re-open trigger DID NOT FIRE — stated, not left silent:** 192.2's whole change here is `+53 / −0` and is a **TYPE only**, zero new runtime exports (measured by grep over the diff), so `196-08`'s mock-factory failure mode again measurably could not fire. Trigger carried forward VERBATIM: **the next phase adding a RUNTIME export or a second concern here**. ⚠ **200 re-confirms it: its whole change is FOUR OPTIONAL FIELDS on an existing interface — the client mirror of transport 3, which `200-02` widened on the PYTHON side and not here, so the wire was sending facts the chat panel could not declare** |
...
| [`backend/app/main.py`](docs/HOT-FILE-LEDGER.md#backendappmainpy) | 72 / 53 / 783 | ⚠ **FIRES** | ⚠ **was ABSENT at FIFTY-THREE phases** (196) — no seam proposed; a registration file doing one thing 40× |
| [`backend/app/config.py`](docs/HOT-FILE-LEDGER.md#backendappconfigpy) | 71 / 42 / 1285 | ⚠ **FIRES** | ⚠ **was ABSENT at FORTY-TWO phases** (196) — **the D-22 headline**; seam named: `MODEL_CAPABILITIES` + its readers out, with a same-commit re-export |
...
| [`scripts/vitest-count-gate.cjs`](docs/HOT-FILE-LEDGER.md#scriptsvitest-count-gatecjs) | 114 / 19 / 3797 | ⚠ **FIRES** | ⚠ **was ABSENT at SIXTEEN phases — on the file that ENFORCES the guardrails** (196; row read `100 / 16 / 3215`). ⚠ Raising a pin necessarily deletes one line; a `grep -c '^-[^-]'` expecting 0 is WRONG — **but 197's edit was `+44 / -0`, because ADDING a pin is not RAISING one** |
...
| `frontend/src/components/workflows/RunTranscript.tsx` | 6 / 2 / 644 | no (2 phases) | young (200, **200.1**) — ⚠ row read `4 / 1 / 514`; 200.1-03 added `+132 / −2` and the row is **one phase below the G-5 threshold**, so it owes a `docs/` section on the NEXT phase that touches it. It **ported the sheet's gutter + live treatment and asserted BOTH shipped refusals as STILL HOLDING, one against a test that passed UNEDITED**; its docblock's claim *"this surface renders neither icon"* is amended BESIDE the original (the mark is a CSS ring, so the glyph budget really is unchanged). ⚠ **The 187-24 trap fired there too, observed RED** — the spinner's docblock QUOTED the sheet's glyph class while refusing it, and the guard is a whole-file grep expecting ZERO. Fixed in the PROSE, never by relaxing the guard. **the run surface's CENTRE**, and the component the canvas made way for. ⚠ **It is not a fourth reading of one run: the GRAIN and the CLOCK are what make it different** — the receipt has one row per step with a duration, this has one line per step stamped at the instant it last became true, ordered by that instant. ⚠ **THE TENSE RULE IS THE HIGH-CONSEQUENCE LOGIC AND IT WAS CORRECTED TWICE**: the state word comes from the phase STREAM and the duration from the durable FETCH, and the page carries a shipped test proving the two disagree. The page's words are used ONLY while they agree with the row's own status — measured against a REAL crossing, **2 of 228 local runs**, where `workflow_phases.phase_index` disagrees with the definition's ordering and D-188-01's index join reports each step's state as its NEIGHBOUR's. ⚠ **The sheet's two-line-per-step rhythm was BUILT AND WITHDRAWN**: its two lines carry authored NARRATION this product does not have, and with only the step's name both lines say the same thing. ⚠ Zero glyphs, zero spelled strings, `--panel-*` forbidden (this is `--background`) |
