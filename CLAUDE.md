...
| [`frontend/src/components/workflows/PhaseSpineGraph.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentsworkflowsphasespinegraphtsx) | 4 / 4 / 278 | ⚠ **FIRES** | ⚠ **was ABSENT at FOUR phases** (added 199) — the AUTHORING spine. ⚠ It reads a DRAFT definition and has **no run**, so any run-time word on it (`TRAVERSED`/`SKIPPED`, a duration, an elapsed) is a FABRICATED claim — sheet c3 col 1 drew exactly that and `199-02` refused it. `NET-NEW · no graph lib` was an implementation note printed to the author; removed, and a repo-wide grep measured **zero** assertions on it first |
