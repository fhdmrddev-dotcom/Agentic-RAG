...
| `frontend/src/pages/WorkflowsPage.tsx` | 103 / 124 / 143 / 152 / 155 / 165 / 184 / 184.1 / 186 / 188 — **21 commits across 10 phases**, **1407 lines**. ⚠ **Added 2026-08-10 during `/gsd:sketch 192`, and the reason it was added matters more than the number: this file was NEVER on the ledger.** Ten phases touched it and not one produced the refactor recommendation G-5 requires, because the audit step scans PLAN.md `files_modified` *against this table* — a hot file absent from the table is invisible to its own guardrail, permanently. Re-derive with: `git log --oneline -- frontend/src/pages/WorkflowsPage.tsx \| wc -l` → 21; `wc -l` → 1407; `git log --format=%s -- <file> \| sed -E 's/^[a-z]+\(([^)]+)\).*/\1/' \| sed -E 's/-.*//' \| sort -u` for the phase list. | **G-5 FIRES — extraction due, and Phase 192 is exactly the phase that must decide it.** 10 phases is more than triple the threshold, and 192 (Workflow Library IA, LIB-01…04) is a *structural* rewrite of this file's whole library view — search, filtering, shelf taxonomy, card actions, create placement — not a mount point like 185's on `PhaseFormPanel`. The concerns already stacked inside one file: the library view, the builder host, the project rail, three distinct card components (`StarterCard` / `PublishedCard` / `DraftCard`), the `RunModal`, and the WFIN-03 victim-naming delete Sheet. The natural seam, named but not taken here: cards to `components/workflows/library/`, the RunModal and delete Sheet to their own modules, leaving the page as composition — the same shape the 188.2 card cut used. **Per G-5, discuss-phase 192 must produce a refactor recommendation as the FIRST option, not the planned feature.** |
