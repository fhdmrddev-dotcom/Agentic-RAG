...
| `frontend/src/components/workflows/PhaseNodeCard.tsx` | 184 (184-03, 184-08) / 185 (185-01 the 137-B rebuild, 185-08 docblocks, 185-09 the corner seal) / 188 (188-06 the run-reading arc, +301/−29) / 188.1 (188.1-04 the WR-04 own-guards, +47/−5) — **7 plans across 4 phases, 797 lines**. ⚠ This cell previously read *5 plans across 2 phases, 478* — both stale: Phases 188 and 188.1 were never added, and the file has grown 67 % since that figure was written. Re-measured 2026-08-06 by `wc -l` and `git log -- <file>`. | **G-5 THRESHOLD CROSSED (4 phases ≥ 3) — audit due at Phase 189's discuss-phase.** This cell was flipped from *Watch — under the 3-phase threshold* on measurement, not on judgement; per G-5's own "audit during discuss-phase" clause the extraction-vs-continue call is the operator's, and no routing is asserted here. 185 spent its budget deliberately: 185-01 rebuilt the geometry to 137-B (D-185-17) and 185-09 added the governance corner seal. Two invariants are now type/test-enforced and must be read before any new mark is proposed: **a third badge is a typecheck error**, and **no focusable control may live inside the card** (the ✕ and ＋ live on the lane). Badge slot 1 is empty and reserved for 188/189. |
