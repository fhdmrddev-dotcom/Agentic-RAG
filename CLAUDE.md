...
| [`backend/app/services/tool_dispatcher.py`](docs/HOT-FILE-LEDGER.md#backendappservicestool_dispatcherpy) | 67 / 28 / 4336 | ⚠ **FIRES** | ⚠ absent for its ENTIRE LIFE — row added 217.1-11 (BE-4 error-path `search.query` write + BE-5 `similarities` key). Seam: `_handle_search_documents`'s audit write is the extraction leaf |
