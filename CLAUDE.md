...
| [`backend/app/api/connectors.py`](docs/HOT-FILE-LEDGER.md#backendappapiconnectorspy) | 32 / 15 / 1757 | ⚠ **FIRES** | ⚠ **the 232-03 discharge claim was FALSIFIED by 232-04** — extraction real (−28 L) but the phase net is **+21** (1736→1757). Extraction still OWED |
...
| [`frontend/src/lib/api/connectors.ts`](docs/HOT-FILE-LEDGER.md#frontendsrclibapiconnectorsts) | 11 / 6 / 594 | ⚠ **FIRES** | ⚠ absent for its ENTIRE LIFE at **6 phases** — row added at 232's close by the REVIEWER, after 232-04 promised it and did not. **`lib/api.ts`'s row is the BARREL, not this module** |
