...
| [`backend/app/api/documents.py`](docs/HOT-FILE-LEDGER.md#backendappapidocumentspy) | 75 / 32 / 2408 | ⚠ **FIRES** | ✅ **DISCHARGED (229)** — mint_document_row + splice_document extracted to services/ingest_splice.py; upload, connectors & email spliced; -127 L |
| [`scripts/vitest-count-gate.cjs`](docs/HOT-FILE-LEDGER.md#scriptsvitest-count-gatecjs) | 159 / 36 / 4687 | ⚠ **FIRES** | honoured by construction (**233**) — 2 BASELINE pins + 2 TARGETS lines + 1 re-pin, NO logic/threshold/check change; ⚠ absent until 196, at 16 phases |
...
| [`frontend/src/pages/LibraryPage.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrcpageslibrarypagetsx) | 40 / 12 / 825 | ⚠ **FIRES** | honoured by construction (**233**) — ONE container class; ⚠ it was the app's only page with NO max-width. ⚠ re-derive with `git log --follow`, else it reads `1` |
...
| [`backend/app/services/sources/preview_service.py`](docs/HOT-FILE-LEDGER.md#backendappservicessourcespreview_servicepy) | 1 / 1 / 549 | no (1 phase) | young (233) — **the ONE classifier**, run twice. ⛔ It writes nothing, and tier-1 identity is an EQUALITY, never a hash |
| [`backend/app/services/sources/import_service.py`](docs/HOT-FILE-LEDGER.md#backendappservicessourcesimport_servicepy) | 2 / 2 / 172 | no (2 phases) | young (232, 233) — ⚠ its four new params all default `None`, so the 216 attach door mints the row it always did |
...
| [`frontend/src/components/library/IngestionTab.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentslibraryingestiontabtsx) | 13 / 4 / 456 | ⚠ **FIRES** | ⚠ absent for its ENTIRE LIFE at **4 phases** — row added 233; honoured by construction: one import, one mount, ZERO branches |
...
| [`backend/app/services/watch_service.py`](docs/HOT-FILE-LEDGER.md#backendappserviceswatch_servicepy) | 2 / 1 / 458 | no (1 phase) | young (234) — ⚠ absent at its own close until 2026-09-06. The milestone's crux engine. ⛔ It hands files to the QUEUE, so it reaches NO rules engine (`BUG-260906-01`) |
| [`backend/app/db/watches.py`](docs/HOT-FILE-LEDGER.md#backendappdbwatchespy) | 1 / 1 / 439 | no (1 phase) | young (234) — the watch data-access leaf; `claim_due_watches` clones `schedules.py`'s SKIP LOCKED claim rather than widening `workflow_schedules` |
| [`backend/app/api/sources.py`](docs/HOT-FILE-LEDGER.md#backendappapisourcespy) | 2 / 0 / 356 | no (0 phases) | young (234) — ⛔ `/sync` POKES the scheduler, it does not sync; its `"scheduled"` reply is true with the loop OFF too (`BUG-260906-02`) |
| [`frontend/src/components/sources/WatchedFoldersSection.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentssourceswatchedfolderssectiontsx) | 2 / 0 / 393 | no (0 phases) | young (234) — mounted in `IngestionTab`, NOT bolted onto `ConnectionsTab` (G-1 pre-empted). ⛔ Renders no `last_run_at`/`last_status`; 235 owes that |
