...
⚠ **THE MECHANICAL GUARDRAILS LIVE IN A VENDORED FRAMEWORK, AND A FRAMEWORK UPDATE CAN CLOBBER THEM SILENTLY.** `.claude/get-shit-done/` is vendored at **v1.42.3**, tracked in git, with a SessionStart update checker — so a future `chore(gsd): apply the pending GSD framework update` can revert any of these edits and nothing would say so. **They are listed here by path so they can be RE-APPLIED as a set** rather than rediscovered one at a time, months apart:
...
| **REG-02** seeds sweep | `workflows/discuss-phase.md` (`<step name="cross_reference_seeds">`) · `workflows/new-milestone.md` (§2.5) · `workflows/plant-seed.md` (the `max(id)+1` allocator + the frontmatter-contract block) | Phase 251 |

⛔ **The call must land in `workflows/`, never in `.claude/commands/gsd/`.** Measured at Phase 251: the two command files are **76- and 45-line ROUTERS with no steps in them**, and **no file under `.claude/commands/gsd/` invokes any `node scripts/…` at all** — a gate added there lands where nothing executes it in order, which is this file's own recurring *"a rule that exists and is not applied is the same as no rule."*
⚠ **Precedent, not guarantee:** the G-7 wiring survived one framework update (`dec2577ae` touched none of its three files). One survival is one data point.
⛔ **And `grep` cannot audit this.** A step that merely *names* a script satisfies `grep -rn "check-seeds-register"` and fires nothing. Phase 251 drove that counterfactual: with the executable fence replaced by a prose mention, the grep still hit while **zero runnable calls** remained. **Audit by extracting the fenced command and RUNNING it**, never by matching the name.
...
| `frontend/src/components/layout/ChatLayout.tsx` | 52 / 27 / 1014 | ⚠ STALE a 3rd time (was `51/26/1010`, `46/24/921`). 262-01: INHERITED rot only; plan 05 adds the branch. ⛔ the trailing fallback is POSITIONAL — order IS the guard |
...
| `frontend/src/components/chat/MessageInput.tsx` | 35 / 17 / 942 | ⚠ STALE (was `34/17/942`). 262-01: INHERITED rot only; plan 05 edits. 260-02 stands — the invite door lives inside '+', ⛔ 0 new top-level controls |
...
| `frontend/src/components/chat/ChatArea.tsx` | 77 / 38 / 882 | ⭐ re-derived 262-01 and **HELD** — the only 1 of 8 that did. 260-03 stands: activeExpert sync via getExpert, mounts ExpertSpotlightCard; plan 05 edits |
...
| `backend/app/services/connector_service.py` | 28 / 11 / 1858 | ⚠ row STALE (`25/9/1772`) — 248 and 252 touched it. NOT modified by 253. honoured by construction (**239-06**) |
...
| `frontend/src/App.tsx` | 33 / 24 / 378 | ⚠ STALE a 3rd time (was `32/23/374`, `31/23/351`); absent its ENTIRE LIFE at 23 phases before. 262-01: INHERITED rot only; plan 05 adds the member. 244-04's fence stands |
...
| `frontend/src/lib/nav-items.ts` | 9 / 6 / 118 | ⚠ STALE (was `8/6/95`). ⛔ 2 of its 3 suites ran in **NEITHER** count-gate knob — 262-01 adopts both. Row added 235 when absent at 6 phases |
...
| `frontend/src/components/experts/expertIcon.tsx` | 0 / 0 / 0 | young (created 262). Row added AT PLANNING. ⛔ The ONE home of expert-icon resolution, a CLOSED 11-key lucide map. It reads `icon` and never `slug`/`name` — that match IS the retired artefact |
| `frontend/src/components/experts/catalog/expertCatalog.ts` | 0 / 0 / 0 | young (created 262). Row added AT PLANNING. Pure search/category/folder-name resolution. ⛔ An unresolvable folder id returns an UNKNOWN marker, never a drop — a blank is a claim nobody made |
| `frontend/src/components/experts/catalog/ExpertCard.tsx` | 0 / 0 / 0 | young (created 262). Row added AT PLANNING. The catalog's 5-element card face, COUNTS only. ⛔ Zero lecturing prose on the face (sketch 261-262) — the NAMES belong to the detail modal |
| `frontend/src/components/experts/catalog/ExpertCatalogPage.tsx` | 0 / 0 / 0 | young (created 262). Row added AT PLANNING. Renders EXACTLY what `listExperts()` returned. ⛔ No grey-out, no locked row: PACK-11's vanish is the server's list, never a filter here |
| `frontend/src/components/experts/catalog/ExpertDetailModal.tsx` | 0 / 0 / 0 | young (created 262). Row added AT PLANNING. PACK-12's only renderer of `when_to_use`/`example_output` and of folder NAMES. ⛔ The CONTENT is asserted, never a `data-testid` |
| `frontend/src/components/experts/catalog/startScopedChat.ts` | 0 / 0 / 0 | young (created 262). Row added AT PLANNING. PACK-13's ONE ordered handoff: create → setThreadActiveExpert → refresh → select → navigate. ⛔ A failed patch does NOT navigate |
