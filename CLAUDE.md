...
| [`frontend/src/pages/WorkflowsPage.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrcpagesworkflowspagetsx) | 34 / 12 / 1176 | **FIRES** | satisfied (192 / 192.1) |
| [`frontend/src/components/workflows/library/WorkflowCard.tsx`](docs/HOT-FILE-LEDGER.md#frontendsrccomponentsworkflowslibraryworkflowcardtsx) | 8 / 3 / 818 | **FIRES** | ⚠ obligation UNDISCHARGED — next phase touching it owes a refactor rec FIRST |
...
| [`backend/app/api/workflows.py`](docs/HOT-FILE-LEDGER.md#backendappapiworkflowspy) | 36 / 18 / 1984 | **FIRES** | extraction due — not taken in q5r or 197. ⚠ **MEASURED NOT MODIFIED by 197** — the `/generate` route declares NO `response_model` and `return result`s the service dict, so a new wire key needed zero change here |
...
| [`backend/app/db/workflows.py`](docs/HOT-FILE-LEDGER.md#backendappdbworkflowspy) | 36 / 18 / 1656 | **FIRES** | honoured by construction (193.2 / 194) — hottest backend module by phase |
...
| [`frontend/src/lib/api.ts`](docs/HOT-FILE-LEDGER.md#frontendsrclibapits) | 171 / 98 / 6174 | ⚠ **FIRES HARDEST** | ⚠ **still the HOTTEST file in the repository** (was ABSENT at 97 phases; row read `170 / 97 / 6154`). Seam **DECLINED as a named decision (197)** — the phase added a TYPE only (`+21 / -1`), so the `196-08` mock-factory failure mode measurably did NOT fire. Re-open trigger: **the next phase adding a RUNTIME export or a second concern here** |
