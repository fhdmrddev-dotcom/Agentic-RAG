...
- **Global scope**: `C:/Vibe Apps/Agentic RAG/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Vibe Apps/Agentic RAG/.claude/notes/`)
...
2. Glob `C:/Vibe Apps/Agentic RAG/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Vibe Apps/Agentic RAG/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Vibe Apps/Agentic RAG/.claude/notes/` — works in any directory
