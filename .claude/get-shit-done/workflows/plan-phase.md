...
@C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/revision-loop.md
@C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/gate-prompts.md
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/skeleton-template.md`.
...
2. **Date the offending code** (`git log --diff-filter=A -- <file>`). Gaps in the last round's own
   output are a signal to stop, not to iterate.
3. Triage each finding: **fast-fix** (G-3 — apply inline), **defer-to-next-phase** (deferred item
   with a concrete re-open trigger), or **accept** (recorded).
4. Proceed ONLY if a ROADMAP success criterion is genuinely unmet, by re-running the check with
   `--unmet-criterion "SC#N: <what is not true>"` (or `--capability-approved "<why>"` for the
   new-capability failure) and recording the override under `STATE.md → Guardrail overrides`.
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
