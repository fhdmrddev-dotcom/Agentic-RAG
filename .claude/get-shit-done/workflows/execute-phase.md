...
@C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/agent-contracts.md
@C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/context-budget.md
@C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/gates.md
</required_reading>
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/planner-antipatterns.md
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       @C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@C:/Vibe Apps/Agentic RAG/.claude/get-shit-done/references/executor-examples.md'}
       </execution_context>
...
**If gaps_found — PROJECT GATE G-7 RUNS FIRST (CLAUDE.md § Workflow guardrails):**
...
**If `G7_EXIT` is 1, do NOT emit the `/gsd:plan-phase {X} --gaps` routing below.** The phase has
already run ≥ 2 gap-closure rounds, and/or a closure plan built a source file that did not exist
when it was written. Instead:

1. Report ROADMAP **success-criteria** status first — if every criterion is verified, say so plainly
   and present *stopping* as a real option. Never route to another round as though it were the only door.
2. **Date the offending code** — `git log --diff-filter=A -- <file>` for each gap's files. A gap in the
   LAST round's own output is a signal to stop, not to iterate.
3. Triage each remaining finding as **fast-fix** (G-3: ≤ 1 file, ≤ 10 lines, no schema/API surface —
   apply inline), **defer-to-next-phase** (log a deferred item with a concrete re-open trigger), or
   **accept** (record it).
4. Only if a ROADMAP success criterion is genuinely unmet, re-run with
   `--unmet-criterion "SC#N: <what is not true>"`, record the override under
   `STATE.md → Guardrail overrides`, and then proceed to the routing below.

Closing a phase with owed manual UAT rows is legitimate — state it as a DECISION, never as a claim
that everything ran, and name which row to run first.

**If `G7_EXIT` is 0, present the standard gap-closure routing:**
```
