...
> ⚠ **AMENDED (Phase 194, D-13/D-14) — the operator-approved original above is kept VERBATIM and this note sits BESIDE it, never over it.** D-14 permits extending this vocabulary and forbids replacing it without stating why; this is the why.
>
> **1. `partial work discarded` is the one sentence D-13 forbids, and the amendment is: `⏹ Run stopped — no deliverable produced · the phases that finished are kept`.** D-13 rules that *"a stopped run KEEPS its completed phases, marks the interrupted one, and is TERMINAL"*, and it explicitly REJECTED *"collapsing to a bare `cancelled` that hides partial work … it discards evidence the database still holds."* Phase 194 then made that literally true in the schema and on the write path: migration 119 adds a **`cancelled`** literal to `workflow_phases_status_check` so the INTERRUPTED phase can be marked honestly, and `cancel_active_phases` carries `AND status = 'active'` precisely so **completed phases are left untouched** (D-07 — *"completed phases' outputs are ALREADY durable"*). Observed on live data in `194-HEAL-RECEIPT.md`: healing a stuck run terminalized its in-flight phase while a `completed` phase stayed `completed` and two `pending` phases stayed `pending`. **A receipt telling a user their partial work was discarded would be false about rows that are still sitting in Postgres.**
>
> **2. The `⏹` in the sentence above is FLAGGED as a net-new mark, and Phase 194 did NOT ship it.** `references/icon-convention.md` §4's canvas glyph table is `⛨ 🔒 ⤳ ＋ ✕ ↶ ↷ ◆` plus the phase-type map — **`⏹` is absent from it, and so is `■`** — and §4's rule is that *"net-new marks must be FLAGGED as proposals"* because *"a sketch that invents a glyph teaches the wrong vocabulary to whoever builds from it."* What 194 built instead spends **zero** net-new glyphs, in two halves: the Stop **CONTROL** reuses the shipped lucide `Square` (`MessageInput.tsx:420`, `ActiveRunsTray.tsx:132`), and the stopped **STATE** uses `■`, chosen by plan `194-04` **by measurement** — 1 render in `frontend/src` (`RunCard.tsx:534`), carrying no second meaning. ⚠ Plan `194-03` refused `■` for its Stop *button* on the complementary ground that *"that is RunCard's cancelled-STATE glyph, a state and not a control"* — **the two decisions are one line drawn from both sides, and `⏹` is on neither side of it.**
>
> **3. ⚠ A FOURTH shipped mark, recorded beside `194-RESEARCH.md`'s three-mark table which MISSED it.** RESEARCH lists `■` (`RunCard.tsx:534`), the lucide `Square` (the control, ×2) and this sketch's designed-never-built `⏹`. Measured, there is a fourth and it ships: **`WorkflowRunPage.tsx:299` renders `"⊘ Cancelled"` as the run band's cancelled sentence.** That correction is what turns `⊘` from a plausible pick into a rejected one — `⊘` renders for **TWO** concepts (`WorkflowRunPage.tsx:299` cancelled, and `admin/ModelDiscoveryPanel.tsx:504` *"No longer offered"*), so it cannot carry a third. `194-04` measured the same fourth mark independently; both readings agree.
>
> **4. ⚠ ONE CORRECTION ON MEASUREMENT AGAINST PLAN `194-05` ITSELF, recorded rather than quietly dropped.** The plan directs *"do the same at `:135` and `:227` if they carry the same sentence."* Measured with `grep -n "partial work discarded"`: **`:25` is the ONLY line in this file carrying it.** `⏹` also appears at `:30`, `:227` and `:263`, but as the composer's `⏹ Cancel` **control** label and the resolve-toggle legend — a different concept, covered by flag 2 above, not by the D-13 copy amendment. `:135` carries CSS for `.runchip .cancel` and no copy at all. **The amendment is therefore correctly scoped to one line, and the plan's conditional is discharged by measurement rather than by assumption.**
