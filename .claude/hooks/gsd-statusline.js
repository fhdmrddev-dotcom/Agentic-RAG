...
/**
 * Clamp a display string to a maximum visible length so a single ballooned
 * field — e.g. a STATE.md `status:` written as a full paragraph instead of a
 * one-word keyword — can never overflow the terminal and shove the context
 * meter off-screen. Truncates on a character boundary with an ellipsis. Apply
 * to RAW text only (before ANSI styling) so escape codes stay intact. Leading
 * segments (milestone + progress bar) are preserved; the overflow tail is cut.
 */
function clampDisplay(str, max = 80) {
...
function formatGsdState(s) {
  ...
  // Balloon guard: a ballooned field (most often a STATE.md `status:` written
  // as a paragraph instead of a keyword) must never blow out the status line
  // and push the context meter off-screen. See clampDisplay.
      ...
      try {
        ...
        if (files.length > 0) {
          try {
            ...
            if (inProgress) task = clampDisplay(inProgress.activeForm || '');
          } catch (e) {}
        }
      } catch (e) {
...
module.exports = {
  readGsdState, parseStateMd, formatGsdState, clampDisplay,
